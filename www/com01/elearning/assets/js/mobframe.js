...
C_iEDIT.prototype = {
	...
	validate: function(newdigits) { // sets this.state.valid according to expected format type and mandatory attributes
		...
		switch(this.state.type) {
			...
			case INPUT_HTMLCOLOR:	this.state.valid = this.isHtmlcolor(newdigits);	break;
	...
	changed: function(paste) { // called on a keyup event. paste [false,true] tells us if a copy/paste was used
		...
		if(paste===true) { // copy/paste escaped the filtering already taking place here (*ed01*)
			...
			switch(this.state.type) { // keep this inline with (*re01*), but keep in mind that they might differ
				...
				case INPUT_HTMLCOLOR:       pattern = new RegExp(/[^0-9,]/g); break;
	...
	setvalidtip: function() { // based on this.state.valid set by this.validate(), here we change the field frame to red (class validNOK) and set a tooltip with error message
		...
		if(wrongentry) {
			switch(this.state.type) {
				...
				case INPUT_HTMLCOLOR: 	msg = 'is not a R,G,B color format'; break;
	...
	keypress: function(e) { // this function returns a propagation true or false => lets the character appear in the field, or do not
		...
		switch(this.state.type) { // keep this inline with (*re01*), but keep in mind that they differ!!
			...
			case INPUT_HTMLCOLOR: 		pattern = new RegExp(/[0-9,]/); 	break;
	...
	isHtmlcolor: function(sText) {
		if (sText == '') return true;
		if (sText.search(/^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\s*,\s*(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\s*,\s*(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/) == -1) // This regular expression ensures that the input string contains exactly three numbers separated by commas, and each number is in the range from 0 to 255
			return false;
		return true;
	}, 
