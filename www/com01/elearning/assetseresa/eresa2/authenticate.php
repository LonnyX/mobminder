...
if($dS_login->eresaAuthent) { // authentication option is active on this web page
	...
	// send the token
			...
			$from  = $dS_login->firstname.' '.$dS_login->lastname.' <'.$fromail.'>';
			$time  = new C_date();
		
			$title = L::XL('e-resa-token-mail title',$language).': '.$token;
		...
		if($demo) { // echo to the tester console because we are in demo mode and/or the 
			...
			$o_dataCom_mail->sendMail($title, $body, $email, $from, $dS_login->language);
		}
} else {
