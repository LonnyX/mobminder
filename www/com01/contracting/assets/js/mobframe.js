...
//
//
	var classOnlyVerbose = monitorModalResaPayments; // (defines which filter to apply) default: false (verbose for all classes)
...
A_cb.prototype = { // sorry for the comments shortage :oP
	...
	cb: function() {
		...
		} } else console.log('A_cb::cb - missing callback function, undefined in context:',this.context);
	},
...
C_iEDIT.exclude = { always:new RegExp(/[|<>\\#"]/g) }	// < and > have been removed from here by PVH on 2019-07 so to allow <b> tags in INPUT_HTMLAREA	
C_iEDIT.prototype = {
	// public functions
	display: function(css, label, labelcss, options) { // return top bottom like: <div>a label</div><input>input field</input>
		...
		if(this.state.rows) textinput = '<textarea'+cssclass+' rows="'+this.state.rows+'" id="'+this.eids.ui+'" '+tabindex+placeholder+' required spellcheck="false" autocomplete="off">'+display+'</textarea>'; /*RQI01*/
			else textinput = '<input type="'+type+'" '+name+' '+pattern+' '+cssclass+' id="'+this.eids.ui+'" value="'+display+'"'+tabindex+placeholder+' '+autocap+' required spellcheck="false" autocomplete="off"/>'; /*RQI01*/
	...
	labelled: function(label, css, options) { // return side by side tds like: <td>a label</td><td>input field</td>
		...
		return '<td class="'+labelcss+'" style="white-space:nowrap; text-align:right;">'+this.state.label+'</td><td>'+this.display(css||'alpha16')+'</td>';
	},
	labelledafter: function(label, css, options) { // return side by side input followed by a label : <input input field><label>a label</label>. This order is needed to have nice label transform effect
		options = options || { xl:true };
		this.state.label = options.xl?C_XL.w(label):label;
		let labelcss = 'label textcolor-light'+(options.labelcss?' '+options.labelcss:'');
		return this.display(css||'alpha16')+'<label class="'+labelcss+'">'+this.state.label+'</label>';
	},
	...
	insert: function(i) { // does not change the caret position ( OBSOLETE, REMOVE )
	...
	getpost: function() { 
		switch(this.state.type) {
			...
			case INPUT_PRICE: return ((this.state.digits)*100)|0; // So we record integers, always using cents as a unit
			default: // INPUT_TEXT
	...
	validate: function(newdigits) { // sets this.state.valid according to expected format type and mandatory attributes
		...
		switch(this.state.type) {
			...
			case INPUT_SMSAREA: this.state.valid = !this.remains().over; break;
		}
	...
	changed: function(paste) { // called on a keyup event. paste [false,true] tells us if a copy/paste was used
		...
		if(paste===true) { // copy/paste escaped the filtering already taking place here (*ed01*)
			
			// this.ui(this.digits().replace(C_iEDIT.exclude.always,'')); // covers a copy/paste operation, that does not loop via keypress, see // this same regexp is used after a copy/paste check, see (*ed01*)
			cleanedup = newentry.replace(C_iEDIT.exclude.always,''); // covers a copy/paste operation, that does not loop via keypress, see // this same regexp is used after a copy/paste check, see (*ed01*)
			let pattern = false;
			switch(this.state.type) { // keep this inline with (*re01*), but keep in mind that they might differ
				case INPUT_MOBILE: 			
				case INPUT_PHONE: 			pattern = new RegExp(/[^0-9+]/g); 	break; // [^ meaning: nothing else but
				...
				case INPUT_IBAN: 			pattern = new RegExp(/[^ A-Z0-9]/g); 	break;
				case INPUT_BIC: 			pattern = new RegExp(/[^A-Z0-9]/g); 	break;
			...
			if(pattern) { 
				cleanedup = cleanedup.replace(pattern,'');
			}
		}
		...
		if(remain.over) // no room remaining for this copy/paste, this protection acts also on ribon key ( maintaining a key pressed for seconds )
			if(newentry.length >= formertxt.length) cleanedup = formertxt; // here we allow only reducing the strnig length
	...
	// events
	keypress: function(e) { // this function returns a propagation true or false => lets the character appear in the field, or do not

		// Important note : we treat here every SINGLE char entry on keyboard. Because they are th result of a 
		// key combination, Copy/paste's are detectable only at keyup event. See this.keyup()
		// 
		...
		switch(this.state.type) { // xcluded characters
			...
			default: xpattern = C_iEDIT.exclude.always; // this same regexp is used after a copy/paste check, see (*ed01*)
		}
		if(!ch.search(xpattern)) return false; // those characters are NEVER accepted through our forms
		
		let pattern = false;
		switch(this.state.type) { // keep this inline with (*re01*), but keep in mind that they differ!!
			// restricted set of characters, they will not appear on screen when you hit the key
			...
			case INPUT_PRICE: 			pattern = new RegExp(/[.0-9-]/);
				if(key==C_KEY.code.s.enter) pattern = false; // we tolerate ENTER key on this field
				break;
			
			case INPUT_IBAN: 			pattern = new RegExp(/[ a-zA-Z0-9]/); break;
			case INPUT_BIC: 			pattern = new RegExp(/[a-zA-Z0-9]/); break;
			...
			case INPUT_TEXT: // INPUT_TEXT, accept all
			case INPUT_SMSAREA:	
			default: 
		}	
		if(pattern) if(!pattern.test(ch)) { console.log('O U T'); return false; } // the key pressed is a prohibited character
		...
		if(this.state.type==INPUT_IBAN) { // special treatment here:
				let v = (el.value+ch); // value forecast
...
class C_iFIELD extends C_iEDIT { // on computer/keyboard device this class is transparent and defaults to C_iEDIT, if is.newtouch (touch device) then this class nests the C_EDIT
	...
	constructor(eid, callbacks, preset) {
		...
		let faketextarea = ''; if(this.state.rows) faketextarea = 'min-height:3em; overflow:auto;';
			...
			let wide100 = this.state.type == INPUT_TEXTAREA ? ' wide100' : '';
...
C_iMODAL.prototype = {
	...
	busy: function(onoff) {
		if(onoff && !this.status.busy) {
			...
			let busy = '<div class="modal-busy" id="'+this.eids.busy+'" style="position:absolute; z-index:'+zindex+'; top:0; left:0; width:100%; height:100%; overflow:hidden;"></div>';
