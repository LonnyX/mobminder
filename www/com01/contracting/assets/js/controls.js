...
C_iCRESTA.prototype = {
	...
	additem: function(v, l, x, optionpreset) {
		optionpreset = optionpreset || {}; // like { tip:C_iTIP, onlabel:A_cb, sortrule:function(a,b) { return (labels[a]>labels[b])?1:-1; }; }
		console.log(v, l, x, optionpreset)
...
C_iTIP.handlers = {
	enter:function(event) {
		...
		C_iTIP.status.timer = setTimeout(C_iTIP.handlers.delayed, 700, this.id);
	},
...
C_iYEAR.prototype = {
	...
	// public
	display: function() {
		let left = '<td id="'+this.eids.left+'" style="width:1%">'+this.months(this.eids.months)+'</td>';
		let right = '<td id="'+this.eids.right+'"></td>';
		let grid = '<div style="position:relative;" id="'+this.eids.grid+'"><table class="year-"><tr>'+left+right+'</tr></table></div>';
		let stats = '<div id="'+this.eids.stats+'">stats:</div>';
		return grid+stats;
	}, 
	activate: function() {
		this.elements.collect(this.eids);
	...
	months: function(eid) { // months names left heading grid rows
		let trs = new Array('<tr><th>'+this.controls.year.display('alpha04 boldy')+'</th></tr>');
		let css;
		for(let m=0;m<12;m++) {
