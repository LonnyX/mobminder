...
if($newxl) {
	...
	$dbio = new mysqli('localhost' /*host*/, 'mobminder' /*user*/, 'tgx23PiQ' /*pw*/, 'contracting' /*db*/);
	$dbio->query('SET NAMES utf8');

	if($id) { // item is already in the DB
		$r = $dbio->query('select id from xl_contract where id='.$id.';');
		if($r===false) echo $dbio->errno.' -> '.$dbio->error;
		$c = $r->num_rows;
		if($c==1) { // id exists, go on with writing
			$sql = 'update xl_contract set '.$lng.' = "'.$newxl.'" where id='.$id.';';
		...
		$r = $dbio->query('insert into xl_contract (tn, page, fr, en, nl, es) values ("'.$tname.'","'.$page.'","'.$l['fr'].'","'.$l['en'].'","'.$l['nl'].'","'.$l['es'].'");');
		if($r===false) echo $dbio->errno.' -> '.$dbio->error;
	}
} 
?>