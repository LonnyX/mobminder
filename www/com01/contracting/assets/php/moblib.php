...
<?php
	...
	if(isset($_GET['ixl'])) { // interactive translation enabler
		if($_GET['ixl'] == 'M0bM1nD3rC0m!') // Password for accessing translation mode
			{ $ixl = 'on'; $_SESSION['ixl'] = 1; }
		else 
			{ $ixl = 'off'; $_SESSION['ixl'] = 0; }
	}
		
	// connect to DB and bring all xl_contract to their respective places
	
	$mysqli = new mysqli('localhost' /*host*/, 'mobminder' /*user*/, 'tgx23PiQ' /*pw*/, 'contracting' /*db*/);
	$mysqli->query('SET NAMES utf8');
	
	function X($tn, $page = false) {
		global $mysqli, $l, $p, $ixl;
		if($page) $qp = $page; else $qp = $p;
		$r = $mysqli->query('select id, fr, '.$l.' from xl_contract where tn = "'.$tn.'" and page = "'.$qp.'";');

		if($r===false) echo $mysqli->errno.' -> '.$mysqli->error;
		$c = $r->num_rows;
		if($c > 1) return '-?many-'.$tn.'-?-'; // 'Many XL for '.$tn.'/'.$qp; // mutliple entries for this combination of technical name and page
		if($c === 0) { 
				$id = 0; // will trigger the creation of a new record in xl_contract DB table
...
window.addEventListener("load",function(event) {  
		console.log('window.onload-moblib.php');
		if('<?=$ixl?>'==='on') {
			$('body').find('xl').each(
				function() {
					$(this)
