...
class C_geofind {
	...
	function __construct($useragent, $userip, $nobot = true) {
		...
		$file = file_get_contents('http://www.geoplugin.net/php.gp?ip='.$this->userip.''); // 120 requests per minute is the maximum rate for this free service : https://www.geoplugin.com/webservices/ssl
		if($file) {
			$geo = unserialize($file);
