...
namespace SmsBox
{
    public class GprsServiceClient //: IServiceClient
    {
        private const string errorprefix = "GprsServiceClient.";
        public string ErrorMessage;
        private string login; 
        private string pwd; 

        public GprsServiceClient(string simphonenumber, string login_, string pwd_)
        {
            login = login_;
            pwd = pwd_;
        }
        ...
        public bool CheckAlive()
        {
            try
            {
                string url = "http://localhost:444/checkalive.html";
        ...
        public bool GetSmsToSend(string iccid, out ReceivedSms sms, int csq)
        {
            try
            {
                ...
                else if (s.StartsWith("0;no data"))
                {
                    ...
                    //Console.WriteLine(s);
                    //sms = ServiceHelper.ConvertHttpResponseToReceivedSms(null);
                    sms = null;
                    return true;
                }
                else
                {
                    //sms = ServiceHelper.ConvertHttpResponseToReceivedSms(s);
                    //TODO
                    sms = null;
                    if (sms == null)
                    {
