...
namespace SmsBox
{
    ...
    {
        // SIGNATURE -----------------------------------------------------------
        private const string _signature = "v4.00";
        //----------------------------------------------------------------------
        ...
        static int Main(string[] args)
        {
            ...
            try
            {
                ...
                if (rpimode)
                {
                    ...
                    //check voltage
                    bool? v = GpioWrapper.CheckVoltage();
                    if (!v.HasValue)
                    {
                ...
                while (true)
                {
                    ...
                    if ((running) && (queuesms.isEmpty()) && (singletondirectack.isEmpty()))  //interdire envoi sms si queue sms et singleton directack ne sont pas vides!!!
                    {
                        ...
                        if (rpimode)
                        {
                            ...
                            {
                                currentpowerstatus = v.Value;
                                LogHelper.WriteLine(ELogLevel.info, "check voltage=" + v.Value.ToString());
                    ...
                    if (singletondirectack.isEmpty())
                    {
                        ...
                        else if (!string.IsNullOrEmpty(smsline))
                        {
                            ...
                            else
                            {
                                ...
                                //smsmessage = "😀N oubliez pas votre RV!";
                                //pduformat = SmsBox.SMS.SMSEncoding.UCS2;
                                ...
                                if ((pduformat == SMS.SMSEncoding.UCS2 && smsmessage.Length <= 70) 
                                    || (pduformat == SMS.SMSEncoding._8bit && smsmessage.Length <= 140)
                                    || (pduformat == SMS.SMSEncoding._7bit && smsmessage.Length <= 160))
                                {
                                    LogHelper.WriteLine(ELogLevel.log, "sending in PDU mode in single part with encoding : " + encoding);

                                    sentreturncode = wrapper.SendSmsPdu(smsnumero, smsmessage, out sentwithsuccess, -1, -1, -1, pduformat);
                                }
                                else
                                {
                                    ...
                                    int packetsize;
                                    if (pduformat == SMS.SMSEncoding.UCS2)
                                        packetsize = 67; //(140-6)/2
                                    else if (pduformat == SMS.SMSEncoding._8bit)
                                        packetsize = 134; //(140-6)
                                    else //7bit
                                        packetsize = 153; // 153; //(160-7)
                                    ...
                                    while (true)
                                    {
                                        ...
                                        sentreturncode = wrapper.SendSmsPdu(smsnumero, currentmsg, out sentwithsuccess, sendsmsgroupeindex, nbpart, currentpart, pduformat);
                    ...
                    if (listsms.Length != 0)
                    {
                        ...
                        foreach (SmsShieldReceivedSms sms in listsms)
                        {
                            ...
                            else if (smstosend.message.Equals("mobhalt"))
                            {
                                LogHelper.WriteLine(ELogLevel.log, "query from mobminder = " + smstosend.message);
                                callmobhalt = true;

                            }
                            else if (smstosend.message.Equals("mobrestart"))
                            {
                                LogHelper.WriteLine(ELogLevel.log, "query from mobminder = " + smstosend.message);
                                callmobrestart = true;
                            }
                            else if (smstosend.message.Equals("mobstop"))
                            {
                                LogHelper.WriteLine(ELogLevel.log, "query from mobminder = " + smstosend.message);
                                callmobstop = true;
                            }
