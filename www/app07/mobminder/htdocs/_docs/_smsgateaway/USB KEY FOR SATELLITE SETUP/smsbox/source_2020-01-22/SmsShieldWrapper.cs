...
namespace SmsBox
{
    public class SmsShieldWrapper
    {
        ...
        public bool SendSmsPdu(string numero, string msg, out bool sentwithsuccess, int groupeindex, int nbpart, int currentpart, SMS.SMSEncoding pduformat)
        {
            if (!wrapper.SendSmsPdu(numero, msg, false, out sentwithsuccess, groupeindex, nbpart, currentpart, pduformat))
            {
                ErrorMessage = new CascadeError(errorprefix + "sending sms", wrapper.ErrorMessage);
                return false;
            }
