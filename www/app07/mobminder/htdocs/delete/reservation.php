...
$bank = @$_POST['bank']; if(!isset($bank)) $bank = 'plitems'; else $bank = substr($bank,0,10); // Options are ['plitems','visiapps','catalyst'], // see (*mr20*)
...
if($attendee_impact_only) $dS_resa->dSsave(); // changes tracking
	else $dS_resa->dSobsolete(); //
	
$perfReport->peak('::deleted');

echo '<code>';
echo '#C_dS_reservation#'.$bank.chr(10).$dS_resa->stream(with_tracking).$nl;
echo '</code>';
echo $nl;
