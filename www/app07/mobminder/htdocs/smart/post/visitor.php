...
if($id>0) { // PVH 2025-01 : preserve the older versions of the smartapp from the new language_code_luxembourgish and gender_code_other
	$dS_visitorPrevious = new C_dS_visitor($id);
	if($dS_visitorPrevious->language==language_code_luxembourgish)
		if(isset($_REQUEST['language'])) $_REQUEST['language'] = language_code_luxembourgish; // keep the existing value if it was luxembourgish
	if($dS_visitorPrevious->gender==gender_code_other)
		if(isset($_REQUEST['gender'])) $_REQUEST['gender'] = gender_code_other; // keep the existing value if it was gender_code_other
}
