...
setrendermode();
set_urls($subdomainname = 'smart'); // see (*dv01*)
...
function fieldvalidation(&$value, $fname, $context, $strong = true) { // field by field checkup on allowed characters
	...
	switch($fname) {
		...
		case 'language':
			...
			if($l<0 || $l>8) abort('0088','Wrong language value:'.$l);
	...
	static public function fieldslist($classname) {
		...
		switch($classname) { 
			...
			case 'C_dS_login':
					...
					$options 	= Array('GMT','weeknumb','color','permissions','accessLevel','tag','decision','aienabled'); // PVH 2025-03: added 'decision','aienabled'
...
class C_apicontext {
	...
	public $resources;
	public $rsc_correllators; // C_dS_synchro_resources
	public $customCsss;
