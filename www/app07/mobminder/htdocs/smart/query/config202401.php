...
// 	  
//		$fieldsA = Array() contains now :
//			'groupId'  <= for wallet specific settings (UNEB)
//			'ePayActive', 'ePayBenefName', 'ePayBenefIBAN', 'ePayBenefBIC', 'ePayconiqKey', 'ePayMarketKey'
//		
// 	  	$fieldsW = Array() contains now : 'price','deposit'
//
...
//   D R O P    D A T A   T O    C L I E N T    S I D E
	...
	$stream = '';
		...
		//    A C C O U N T
			$fieldsA = Array('groupId','id','name','color','pattern','tag','note','language'
			...
			, 'ePayActive', 'ePayBenefName', 'ePayBenefIBAN', 'ePayBenefBIC', 'ePayconiqKey', 'ePayMarketKey'
			, 'ePayHardPayClientId','ePayHardPayClientSecret','ePaySoftPayClientId','ePaySoftPayClientSecret','ePaySoftPayAppId'
			);
			...
			if ($context->dS_account->ePayBenefName) 			$context->dS_account->ePayBenefName='1';
			if ($context->dS_account->ePayBenefIBAN) 			$context->dS_account->ePayBenefIBAN='1';
			if ($context->dS_account->ePayBenefBIC) 			$context->dS_account->ePayBenefBIC='1';
			if ($context->dS_account->ePayconiqKey) 			$context->dS_account->ePayconiqKey='1';
			if ($context->dS_account->ePayHardPayClientId) 		$context->dS_account->ePayHardPayClientId='1';
			if ($context->dS_account->ePayHardPayClientSecret) 	$context->dS_account->ePayHardPayClientSecret='1';
			if ($context->dS_account->ePaySoftPayClientId) 		$context->dS_account->ePaySoftPayClientId='1';
			if ($context->dS_account->ePaySoftPayClientSecret) 	$context->dS_account->ePaySoftPayClientSecret='1';
			if ($context->dS_account->ePaySoftPayAppId) 		$context->dS_account->ePaySoftPayAppId='1';
	...
	// W O R K   C O D E S
		...
		//    W O R K     C O D E S  (keep this after experts streaming)
			$fieldsW = Array('id','name','note','duration','staffing','cssColor','cssPattern','tag','tags','ereservable','price','deposit');
	...
	//
		...
		$stream.= '#C_dS_system'.$nl.$dS_system->stream(no_tracking, '|');
...
// if paymean alive counter is greater than limit, then paymean is disabled and booking is allowed without any payment
//provider_threshold==0 => check is disabled
echo 'payalivelimit='.C_dS_system::$provider_threshold.$nl;  	
