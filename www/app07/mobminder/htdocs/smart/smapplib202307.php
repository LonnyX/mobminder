...
function fieldvalidation(&$value, $fname, $context, $strong = true) { // field by field checkup on allowed characters
	...
	switch($fname) {
		...
		case 'language':
			...
			if($l<0 || $l>7) abort('0088','Wrong language value:'.$l); // must be adjusted if a new enum value is defined, see (*lc01*)
		...
		case 'gender': // see (*gc01*)
			...
			if($g<0 || $g>6) abort('0089','Wrong gender value:'.$g.''); // must be adjusted if a new enum value is defined, see (*gc01*)
			return true;
