...
//////////////////////////////////////////////////////////////////////////////////////////////
//
//  A P I   /   P O S T    R E S E R V A T I O N 
//
//
...
h2('Input fields check up');
	...
	$visitors = @$_REQUEST['visitors']; if(!isset($visitors)) $visitors = false; if($visitors) $visitors = explode('!', $visitors); 
	...
	if($replan) indent('o replanned reservation id: '.$replan,6);
	...
	if($isanewresa) { 
		...
		h3('Checking for mandatory fields');
			...
			if(!$cueOut) abort('5511','missing mandatory field: cueOut');
			indent('cueIn: '.$cueIn,6);
			indent('cueOut: '.$cueOut,6);
		
			if($total_posted_attendees == 0) abort('5512','for a new reservation, one of bCals, uCals or fCals is mandatory');
			
			foreach($posted_attendees as $class => $cals) {
				$t = $context->get_rclass_type($class);
				$c = $count_posted_attendees[$class];
				if($c) indent('o '.$t.': '.implode(', ',$cals).' will be attached to the reservation if ids are valid',6);
					else indent('o '.$t.': none (the created reservation will have no '.$t.' attached)',6);
			}
		...
		h3('Optional fields');
			...
			if($visitors===false) indent('o visitors: ommited (the created reservation will have no visitor attached)',6);
				else if($visicount==0) indent('o visitors: no visitor will be attached to the reservation',6);
					else indent('o visitors: '.implode(', ',$visitors).' will be attached to the reservation if ids are valid',6);
		
			
			if($workcodes===false) indent('o workcodes: ommited (the created reservation will have no performance attached)',6);
				else if($wkcount==0) indent('o workcodes: the created reservation will have no performance attached',6);
					else indent('o workcodes: '.implode(', ',$workcodes).' will be attached to the reservation if ids are valid',6);
			...
			if($expires!==false) {
				...
				if($expires==0) {
					indent('o pre-booking: preb is zero, ignored',9);
					$expires = false;
				}
				
				indent('o expires: '.$expires.'min from reservation deletion',6);
			}
			...
			if($bookingcode!==false) {
				...
				indent('o bookingcode: '.$bookingcode.'',6);
			}
			...
			if($recurr!==false) {
				...
				foreach($recurr as $rcuein) {
					indent('o recurrent reservation: -'.(++$a).'- '.Date('Y-m-d H:i', $context->timeStampToUTC($rcuein, $utc)),9);
				}
			}
			...
			h3('Optional fields');
				...
				foreach($posted_attendees as $class => $cals) {
					...
					if($cals!==false) { // the post contains ids for resources
						$total_attendees += $c;
						if($c) indent('o '.$t.': '.implode(', ',$cals).' will be attached to the reservation if ids are valid',6);
							else indent('o '.$t.': will be removed if any',6);
					}
						...
						if($c) indent('o '.$t.': ommited in input parameters, attached resources remain identical for type '.$t.':'.implode(', ',$cals),6);
							else indent('o '.$t.': ommited in input parameters, the existing reservation had no such type attached',6);
					}
					unset($cals);
				}
				...
				if($visitors===false) {
						$vs = $visitors = $resa_as_existing->visitors->getKeysList();
						$c = $visicount = count($visitors);
						if($c) indent('o visitors: ommited in input parameters, attached visitors remain identical:'.implode(', ',$vs),6);
							else indent('o visitors: ommited in input parameters, the existing reservation had no visitors attached',6);
					}
					else if($visicount==0) indent('o visitors: visitors if any will be removed from the existing reservation',6);
						else indent('o visitors: '.implode(', ',$visitors).' will be attached to the reservation if ids are valid',6);
				...
				if($workcodes===false)  {
						...
						if($c) indent('o workcodes: ommited in input parameters, attached workcodes remain identical:'.implode(', ',$wks),6);
							else indent('o workcodes: ommited in input parameters, the existing reservation had no workcode attached',6);
					}
					else if($wkcount==0) indent('o workcodes: workcodes if any will be removed from the existing reservation',6);
						else indent('o workcodes: '.implode(', ',$workcodes).' will be attached to the reservation if ids are valid',6);
				...
				if(!$expires) { // omitted or zero
					...
					if($q->ids()) {
						...
						if($expires==0) indent('o pre-booking: preb is zero but this reservation is not in the pending pre-booking queue. Discarding preb.',6);
					}
				}
				else indent('o pre-booking: updating the pre-booking expire timer: '.$expires.'min',6);
				
				if(isset($stitle)) 
					indent('o stitle: update the serie title to '.$stitle.'',6);
		}
	...
	h3('Fields format/value validation');
		...
		if($visitors) 
			...
			foreach($visitors as $vid) {
				...
				if(!$q->c()) abort('5052','You must specify a valid visitor id:'.$vid.' is unknown');
					else indent('o visitor id :'.$vid.' is valid',6);
			}
			pad(0);
		}
		...
		foreach($posted_attendees as $class => $cals) {
			...
			if($cals) {
				...
				if($c) {
					foreach($cals as $rid) 
						if($rname = $context->isValidResource($rid)) indent('o resource id :'.$rid.' is an existing resource: '.$rname,12);	
						else abort('5053','You must specify valid resource ids. '.$rid.' is unknown');
					foreach($cals as $rid) 
						if($rcl = $context->isValidResourceClass($rid, $class)) indent('o resource id :'.$rid.' has the right resource type: '.$class,12);	
		...
		if($replan)
			if($replan==$resaId) abort('5060','the replanned reservation must be different from the newly planned reservation');
				else if($context->isValidReservation($replan)) indent('o reservation id :'.$id.' is valid',6);
...
if($archived) {
    ...
    //
    $dS_reservation = new C_dS_reservation(0, $accId, $_REQUEST); // archived resa goes back to live table and gets a new id
...
if($isexistingresa) // PVH-2020-11 fix limited view resa post problem
	if($context->is_limited_view) { // add attendees that are out of view scope (complete the set with resources that are out of login's view, so they receive the expected treatment)
		...
		indent('Out of view attendees: '.implode(',',$as_is_resources),6); // should restore resources eventually out of view scope BUT NOT the ones in the view scope
		$to_be_resources = $attendees->getResourceIdsList(list_as_array);
		indent('Posted in view attendees: '.implode(',',$to_be_resources),6); // list of resources as passed through the POST, view limited players can only mange and post in view scope resources 
		...
		foreach($as_is_resources as $rscid => $v)
			if(!array_key_exists($rscid,$to_be_resources)) {
				...
				indent('- adding rscid : '.$rscid,9);
			}	
		indent('Final attendees set: '.$attendees->getResourceIdsList(),6); // list of resources as passed through the POST, view limited players can only mange and post in view scope resources 
		pad();
	}
...
//
$formerly_existing_attendees = false; // will contain only attendees that can be managed by this login
...
$dS_reservation->deleted = 0; // when saving a deleted reservation, you restore it
...
if($archived) {
    ...
    new Q('update reservations set id = '.$archived.' where id = '.$resaId.';'); // see (*23*), the reservation as moved to the current reservations table but keeps the same id
...
span('&ltdata&gt');
	...
	// echo $resaparts->stream(false, $bank);
	// echo $toggles->stream();
			
		// $fieldsR = Array('id','created','creator','creatorId','changed','changer','changerId','deleted','deletorId'
	...
	echo $rec_resas->addmeta_prebooking()->setstringtimeformat($utc)->stream(no_alias, no_bank, no_tracking, $sep = '|', $flds = $fieldsR); // embraces recurrent or non recurrent dS_reservation(s)
	
		$fieldsS = Array('id','created','creator','creatorId','changed','changer','changerId','deleted','deletorId','stitle');
	if($dS_serie) echo '#C_dS_resa_serie'.'#'.$bank.$nl.$dS_serie->stream(no_tracking, '|', $fieldsS).$nl;
...
pad();
	...
	if($dS_serie) explainclass($dS_serie		, $fieldsS, '|');
...
if(!$expires) 
{
    ...
    if($formerly_existing_attendees) 
        $dS_cronofy_manager_reservation->DeleteByAttendees($dS_resa,$formerly_existing_attendees); // which is limited to the view
