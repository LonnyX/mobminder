...
$where 	= 'WHERE reservations.groupId = '.$accountId.' AND resourceId IN ('.$keyview->get4sql().') AND ('.$deleted.' OR '.$changed.' OR '.$created.') '.$exclude;
...
if($resas->count()) {
	...
	$ccss = new C_dbAccess_customCss($dS_accesskey->accountId);
	...
	// here we associate the custom ccss remoteId with the mobminder ccss ids
	$rccss = new C_dbAccess_synchro_ccss($skeyId);
		foreach($ccss->keyed as $cid => $dS_ccss) $dS_ccss->remoteId = ''; // empty when not synced (some colors may not be synced)
	...
	$remvis = new C_dbAccess_synchro_visitors(); if($visiIds) $remvis->loadOnLocalId($visiIds, $skeyId);
		if($visiIds) foreach($visis->keyed as $vid => $dS_v) $dS_v->remoteId = 0; // 0 is default when not synced
	...
	foreach($resas->keyed as $resaId => $dS) {
		...
		if($dS->cssColor) { // ccount may be zero when no colour is applied
			$rccss = $ccss->get($dS->cssColor,'','remoteId'); // do not communicate about custom css that are not synced
		}
		
		// visitors
		$vids = Array(); $rvids = Array(); $vcount = 0; if(isset($attvs->grouped[$resaId])) $vcount = count($attvs->grouped[$resaId]);
		if($vcount) foreach($attvs->grouped[$resaId] as $attId => $d) {
			$v = $visis->get($d->resourceId);
			$vids[] = $v->id; 
			$rvids[] = $v->remoteId; 
		}
		$vids = implode('!',$vids);
...
if($web) echo '<file>'; // enclose the filecontent within the stream
	...
	echo $resas->csvstream($filename, $fields);
if($web) echo '</file>';
