...
//////////////////////////////////////////////////////////////////////////////// 
//
//  IMPORTS APPOINTMENTS FROM EPICURE EXPORTS - link patients through their mobile number
//
//	following columns are expected:
//
//  appdate	appfrom	appto	name(lastname+firstname)	mobile	phone	registration	note
//
//
//  Usage: http://127.0.0.1/utilities/epicure.php?id=2891&rsc=7160   <= test mode
//  Usage: http://127.0.0.1/utilities/epicure.php?id=3059&rsc=7422&cleanup=1&do=1   <= clean up and action mode
// 
//  http://be.mobminder.com/utilities/epicure.php?id=3510&rsc=9531&do=1
//
//  The filename has a rigid format, like 2891_7160_apps.csv';
//
//  !! 1- IMPORT VISITORS FIRST !!
//
//
...
if(!in_array('name',$headers)) error('Missing mandatory header: combined lastname firstname', $handle);
...
// mandatory fields
$ln_position = array_keys($headers, 'name'); 	$ln_position = $ln_position[0]; 
...
$byName = Array(); // an array that references the visitors using their original id from (id in the migrated DB)
	...
	$SQL = 'SELECT id, mobile, lastname, firstname FROM visitors WHERE groupId = '.$groupId.';';
...
for($x = $from-1; $x<$to; $x++,$from++) { 
		...
		$phone = $split[$mb_position];  
		$appNote = $split[$nt_position]; 
			
		if($mobile) $mobile = '33'.$mobile; else $mobile = false;
		$name = str_replace(' ','',strtolower($lastname));
		$visiId = false; $match = '';
		if($mobile) {
		...
		if(!$visiId) {
			notice('<b>NO MATCH: </b>'.($nomatch++).' (line '.$x.', no reference for the visitor:'.$lastname.')');
			$appNote = $lastname.', '.$mobile.' - '.$appNote;
			
			if(($lastname&&$mobile)||($lastname&&$phone)) {
				// we create a visitor based on the info given in the epicure appointment
				...
				$dd = substr($date,0,2); // date is originally in epicure like 26-08-16
				$mm = substr($date,3,2);
		...
		$o_dS_reservation->note = $appNote; //iconv('ISO-8859-1', 'UTF-8', $appNote); the file is already UTF-8
