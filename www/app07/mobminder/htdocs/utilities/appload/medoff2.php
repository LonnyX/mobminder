...
if(1)
	...
	$expected_headers = array('resourceId','appfrom','appto','reference','note','perfId');
...
$lcount = count($lines);
$out .= notice('The file has been read in: '.$cuedelta.' mseconds, it counts '.$lcount.' valid lines');
fclose($handle);
...
$cueout = microtime(true);
$cuedelta = (($cueout-$cuein)*1000)|0;
$out .= notice($visiTotal.' visitors have been loaded in: '.$cuedelta.' mseconds');
...
foreach($lines as $x => $split) { // scans the appointments csv
	...
	$cuein = $split[$ph['appfrom']]; $cuein = substr($cuein,0,2).':'.substr($cuein,-2);
	$cueout = $split[$ph['appto']];  $cueout = substr($cueout,0,2).':'.substr($cueout,-2);
	$visiref = $split[$ph['reference']];  
	$appnote = $split[$ph['note']];   
	$performance = $split[$ph['perfId']];  
	...
	if($visiref!==''&&$visiref!=='0') {
		...
		if(0) {
			$visiref = strtolower(trim($visiref));
			...
			if(isset($byReference[$visiref])) { // best case
				...
				$subname = '';
				$pattern = "/\d{2}\/\d{2}\/\d{4}/";
				if(preg_match($pattern, $visiref, $matches)) {
					$pos = strpos($visiref, $matches[0]);
					// $out .= warning('Found date '.$matches[0].' position '.$pos.' from '.$visiref);
					$subwbd = substr($visiref,0,$pos+10);
					$subname = substr($visiref,0,$pos-1);
					// $out .= warning('Cut reference '.$subwbd.' subname:'.$subname);
				}
				if(isset($subwbd)) { 
					if(isset($byReference[$subwbd])) { // best case
						$visiId = $byReference[$subwbd];
						$matchcount++;
					}
				} else {
					$pattern = "/\d{2}\/\d{2}\/\d{2}/";
					if(preg_match($pattern, $visiref, $matches)) {
						$pos = strpos($visiref, $matches[0]);
						// $out .= notice('Found date '.$matches[0].' position '.$pos.' from '.$visiref);
						$subname = substr($visiref,0,$pos-1);
						// $out .= notice('Cut reference '.$subname);
					}
					if($subname){
						if(isset($byName[$subname])) { // match on name only
							$visiId = $byName[$subname];
							$matchonname++;
						}
					} else {
						$nomatchcount++;
	...
	if($performance) {
		if(0) {
			switch(trim($performance)) {
				// Souery
				case 'Consult. Dr Souery': 	$performance = 24839; break;
				case 'RDV Dr Souery 00:30': $performance = 24839; break;
				// case 'Nouveau Patient': 	$performance = 24841; break;
				case 'LABO Délégués': 		$performance = 24840; break;
				case 'Réunion': 			$performance = 24482; break;
				
				// Du Roy
				case 'psy': 			$performance = 24843; break;
				case 'la ramée': 		$performance = 24905; break;
				case 'Nouveau patient': $performance = 24841; break;
				case 'Consultation Aqueduc': $performance = 24904; break;
				case 'Consultation': 	$performance = 24843; break;
				
				default:
					$out.= warning('Ouuups! this performance was not expected: '.$performance);
					$performance = 0;
			}
		}
	}
	...
	if(isset($appnote))
		if(!$visiId) $o_dS_reservation->note = $visiref.chr(10).$appnote;
		else $o_dS_reservation->note = $appnote;
	if($verbose) {
...
$out .= h2(''.$icount.' items have been inserted. Scanned in: '.$cuedelta.' mseconds');
