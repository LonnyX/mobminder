...
//////////////////////////////////////////////////////////////////////////////// 
//
//  IMPORTS APPOINTMENTS FOR BUROGEST
//
//  Usage: localhost/utilities/google_csv.php?id=3636&rsc=10389 (Sébastien)
//  Usage: localhost/utilities/google_csv.php?id=3636&rsc=10632 (Joannes)
//
...
if(!isset($from)) { // then this page is called for the first time, echo request for sending
	...
	$html->pushHTML('<a href="'.$thisScript.'?id='.$groupId.'&rsc='.$rescId.'&f=1">Click to Start creating the appointments</a>');
	$html->dropPage(); 
	die();
}
...
//  LOADING VISITORS
//
if(false) // manual switch if former reservations should be deleted (we have switched this off because holidays where removed while the account was made ready by Séverine)
	if($from==1) { // first pass, remove older stuff
		
		// current table
		$resas = new C_dbAccess_reservations(); $resas->loadOnGroup($groupId);
		$resaIds = $resas->getIdsList();
		
		if($resaIds != '') {
			
			new Q('DELETE FROM attendees WHERE groupId IN ('.$groupId.');');
			
			new Q('DELETE FROM att_visitors WHERE groupId IN ('.$groupId.');');
			
			new Q('DELETE FROM reservations WHERE id IN ('.$resaIds.');');
		}
		
		// archive table
		$resas = new C_dbAccess_reservations('archive_'); $resas->loadOnGroup($groupId);
		$resaIds = $resas->getIdsList();
		
		if($resaIds != '') {
			
			new Q('DELETE FROM archive_attendees WHERE groupId IN ('.$groupId.');');
			
			new Q('DELETE FROM archive_att_visitors WHERE groupId IN ('.$groupId.');');
...
for($x = $from-1; $x<$to; $x++,$from++) { 
		...
		$iscan++;
		$split = $table[$x];
		$note = $split[0]; $cueIn = $split[1]; $cueOut = $split[2]; // fixed expected headers position is used here
		
		// if(!isset($byRegistr[$visiId])) {
			// notice('<b>Skipping line: </b>'.$x.' (no reference for the visitor:'.$visiId.')'); continue;
		// } else $visiId = $byRegistr[$visiId];
		
			$cueIn = new C_date($cueIn);
			$cueOut = new C_date($cueOut);
		...
		$o_dS_reservation->note = utf8_encode($note);
...
if($from < $total) {
	...
	$html->pushHTML('<a href="'.$thisScript.'?id='.$groupId.'&rsc='.$rescId.'&f='.$from.'">Click to Start sending next bunch</a>');
	$html->dropPage(); 
	die();
}
