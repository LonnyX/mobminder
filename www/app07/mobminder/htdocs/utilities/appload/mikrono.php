...
//////////////////////////////////////////////////////////////////////////////// 
//
//  imports APPOINTMENTS from mikrono exports - link patients through their mobile number
//
...
//
//  Usage: http://localhost/utilities/mikrono.php?id=2944   <= single account, or multi with resourceid field in input file
//  Usage: http://localhost/utilities/mikrono.php?id=2944&rsc=9994   <= multi account with specific resource
//  Usage: http://localhost/utilities/mikrono.php?id=3059&rsc=7422&cleanup=1&do=1   <= clean up and action mode
// 
//  http://be.mobminder.com/utilities/mikrono.php?id=3510&rsc=9531&do=1
//
//  
//	Pre-requisites: 
//
...
//
//	2. set this headers at the top of mikrono file and remove unnecessary columns
//
...
//  HTML Headers 
//
$out .= h1('mobminder appointments generator from mikrono export');
...
if($cleanup) { // first pass, remove older stuff
	...
	// current table
		...
		$q = new Q('select id from reservations where groupId = '.$accountId.';');
		$resaIds = $q->ids();
	...
	// archive table
		...
		$q = new Q('select id from archive_reservations where groupId = '.$accountId.';');
		$resaIds = $q->ids();
...
if(!in_array('appfrom',$headers)) error('Missing mandatory header: appfrom', $handle); // YYYY-MM-DD HH:MM
if(!in_array('appto',$headers)) error('Missing mandatory header: appto', $handle); // YYYY-MM-DD HH:MM
...
if(!in_array('birthday',$headers)) error('Missing mandatory header: mobile', $handle);
...
$bh_position = array_keys($headers, 'birthday'); 		$bh_position = $bh_position[0]; 
...
$lines = array();
	while (!feof($handle)) {
		...
		if(substr($line,0,1)=='D') continue; // skips mikrono header lines
...
$byBirthday = Array(); // an array that references the visitors using their original id from (id in the migrated DB)
$byName = Array(); // an array that references the visitors using their original id from (id in the migrated DB)

	$SQL = 'SELECT id, mobile, birthday, lastname, firstname FROM visitors WHERE groupId = '.$accountId.';';
	...
	while ($row = $result->fetch_array()) {
		...
		if($m) $byMobile[$name.$m] = $row['id'];
		
			$p = $row['birthday'];
		if($p) $byBirthday[$name.$p] = $row['id'];
...
foreach($lines as $x => $split) {
		...
		$birthday = preg_replace('/[^0-9]/', '', $split[$bh_position]);
		...
		if($mobile) {
			$m = substr($mobile,-9);
			if(isset($byMobile[$name.$m])) {
				$visiId = $byMobile[$name.$m];
		...
		if(!$visiId) {
			if($birthday) {
				$b = substr($birthday,-9);
				if(isset($byBirthday[$name.$b])) {
					$visiId = $byBirthday[$name.$b];
		...
		if(!$visiId) {
			$out .= warning(($novmatch++).' <b>NO VISITOR MATCH: </b>(line '.$x.', no reference for the visitor:'.$lastname.', '.$firstname.')');
			$appNote = $lastname.', '.$firstname.' '.$birthday.' '.$mobile.' - '.$appNote;
			
			// we create a visitor based on the info given in the mikrono appointment (uncomment if needed)
		...
		$o_dS_reservation->note = $appNote;
			$out .= notice('<b>Appointment: </b>'.$match.' => '.$cueIn->getDateTimeString().' to '.$cueOut->getDateTimeString().' with '.$name); // debug
...
$cuedelta = (($cueout-$cuein)*1000)|0;
	...
	$out .= notice('<b>New visitors should be created or linked: </b>'.$newVisiCount.' in total.');
