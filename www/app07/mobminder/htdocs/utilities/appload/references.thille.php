...
//	- merging ? if multiple csv file, copy *.csv fichier_final.csv
//  - import csv content into excel file
//	- remove empty lines
//	- remove duplicated header lines (in case of merging file)
//	- remove unused columns
// 	- prepare datettime column
//		if 2 column date & time => formula = Col 1 + col 2
// 		custom format = aaaa-mm-jj hh:mm
//		remove date & time unused columns 
//		rename datetimein & datetimeout
//	- rename reference column
//	- rename note column
//  - rename performance	
//	- clean invalid characters (accent?)
//		=SUBSTITUE(SUBSTITUE(SUBSTITUE([@address];"?ois";"çois");"?";"e");"\/";"/")
//	- performance mapping
//		create a new references.php file (copy/paste)
//		configure performance mapping switch cases
//	- check if empty reference to visitor => try to copy visitor name into note column
//		=SI(ET(SUPPRESPACE(A2)=""; SUPPRESPACE(BE2)<>""); CONCATENATE(B2;", ";C2); C2)
//  - save as csv file
//  - try to convert to utf-8 with notepad++ or notepad (if notepad++ doesn't)
//	- testing
// 		http://192.168.0.16/mobminder/htdocs/utilities/appload/references.thille.php?id=2935
//		check log traces
//		do=1 to go produciton
//		cleanup=1 if necessary
// - check datetime in and out in webapp
// - do not forget to configure resource mapping
//
// // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
// !!! after files imports : remove all CSV file from production server!
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
// $iterator = new FilesystemIterator($cwd);
// echo "<h3>" . $iterator->getPath() . "</h3>";
// foreach ($iterator as $entry) {
    // $name = $entry->getBasename();

    // if (is_dir($cwd . '/' . $name)) {
        // echo "D: <a href='?dir=" . $cwd . "/" . $name . "'>" . $name . "</a><br />";
    // } elseif (is_file($cwd . '/' . $name)) {
		// if(substr($name,-3)=='csv')
			// echo "F: <a href='?download=" . $cwd . '/' . $name . "' download='" . $name . "'> " . $name . " </a><br />";
    // }
// }
...
if(in_array('note',$headers)) { // appointment note
	...
	if(!in_array('note2',$headers)) {
		$has_note2 = true;
		$positions['note2'] = array_search('note2', $headers);
	}
	
	if(!in_array('note3',$headers)) {
		$has_note3 = true;
		$positions['note3'] = array_search('note3', $headers);
	}
}
...
function eu_rewritedate($datetime) {  // datetime arrives like 15/01/2016 10:30  
	...
	$in_date = substr($datetime,0,10); // like 15/01/2016
	$in_time = substr($datetime,11,5); // input like '10:30:00' turns into '10:30'
	
	$h = substr($in_time,0,2); 
	$m = substr($in_time,3,2);
...
foreach($lines as $x => $split) {
	...
	if($has_resc_name) { 
		...
		switch($resource) {
			...
			//try joly
			//case 'lagrou': 	$rescId = 22124; break; // CB
	...
	$perfid = false;
	if($has_perf_name) { 
		$perfalias = $split[$positions['performance']];   
		switch($perfalias) { // this mapping is manual and should be adapted for each import

			//case 'Renouvellement de traitement, Vaccination': $perfid = 54036; break;

			//Motif UBicentrex :  Bilan Sénologique 
			//Nom Mobminder : BS 
			//ID Mobminder : 57511
			case 'Bilan Sénologique': 	$perfid = 57511; break; 

			//Motif UBicentrex :  Bilan Sénologique + Osteodensitométrie
			//Nom Mobminder : MAMMO + OSTEO
			//ID Mobminder : 57519
			case 'Bilan Sénologique + Osteodensitométrie': 	$perfid = 57519; break; 

			//Motif UBicentrex :  Angiomammographie
			//Nom Mobminder : ANGIOMAMMO
			//ID Mobminder : 57520
			case 'Angiomammographie': 	$perfid = 57520; break; 

			//Motif UBicentrex :  Biopsie 30' S/ASPI
			//Nom Mobminder : BIOPSIE
			//ID Mobminder : 57512
			case 'Biopsie 30\' S/ASPI': 	$perfid = 57512; break; 

			//Motif UBicentrex :  Biopsie 30' S/Echographie
			//Nom Mobminder : BIOPSIE
			//ID Mobminder : 57521 (même que précédemment)
			case 'Biopsie 30\' S/Echographie': 	$perfid = 57512; break; 

			//Motif UBicentrex :  Biopsie 45' S/TOMO
			//Nom Mobminder : BIOPSIE
			//ID Mobminder : 57521 (même que précédemment)
			case 'Biopsie 45\' S/TOMO': 	$perfid = 57512; break; 

			//Motif UBicentrex :  Echo Pelvienne Dr Thille
			//Nom Mobminder : ECHO PELVIENNE
			//ID Mobminder : 57523
			case 'Echo Pelvienne Dr Thille': 	$perfid = 57523; break; 

		
			//Motif UBicentrex :  Echographie
			//Nom Mobminder : ECHO
			//ID Mobminder : 57514
			case 'Echographie': 	$perfid = 57514; break; 

			//Motif UBicentrex :  Mammotest
			//Nom Mobminder : MAMMOTEST
			//ID Mobminder : 57516
			case 'Mammotest': 	$perfid = 57516; break; 

			//Motif UBicentrex :  Osteodensitométrie
			//Nom Mobminder : OSTEO
			//ID Mobminder : 57518
			case 'Osteodensitométrie': 	$perfid = 57518; break; 

			//Motif UBicentrex :  Rendez-vous
			//Nom Mobminder : MAMMO + OSTEO
			//ID Mobminder : 57519
			//(ce sont des rdvs chez d'autres praticiens mais ceux-ci sont identifiés avec des blocs horaires et non des ressources
			case 'Rendez-vous': 	$perfid = 57519; break; 

			//Motif UBicentrex :  URGENCE
			//Nom Mobminder : URGENCE
			//ID Mobminder : 57521
			case 'URGENCE': 	$perfid = 57521; break; 
	...
	$pivot = new C_date();
	$pivot->mIncrement(-20*12); // we limit the import to the 7 last years.
