...
//////////////////////////////////////////////////////////////////////////////// 
//
//  IMPORTS APPOINTMENTS FROM CLIC RDV EXPORTS - link patients through their mobile number
//
...
//
//
//  Usage: http://localhost/utilities/clicrdv.php?id=2944&rsc=9994   <= test mode
//  Usage: http://localhost/utilities/clicrdv.php?id=3059&rsc=7422&cleanup=1&do=1   <= clean up and action mode
// 
//  http://be.mobminder.com/utilities/clicrdv.php?id=3510&rsc=9531&do=1
//
//  The filename has a rigid format, like 2891_7160_apps.csv';
//  !! IMPORT VISITORS FIRST !!
//
//  2950_12814_apps  (Dr Lebecq)
//  2950_12815_apps  (Dr Van Melbeckeke)
...
function rewritedate($input) {  // datetime arrives like 15-01-2016 10:30:00
	
	$output = '';
	$in_time = substr($input,-9); // like ' 10:30'
...
for($x = $from-1; $x<$to; $x++,$from++) { 
		...
		if($mobile) {
			$mobile = preg_replace("/[^0-9']/si",'',$mobile);
			$mobile = '32'.substr($mobile,-9); 
		} else $mobile = false;
		...
		if(!$visiId) {
			notice('<b>NO MATCH: </b>'.($nomatch++).' (line '.$x.', no reference for the visitor:'.$lastname.', '.$firstname.', mobile:'.$mobile.')');
