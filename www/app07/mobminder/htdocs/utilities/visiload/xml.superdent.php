...
//
//  Usage: 127.0.0.1/utilities/xml.superdent.php?fn=roland.xml&gid=3014
//
...
while (!feof($handle)) {
	...
	if($line=='<patient_medical_info>') {  // starting a patient section
		...
		$patient = array('gender'=>false, 'firstname'=>false, 'lastname'=>false, 'birthday'=>'', 'mobile'=>'', 'phone'=>'', 'address'=>'', 'zipCode'=>'', 'city'=>'', 'country'=>''); }
	...
	if($inPatient) {
		...
		if($inPersonalData) {
			if(strpos($line,'<First_name>')!==false) $patient['firstname'] = getTextBetweenTags($line,'First_name');
			if(strpos($line,'<Name>')!==false) $patient['lastname'] = getTextBetweenTags($line,'Name');
			if(strpos($line,'<Sexe>')!==false) {
				$v = getTextBetweenTags($line,'Sexe');
				$patient['gender'] = $v == 'M' ? 1 : 0;
			}
			if(strpos($line,'<Date_of_birth>')!==false) {
				$v = getTextBetweenTags($line,'Date_of_birth');
				if($v != '') {
					$split = explode('/',$v); 
					if(strlen($split[0])==1) $split[0] = '0'.$split[0];
					if(strlen($split[1])==1) $split[1] = '0'.$split[1];
			...
			if(strpos($line,'<Phone1>')!==false) {
				$number = getTextBetweenTags($line,'Phone1');
				if($number) {
					$number = str_replace('.' , '', $number);
					$number = str_replace('/' , '', $number);
					$number = str_replace(' ' , '', $number);
					$number = str_replace('-' , '', $number);
					if(strlen($number)==6) $patient['phone'] = '071'.$number; // Charleroi
					if(strlen($number)==9) $patient['phone'] = $number;
					if(strlen($number)==10) $patient['mobile'] = $number;
				}
			}
			if(strpos($line,'<Phone2>')!==false) {
				$number = getTextBetweenTags($line,'Phone2');
				if($number) {
					$number = str_replace('.' , '', $number);
					$number = str_replace('/' , '', $number);
					$number = str_replace(' ' , '', $number);
			...
			if(strpos($line,'<Address>')!==false) $patient['address'] = getTextBetweenTags($line,'Address');
			if(strpos($line,'<Postal_code>')!==false) $patient['zipCode'] = getTextBetweenTags($line,'Postal_code');
			if(strpos($line,'<City>')!==false) $patient['city'] = getTextBetweenTags($line,'City');
			if(strpos($line,'<Country>')!==false) $patient['country'] = getTextBetweenTags($line,'Country');
		}
		if($inAddressData) { // this section does not exist in superdent exports, it resides in personal_data
			if(strpos($line,'<Address>')!==false) $patient['address'] = getTextBetweenTags($line,'Address');
			if(strpos($line,'<Postal_code>')!==false) $patient['zipCode'] = getTextBetweenTags($line,'Postal_code');
			if(strpos($line,'<City>')!==false) $patient['city'] = getTextBetweenTags($line,'City');
			if(strpos($line,'<Country>')!==false) $patient['country'] = getTextBetweenTags($line,'Country');
		}
		if($inPhones) { // this section does not exist in superdent exports, it resides in personal_data
			if(strpos($line,'<Mobile>')!==false) {
				$mobile = getTextBetweenTags($line,'Mobile');
				$mobile = str_replace('.' , '', $mobile);
				$patient['mobile'] = str_replace('/' , '', $mobile);
			}
			if(strpos($line,'<Private_phone>')!==false) {
				$phone = getTextBetweenTags($line,'Private_phone');
				$phone = str_replace('.' , '', $phone);
				$patient['phone'] = str_replace('/' , '', $phone);
			}
		}
	}
	$lcount++;
