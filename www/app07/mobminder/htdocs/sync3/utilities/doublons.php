...
function micro($msg) { 
	// return '<p style="color:blue; padding-left:3em; font-size:80%; line-height:70%;">'.$msg.'</p>'; 
}
...
//////////////////////////////////////////////////////////////////////////////// 
//
//   I D E N T I F Y I N G      D O U B L O N S 
//
...
foreach($dsVisitors->keyed as $vid => $dSvisitor) {
	...
	if($dSvisitor->mergedTo) continue;
	...
	if($underbreak)	{ // this one was a doublon, let's keep record of this
			$d++; $i--;
			$dSvisitor->mergeTo = $passed; // see (*dr01*) $passed is an instance of dS_visitor
...
if($do) {	
	...
	$cuein = microtime(true); $c = 0;
		foreach($screened_dSvs as $vid => $dS) { 
			$c++; unset($dS->lower_lastname); unset($dS->lower_firstname); 
			$dS->dSsave(); // not needed, see (*dr02*)
		};
	$cueout = microtime(true);
	...
	$cuein = microtime(true); $c = 0;
		foreach($doublons_dSvs as $vid => $dS) { // doublons need to be actually merged 
			$c++;
			$dS->mergeTo();	// dS containing a ->mergeTo attribute	// Watch out performance
		};
	$cueout = microtime(true);
	$cuedelta = deltasec($cuein,$cueout);
	...
	// update sync table in such a way that merged items 
	//
	new Q('	update synchro_visitors left join visitors on visitors.id = synchro_visitors.localId 
			set synchro_visitors.localId = visitors.mergedTo
			where skeyId = '.$skeyId.' and visitors.id is not null and visitors.mergedTo <> 0;');
	$cueout = microtime(true);
	$cuedelta = deltasec($cuein,$cueout);
	$out .= h2($c.' Synchro links where re-linked to kept (unmerged) visitors: it took '.$cuedelta.'.');
