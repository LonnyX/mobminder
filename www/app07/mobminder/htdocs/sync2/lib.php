...
function quicklogin() {
	...
	if(!$dS_login) abort('005','gate::Wrong login/pass combination');
			...
			else C_dbIO::logged($dS_login,'(sync2)');
...
class C_syncContext { // should be used in all sync scripts in the next design lift, now only used for backups
	...
	public function rem_resa($resaIds = false, $archiveInc = true, $remotePOV = false, $stampSearchIn = false, $days = 1) { // oxteo displays list of appointments
		...
		if($c) foreach($avidx as $resaid => $vids) { 
			...
			foreach($vids as $id => $vid) { 
				if($remotePOV) { // $a[] should contain the remoteId
					if(isset($vidx[$vid])) $a[] = $vidx[$vid]; 
						...
						$a[] = 0; 
						$dS_r = $this->resas->keyed[$resaid]; // pvh 2021-01 : warnings on apache2 log $resaid not referenced in $this->resas, $resaid is from another query, TBD check the coherence between the 2 queries!!
			...
			$m = implode('!',$a); // a string list of visitors ids (remote id when remotePOV is true)
...
class postedfile {
	...
	public function __construct($dS_account, $email = '') {
		...
		$this->filter_line 		= utf8_encode('/[^#_ÀàâãçËÈÉéèëêôöîïñûüùÿ@ A-Za-z0-9+€&:;,!=\*\?\/\'\"\.\(\)\[\]\-]/');
		$this->filter_address 	= utf8_encode('/[^_ÀàâãçËÈÉéèëêôöîïñûùüÿ\.,a-z A-Z0-9-\'\/\(\)]/');
		$this->filter_name 		= utf8_encode('/[^_ÀàâãçËÈÉéèëêôöîïñûùüÿ\'\.a-z A-Z-]/');
	...
	public function tolines($encoding = false) {
		...
		while(!feof($handle)) {
			...
			if($c++<20) $this->log(chr(9).'line '.$c.' >'.$buffer);
	...
	public function csv_toVisitors($skeyId = false, $section = false) { // leaving skeyId to false forces creation of all new dataSets
		...
		if(!in_array('lastname',$headers)) $this->abort('240', 'csv visitors headers processing::::Missing mandatory header: lastname');
			...
			switch($key) {
				...
				case 'birthday': 
					...
					if(!is_numeric($item)) { // $that->log(chr(9).'found a non numeric birthday:'.$item);
						$i = (string) $item; 
						if(strlen($item)>=10) 
							if($item[4]=='-'&$item[7]=='-') // format enters like AAAA-MM-JJ
								$item = substr($item,0,4).substr($item,5,2).substr($item,8,2);
		...
		for($lx = $linein; $lx<=$lineout; $lx++, $lcount++) { $line = $this->lines[$lx];
			...
			$existingId = 0;
			if($this->hasRemoteId) { // the visitors file arrived with remoteId, it is an update or a backup restore. When init from scratch, there is no remoteId field and they are created for the complete file.
				...
				if($skeyId) { // if a synchro key is passed, first check if this item is not already present in DB
					if($remoteId) {
						...
						$cnt = $q->cnt(); 
						if($cnt>1) {
		C_dS_exception::put('sync2/lib.php postedfile' /*class*/, 'csv_toVisitors()' /*function*/, 'Multiple localId match for a single remoteId > '.$sql /*msg*/, 0 /*account id*/);
	...
	public function csv_toAppointments($skeyId = false, $section = false) { // leaving skeyId to false forces creation of all new dataSets
		...
		for($lx = $linein; $lx<=$lineout; $lx++, $lcount++) { $line = $this->lines[$lx];
			...
			if($skeyId && $this->hasRemoteId) { // if a synchro key is passed, first check if this item is not already present in DB
				$remoteId = $split[$remoteIdPosition];
				if(!is_numeric($remoteId)) abort('256', 'csv appointments processing::remoteId must be a numeric, you sent:'.$remoteId);
				$remoteId = intval($remoteId); if(!is_int($remoteId)) abort('257', 'csv appointments processing::remoteId must be an integer, you sent:'.$remoteId);
				if($remoteId) {
					$q = new Q('SELECT localId as id FROM synchro_reservations WHERE skeyId = '.$skeyId.' AND remoteId = '.$remoteId.';');
					$existingId = $q->ids(); if(!$existingId) $existingId = 0; // then the item is new
					if($q->cnt()>1) abort('258', 'csv appointments processing::Many reservations have the same remoteId:'.$remoteId.' - you need to reset'); // that is a messy situation where the remote side has loaded many new items having a common remote id
				}
			}
		...
		$lcount -= $skipcount;
		if(!$lcount) $this->log('259', 'csv appointments processing::No valid lines could be read from the file');
		$this->log($lcount.' valid lines have been read');
		return $a;
	}
	public function csv_toIdsMatch($class) {
		...
		// scanning lines
		//
		$lcount = 0; $skipcount = 1; $a = array(); $datalines = count($this->lines)-1;
		...
		if($datalines) {
			foreach($this->lines as $line) {
				...
				if($dS_s->localId == '')	{ $skipcount++; $this->log(chr(9).'Skipping line: '.$lcount.' (mobminderId is empty)'); continue; }; 
				if($dS_s->remoteId == '')	{ $skipcount++; $this->log(chr(9).'Skipping line: '.$lcount.' (remoteId is empty)'); continue; }; 
				$a[] = $dS_s;
			}
			$lcount -= $skipcount;
			if(!$lcount) $this->abort('283', 'acknowledge::No valid lines could be read from the file');
		}
...
function fixgenders($dS_v_array, $preload = false, $verbose = false) { // accepts an array of C_dS_visitor
	...
	$count = Array(); $count[0] = 0;  $count[1] = 0; 
	$genders = new C_dbAccess_genders($preload);
	$log = '';
	foreach($dS_v_array as $c => $dS_v) {
		$g = $dS_v->gender;
		if($g===false) { $dS_v->gender = $genders->guess($dS_v->firstname); if($verbose) $count[$dS_v->gender]++; continue; }
		...
		$dS_v->gender = $out;
		if($verbose) $count[$dS_v->gender]++;
	}
	if($verbose) msg('fixgenders log: '.$log.' (males:'.$count[1].' females:'.$count[0].')');
}
