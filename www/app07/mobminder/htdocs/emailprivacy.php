...
// let's make sure that bots do not get in here

		if(!isset($_REQUEST['aid'])) die('You have no access to this script <command>logoff</command>');
		$aid = $_REQUEST['aid'];
		if(!is_numeric($aid)) die('Access aid invalid <command>logoff</command>');

		if(!isset($_REQUEST['tid'])) die('You have no access to this script <command>logoff</command>');
		$tid = $_REQUEST['tid'];
		if(!is_numeric($tid)) die('Access tid invalid <command>logoff</command>');

		if(!isset($_REQUEST['eid'])) die('You have no access to this script <command>logoff</command>');
		$eid = $_REQUEST['eid'];
		if(!is_numeric($eid)) die('Access eid invalid <command>logoff</command>');

		if(!isset($_REQUEST['rid'])) die('You have no access to this script <command>logoff</command>');
		$rid = $_REQUEST['rid'];
		if(!is_numeric($rid)) die('Access rid invalid <command>logoff</command>');
	
	$dS_account = new C_dS_group($aid);
	if(!$dS_account->id) die('No account <command>logoff</command>');
	
	$dS_template = new C_dS_emailTemplate($tid);
	if(!$dS_template->id) die('No template <command>logoff</command>');
	
	switch($dS_template->target) {
		case reminder_target_visitor: // a dS_visitor email is the recepient
			$dS_target = new C_dS_visitor($rid);
			// echo 'reminder_target_visitor<br/>';
			if($dS_target->groupId <> $aid) die('Inconsistent visitor <command>logoff</command>');
			break; 
		case reminder_target_bCal: // a dS_login email is the recepient
		case reminder_target_uCal:
		case reminder_target_fCal:
			// echo 'reminder_target_login<br/>';
			$dS_target = new C_dS_login($rid);
			break; 
	}
	// print_r($dS_template); echo '<br/>';
	// print_r($dS_account); echo '<br/>';
	// print_r($dS_email); echo '<br/>';
	// print_r($dS_target); echo '<br/>';
	
	if(!$dS_target->id) die('No resource <command>logoff</command>');
	
	if($dS_template->groupId!=$aid) die('Inconsistent template again <command>logoff</command>');
...
$html->pushLink('./eresa2/e-privacy.css?b='.$baseline, 'stylesheet', 'text/css');
...
$html->pageTitle('Mobminder - Privacy policy');
		...
		$icon = '<div class="fa fa-2x fa-user-secret" style="color:steelblue; text-align:left; display:inline-block; padding:.5em;"></div>';
	$title = '<h1 style="color:white; padding-bottom:.5em; padding-top:.2em;">'.$dS_account->name.$icon.'Politique de confidentialité'.'</h1>';
