...
//
const measureAjax = false; // execution time round around and local treatment
const measurePlitem = false; // measure the display time of entering plitems data
const measureStreaming = false;

const verboseOn = false; // turns on/off the console.log from function vlog(); // dev urls are also presented under the login actions, when verbose is On
const traceCallbacks = false;
const fileVerbose = false; // default: false (verbose for all controls in a given file)
const tipsdonotvanish = false; // very useful when debugging C_iTIP, it leaves the sticker on screen when your mouse goes away. used in C_iTIP.handlers in controls.js
...
//
	//// dBaccess
const monitorDBaccess = { 0:'C_dS_trackingCC', 1:'C_dS_reservation', 2:'C_dS_resource', 3:'C_dS_context', 4:'C_dS_attendee' };
	

	//// easy & framework basics
const monitorFramework = {  0:'C_iMENU', 1:'A_ct', 3:'C_KEY', 4:'T_bar', 5:'C_iTABS' }; // 0:'A_ct', 1:'A_cb', 2:'C_iMODAL',, 4:'C_iCLIK', 5:'C_iWIN', 6:'C_iBUTTON'
const monitorCueCtrls = { 0:'C_iDDWN', 1:'C_iSELECT', 3:'C_iRANGE', 4:'C_iCUE', 5:'C_iINOUT', 6:'C_iOFFD', 7:'C_iSPAN' };
const monitorCresta = { 0:'C_iCRESTA', 1:'C_iACPICK', 2:'C_iAC', 3:'C_iCLIK', 4:'C_iDDWN', 5:'C_iMENU', 6:'C_iTABLE' };
const monitorOptions = { 0:'C_iSELECT' };
const monitorDragDrop = { 0:'C_iDRAG', 1:'C_iDROP', 2:'P_hrow', 3:'P_htype', 4:'P_horiz' };
const monitorPlusClass = { 0:'C_iPLUS', 1:'C_iDRAG', 2:'C_iDROP' };
const monitorTooltip = { 0:'C_iTIP' };
const monitorMenu = { 0:'C_iCLIK', 1:'C_iMENU', 2:'C_iDDWN', 3:'C_iAC' };
const monitorCommToggles = { 0:'C_iCOMM', 1:'M_RESA', 2:'C_iTRIGGER' };
const monitorAC = { 0:'C_iAC' }; // , 1:'C_iMENU', 2:'C_iEDIT'
	

	//// modals
const monitorModals 			= { 0:'M_RESA', 1:'M_VISI', 2:'M_RESC', 3:'C_iMSG', 4:'C_iARRAY', 5:'C_iDRAG', 6:'C_iDROP', 7:'C_iCLIK'};
const monitorModalResa 		= { 0:'M_RESA', 1:'C_iINOUT', 2:'C_iCUE', 3:'C_iSPAN', 4:'C_iDPpop', 5:'C_iDP', 6:'C_iOFFD', 7:'C_iCSS', 8:'C_iMENU', 9:'C_iDDSTACK', 10:'C_iCLIK', 11:'C_iEDIT', 12:'C_iCRESTA' /*, 10:'C_dS_reservation', 11:'C_dS_attendee', 12:'C_dS_att_visitor', 13:'C_dS_performance'*/};
const monitorModalResaSerie 	= { 0:'M_RESA', 2:'C_iDPpop', 3:'C_iDP', 4:'C_iRECUR', 5:'C_iWDAYs' };
const monitorModalResaPayments= { 0:'M_RESA', 1:'M_PAYMENT', 2:'C_dS_payment', 3:'C_iBILL', 4:'C_iPAY', 5:'M_iPairGoCrypto' };
const monitorModalRscr 		= { 0:'M_RESC', 1:'C_iPLUS', 3:'M_Huser', 4:'M_Hourly', 5:'C_iDP', 6:'C_iOFFD', 7:'C_iCSS', 9:'C_iYEAR', 2:'C_dS_reservation' };
const monitorHourlies 		= { 0:'M_SHADOW', 1:'M_NewHourly', 2:'P_Sticker', 3:'P_column', 4:'P_hourly', 5:'P_GRID', 6:'C_iRANGE', 7:'C_iCUE'};
const monitorModalTboxing 	= { 0:'M_TBOXING', 1:'C_iSKIN' };
const monitorModalFile 		= { 0:'M_file', 1:'C_iCSS', 2:'C_iFILE', 3:'C_iDQS' };
const monitorModalLogin 		= { 0:'M_LOGIN', 1:'C_iSTAFF', 2:'C_iEPREV',3:'C_iIMG', 4:'C_iEBOOK' };
const monitorModalVisitor 	= { 0:'M_VISI', 2:'C_iCSS' };
const monitorModalTemplate 	= { 0:'M_SMST', 1:'C_iTRIGGER', 2:'C_iACTLOGIN' };
const monitorModalWorkcode 	= { 0:'M_WRKCD', /*1:'C_iCRESTA', 2:'M_RESA', 3:'C_dS_reservation'*/ };
const monitorModalResaTAGs 	= { 0:'M_RESA', 4:'C_iCSS' }; // 2:'C_iTABLE', 3:'C_iTEM'
	
	
	//// backoffices
const monitorVisitorsPage = { 1:'C_FILTERTABS', 2:'Date'};
const monitorBackVisi = { 0:'C_backVISI', 1:'C_iTABLE', 2:'C_iSTAFF', 4:'C_iARRAY' };
const monitorBackStats = { 0:'C_backSTAT', 1:'C_iDP', 2:'C_iARRAY', 3:'C_iDRAG', 4:'C_iDROP' };
const monitorBackoffice = { 2:'C_backPREFS', 3:'C_iRANGE', 4:'C_iPLUS', 5:'M_CCSS', 6:'M_LOGIN', 7:'M_SMST', 8:'C_iCUE', 9:'C_GDLN', 10:'M_TBOXING', 11:'C_iDATEPRESET' };
const monitorPreferences = { 0:'C_iONOFF' };
const monitorConnections = { 0:'C_backCNX', 1:'C_iARRAY', 2:'C_iAC' };
const monitorArchives = { 0:'C_backARCH', 1:'C_iARRAY', 2:'C_iSTAFF', 3:'C_iACPICK', 4:'C_iDP' };
const monitorFinder = { 0:'C_backFIND', 1:'C_iARRAY', 2:'C_iSTAFF', 3:'C_iACPICK', 4:'C_iDP', 5:'M_duplacc', 6:'C_iACTIONS' };


	//// application
const monitorHorizontal = { 0:'C_iMOB', 1:'C_iDNAV', 2:'P_AgendaHVL', 3:'P_horiz', 4:'P_htype', 5:'P_hrow', 6:'C_iPLAN', 9:'P_RULER', 10:'P_Vertical'};
const monitorVertical = { 0:'C_iMOB', 2:'P_AgendaHVL', 3:'P_Vertical', 4:'P_column', 5:'C_iPLAN', 6:'P_RULER', 7:'P_GRID' }; //, 4:'P_column',, 1:'R_search'
const monitorListView = { 0:'C_iMOB', 1:'R_search', 2:'C_iPLAN', 3:'P_AgendaHVL', 4:'P_List'};

const monitorStickers = { 0:'P_Sticker', 1:'P_GRID', 2:'P_column', 3:'P_horiz', 4:'P_htype', 5:'P_Vertical', 6:'P_AgendaHVL', 7:'P_hrow', 8:'C_iPLAN'};
const monitorSurferSwitch = { 0:'T_login', 1:'C_iMOB', 2:'T_logged', 3:'C_iPLAN', 4:'R_search', 5:'C_XL', 6:'T_bar' };
const monitorNotesAndTasks = { 0:'T_notes', 1:'M_NOTE', 2:'T_tasks', 3:'M_TASK', 4:'C_iPERIOD', 5:'C_iTDONE', 6:'M_VISI' };
const monitorChat 	= { 0:'T_chats', 1:'M_chat', 2:'C_iUSERS', 3:'C_iTHREADS', 5:'C_iTHREADSdog', 6:'C_iWatchdog' };
const monitorSearch 	= { 0:'R_search', 1:'C_iACPICK', 2:'C_iAC', 3:'C_iPLAN', 4:'C_iAMPM', 5:'C_iBEFORE', 6:'C_iSLOT', 8:'C_iDUR', 9:'A_ct' };
const monitorTopMenu 	= { 0:'C_iMOB', 1:'T_bar', 2:'T_login', 3:'T_notes', 4:'T_tasks', 5:'C_iCAL', 6:'T_backoptions', 7:'T_dp' };
const monitorEresa 	= { 0:'e_MOB', 1:'C_eSearch', 2:'C_eRESA', 3:'C_eREG', 4:'C_eVisitor', 5:'C_eCheckIn', 6:'C_iSTAFF', 7:'C_eProcess', 8:'C_iSLIDE' };
const monitorProximus = { 0:'e_MOB', 1:'C_pSearch', 2:'C_pRESA', 3:'C_pREG', 4:'C_pVisitor', 5:'C_pVident', 6:'C_iSTAFF' };

	//// application
const monitorSMStemplates = { 0:'C_dS_smsTemplate', 1:'C_iCOMM' };


	//// small utilities apps
const synctest = { 0:'sync_MOB', 1:'C_iCRESTA', 2:'C_iFILE', 3:'C_iCHECKIN', 5:'C_SYNCinit', 6:'C_SYNCupdate', 7:'C_SYNCbackup', 8:'C_iONOFF' };
const smstest = { 0:'sms_MOB', 5:'M_SMST' }; // , 1:'C_iEDIT'

const smartphones1 = { 0:'s_MOB', 1:'C_sDP', 2:'C_sVslider', 3:'P_Vertical', 4:'P_column', 5:'P_Sticker', 6:'C_iEDIT', 7:'C_iFIELD', 8:'M_RESA' };
const smartphones2 = { 0:'C_sCalview', 1:'C_sVslider', 2:'P_Vertical', 3:'C_tMODAL', 4:'C_sDP', 5:'C_sDF' };
const smartphones3 = { 0:'C_iAC', 1:'C_iACPICK', 2:'C_iFIELD', 3:'C_iEDIT', 4:'C_iDDWN', 5:'C_iMENU', 6:'C_KEY' };

const C_iAC_multiSelect = { 0:'C_iAC', 1:'C_iACPICK', 2:'C_iFIELD', 3:'C_iEDIT', 4:'C_iDDWN', 5:'C_iMENU', 6:'C_KEY', 6:'C_iCLIK' };



	//// synchronization
const synchro = { 0:'C_dbSynchro', 1:'C_iSYRSC', 2:'C_iSYCSS' }
...
//
//
	const classOnlyVerbose = {}; // (defines which filter to apply) default: false (verbose for all classes)
	// var classOnlyVerbose = monitorBackoffice; // (defines which filter to apply) default: false (verbose for all classes)
	
	// var activate = 'activate';
	const functionOnlyVerbose = false; // default: false (verbose for all functions)
	
	
	
const warning = function(file, classid, functionid, msg, more) {
...
let vlogcount = 0, vbs = verboseOn, vbspad = '    | ';
const vlog = function(file, classid, functionid, msg, more, raw) {
	if(fileVerbose) {
...
String.prototype.composition = function() { // returns one of ['', 'alpha', 'alphanum', 'num'],  // see (*mv01*)
    let is = '';
...
function limitlength(s,l=255) { // s for string, l for length
		s = s+''; // turns it into a String for sure
	const nc = s.length; 
	return s.substr(0,l) + (nc>l?'...':'');
}
...
A_cb.prototype = { // sorry for the comments shortage :oP
	argsarray: function() {
		const args = new Array(); for(let x=0; x<arguments.length; x++) args.push(arguments[x]);
		return args;
	},
	cb: function() {
		const args = this.argsarray.apply(this, arguments);
...
A_hn.prototype = {
	add: function(name, callback, delay) { // please avoid this name => 'add' for your handler :o)
		if(delay) this[name] = function(e) {
			const args = new Array(); for(let x=0; x<arguments.length; x++) args.push(arguments[x]); args.unshift(delay);
...
A_df = function(defaults, depth, path) { 
	...
	for(let x in defaults) {
		const d = defaults[x];
		if(typeof(d)=='object') {
			const isAdf = (d instanceof A_df);
			const isDte = (d instanceof Date); 
...
// PVH 2025 : a new version that ufcked up the hourlies management... so I fell back on the previous verion (hereunder).



C_r = function(sort) { // one register
	if(sort) this.order = { 
...
C_r.prototype = {
	...
	del_try: function(itemORnode, level) { // PVH 2025 - improvement I tried but it failes...
	
		// deletes every ends equal to item, or parts of the tree, or one part of the tree at given level
		
		const item = itemORnode instanceof Object ? itemORnode : false;
		const node = !item ? itemORnode : false;
		for(let x in this) 
			if(this[x] instanceof C_r) this[x].del(itemORnode, level?--level:level); // begin with recursing sub levels of C_r instances
		
		if(level==undefined)
			for(let x in this) {
				if(x in C_r.prototype) continue; 
				if(item) if(this[x] == item) { 
					if(this.order) { this.order.drop(x); delete this[x]; }
			// console.log('C_r.del(on item, '+x+')');
				}
				if(node) if(x == node) {
					if(this.order) { this.order.drop(x); delete this[x]; }
			// console.log('C_r.del(on node,'+x+')');
				}; // delete ends
			}
			
		if(level!==undefined)
			if(!level) // then you reached the first level that recursion requests to be cleaned up
				for(let x in this) 
					if(x in C_r.prototype) continue; 
						else if(node) if(x==node) delete this[x]; // delete parts of the tree (item is a node) when level reaches zero, or any level if level is undefined
	},
...
//
// 
C_regS = function(reg1, reg2, reg3, reg4) { // a collection of registers
...
A_el.prototype = {
	collect: function(eids) { // eids is an object, possibly nesting other objects, where members are element ids
		for(let x in eids) {
			let eid = eids[x]; // can be an eid OR a sub-structure 
			if(typeof(eid)=='object') { 
				this.get[x] = this[x] = new A_el(eid); // when eids are provided, the collect() function is called by the constructor
			} else {
				this.get[x] = this[x] = document.getElementById(eid);
				// if(!this.get[x]) console.log('A_el::collect() - found no element with id '+eid);
				// else console.log('A_el::collect() - found element with id '+eids[x]);
			}
		}
		return this;
	},
	pick: function(x, eid) { // adds one element to the collection
		this.get[x] = this[x] = document.getElementById(eid);
		return this[x];
	},
	...
	isvalid: function() {
		...
		for(let control in this.get) {
			if(this[control]['valid']) { // only if the function is defined for this control
				...
				if(!valid) {
					// let value = '|'+this[control].value()+'|'||'no value() function';
					// if(vbs) vlog('mobframe.js','A_ct','isvalid','control:'+control+' is NOT valid: '+value);
					// console.log('mobframe.js','A_ct','isvalid','control:'+control+' is NOT valid: '+value);
				}
			}
		...
		// console.log('mobframe.js','A_ct','isvalid','FINAL:'+macrovalid);
...
// © Copyright 2007-2026 - PASCAL VANHOVE -70.12.30-097.72 - Belgium
//
const A_ps = function(controls, names, url, callbacks, options) { // callbacks like { onreply:A_cb, ontimeout:A_cb };
	...
	for(controlName in names) {
		...
		const postName = names[controlName]; // is a string or an object containing string
		const value = control.getpost(); // is a scalar or a string, or an object containing scalars or/and strings
		
		const resolve = function(postName, value) { 
	...
	const jpost = post.join('&');
	this.send(url, jpost);
	if(measureAjax) this.microperf.cue('post sent');
}
A_ps.defaults = new A_df({timeout:20000});
A_ps.callid = { // a little register that we use to store our requests :)
	id: (Math.random()*10000000)|0, // which is initial value, it get incremented by record();
	register: new Array(),
	record: function(post) { const id = ++this.id; this.register[id] = post;  this.monitor(id,'record'); return id; },
	get: function(id) { if(this.register[id]) return this.register[id]; return false; },
	free: function(id) { if(this.register[id]) { delete this.register[id]; this.monitor(id,'free'); return true; } return false; },
	monitor: function(commid, from) { 
		let active = 0; for(let x in this.register) active++;
		if(mobminder.app.controls.top) mobminder.app.controls.top.monitorXM(active, commid, from);
	} // monitors transmition with server (let a visual display appear/disappear)
}
A_ps.timeout = function(callid) {
	const p = A_ps.callid.get(callid);
	if(measureAjax) p.microperf.cue('timed out');
	if(!p) return false; // this feedback comes later than the server feedback
	A_ps.callid.free(callid); // PVH2025 - I intentionaly set .free() before calling the client callback
	p.timeout();
...
A_ps.prototype = {
	send: function(url, post) {
		...
		const callid = A_ps.callid.record(this);
		http.open("POST", url, true /*asynchronous*/);
		http.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
		http.onreadystatechange = 
			function() { 
				const r = this.readyState;
				const s = (r==1||r==2)?200:this.status; 		// was introduced for MSIE8 compatibility
				const t = (r==1||r==2)?'':this.responseText; 	// was introduced for MSIE8 compatibility
				return A_ps.feedback(callid, s, r, t); 
			}
		http.send(post);
		if(this.state.timeout) this.timer = setTimeout(A_ps.timeout, this.state.timeout, callid);
	},
	stream: function(stream) { // called on server response
		if(this.timer) clearTimeout(this.timer);
		if(this.callbacks.onreply) this.callbacks.onreply.cb(stream);
	},
	timeout: function() { if(this.callbacks.ontimeout) this.callbacks.ontimeout.cb(); } // called on non server response
}
...
var device = { 
	...
	, language: (function() { // detects the browser language, usefull when no user is logged yet
				let navlanguage = navigator.language || navigator.userLanguage || 'en-US';
			let lcs = navlanguage.split('-'); // is an array like ['en','US'] explaining language and country
		...
		if(!navlanguage in la) console.log('! the browser language does not match any expected');
		
		let moblabr = l, mobcode = 0; // language final, reduced according to our translation scope
		if(!l in mobminder.language.abr) {
			moblabr = 'en';
			console.log('this browser language ('+l+') is outside mobminder translations scope, final language is '+moblabr); // see (*is01*)
		} else {
			for(let x in mobminder.language.abr) if(mobminder.language.abr[x]==moblabr) { mobcode = x; break; }
			console.log('this browser language is |'+moblabr+'| for country |'+navlanguage+'|');
		}
			
		return { 'mobcode':mobcode, 'moblabr':moblabr, 'country':c }; // that you fetch globally through device.language.mobcode and device.language.country
...
, dev: (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' || window.location.hostname === '192.168.0.37')
};
...
(C_iWIN.setbodyclass = function(cursor) {
		const bodymode = is.tactile ? ' touch' : ' mouse';
		const bodysize = is.small ? 'small-device' : 'large-device';
	$('body').addClass(bodysize+bodymode).noContext();
		const oriclass = is.landscape?'landscape':'portrait';
	...
	if(cursor)
		...
		$(document.body).removeClass ( // only the classes that contain the cursor specifications
			function (index, classes) { 
				const matches = classes.match(/\bcursor-[^\b]*\b/gi) || [];
				return (matches.join(' '));
			}
		);
};
...
Date.prototype.calendar = function(d) { // both cues in and out sticks to midnight. This is a calendar span.
	if(this.seconds()) return false;
	if(d.seconds()) return false;
	return this.span(d)/86400; // returns the number of entire days 
}
...
Date.prototype.HHmm = function(seconds, prefill) {
		if(prefill==undefined) prefill = '0'; // prefills the string if hour < 10, like 2:35 becomes 02:35
	let h = this.getHours();
...
Date.prototype.getWeekNumber = function() {
  // Copy date and clear the time portion
  const d = new Date(Date.UTC(this.getFullYear(), this.getMonth(), this.getDate()));
  // ISO week starts on Monday; getUTCDay: 0=Sun…6=Sat, so map Sun→7
  const dayNum = d.getUTCDay() || 7;
  // Shift date to the Thursday of this week: Thu is ISO’s week anchor
  d.setUTCDate(d.getUTCDate() + 4 - dayNum);

  // Year of the Thursday is the ISO-week-year
  ...
  // Calculate full weeks between our week‐Thursday and the year’s first‐Thursday
  const weekNumber = Math.round(
    (d - firstThu) / (7 * 24 * 60 * 60 * 1000)
  ) + 1;

  return { y: isoYear, wn: weekNumber };
};
...
timeOBS = function(seconds,sep,str8,mn) { // returns HHMM string given a number of seconds from midnight (replaced by time() using Math.floor, more efficient
	sep = sep || ':';
	mn = mn || '';
...
function time(seconds, sep = ':', str8 = false, mn = '') { // returns HHMM string given a number of seconds from midnight
  // hours = whole 3600-second chunks
  const hNum = Math.floor(seconds / 3600);
  // minutes = leftover seconds divided by 60
  const mNum = Math.floor((seconds % 3600) / 60);
  // zero-pad hours if requested
  const h = str8
    ? String(hNum).padStart(2, '0')
    : String(hNum);
  // always zero-pad minutes to two digits
...
tohumanbdate = function(digits, options) { // birthday enters like '19701230', so YYYYMMDD
	if(!digits) return '';
	options = options || {};
	digits = digits.toString();
	if(digits.length!=8) return '';
	...
	const jsdate = new Date(formated);
	return C_XL.date(jsdate, { abreviation:options.abreviation, weekday:false, time:false, year:true });
}
...
hhmm_fromUNIXtime = function(unixTime, options) { // For unix time input, returns HH:MM
	let jsdate = new Date(unixTime*1000); 
	return jsdate.HHmm(options); 
};
humandate = function(t, options) { // returns human readable date time from either unix or sql timestamp format ( for tracking CCDA )
	...
	if(options.time===undefined) options.time = true;
	...
	let string;
	if(options.relative) string = C_XL.daterelative(jstime, {abreviation:options.abreviation, weekday:true, time:options.time });
		else string = C_XL.date(jstime, { abreviation:options.abreviation, weekday:true, time:options.time });
	return string; 
}
...
//
C_iPASS = function(items) { // eg. new C_iPASS({id:this.dS.id, delta:0});
	this.items = items;
}
C_iPASS.prototype = { 
	activate: function() {}, 
	...
	value: function() { return this.items; } // smth like {id:this.dS.id, delta:0}
}
...
const class_resa_any = 10; // is a triggerClass for communication

const class_note = 14;
const class_task = 15;
const class_chat = 16;
const class_file = 17;
const class_product = 18;

const resaclass = { // used in datasets, see (*rc01*)
...
const class_uCal 		= 1;
const class_bCal 		= 2;
const class_visitor 	= 3;
const class_fCal 		= 4;
const agClass = { bCal:2, uCal:1, fCal:4 };
const agClassMatrix = function(b,u,f) { return { 2:b||[], 1:u||[], 4:f||[] }; }
...
function symbol(which,css,style,eltype) { // (*sybl01*)
	let s = false; // will contain a suffix to fa-
	css = css || '';
	style = style || '';
	eltype = eltype || 'i'; // '<i>', and  if you pass div, you get a <div> back
	switch(which) {
		...
		// used in modals h1
		case 'user': 		s = 'user'; 		css = css || 'fa-13x'; 	style = 'padding:0 .2em 0 .7em;'; break;
		case 'user-new':	s = 'user-plus'; 	css = css || 'fa-13x'; 	style = 'padding:0 .2em 0 .7em;'; break;
		case 'user-deleted':s = 'user-times'; 	css = css || 'fa-13x'; 	style = 'padding:0 .2em 0 .7em;'; break;
		case 'calendar': 	s = 'calendar'; 	css = css || 'fa-13x'; 	style = 'padding:0 .2em 0 .7em;'; break;
		case 'task': 		s = 'thumbtack'; 	css  = css || 'fa-13x'; style = 'padding:0 .2em 0 .7em;'; break;
		case 'note': 		s = 'tags'; 		css = css || 'fa-13x'; 	style = 'padding:0 .2em 0 .7em;'; break;
		case 'chat': 		s = 'comments'; 	css = css || 'fa-13x'; 	style = 'padding:0 .2em 0 .7em;'; break;
		
		case 'child': 		s = 'child'; 		css = css || 'fa-13x'; 	style = 'padding:0 .2em 0 .7em;'; break;
		case 'baby': 		s = 'baby'; 		css = css || 'fa-13x'; 	style = 'padding:0 .2em 0 .7em;'; break;
		...
		case 'jpg': case 'jpeg': case 'gif': case 'png': case 'jfif': case 'webp': 	s = 'file-image'; break;
	...
	let span = '<'+eltype+' style="'+(style||'padding-left:1em;')+'" class="fa fa-gray '+(css||'fa-11x')+' fa-'+s+'"></'+eltype+'>';
	return span;
}
...
C_XL.w = function(term, options, fallback) { // term can be a bunch of terms like { 1:term1, 2:term2, ...} 
	...
	if(typeof(term)=='object') { // then you passed a bunch of terms like { 0:'term1', 1:'term2', ... }
		const xlted = {}; // keeps the input object intact!
		for(let x in term) xlted[x] = C_XL.w(term[x],options,fallback); // note the recursion (*xl02*)
		return xlted;
	}
	
	const l = options.language;
	const tx = C_XL.d[term]; 
	if(tx===undefined) { // this term is not defined in our dictionnary, for an example of fallback usage see (*fb01*)
		if(verboseOn) {
			let f = ''; if(fallback) f = ' ( fallback value |'+fallback+'| )';
			const x = 'C_XL.w() term not found for |'+term+'| ';
			console.log(x+f);
		}
	...
	const hasvisitors = t.includes('visitors');
	
	// keep plural above or it will screw it up
	...
	const hasvisitor = t.includes('visitor');
		c = 0;
		go = hasvisitor; while(go) {
			t = t.replace('visitor', C_XL.d['visitor'][l]);
			go = t.includes('visitor'); // keeps going until purged
			if(++c>10) break; // let's keep it safe
		}


	// bCals keyword fusion
	const has_bCals = t.includes('bCals');
	
		c = 0;
		go = has_bCals; while(go) {
			t = t.replace('bCals', C_XL.d[class_bCal][l]);
			go = t.includes('bCals'); // keeps going until purged
			if(++c>10) break; // let's keep it safe
		}

	// uCals keyword fusion
	const has_uCals = t.includes('uCals');
	
		c = 0;
		go = has_uCals; while(go) {
			t = t.replace('uCals', C_XL.d[class_uCal][l]);
			go = t.includes('uCals'); // keeps going until purged
			if(++c>10) break; // let's keep it safe
		}

	// fCals keyword fusion
	const has_fCals = t.includes('fCals');
	
		c = 0;
		go = has_fCals; while(go) {
			t = t.replace('fCals', C_XL.d[class_fCal][l]);
			go = t.includes('fCals'); // keeps going until purged
			if(++c>10) break; // let's keep it safe
		}
	...
	if(options.nested) {
	  for(let key in options.nested) {
		t = t.replaceAll('$'+key+'$', options.nested[key]);
	  }
	}
...
C_XL.capitalize = (s) => { s = s.toLowerCase(); return s.charAt(0).toUpperCase() + s.slice(1); };
...
C_XL.date = function(jsdateORstamp, preset, language) {
	...
	const opt = C_XL.date.defaults.align(preset);
	const separator = opt.abreviation == 'min' ? '' : ' ';
	
	const date = jsdate.getDate(); 						
	const month = C_XL.month(1+jsdate.getMonth(), opt.abreviation, language);
	...
	switch(language) { 
		...
		case mobminder.language.codes.english: // reversed English sequence
			...
			s = [month, date+post]; 
			if(opt.weekday) { const weekday = C_XL.weekday(jsdate.getPHPday(), opt.abreviation, language); s.unshift(weekday); }
			if(opt.year) { const year = jsdate.getFullYear(); if(opt.yearfirst) s.unshift(year); else s.push(year); };
		...
		default:
			s = [date,month]; 
				if(opt.weekday) { const weekday = C_XL.weekday(jsdate.getPHPday(), opt.abreviation, language, preset.cap); s.unshift(weekday); }
				if(opt.year) { const year = jsdate.getFullYear(); if(opt.yearfirst) s.unshift(year); else s.push(year); };
...
C_XL.weekday = function(daycode, abreviation, language, cap) { // php daycode [1-7]
	
	// console.log('C_XL.weekday',daycode, abreviation, language, cap);
	
	abreviation = abreviation || 'full'; // can be ['full','abr','min','plural','every']
	language = (language!==undefined)? language:mobminder.context.surfer.language;
	
	let weekday = C_XL.weekdays[abreviation]['weekday'+daycode][language];
	if(cap!==undefined) if(cap == 0) return weekday; // Force non cap
	return C_XL.capitalize(weekday); // Capitalizing is the default.
}
C_XL.month = function(number, abreviation, language) {
	
	if(!abreviation) abreviation = 'full';
	language = (language!==undefined)? language:mobminder.context.surfer.language;
	return C_XL.capitalize(C_XL.months[abreviation]['month'+number][language]);
}
...
C_XL.duallettercountrycode = function(dd) { // like 'fr' or 'ch'
	return countryNames[dd] || dd+'(no match)';
}
...
function C_iCLIK(eid, callbacks, preset) {
	...
	this.elements = new A_el();
	this.callbacks = callbacks; // like { down:, up:, click:, hold:, dblclick:, hover:, mousemoves:, gone: }
	this.state = C_iCLIK.defaults.align(preset);
		const p = this.state.value?('_'+this.state.value):''; // eids postfix
	this.eids = { eid:eid, ui:eid+'_ui'+p, wrapme:eid+'_wrp'+p };
	this.handlers = new A_hn(
		{	/* mouse */ down:new A_cb(this, this.down), up:new A_cb(this, this.up), enter:new A_cb(this, this.enter), moves:new A_cb(this, this.moves), leave:new A_cb(this, this.leave), leavehover:new A_cb(this, this.leavehover),
	...
	if(this.state.tip) {
		if(typeof(this.state.tip)==='string') // easy case, some text was passed, to be displayed in standard tip format
			this.state.tip = new C_iTIP(this.eids.ui,  { text:this.state.tip } ); // css:'help-tip' is applied by default
		else
			if(typeof(this.state.tip)==='object') // a tip preset like { text:'blabla', css:'sticker-tip'} was passed
				if(this.state.tip.text)
					this.state.tip = new C_iTIP(this.eids.ui, this.state.tip); 
	}
...
C_iCLIK.prototype = {
	...
	display: function(css, rowspan, tag) {
		...
		const id  = ' id="'+this.eids.ui+'"'; // not used for tag 'input'
		const clss  = ' class="click '+css+'"'; // not used for tag 'input'
		const indent = this.state.indent ? ' padding-left:'+this.state.indent+'em;' : '';
		let bullet = ''; // bullets indicate the presence of a section split in a C_MENU list.
		if(this.state.section) { 
			const s = this.state.section;
			if(s===true) bullet = '';
			else if(typeof s === 'object' && !Array.isArray(s)) { // the bullet was passed as a duet { bullet:'', css:'' }, css will be applied here (*sb02*)
				bullet = s.bullet?s.bullet+'&nbsp;':'';
			} else if(typeof s === 'string') // the bullet was passed as a single string
				bullet = s?s+'&nbsp;':'';
		}
		const sign = this.state.sign?'<div style="display:inline-block;" class="fa fa-lefter fa-11x fa-'+this.state.sign+'"></div>':'';
		const label = bullet+sign+this.state.ui; // see (*sb01*)
	...
	activate: function() {
		...
		if(this.elements.ui) { // do nothing if this control was not displayed
			...
			if(this.state.idle) this.idle(this.state.idle);
			...
			if(this.callbacks.mousemoves) { // sets animation to start point
				this.radialsetinitial();
			}
		}
	},
	...
	disable: function(onoff) { // enable/disable, when disabled, the control is grayed
		return this.enable(!onoff);
	},
	idle: function(onoff) {
		this.state.idle = !!onoff;
		if(this.state.idle) $(this.elements.ui).addClass('idle'); else $(this.elements.ui).removeClass('idle');
		return this.active();
	},
	...
	hide: function(hidden) { 		
		const eltohide = this.elements.wrapme || this.elements.ui; // when defined, we hide the wrapper of the label (e.g. a table td) instead of the label div. This prevents having empty td's in a a table, they take room on the screen.
	...
	inlabel: function(regexp, options) { // highlights in HTML ( not in this.state.ui !! ) the digits given through regexp, options: {elsehide: }
			options = options || {};
			
		const cleanup = new RegExp(/(<b class="match">|<\/b>)/gi); // cleans up previous highlights
		if(this.elements.ui) this.elements.ui.innerHTML = this.elements.ui.innerHTML.replace(cleanup,'');
		
		const ui = this.state.ui.normalize('NFD').replace(/\p{Diacritic}/gu, ''); // see (*im01*)
		
		const where = ui.search(regexp); // returns start position // see (*ie01*)
		const found = (where >= 0);
		
		if(found) { // should match and highlight "andré" in the same way as "andre", diacritics should have no influence on search
		
				const leftcount = RegExp.leftContext.length;
				const matchlen 	= RegExp.lastMatch.length;
				const taillen 	= RegExp.rightContext.length;
				const nativelen = this.elements.ui.innerHTML.length;
				const nativeui	= this.elements.ui.innerHTML;
			...
			const head = RegExp.leftContext;
			const core = nativeui.substr(leftcount, matchlen); // note that we here start from the non diareduced original, containing the accentuations
			const tail = RegExp.rightContext;
	...
	// PRIVATE
	section: function(section) { // elements must exist in the DOM.
		// this function allows C_MENU to create sections in the menu list. It is useful for no other control.
		if(!this.elements.ui) return this;
		if(!section) return this;
		let css = ' mobtext'; // the default
		if(typeof section === 'object' && !Array.isArray(section)) css = ' '+section.css; // if the user provided a css, then replace the default one (even if empty).
		$(this.elements.ui).removeClass('click').addClass('section'+css); // see (*sb02*)
		return this;
	},
	...
	radialsetinitial: function(w,h) { // initial location of the highlight that follows the mouse pointer
			const r = this.elements.ui.getBoundingClientRect(); // this triggers the error e.getBoundingClientRect() is not a function. You have the details on the screenshot. Can you tell me why the div element doesn't have this function. We need only to locate the mouse-pointer relative to the axis origin of the element.
		const x = r.width / 2; // in the middle
		const y = r.height + 10; // 30px under the horizon
		this.setcircleproperties(x,y);
		return this;
	},
	setcircleproperties: function(x,y) { // applies circle animation central position into DOM
		if(this.elements.ui) {
			this.elements.ui.style.setProperty('--circle-xpx',x+'px'); // see (*ic04*) to find the css animation code side
			this.elements.ui.style.setProperty('--circle-ypx',y+'px');
		}
	},
	...
	dblclick: function(e) {  		
		...
		if(this.callbacks.dblclick) return this.callbacks.dblclick.cb(this, {ctrlkey:ctrlkey, shiftkey:shiftkey}, e);
		return true;
	},
	down: function(e, t) { // t indicates that the interactivity comes from a touch event
		...
		if(!is.tactile) $(this.elements.ui).mouseleave(this.handlers.leave); // see (*ic02*)
		...
		if(this.callbacks.dblclick) {
			if(++C_iCLIK.state.dbltap.taps >= 2) { 
				...
				propagate = this.dblclick(e); /* double tap or double mouse click */ 
				return propagate; 
			}
		...
		if(this.callbacks.hold) {
			...
			C_iCLIK.state.hold = setTimeout(handler.hold, 1000);
		}
		
		if(this.callbacks.down) propagate = this.callbacks.down.cb(this, {ctrlkey:ctrlkey, shiftkey:shiftkey}, e);
		return propagate;
	},
	up: function(e) {
		...
		const ctrlkey = e.ctrlKey;
		const shiftkey = e.shiftKey;
		if(!is.tactile) $(this.elements.ui).unbind('mouseleave',this.handlers.leave); // see (*ic02*)
		$(this.elements.ui).removeClass('touch-in'); 
		
		if(vbs) vlog('controls.js','C_iCLIK','up','eid:'+this.eids.ui+', taps:'+C_iCLIK.state.dbltap.taps+', CtrlKey:'+ctrlkey+', ShiftKey:'+shiftkey);
		
		if(this.callbacks.hold) { clearTimeout(C_iCLIK.state.hold); }
		if(this.callbacks.up) propagate = this.callbacks.up.cb(this, {ctrlkey:ctrlkey, shiftkey:shiftkey}, e);
		if(this.callbacks.click) // event is defined
			if(!this.callbacks.dblclick) { // if dblclick is defined, it has to fire in the tout() 
				if(this == C_iCLIK.state.down) { // down and up on identical DOM element
					propagate = this.callbacks.click.cb(this, {ctrlkey:ctrlkey, shiftkey:shiftkey}, e); // ctrlkey is true or false, passes the mouse event as well
				}
	...
	enter: function(e) { 
		...
		$(this.elements.ui).mousemove(this.handlers.moves).mouseleave(this.handlers.leavehover);
	},
	leave: function() { // this one is called ONLY when mouse pointer is down on the element, see (*ic02*)
		$(this.elements.ui).unbind('mouseleave',this.handlers.leave).unbind('mousemove',this.handlers.moves).unbind('mouseleave',this.handlers.leavehover);
		if(this.callbacks.hold) { clearTimeout(C_iCLIK.state.hold); } 
		$(this.elements.ui).removeClass('touch-in'); 	
	},
	moves: function(e) {
		if(this.callbacks.mousemoves) { 
			// having that callback defined causes the .click element to animate if it has together 
			// one of the css class defined here (*ic05*) in controls.css
			// like &.ical-button, &.button.button-wlist, &.button.button-search, &.top-button-face, &.chat-button-face
			
				const mx = e.clientX; // viewport refering absolute x mouse position
				const my = e.clientY; // viewport refering absolute y mouse position
			...
			// const y = clamp(((e.clientY - r.top) / r.height) * 100, 0, 140); // allow >100 if you like
			...
			const dx = mx - rx; // which is the exact mouse pointer position in the button (rx, by) rectangle referential
			const dy = my - by;
			
			const x = -dx; // that I flip to mirror the mouse position in the rectangle
			const y = -dy;
			
			// console.log(x, y, '/', mx, my, ' R', lx, ty, '-', rx, by );

			this.setcircleproperties(x,y);
			// this.elements.ui.style.setProperty('--circle-xpx',x+'px'); // see (*ic04*) to find the css animation code side
			// this.elements.ui.style.setProperty('--circle-ypx',y+'px');
			
			this.callbacks.mousemoves.cb(e, x, y);
		}
	},
	leavehover: function(e) {
		...
		if(this.callbacks.mousemoves) { // sets animation back to start point
			this.radialsetinitial();
		}		
	},
	tstart: function(e) { 
	...
	keys: function(keycode) {
		...
		const shifton = !!(keycode&C_KEY.code.s.shift); 
		const ctrlon = !!(keycode&C_KEY.code.s.ctrl); 
		const nudekey = keycode&=(C_KEY.code.s.alt-1); // does not have the command keys (Ctrl, Alt, Shift, etc..)
...
const INPUT_TOKEN 	= 136  // e-resa url postfix
...
C_iEDIT = function(eid, callbacks, preset) {
	...
	this.tooltip = false;
	if(preset.type == INPUT_LOGIN) this.logindelay = new A_cb(this, this.isUniqueLogin);
...
C_iEDIT.defauts = new A_df( { 
	/* user presets */ label:'', placeholder:'', digits:'-', type:INPUT_TEXT, enabled:true, hidden:false, focus:false
						, tabindex:false, max:false, min:false, mandatory:false, loginId:0, dblclick:true, units:'', showvalid:true
...
C_iEDIT.prototype = {
	...
	display: function(css, label, labelcss, options) { // return top bottom like: <div>a label</div><input>input field</input>
			...
			css = css || '';
		const display = this.state.digits;
		
		const tabindex = (this.state.tabindex) ? ' tabindex="'+this.state.tabindex+'"' : '';
		let textinput, type='text', pattern='', name='', moreoptions='';
		...
		switch(this.state.type) {
			...
			case INPUT_MOBILE: 
				type='tel'; 
				if(this.state.placeholder) // arrives as a country code, like [ 31, 32, 33, 34, 44, ... ], see geocheck.php
					this.state.placeholder = this.mobileplaceholder(this.state.placeholder); // turned into +32493112233 as an example fitting the surfer's region mobile format
				// pattern = 'pattern="[0-9]*"'; // for soft keyboards switching, this misses the + option
				break; 
			case INPUT_PASSWD: case INPUT_PASSWD:
				moreoptions='autocomplete="new-password"';
				break;
			default: pattern = '';
		}
		
		const placeholder = (this.state.placeholder) ? ' placeholder="'+this.state.placeholder+'"' : '';
		const cssclass = css ? ' class="'+css+'"' : '';
		...
		if(this.state.rows) textinput = '<textarea'+cssclass+' rows="'+this.state.rows+'" id="'+this.eids.ui+'" '+tabindex+placeholder+'>'+display+'</textarea>';
			else textinput = '<input '+moreoptions+' type="'+type+'" '+name+' '+pattern+' '+cssclass+' id="'+this.eids.ui+'" value="'+display+'"'+tabindex+placeholder+' '+autocap+'/>';
	...
	labelled: function(label, css, options, blueprint) { // return side by side tds like: <td>a label</td><td>input field</td>
		options = options || { xl:true };
		this.state.label = options.xl?C_XL.w(label):label;
			const labelcss = 'label textcolor-light'+(options.labelcss?' '+options.labelcss:'');
			const bp = blueprint ? '<div style="display:inline-block;">'+blueprint+'</div>' : '';
			const tdlabel = '<td id="'+this.eids.label+'" class="'+labelcss+'" style="white-space:nowrap; text-align:right;">'+this.state.label+'</td>';
			const tdctrl = '<td>'+this.display(css||'alpha16')+bp+'</td>';
		return tdlabel+tdctrl;
	},
	td: function(css, tdcss) { // return one td like: <td>input field</td>
		const cssclass = tdcss ? ' class="'+tdcss+'"' : '';
		...
		if(this.elements.ui) { // that is only if the element was activated
			switch(onOff) {
				case true:
					...
					// $(this.elements.label).removeClass('disabled');
					if(is.tactile) {
						this.elements.ui.ontouchstart =  this.handlers.tstart;
						this.elements.ui.ontouchend = this.handlers.tend;
				...
				case false: 
					if(is.tactile) {
						...
						// $(this.elements.label).addClass('disabled');
	...
	obfuscate: function(onoff) { if(this.elements.ui) this.elements.ui.type = onoff?'password':'text'; },
	...
	set: function(digits, options) { // options like { propagate:false }
		...
		if(vbs) vlog('mobframe.js','C_iEDIT','set','digits:'+digits+', propagate:'+(options.propagate?'YES':'nope'));

		switch(this.state.type) {
			case INPUT_PRICE: digits/=100; digits = digits.toFixed(2); break;
			case INPUT_NUMER: digits = digits.toString(); break; // integer values must be converted into a string to achieve this.state.digits.substring() inside this.keypress() function.
		}
		this.ui(digits);
		if(options.propagate) {
			// console.log('C_iEDIT::set('+digits+') propagate is TRUE');
			this.changed();
		}
		else {
			// this.state.digits = digits; // PVH2025 was not calling changed() which takes care of setting this.state.digits
			this.changed(false, {propagate:options.propagate});
			// console.log('C_iEDIT::set('+digits+') propagate is FALSE');
		}
		return this;
	},
	...
	insert: function(i) { // does not change the caret position ( OBSOLETE, REMOVE !! still used by M_EMLT::ontag() )
					const el = this.elements.ui;
				// const caretPosition = el.selectionEnd;
				const caretPosition = this.caretPosition();
			const textBefore = el.value.substring(0,caretPosition);
			const textAfter = el.value.substring(caretPosition);
		const newText = textBefore + i + textAfter;
	...
	getpost: function() {
		...
		const dg = this.state.digits;
		const d = this.cleanup(dg);
		switch(this.state.type) { // we change the format for some types here, so they match the expected at server side
			...
			case INPUT_PRICE:
				if(d==0) return 0; // some controls are initialized at zero and never accessed by the user, so they arrive here as just zero. See (*bi01*)
				const time100 = parseInt(d.replace('.', ''), 10); // the format arriving here is always "..hhh.cc", so by removing the dot, we multiply by 100
				return time100; // So we record integers, always using cents as a unit
			default:
		}
	...
	androidkeys: function(e) { // only for Android smartphone keyboards, replaces the keypress function. 
		...
		if(this.state.type==INPUT_BDATE) { 
			...
			const el = this.elements.ui;
			const v = el.value; // value as is
			const ch = v.slice(-1);
			const prior = v.substring(0,v.length-1);
	...
	setSelectionRange: function(selectionStart, selectionEnd) { // can be used to set the caret position if End is omitted
		...
		if (this.elements.ui.setSelectionRange) { // Firefox, Webkit, Gecko
			...
			const range = this.elements.ui.createTextRange();
			range.collapse(true);
	...
	caretPosition: function() { // Obtain caret position through the field
		 if(this.elements.ui.selectionStart) { return this.elements.ui.selectionStart; } // Firefox, Webkit, Gecko
			...
			{ 
				...
				const re = this.elements.ui.createTextRange(), rc = re.duplicate(); 
	...
	ui: function(digits) {  // writes in ui dom elements but does not update internal this.state.digits	
		if(this.elements.ui) {
			...
			// console.log('C_iEDIT::ui('+digits+'), eid:'+this.elements.ui.id);
	...
	mobileplaceholder: function(ccode) { // International Country codes, please keep this function in sync with this.isMobile::NSNdigits
		
		switch(ccode) {
			case 1: return '+1 418 2652311'; // North america. E.g. Canada Quebec: +1 418 2652311
			
			case 216: return '+216 98 123456'; // Tunisia: +216 yy 123456
			case 230: return '+230 59 123456'; // Mauritius island: +23 [0 yyy xxxx] 
			
			case 30: return '+30 699 1234567'; // Greece, mobiles like +30 6xx 1234567
			case 31: return '+31 793 123456'; // Netherland
			case 32: return '+32 493 123456'; // Belgium
			case 33: return '+33 987 123456'; // France
			case 34: return '+3471 123 1234'; // Spain
			
			case 351: return '+351 969 123456'; // Portugal
			case 352: return '+352 621 123456'; // Luxembourg: open plan (numbers may have whatever number of digits)
			case 36: return '+36 97 1234567'; // Hungary
			
			case 370: return '+370 686 12345'; // Lithuania: +370 686 12345
			case 371: return '+371 640 12345'; // Latvia:  +371 640 12345
			case 372: return '372 58 123456'; // Estonia: +372 81xx xxxx
			case 373: return '+373 69 123456'; // Moldova: is an exception inside the +37, it has 8 NSN digits
			
			case 385: return '+385 98 123456'; // Croatia: +385 98 123456 or +385 91 1552692

			case 39: return '+393 939 123456'; // Most general case Italy, mobiles like +39 3xx 1234567
			
			case 40: return '+40 78 1234567'; // Romania
			case 41: return '+41 79 123456'; // Switzerland
			case 45: return '+45 81 123456'; // Denmark
			
			case 43660: return '+43 660 1234567'; // +43 is for Austria, they have 10 NSNs in Cell phones like +43 660 1234567
			case 43: return '+43 699 11223344'; // +43 is for Austria, they have 10 or 11 NSNs in Cell phones like +43 699 11223344
			case 44: return '+44 7199 123456'; // +44 is the UK, they have 10 NSN
			case 45: return '+45 91 123456'; // +45 is denmark mobile phones are like +45 20 123456
			
			case 49: return '+49 991 12345'; // +49 is Germany, they have 10 NSN in regular 017 and 016 numbers
			
			case 601: return '+60 192 123456';
			case 687: return '+687 999 123'; // New Caledonia numbers like +687 123 456
			
			case 90: return '+90 533 1234567'; // Turkey, mobiles like +90 533 1234567 (533 is the operator code, but portability is present)
			case 961: return '+961 70 123456'; // Lebanon, mobiles like +96170 123456
		
			default: return 'your mobile';
		}
	},
	...
	validate: function(newdigits) { // sets this.state.valid according to expected format type and mandatory attributes
		...
		switch(this.state.type) {
			...
			case INPUT_LOGIN: 	this.state.valid = this.isLogin(newdigits); if(this.state.valid) if(this.callbacks.onfreelogin) this.logindelay.dcb(600, newdigits); break; // see this.isUniqueLogin()
			case INPUT_PASSWD: 	this.state.valid = this.isPassword(newdigits); break;
	...
	filterfor: function(inputtype) { // provides a filter for allowed chars depending on field type.
		...
		switch(inputtype) { // keep this inline with (*re01*), but keep in mind that they might differ
			...
			case INPUT_TOKEN: 	f = 'a-z0-9'; break; // see(*qt02*)
	...
	changed: function(paste, options) { // called on a keyup event. 

		options = options = options||{ propagate:true };
		...
		if(vbs) vlog('mobframe.js','C_iEDIT','changed','paste:'+(paste?'YES':'NOPE')
			, 'former txt: '+tab+'|'+formertxt+'|'+' /'+(formertxt.length)+'chars'
			, 'new entry:' +tab+'|'+newentry+'|'+' /'+(newentry.length)+'chars'
			, 'corrected: '+corrected
			, 'changed: '+changed
			);
		// console.log('mobframe.js','C_iEDIT','changed','paste:'+(paste?'YES':'NOPE')
			// , 'former txt: '+tab+'|'+formertxt+'|'+' /'+(formertxt.length)+'chars'
			// , 'new entry:' +tab+'|'+newentry+'|'+' /'+(newentry.length)+'chars'
			// , 'corrected: '+corrected
			// , 'changed: '+changed
			// , 'cleanedup: '+cleanedup
			// , 'eid:'+(this.elements.ui?this.elements.ui.id:'not set yet')
			// );
		...
		if(changed||corrected) { 
			...
			if(this.callbacks.onfchange&&options.propagate) {
	...
	setvalidtip: function() { // based on this.state.valid set by this.validate(), here we change the field frame to red (class validNOK) and set a tooltip with error message
		...
		if(!this.state.showvalid) return this;
		...
		if(wrongentry) {
			switch(this.state.type) {
				...
				case INPUT_PASSWD: 	msg = C_XL.w('bad pass'); break;
			}
	...
	keypress: function(e) { 
		...
		// let dg = this.digits();
		// console.log('>>1 |'+dg+'| ('+dg.length+'chars)');
		...
		const ch = String.fromCharCode(key); // the string character version of the key code
	console.log('keypress',this.state.digits);	
	
			const textBefore = this.state.digits.substring(0,caretPosition);
			const textAfter = this.state.digits.substring(caretPosition);
		const newtxt = textBefore + ch + textAfter;
		
		if(vbs) vlog('mobframe.js','C_iEDIT','keypress','eid:'+this.eids.ui+', key:'+key+', caret pos:'+this.statecaretvalue()+', keyraw:'+keyraw+', newtxt: |'+newtxt+'| ('+newtxt.length+'chars)');
		
			const disallowed = this.disallowed(this.state.type); // something like ._a-zA-Z0-9-@ depending on the input type
		const x = '['+disallowed.filter+']';
		const regxp = new RegExp(x);
		
		// dg = this.digits();
		// console.log('>>0 |'+dg+'| ('+dg.length+'chars)');
		...
		if(this.remains(newtxt).over) return false; // no room for anymore character, according to this.state.max setting, copy/paste cases are treated here (*ed02*)
			...
			let f4 = this.filterfor(this.state.type); // something like ._a-zA-Z0-9-@ depending on the input type
		if(f4.filter) {
			const x = '['+f4.filter+']'; // something like /[^._a-zA-Z0-9-@]
			const regxp = new RegExp(x,'gu');
		...
		if(this.state.type==INPUT_MOBILE) { 
			...
			if(usetrunk) // For Luxembourg, Spain and North america (USA and Canada): people are used to enter no trunk number, see (*tr01*)
				if(newtxt[0]!='0' && newtxt[0]!='+')  { console.log('A'); return false; } // you must place a 0 or a + at the beginning of your mobile number
			
			if(/[+0123456789]{1,}[+]{1,}/.test(newtxt))  { console.log('B'); return false; } // any of [digits] may not be followed by a new +
		}
		if(this.state.type==INPUT_BDATE) { 
			
			const v = el.value+ch; // value forecast, as ch is not yet written in the field on keypress()
			if(v.length==1) if(/[456789 .\-]/.test(ch)) return false; // no leading space, dot, coma or minus, nor 456789 (month day number)
			if(v.length==4) if(/[23456789 .\-]/.test(ch)) return false; // no leading space, dot, coma or minus, nor 23456789 (month number)
			if(v.length==7) if(/[03456789 .\-]/.test(ch)) return false; // no space, dot, coma or minus, nor 456789 (month day number)
		...
		if(this.state.type==INPUT_PRICE) { // authorizes '.0-9-'+cr
				const cc = (/[,\uFF0C\uFF64\uFE50\u201A\u060C]/u.test(ch)) ? '.' : ch; // immediately converts ','(and any exotic similar UTF-8) into a nice international dot '.'
			const isanewdot = (cc == '.');
			const hasadotyet = el.value.indexOf('.')+1; // 0 means no occurence

		// console.log(' ch: |'+ch+'|'+', cc: |'+cc+'|'+', isanewdot: |'+isanewdot+'|'+', hasadotyet: |'+hasadotyet+'|');

			if(isanewdot) { // has a dot already
				if(hasadotyet) return false;
				el.value = (el.value+'.'); // we impose the right digit
				return false;
			} else
				if(key==C_KEY.code.s.enter) {
					const newdigits = this.digits();
					if(newdigits.indexOf('.')==-1) this.set(newdigits*100); // such that 10 + Enter makes it 10.00€
					if(vbs) vlog('mobframe.js','C_iEDIT','keypress[INPUT_PRICE, ENTER]','eid:'+this.eids.ui+', newdigits:'+newdigits);
					return false;
				} else {
					// these a regular digits
					
					if(hasadotyet) { // only 2 digits max after a .
						const decimals = el.value.slice(hasadotyet);
		...
		// dg = this.digits();
		// console.log('>>2 |'+newtxt+'| ('+newtxt.length+'chars)');
		
		return true; // ch gets adopted
	},
	keyup: function(e) { // this event is triggered after the character appeared in the input field
		...
		let newtxt = this.digits();
		if(vbs) vlog('mobframe.js','C_iEDIT','keyup','eid:'+this.eids.ui+', caret pos:'+this.statecaretvalue()+', key:'+key+', newtxt: |'+newtxt+'| ('+newtxt.length+'chars)');
	...
	onfocus: function() { 
		...
		console.log('mobframe.js','C_iEDIT','onfocus','eid:'+this.eids.ui+'caret pos:'+this.statecaretvalue());
		return false;
	},
	onblur: function() { 
	...
	passwords: /^(?=(.*[a-zØ-öø-ÿ]){2})(?=(.*\d){2}).{9,32}$/, // this should stay aligned with (*cr11*), example of usage stands here (*cr14*)
	logins: /^(?=(.*[a-zØ-öø-ÿ]){2})(?=(.*[A-ZÀ-Ö]){2})(?=(.*\d){2})(?=(.*[\s\[\]\/+\-='€?_,.:;(){}!@$%&*]){2}).{9,32}$/, // note that we secretly authorize the single MSwindows single quote
	isPassword: function(sText) { // see (*cr03*)
		let valid = true;
		if(!sText) {
			// if the field is mandatory, it displays an appropriate msg
			if(this.state.mandatory) if(!this.state.filled) valid = false;
		}
		else
			if(!this.passwords.test(sText)) valid = false;
		return valid;
	}, 
	isLogin: function(sText) { // see (*cr03*)
		let valid = true;
		if(!sText) { 
			// if the field is mandatory, it displays an appropriate msg
			if(this.state.mandatory) if(!this.state.filled) valid = false;
		} else
			if(!this.logins.test(sText)) valid = false;
		return valid;
	}, 
	isUniqueLogin: function(sText) {
		this.state.valid = true;
	...
	isMobile: function(sText) {
		...
		let NSNdigits = function(digits) { // National Significant Numbers, check also (*nsn*)
			...
			// please keep this function in sync with this.mobileplaceholder()
	...
	isLoginFree: function(dS, stream) { // stream is 0 or the id of an existing login
		let isfree = false;
		// stream = stream.split('#').shift(); // keeps the heading part
		const io = stream.extract('<in>','</out>').match;
		if(io) { // login is authorized when not in use yet, or when in use by this same loginId
			let ids = io.split('!');
			for(let x in ids) { let id = ids[x]; if(id == this.state.loginId) { isfree = true; break; } } // allows multiple identical login names
		} else
			isfree = true; // the login name is not used yet
		
		if(!isfree) { 
			this.state.valid = false; // if is free, does not change the valid settings set by this.isLogin(), where format check is done
			if(!this.state.mandatory) if(this.digits()=='') this.state.valid = true;
		}
		if(this.callbacks.onfreelogin) this.callbacks.onfreelogin.cb(isfree);
		this.setvalidtip();
	},
	isTokenFree: function(dS, stream) { // stream is 0 or the id of an existing login
		this.state.valid = false;
		if(stream) { // eresa prefix is authorized when not in use yet, or when in use by loginId
			// if(stream == this.state.loginId) this.state.valid = true;
			// if(stream == 0) this.state.valid = true;
			const io = stream.extract('<in>','</out>').match; // PVH2025 : added in / out tags
			if(io == this.state.loginId) this.state.valid = true;
			if(io == 0) this.state.valid = true;
		}
		this.setvalidtip();
	}
...
var gendercode = { female:0, male:1, sa:2, sprl:3, miss:4, boy:5, girl:6, other:7 }; // see (*gc01*)

var genders = { english:(function(){ // builds object like { 0:'female', 1:'male', ... }
		const genders = {};
	for(let m in gendercode) genders[gendercode[m]] = m;
	genders[gendercode.other] = 'othergender';
	return genders;
})() }
...
C_XL.ccss = function(oclass, csstype, options) { // options like { typeonly:bool, plural:bool, translate:bool }
	...
	switch(oclass) {
		...
		case class_product: 			postfix = 'product'; break;
	}
...
C_iPOSITION = function(padElement, options, bodyElement) {
	...
	if(options.mouse) { // positionning & swap is relative to a click event on the screen
		...
		this.mouse = { x:options.mouse.event.pageX, y:options.mouse.event.pageY };
		this.anchor = this.mouse; // to what spot the pad is hooked up
		this.mouseAnchor();
	} else // default to screen positionning
		this.screenAnchor(bodyElement);
...
C_iPOSITION.prototype = {
	...
	mouseAnchor: function(padElement) { // in this mode, the Pad appears under the mouse click
		...
		this.setSwap();
		this.setDisplayPosition(); // records new position according to swap conditions		
	},
	screenAnchor: function(bodyElement) { // hooks up the modal according to its size & modal size, centered on screen
		...
		const sizemode = { x:(bodyElement.style.width!='')?'fixed':'free', y:(bodyElement.style.height!='')?'fixed':'free' };
		...
		const size = { x:this.padxy.elsize.w, y:this.padxy.elsize.h }; // measures the full modal size
		const padding = { x:(C_iWIN.size.w-size.x)/2, y:(C_iWIN.size.h-size.y)/4 };
		
		// console.log('C_iPOSITION.screenAnchor()');
		// console.log('window size:',C_iWIN.size,'modal size:',size,'final padding:',padding);
		// console.log('this.offset:',this.offset);

		this.display = { x:padding.x+this.offset.x, y:padding.y+this.offset.y };
...
C_iMODAL = function(html, callbacks, preset) { 
	...
	// preset.style can be ['modal','pad','dp','message', ...]
	...
	const layer = C_iMODAL.layer;
	const mid = C_iMODAL.ever++; // builds a unique id for each modal (layer is not enough if we want to animate the fade off of modals)
	
	this.callbacks = callbacks; // { escape:, top:, left:, bottom:, right:, scrollstart: } all members are A_cb
	
		const eidp = '_'+layer+'_'+mid;
	this.eids = { overlay:'mOver'+eidp, outset:'mOutset'+eidp, inset:'mInset'+eidp, header:'mHeader'+eidp, body:'mBody'+eidp, busy:'mBusy'+eidp  }; 
	this.elements = new A_el();
	
	const events = { escape:new A_cb(this, this.escape), scroll:new A_cb(this, this.scroll)
				...
				, mouseleave:new A_cb(this, this.mouseleave)				}
	this.handlers = new A_hn(events);
	this.status = C_iMODAL.defauts.align(preset);
	
	// console.log('C_iMODAL.this.status',this.status);
	// console.log('---------------');
	
	this.interactivity = { dragging:false, mouse:false  }
	if(is.tactile && this.status.style=='modal') this.status.fullscreen = true; 
	
	this.is = { screen:false };
	if(this.status.position.drop||this.status.position.side||this.status.position.mouse) { // the modal is positionned relatively to another element on page (case of e.g. a drop down list)
	...
	C_iMODAL.register[++C_iMODAL.layer] = this;
		...
		// console.log('new C_iMODAL on layer: '+C_iMODAL.layer);
...
C_iMODAL.defauts = new A_df( { morecss:'',  style:'modal', location:{x:0, y:0}, moves:false, fullscreen:false, autopop:true, embedded:false
	, position:{ style:'', offset:{x:0, y:0}}, size:{x:'', y:'', maxy:false, miny:false } /*free size by default*/
	, busy:false, scrolling:false, isclosed:false, fadeoff:300, invisible:false /* fadeoff are ms, at modal close it is the time before removal of the <div outset> from dom. Css visual effects must finish before fadeoff delay. See (*md03*) */  
	, flies:false // a flying modal escapes as the mouse pointer moves away from its borders
});
C_iMODAL.layer = 0;
C_iMODAL.ever = 0;
...
C_iMODAL.prototype = {
	...
	busy: function(onoff) {
		if(onoff && !this.status.busy) {
			const zindex = 1000+C_iMODAL.layer;
			
				const spinner = '<div class="spinner">'+'</div>';
				const busystyle = 'z-index:20; position:absolute; top:0; left:0; width:100%; height:100%; overflow:hidden; display:flex; justify-content:center; align-items:center;';
			const overlay = '<div class="modal-busy" id="'+this.eids.busy+'" style="'+busystyle+'">'+spinner+'</div>';
			...
			const element = document.getElementById(this.eids.busy);
	...
	mposition: function() { // lets the modified modal inner be replaced as required on screen (not involved for pads, messages or actionpads
		const position = new C_iPOSITION(this.elements.inset, this.status.position, this.elements.body); // adjust size and position
		
		let screenoffsetx = 0, screenoffsety = 0;
		const xfactor = (C_iMODAL.layer-1)<<6; // shifts the modal according to its layer position, in such a way that they do not overlap perfectly
		const yfactor = (C_iMODAL.layer-1)<<5;
		if(this.is.screen) {
			if(this.status.position.offset.x == 0)
				screenoffsetx = xfactor; // avoid alignment of many modals opened
			if(this.status.position.offset.y == 0) {
					let yscroll = bodyscroll(); // put modal on visible body even when body is scrolled
				if(yscroll) screenoffsety = yscroll;
				screenoffsety += yfactor;
			} 
		}
		const x = position.display.x+screenoffsetx;
		const y = position.display.y+screenoffsety;
		
		// console.log('C_iMODAL.mposition('+x+','+y+')');
		// console.log('C_iMODAL.layer: '+C_iMODAL.layer);
		// console.log('-------------------------------------------------');
		
		this.position(x,y); // actually puts the DOM element in place
		return this;
	},
	...
	setmborder: function(ccode) {
		const inset = this.elements.inset;
	...
	display: function(html) { // html is like { header:'some html for the header', body:'some html for the body'}
		const zindex = 100+C_iMODAL.layer;
		const style = this.status.style; // that is e.g 'modal' or 'message' or 'info'
		
		const p = this.status.fullscreen?'fs-':'zi-'; // fs for full screen, zi for z-indexed
		const c = { header: style+'-header', body: style+'-body', overlay: style+'-overlay'	// makes modal-body, or pad-body, modal-overlay
		...
		const header = html.header==undefined ? '' : '<div class="'+c.header+'" id="'+this.eids.header+'">'+html.header+'</div>';
			
			const inner = html.body?html.body:'<div></div>'; // when inner is empty, the body div does not repond to the height:100% css rule
			const maxheightpad = is.newtouch?' height:100%; max-height:100%;':'';
			const bd = '<div class="'+c.body+'" id="'+this.eids.body+'" style="position:relative;'+maxheightpad+'">'+inner+'</div>'; // relative necessary for iscroll and for process (*sc03*).
		const body = '<div class="body-wrapper">'+bd+'<div>';
		
			const moreclass = this.status.morecss?(this.status.morecss.inset?(' '+this.status.morecss.inset):''):'';
		const inset = '<div class="'+c.inset+' '+c.shape+moreclass+'" id="'+this.eids.inset+'" style="position:relative;'+maxheightpad+'">'+header+body+'</div>';
		...
		if(this.status.fullscreen) // then modals are full screen
			...
			outset = '<div class="'+c.outset+' '+c.shape+' onscreen '+moreclass+'" id="'+this.eids.outset+'" style="position:absolute; z-index:'+zindex+'; '+bottom0px+'">'+inset+'</div>';
		}
	...
	activate: function() { // this modal is activated through this.pop(); and should NOT be reactivated thereafter programmaticaly
		this.elements.collect(this.eids);
		...
		if(is.tactile && !this.status.fullscreen) { // TACTILE PADS that are NOT FULLSCREEN
			...
			if(this.status.flies) $(this.elements.outset).mouseleave(this.handlers.mouseleave);
			else $(this.elements.overlay).click(this.handlers.escape);
			
			if(this.elements.header && this.status.moves) $(this.elements.header).mousedown(this.handlers.down);
		}
		
		if(this.status.style == 'message' || this.status.style == 'info') {
	...
	show: function() {
		...
		if(this.status.fullscreen) { 
			...
			$(this.elements.body).width(this.status.size.x).height(this.status.size.y);
				const maxy = this.status.size.maxy; // is a string like '18em', cannot be devided
				const miny = this.status.size.miny; // is a string like '18em', cannot be devided
			if(is.newtouch) {
				// maxy=false; 
			...
			// console.log('C_iMODAL.show() - position',position);
			// console.log('C_iMODAL.layer: '+C_iMODAL.layer);
			// console.log('     -----------------------');
		}
		...
		if(is.tactile) { 
				...
				const options = { hScroll:false, bounce:true, momentum:true, scrollbars:true };
	...
	position: function(x,y) { // does position the outset on the screen
	
			// console.log('C_iMODAL.position('+x+','+y+')');
			// console.log('     -----------------------');
		if(this.status.fullscreen) return;
		if(this.status.invisible) x = -x -3000; // in invisible mode, the modal is placed outside the viewport.
	...
	toremove: function(options) {
		...
		if(this.status.flies) $(this.elements.outset).unbind('mouseleave');
		else $(this.elements.overlay).unbind('click', C_iMODAL.click).addClass('offscreen'+cssslow); // this item stays temporarily idle until removed by this.remove()
		
		$(this.elements.outset).removeClass('onscreen').addClass('offscreen'+cssslow);

		if(options.now) return this.remove();
		if(delay) // soft removal using animation defined in controls.css
	...
	// handlers
	escape: function() {
		// console.log('C_iMODAL escape');
		if(this.status.isclosed) return false;
		C_iTIP.handlers.quit(); 
		let remove = true;
	...
	scroll: function(e) {
		const scrollPosition = this.elements.body.scrollTop;
		const scrollMax = this.elements.body.scrollHeight-this.elements.body.clientHeight;
		if(scrollPosition<=0) {
	...
	move: function(e) {
		if(this.interactivity.dragging) { 
			const vector = { x:e.pageX-this.interactivity.mouse.x, y:e.pageY-this.interactivity.mouse.y };
			this.vector(vector);
		}
		this.interactivity.mouse = {x:e.pageX, y:e.pageY};
		return false;
...
C_tMODAL = function(html, callbacks, preset) { // this one is for the contextual menus (most used in hourlies)
	this.html = html;	
	this.elements = new A_el();
	this.callbacks = callbacks;
...
C_tMODAL.prototype = {
	...
	// public
	display: function() {
		const header = '<div id="'+this.eids.header+'">'+this.html.header+'</div>';
			
			const padder = '<div style="height:10em; border-bottom:5px solid silver;">&nbsp;</div>'
		const slide = '<div id="'+this.eids.slide+'" class="iSlide">'+this.html.body+padder+'</div>';
		const wrapper = '<div id="'+this.eids.wrapper+'" style="position:relative; height:100%; max-height:100%; width:100%;">'+slide+'</div>';
		const vwport = '<div id="'+this.eids.vwport+'" style="position:absolute; top:'+this.state.position.top+'; bottom:0; min-width:100%; max-width:100%; overflow:hidden;">'+wrapper+'</div>';

		const btop = '<div id="'+this.eids.bouncytop+'" class="minder-background" style="z-index:10; position:absolute; top:'+this.state.position.top+'; min-width:100%; max-width:100%; overflow:hidden; display:none;">'+'TOP'+'</div>';
		const bbot = '<div id="'+this.eids.bouncybottom+'" class="minder-background" style="z-index:10; position:absolute; bottom:0; min-width:100%; max-width:100%; overflow:hidden; display:none;">'+'BOTTOM'+'</div>';
		
		return header+vwport+btop+bbot;
	},	
	activate: function() {
		this.elements.collect(this.eids);
		const handlers = new A_hn({ scrolstart:new A_cb(this, this.scrolstart), scrolldone:new A_cb(this, this.scrolldone), bounce:new A_cb(this, this.bounce) });
		const options = { 
...
C_KEY = function(keys, onhit, client) { // This is how you bind a callback function to a given keyboard key. Watch the return value of your call back function!
	...
	for(let x in keys) {
		...
		if(verboseOn) { // get a verbose detailed insight on what keys are associated to what event, activate verboseOn in mobframe.js and choose classOnlyVerbose = monitorFramework;
				...
				const l = layer;
				let ctrlkey = '', keyname = '';
				const iaf = arrayINVERT(C_KEY.code.alpha);
				const inm = arrayINVERT(C_KEY.code.num);
				const isp = arrayINVERT(C_KEY.code.s);
				const ifn = arrayINVERT(C_KEY.code.F);
				const ikn = arrayINVERT(C_KEY.code.kpad.n);
				const iks = arrayINVERT(C_KEY.code.kpad.s); // console.log(bitmap(k)+' check on 2048 '+bitmap(k&2048)+' -> '+((k&2048)==2048) );
...
C_KEY.cleanUpLayer = function(mlayer) { // removes all associated callbacks from the layer // see (*md01*)
	if(vbs) vlog('mobframe.js','C_KEY','cleanUpLayer','cleaning up layer '+mlayer);
	let r = C_KEY.register[mlayer]; if(!r) return false; // in the current layer, there is no callback defined
	delete C_KEY.register[mlayer];
};
C_KEY.count = 0;
C_KEY.register = new Array(); // we organize it like C_KEY.register[C_iMODAL.layer][key] = C_KEY instance
C_KEY.onkeydown = function(e) {	
	const keycode = (e.keyCode) ? e.keyCode : e.which; // MSIE compatibility
	if(keycode == 16 || keycode == 17 || keycode == 18) return false;
	const checkCode = C_KEY.specials.check(e); // note that a single Ctrl key down this not trigger C_KEY.onkeydown, that happens only if together another keyboard key si pressed
	let lastPropagation = true;
	const r = C_KEY.register[C_iMODAL.layer]; if(!r) return lastPropagation; // in the current layer, there is no callback defined
...
(function (w,d) {  // disables backspace on page except on input fields and textarea.
...
C_iTABLE.prototype = {
	...
	display: function(css) { 
		...
		const table = '<table id="'+this.eids.wtbl+'" class="C_iTABLE '+(css||'')+'">'+this.rows()+'</table>';
		return table;
	},
	activate: function() { 
	...
	rows: function() { // display with top/bottom mode: <div>title</div><table>control</table>
			...
			const i = this.items.cnt(); if(!i) return '<tr><td></td></tr>';
			
		// size the table
			const mxr = this.state.maxrows, mxc = this.state.maxcols; 
		...
		r = 1+((i/c)|0);
			const e = r*c-i; // empty cells remaining in table when all items are placed
			const f = c-e; // number of columns that are fully filled, other columns miss each one item
...
//
// Smoothly scrolls element `el` to vertical position `target` over `duration`ms.
//
...
C_iPLAY = function(presets, altformats) { // provide alternative formats for browsers portability
	...
	for (let i=1; i<arguments.length; i++){
		const filename = this.state.path+arguments[i];
		const sourceel = document.createElement('source'); sourceel.setAttribute('src', filename)
...
const ctrlshiftplay = function(ctrlshift) {
	switch(ctrlshift) {
		case 1: mobminder.sounds.droplet.play(); break; // Ctrl // droplet, popup
		case 2: mobminder.sounds.attention.play(); break; // Shift whatif
		case 3: mobminder.sounds.unexpected.play(); break; // Ctrl + Shift
	}
}
