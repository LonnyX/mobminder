...
class C_b64_email { // this is the 2023 version by BSP, featuring base64 encoding and files attachment
	...
	public function sendMail($addressee, $vid=0, $replyTo='', $subject='', $htmlbody='', $bgcolormail='') { // invokes PHP mail to mailsever
		...
		if($bgcolormail)
		{
			$this->bgcolormail = $bgcolormail; // can be passed at construction or at send() event
		}
		...
		$unsubvid = ''; if($vid) $unsubvid = '?vid='.$vid;
		$headers .= 'List-Unsubscribe: <https://www.mobminder.com'.$unsubvid.">\r\n";
