...
//  - true => https://api.ext.payconiq.com
//  - false=> https://api.payconiq.com
//
// for TEST env. : do not forget to update payconiq feedback url (using ngrok) at 2 places
// 1. app05/booking/post/reservation.php (online booking)
// 2. app05/htdocs/post/payment.php (webapp)
//
/////////////////////////////////////////////////////////////////////////
//define('payconiqtesturl',false); 
define('tmpcomputopcapturemode',false); 
define('forceaccountidstosuccess','-1'); //3925 -1 (comma separator)

/////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////
//  FEEDBACK TEST / ENVIRONMENT  
//  prefix url for payconiq AND computop feedbackurl
//	(use ngrok for local dev environment)
// do not forget to change ngrok key (cc22-109-89-149-142) every time you start a new ngrok session

//define('ngrokkey','17a29f372a4f.ngrok-free.app');
//define('payconiqcallbackurl','https://'.ngrokkey.'/mobminder/htdocs/feedback/payconiq.php');
//define('computopcallbackurl','https://'.ngrokkey.'/mobminder/htdocs/feedback/computop.php');
//define('computopreturnurl','https://'.ngrokkey.'/booking/');

define('payconiqcallbackurl','https://be.mobminder.com/feedback/payconiq.php');
define('computopcallbackurl','https://be.mobminder.com/feedback/computop.php');
define('computopreturnurl',  'https://booking.mobminder.com/');


function tmplog($log)
{
	$time = @date('[d/M/Y:H:i:s]');
    $result = file_put_contents('/var/log/computop.log',$time.' - ' . $log . "\n", FILE_APPEND | LOCK_EX);
}

function isAccountIdTestForPayment($accountid)
{
    $integerIDs = array_map('intval', explode(',', forceaccountidstosuccess));
    return in_array($accountid, $integerIDs);
}
...
//create payment allowed values: PENDING
//callback  llowed values: PENDING, IDENTIFIED, AUTHORIZED, AUTHORIZATION_FAILED, 
//                         SUCCEEDED, FAILED, CANCELLED, EXPIRED.

function converterPayconiqtoPaymentStatus($status)
{
    if($status=='AUTHORIZED') 					return payment_status_pending;       // => pending
    else if($status=='AUTHORIZATION_FAILED') 	return payment_status_unauthorized;  // => unauthorized   (final status)
    else if($status=='CANCELLED') 				return payment_status_cancelled;     // => cancelled   (final status)
    else if($status=='EXPIRED') 	return payment_status_expired;    //=> expired        (final status)
    else if($status=='FAILED') 	return payment_status_failed;     //=> failed         (final status)
    else if($status=='IDENTIFIED') return payment_status_identified; //=> identified (scanned by payconiq mobile app)
    else if($status=='PENDING') 	return payment_status_pending;    //=> pending
    else if($status=='SUCCEEDED') 	return payment_status_success;    //=> success        (final status)
    
    else return payment_status_failed; //failed
}

/*function converterComputoptoPaymentStatus($status)
{
    //waiting for computop answer about status returned if payed or if captured???
    if($status=='AUTHORIZED') 
    {
        if(tmpcomputopcapturemode)
            return payment_status_reserved;
        else
            return payment_status_success;  //=> success    (final status)  
    }
    else if($status=='OK') 	    return payment_status_success;  //=> success    (final status)
    else if($status=='FAILED') 	return payment_status_failed;   //=> failed     (final status)
    else                            return payment_status_failed;    //=> failed     (final status)
}*/

function truncateString($source,$maxlen)
{
    return (strlen($source) > $maxlen) ? substr($source, 0, $maxlen) : $source;
}

class C_PayconiqCreatePaymentResponse
{
    public $status;
    public $paymentid;
    public $reference;
    ...
    public function __construct($json,$returnurl,$payconiqtesturl)
    {
        //echo $json;
        ...
        $this->qrcodeurl = $obj->{'_links'}->{'qrcode'}->{'href'};
        $this->deeplink=strtolower($obj->{'_links'}->{'deeplink'}->{'href'});
        
        if(property_exists($obj->{'_links'}, 'checkout'))
        {
            $this->checkouturl = $obj->{'_links'}->{'checkout'}->{'href'};

            //bug in payconiq api ! test env send prod url
            if($payconiqtesturl)
            {
                //echo 'checkouturl=test='.$this->checkouturl."\n";
                $this->checkouturl=str_replace('payconiq.com','ext.payconiq.com',$this->checkouturl);
            }
            //else
            //{
            //    //echo 'checkouturl=production'."\n";
            //}
        ...
        /*if(property_exists($data, 'debtor')) 
        {
            $this->debtorname=$data->{'debtor'}->{'name'};
            $this->debtoriban=$data->{'debtor'}->{'iban'};
        }*/


        if(isset($returnurl))
        {
            $this->universalurl = $this->deeplink.'?returnUrl='.urlencode($returnurl);
        }
        else
        {
            $this->universalurl = $this->deeplink;
        }
    }
}
class C_PayconiqCallbackResponse
{
    public $paymentid;
    public $status;
    public $reference;
    ...
    public function __construct($json)
    {
        ...
        if(property_exists($data, 'debtor')) 
        {
            ...
            if(property_exists($debtor, 'name')) 
                $this->debtorname=$debtor->{'name'};
            
            if(property_exists($debtor, 'iban')) 
                $this->debtoriban=$debtor->{'iban'};
        }
    }
}
class C_PayconiqGateaway
{
    //https://developer.payconiq.com/online-payments-dock
    ...
    public function __construct($key=false)
    {
        ...
        //$this->payconiqtesturl = ($key=='22868856-4835-4018-9130-4c7a9442ef90');
        $this->payconiqtesturl = ($key=='0a580cff-5362-4363-b4d6-9d7e42115fa5');
            

        //EXT URL Definition: https://api.ext.payconiq.com/v3/payments
        //PROD URL Definition: https://api.payconiq.com/v3/payments
        if($this->payconiqtesturl)
        {
            //$this->baseurl = 'https://api.ext.payconiq.com';
            $this->baseurl = 'https://merchant.api.preprod.bancontact.net';
        }
        else
        {
            //$this->baseurl = 'https://api.payconiq.com';
            $this->baseurl = 'https://merchant.api.bancontact.net';
        }

        $this->payconiqheaders = [
            'Authorization: '.$key,
    ...
    private function curlPostbase($requestbody,$suffixurl)
    {
        ...
        $fullcurl = $this->baseurl.$suffixurl;
    ...
    public function createPayment($amountincent,$description,$reference,$returnurl) 
    {
        ...
        (
            'amount' => $amountincent,
            'currency' => 'EUR',
            'description' => truncateString($description,140),
        ...
        $suffixurl = '/v3/payments';

        $result = $this->curlPostbase($requestbody,$suffixurl);
        //echo($result);

        if(!$result)
        {
            //lasterrorcode and lasterrormessage are set by curlpostbase
            return false;
        }
        else
        {
            if(substr($this->lasthttpcode,0,1)=='2') //success
            {
                ...
                return new C_PayconiqCreatePaymentResponse($result,$returnurl,$this->payconiqtesturl);
            }
            else if(substr($this->lasthttpcode,0,1)=='4'||substr($this->lasthttpcode,0,1)=='5') //payconiq error
            {
                ...
                {
                    $this->lasterrorcode=$obj->{'code'};
                    $this->lasterrormessage=$obj->{'message'};
                    return false;
                }
                else
                {
                    $this->lasterrorcode= 'OTHER_ERROR';
                    $this->lasterrormessage=$result;
                    return false;
                }
            }
            else
    ...
    {
        $suffixurl = '/v3/payments/'.$paymentid;
        ...
        if(substr($this->lasthttpcode,0,1)=='2') //success
        {
            ...
            return new C_PayconiqCreatePaymentResponse($response,null,$this->payconiqtesturl);
    ...
    {
        //default payconiqkey = false in constructor)
        //key = false 
        //=> payconiqtesturl = false;
        //=> host = production url => ok
...
class C_SepaPayment
{
    //https://fr.qr-code-generator.com/solutions/epc-qr-code/
	...
	//https://blog.yavin.com/paiement-electronique-belgique/
	//https://support.accountingplatform.eu/fr/support/solutions/articles/9000203504-utiliser-le-code-qr-comme-mode-de-paiement
	//https://www.b2bpay.co/sepa-countries
	//https://www.scan2pay.info/#where
    ...
    public function __construct($beneficiaryaccountnumber,$beneficiaryname)
    {
    ...
    public function createQrCodeString($amountincent,$description)
    {
        if(empty($this->beneficiaryaccountnumber))
        {
            $this->lasterrorcode='MISSINGACCOUNTNUMBER';
            $this->lasterrormessage='beneficiary account number is empty';
            return false;
        }
        if(empty($this->beneficiaryname))
        {
            $this->lasterrorcode='MISSINGBENEFICIACYNAME';
            $this->lasterrormessage='beneficiary name number is empty';
            return false;
        }
        if(empty($amountincent))
        {
            $this->lasterrorcode='MISSINGAMOUNT';
            $this->lasterrormessage='amount is empty';
            return false;
        }
        /*if(!is_int($amountincent))
        {
            $this->lasterrormessage='amount must be a number >= 100';
            return false;
        }*/
        /*if($amountincent<100)
        {
            echo 'amountincent='.amountincent;
            $this->lasterrormessage='amount must be a number >= 100';
            return false;
        }*/
        ...
        {
            $this->lasterrorcode='MISSINGDESCRIPTION';
            $this->lasterrormessage='description is empty';
            return false;
        }

        //old algo is not working with number smaller than 100 (1€)
        //$samount = strval($amountincent);
        //$pos = strlen($samount)-2;
        //$samount = substr($samount, 0, $pos) . '.' . substr($samount, $pos);
        
        $samount = $amountincent / 100;
        ...
        //AT-23 BIC of the Beneficiary Bank The BIC will continue to be mandatory for SEPA payment transactions involving nonEEA countries.
        //The BIC will remain mandatory for payment transactions from PSPs from EU /EEA Member States using another currency than the euro until 31 October 2016.
        //V1: M
        //V2: O/M

        $separator = "\n";
        $response = 'BCD'.$separator                        //Service Tag : 'BCD'
                    .'002'.$separator                       //Version 001 or 002
                    .'1'.$separator                         //character set 1=UTF-8
                    .'SCT'.$separator                       //identification code : 'SCT'
                    .''.$separator                          //AT-23 BIC of the Beneficiary Bank (empty in V2 for EUR curreny)
                    .truncateString($this->beneficiaryname,70).$separator            //AT-21 Name of the Beneficiary
                    .truncateString($this->beneficiaryaccountnumber,34).$separator          //AT-20 Account number of the Beneficiary
                    .'EUR'.$samount.$separator                  //AT-04 Amount of the Credit Transfer in Euro
                    .''.$separator                          //AT-44 Purpose of the Credit Transfer
                    .''.$separator                          //AT-05 Remittance Information (Structured)
                    .truncateString($description,140).$separator    //AT-05 Remittance Information (Unstructured)
...
class C_ComputopRequest
{
    ...
    //rmq 1 cardholder could contain space character, even if space are send as url parameter
    //because parameters string is splited by '&' on front side and send as POST parameter.
    
    //rmq 2 url could exceed 512 characters and will be trunced when save into database
    
    //MerchantID/Len/Data/Language/$Url => common parameters
    //number/securityCode/expiryDate/brand/cardholder => only used by credit/debit cards
    //template => only used by bancontact EMS 
    public function __construct($MerchantID_,$Len_,$Data_,$Language_,$Url_,$number=false,$securityCode=false,$expiryDate=false,$brand=false,$cardholder=false,$template=false)
    {
        $this->MerchantID=$MerchantID_;
        $this->Len=$Len_;
        $this->Data=$Data_;
        ...
        if($number==null){
            $params = array('MerchantID='.$MerchantID_,'Len='.$Len_,'Data='.$Data_,'Language='.$Language_);
        }
        else{
            $params = array('MerchantID='.$MerchantID_,'Len='.$Len_,'Data='.$Data_,'Language='.$Language_,
            'number='.$number,'expiryDate='.$expiryDate,'brand='.$brand,'cardholder='.$cardholder); //'securityCode='.$securityCode,
        }
        ...
        $this->GETUrl=$Url_.'?'.$plaintext;
    }
}
class C_ComputopResponse
{
    private $Values;

    public function __construct($Values_)
    {
       $this->Values = $Values_;
    }
    private function getValue($key)
    {
        ...
        return $myPayGate->ctSplit($this->Values, '=', $key);
    }
    public function getMid()        { return $this->getValue('mid');}
    public function getPayID()      { return $this->getValue('PayID');}
    public function getTransID()    { return $this->getValue('TransID');}
    public function getXID()        { return $this->getValue('XID');}
    public function getCode()       { return $this->getValue('Code');}
    public function getStatus()     { return $this->getValue('Status');}
    public function getDescription(){ return $this->getValue('Description');}
    public function getType()       { return $this->getValue('Type');}
    public function getMsgver()     { return $this->getValue('msgver');}
    public function getMAC()     { return $this->getValue('MAC');}
    //threedsdata
    //resultsresponse

}
class C_ComputopGateaway
{
    public $curltimeout = 2000;

    private $MerchantID;
    private $BlowfishPassword;   
    private $HmacPassword;  


    public function __construct($BlowfishPassword_,$MerchantID_=false,$HmacPassword_=false)
    {
        $this->BlowfishPassword=$BlowfishPassword_;
        $this->MerchantID=$MerchantID_;
        $this->HmacPassword=$HmacPassword_;
    }
    public function parseResponse($request)
    {
        ...
        $Values = explode('&', $plaintext);
        return new C_ComputopResponse($Values);
    }
    public function parseResponseText($request)
    {
        //echo "parseResponseText=".$request;
        $tmp = explode('&', $request);
        foreach ($tmp as $row)
        {
            $tmp2 = explode('=', $row);
            if($tmp2[0]=='Len')  $Len = $tmp2[1];
            else if($tmp2[0]=='Data')  $Data = $tmp2[1];
        }
    ...
    public function createDirectPostRequest($TransID,$Amount,$OrderDesc,$ReturnUrl,$accountId,$languageIso2,
        $number,$securityCode,$expiryDate,$brand,$cardholder,
        $colordepth,$screenheight,$screenwidth,$timezoneoffset,$visitor=false)
    {
        $sMerchantID='MerchantID='.$this->MerchantID;
        $Currency='EUR';
        $Msgver='2.0';
        $pTransID = 'TransID='.$TransID;
        $pAmount = 'Amount='.$Amount;
        $pCurrency = 'Currency='.$Currency;
        $pURLSuccess = 'URLSuccess='.$ReturnUrl;
        $pURLFailure = 'URLFailure='.$ReturnUrl;
        $pURLNotify = 'URLNotify='.computopcallbackurl.'?accid='.$accountId.'&paymean=cards';
        $pOrderDesc = 'OrderDesc='.$OrderDesc;
        $sMsgver='msgver='.$Msgver;
        $myPayGate = new ctPaygate;
        $MAC = $myPayGate->ctHMAC('', $TransID, $this->MerchantID, $Amount, $Currency, $this->HmacPassword);
        $pMAC = 'MAC='.$MAC;
        $pCapture = 'Capture=AUTO';

        //$json = '{"acceptHeaders": "text\/html,application\/xhtml+xml,application\/xml;q=0.9,image\/webp,image\/apng,*\/*;q=0.8","ipAddress": "2001:0db8:85a3:0000:0000:8a2e:0370:7334","javaEnabled": false,"javaScriptEnabled": true,"language": "en-US","colorDepth": 24,"screenHeight": 723,"screenWidth": 1536,"timeZoneOffset": "300","userAgent": "Mozilla\/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit\/537.36 (KHTML, like Gecko) Chrome\/70.0.3538.110 Safari\/537.36"}';
        /*$json = '{
        ...
        */
        ...
        //Notice: Meanwhile exist the new color depths 30 and 36 bits, but these are not included within den EMVCo specifications 
        //and should not be used because they can cause rejections. Please send the next lower value instead. Currently allowed are 1, 4, 8, 15, 16, 24, 32, 48 bits.
        if($colordepth==30) 
            $colordepth=24;
        ...
        {
            //BrowserInfo String 'fr,fr-FR;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6,fr-BE;q=0.5' exceeds maximum length. Path 'language'

            $httpacceptlanguage = $_SERVER['HTTP_ACCEPT_LANGUAGE']; //take all
            $httpacceptlanguage = explode(",", $httpacceptlanguage); //explode by ','
            $httpacceptlanguage = $httpacceptlanguage[0]; //take first item
        }
        


        $browserInfo = array(
            'acceptHeaders' => isset($_SERVER['HTTP_ACCEPT'])?$_SERVER['HTTP_ACCEPT']:'',
        ...
        $browserInfo = json_encode($browserInfo);
        $browserInfo = 'browserInfo='.base64_encode($browserInfo);
        ...
        ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        //add customer information, specially for VISA
        //Please note that Visa requires new mandatory fields for 3-D Secure transactions starting February 12, 2024. 
        //To be prepared, we recommend implementing the changes early.
        // According to Visa requirements, you will have to submit the following fields for Visa Secure transactions 
        //in addition to those already mandatory:
        //In JSON object “address”: AddressLine1, PostalCode, City, Country (in A3-format), State (if applicable)
        //In JSON object “customerInfo”: email, phone (or mobile phone)
        //We strongly advise you to make the necessary changes to avoid losses on future card payments. You can find all the 
        //information on implementing 3-D Secure and the new mandatory fields in our developer documentation. 
        //Our Merchant Services Team will gladly assist you with any problems that may arise during the changeover
        if (true && $visitor)
        {
            ...
            $billToCustomer = json_encode($billToCustomer);
            //tmplog('billToCustomer='.$billToCustomer);
            $bllToCustomer = 'billToCustomer='.base64_encode($billToCustomer);
            ...
            'postalCode' => '4520',
            //'state' => 'bspoden@gmail.com',
            );
            $billingAddress = json_encode($billingAddress);
            tmplog('billingAddress='.$billingAddress);
            $billingAddress = 'billingAddress='.base64_encode($billingAddress);*/

            $query = array($sMerchantID,$sMsgver,$pTransID, $pAmount, $pCurrency,$pMAC, $pURLSuccess, $pURLFailure, $pURLNotify, $pOrderDesc,$pCapture,$browserInfo,$billToCustomer);
        }
        else
        {
            $query = array($sMerchantID,$sMsgver,$pTransID, $pAmount, $pCurrency,$pMAC, $pURLSuccess, $pURLFailure, $pURLNotify, $pOrderDesc,$pCapture,$browserInfo); //,$billToCustomer,$billingAddress); 
        }
        ...
        $payurl = 'https://www.computop-paygate.com/payNow.aspx';
        
        $language=$languageIso2;
      
        return new C_ComputopRequest($this->MerchantID,$Len,$Data,$language,$payurl,
            $number,$securityCode,$expiryDate,$brand,$cardholder,false);
    }
    public function createBancontactEMSRequest($TransID,$Amount,$OrderDesc,$ReturnUrl,$timeoutsec,$accountId,$languageIso2,$capturemanual=false)
    {
        $sMerchantID='MerchantID='.$this->MerchantID;
        $Response = 'encrypt';
        $Currency='EUR';
        ...
        $pURLSuccess = 'URLSuccess='.$ReturnUrl;
        $pURLFailure = 'URLFailure='.$ReturnUrl;
        $pURLBack = 'URLBack='.$ReturnUrl.'?ctptransid='.$TransID;
        $pURLNotify = 'URLNotify='.computopcallbackurl.'?accid='.$accountId.'&paymean=bancontact';
        $pOrderDesc = 'OrderDesc='.$OrderDesc;
        $sMsgver='msgver='.$Msgver;
        ...
        //echo "date=".gmdate("Y-m-dH:i:s")."</br>"; 
        //echo "date=".gmdate("Y-m-dH:i:s",(time()+$timeoutsec))."</br>"; 
        
        $expirationTimeUTC = (time()+$timeoutsec);
        $expirationTime = gmdate("Y-m-d",$expirationTimeUTC).'T'.gmdate("H:i:s",$expirationTimeUTC);
        $sexpirationTime = 'expirationTime='.$expirationTime;

        //let urlback = ngrock+'/testing/poccomputop/back.php';
        //echo 'MerchantID='.$this->MerchantID.'</br>';
        //echo 'HmacPassword='.$this->HmacPassword.'</br>';
        //echo 'BlowfishPassword='.$this->BlowfishPassword.'</br>';

        if($capturemanual)
            $pCapture = 'Capture=MANUAL';
        else
            $pCapture = 'Capture=AUTO';

        $query = array($sMerchantID,$sMsgver,$pTransID,$sexpirationTime, $pAmount, $pCurrency,$pMAC, $pURLSuccess, $pURLFailure,$pURLBack, $pURLNotify, $pOrderDesc,$pResponse,$pCapture); //$pUserData,$pMAC
        $plaintext = join('&', $query);
        ...
        $payurl = 'https://www.computop-paygate.com/payssl.aspx';
        
        //with payment choice
        //$payurl = 'https://www.computop-paygate.com/paymentPage.aspx';
        
        $language=$languageIso2;
        $template='ct_bcmc';
      
        return new C_ComputopRequest($this->MerchantID,$Len,$Data,$language,$payurl,false,false,false,false,false,$template);
    }
    public function createBancontactPPRORequest($TransID,$Amount,$OrderDesc,$ReturnUrl,$accountId,$languageIso2)
    {
        $sMerchantID='MerchantID='.$this->MerchantID;
        $Response = 'encrypt';
        $Currency='EUR';
        ...
        $query = array($sMerchantID,$pTransID,$sexpirationTime, $pAmount, $pCurrency,$pMAC, $pURLSuccess, $pURLFailure, $pURLNotify, $pOrderDesc,$pResponse); 
        ...
        $payurl = 'https://www.computop-paygate.com/Bancontact.aspx';
        
        $language=$languageIso2;
      
        return new C_ComputopRequest($this->MerchantID,$Len,$Data,$language,$payurl,false,false,false,false,false,false);
    }
    
    public function confirmPayment($TransID,$Amount,$PayID,$confirm=true)
    {
        $sMerchantID='MerchantID='.$this->MerchantID;
        $Currency='EUR';
        $pTransID = 'TransID='.$TransID;
        $pReqId = 'ReqId='.$TransID;
        $pAmount = 'Amount='.$Amount;
        $pCurrency = 'Currency='.$Currency;
        $pPayID = 'PayID='.$PayID;

        $myPayGate = new ctPaygate;
        $MAC = $myPayGate->ctHMAC($PayID,$TransID, $this->MerchantID, $Amount, $Currency, $this->HmacPassword); //$PayID, 
        //function ctHMAC($PayId = "", $TransID = "", $MerchantID, $Amount, $Currency, $HmacPassword)
        
        $pMAC = 'MAC='.$MAC;
        //echo $pMAC."<br/>";

        $query = array($this->MerchantID,$pTransID,$pReqId,$pAmount, $pCurrency,$pMAC,$pPayID); ////$pUserData,$pMAC
        $plaintext = join('&', $query);
        //echo 'plaintext='.$plaintext.'</br>';
        $Len = mb_strlen($plaintext);  // Length of the plain text string
        $Data = $myPayGate->ctEncrypt($plaintext, $Len, $this->BlowfishPassword);
        ...
        if($confirm)
            $url = 'https://www.computop-paygate.com/capture.aspx';
        else
            $url = 'https://www.computop-paygate.com/reverse.aspx';
    

        $url = $url .'?'.$sMerchantID.'&'.$pLen.'&'.$pData;
        //echo 'url='.$url."<br/>";
        ...
        if(!$response) { // The server is unreachable
            ...
            //$errtitle = $code.'';
            $msg =  'curl error calling : '.$url;
            //echo $msg;
            C_dS_exception::put('lib_moblib/onlinepayment.php', 'The server is unreachable,C_ComputopGateaway.confirmPayment error',$msg);
            curl_close($curl);
            return false;
        }
        else
        {
            ...
            if(substr($code,0,1)=='4'||substr($code,0,1)=='5') 
            {
                ...
                $msg =  'curl error '.$code.' calling : '.$url;
                //echo $msg;
                C_dS_exception::put('lib_moblib/onlinepayment.php', 'C_ComputopGateaway.confirmPayment error',$msg);
                curl_close($curl);
                return false;
            }
            else
            {
                ...
                $result = $this->parseResponseText($response);
                /*$mid = $result->getMid();
...
class ctPaygate extends Blowfish
{
    ...
    function ctHMAC($PayId = '', $TransID = '', $MerchantID, $Amount, $Currency, $HmacPassword)
    {
        $tmp = "$PayId*$TransID*$MerchantID*$Amount*$Currency";
        //echo "tmp=".$tmp;
        return hash_hmac('sha256',$tmp , $HmacPassword);
    }
...
class Blowfish
{
    ...
    function bf_enc($ll, $r, $n)
    {
        ...
        $b1 = $this->sk[($r >> 24) & 0xff];
        $b2 = $this->sk[256 + (($r >> 16) & 0xff)];
    ...
    function sec_add($a, $b)
    {
        ...
        $highsum = $higha + $highb;
        $lowsum = $lowa + $lowb;
        # overflow low-addition add high-addition
        $highsum += ($lowsum & 0x00010000) >> 16;
