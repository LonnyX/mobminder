...
////////////////////////////////////////////////////////////////////////////////////////////////////////
//
//       Moby  A I   A P I   L I B R A R Y 
//
//   Note: this file is an equivalent of webapp::ajax_session, 
//   but implements more dev & debug functions and more security checks
//

require '../webrender.php';

require '../../lib_mobphp/language_basics.php'; // pvh202501: only used by ai api so far but hopefully /booking and /webapp will ever make use of a library (like here and in smartapp) that make possible the inclusion of language_basics.php
...
$jason = false; // streams data in jason format iso mobminder format

setrendermode();
set_urls($subdomainname = 'ai'); // see (*dv01*)

$xpected_alevel = [aLevel_synchro,aLevel_operator,aLevel_supervisor,aLevel_manager,aLevel_seller,aLevel_admin]; // this is the default for this api. It is used at C_apicontext construction
...
function explainstream($classname,$s,$fields) { // $s is the field separator. E.g. | or , 
	global $web;// if(!$web) return;
	$e = '#';
	$e.= $classname.': ';
		$S = '';
	...
	// #C_dS_reservation: id | created | creator | creatorId | changed | changer | changerId | deleted | deletorId | rversion | peerId | iscluster | cueIn | cueOut
}
...
function explainclass($instance, $fields, $sep) { // set fields to false to get an overview of all fields
		...
		if($classname=='C_dS_group') $classname='C_dS_account'; // special cast for clarity at AI side
		if($classname=='C_dS_login'||$classname=='C_dS_login_min') $classname='C_dS_collaborator'; // special cast for clarity at AI side
		
		if(!$fields) { // pile up all the possible fields in this class
			$fields = Array('id','groupId');
			foreach($dS->getDefaults() as $fieldname => $explained) $fields[] = $fieldname;
		}
			
		if($web) h4('class '.$classname);
		$brk = ''; if($web) $brk = '<br/>&nbsp;';
		notice(explainstream($classname,$sep,$fields).$brk);
			
		foreach($fields as $f) indent($f.': '.$dS->explain($f)); // 'o '. << if you want bullets like display
		if($web) pad();
		
		$AIinstx = $dS->AIinstruction();
		if(count($AIinstx)) {
			indent('<description>'); if($web) span('&ltdescription&gt');
			foreach($AIinstx as $which => $what) indent($which.': '.$what);
			indent('</description>'); if($web) span('&lt/description&gt');
		}
		if($web) pad();
}
...
function checkRequest4sqlInjection() {
	global $web;
	$sqlinj = SQLinjectionScreen($_REQUEST); // defined in dbio.php
	if($web) h2('Sql injection screening');
		if(!$sqlinj) { if($web) indent('no thread'); }
...
function fieldvalidation(&$value, $fname, $context, $strong = true) { // field by field checkup on allowed characters
	...
	switch($fname) {
		...
		case 'birthday': 
			...
			if(strlen($birthday)>=6) $mm = substr($birthday,4,2); else $mm = '';
				if($mm) if($mm<1 || $mm>12) abort('0084','Wrong birthday month value:'.$mm);
			if(strlen($birthday)==8) $dd = substr($birthday,6,2); else $dd = '';
				if($dd) if($dd<1 || $dd>31) abort('0085','Wrong birthday calendar day value:'.$dd);
			// $bd = $yyyy.'-'.$mm.'-'.$dd;
			// $today = date("Y-m-d");
			// if($today<$bd) abort('0086','Wrong birthday value, this date is in the future:'.$yyyy.'-'.$mm.'-'.$dd.', today is '.$today.', nobody can be born in the future.');
			
			wo_indent('birthday format ok: '.$birthday,6);
			return true;
		...
		case 'language':
			...
			wo_indent('o '.$fname.' passed: '.$languagename.' ('.$l.')',6);
			return true;
		...
		switch($classname) { 
			case 'C_dS_visitor':
				// return Array('id','created','creator','creatorId','changed','changer','changerId','deleted','deletorId','mergedTo',
							 ...
							 // 'cssColor','cssPattern','cssTags');
				return Array('id',
							 'gender','lastname','firstname','company','address','residence','zipCode','city','country',
							 'email','mobile','phone','language','birthday','note');
				break;
			//new field billamount in 2023/07 version
			case 'C_dS_availability':
				// return Array('id','created','creator','creatorId','changed','changer','changerId','deleted','deletorId','rversion','peerId','iscluster',
					...
					// 'waitingList','cssColor','cssPattern','cssTags','rescheduled','serieId','snext','sprev','archived','prebooking','billamount');
				return Array('id','cueIn','cueOut','workcodeIds','resourceIds');
				break;
			case 'C_dS_reservation':
				// return Array('id','created','creator','creatorId','changed','changer','changerId','deleted','deletorId','rversion','peerId','iscluster',
					// 'cueIn','cueOut','note','bookingcode',
					// 'waitingList','cssColor','cssPattern','cssTags','rescheduled','serieId','snext','sprev','archived','prebooking','billamount');
				// return Array('id','cueIn','cueOut','note','billamount','visitorIds','workcodeIds','resourceIds');
				return Array('id','cueIn','cueOut','billamount','note','visitorIds','workcodeIds','resourceIds');
				break;
			...
			case 'C_dS_login':
				...
				break;
			//new C_dS_payment in 2023/07 version
			case 'C_dS_payment':
				return Array('id','groupId','created','creator','creatorId','changed','changer','changerId',
					'paymean','amount','transid','transnote','transtatus','opstatus','accountholder','accountIBAN','qrcodestring');
				break;

			case 'C_dS_att_visitor':
					return Array('groupId','resourceId');
				break;
			...
			case 'C_dS_performance':
					return Array('groupId','workCodeId');
				break;
		}
	}
}
...
class C_shortlogin { // login without loading context
	...
	public function __construct($xpected_alevel = false, $loadcontext = 0) {
		...
		if($xpected_alevel) { // AI gateway
			if(!in_array($al,$xpected_alevel))
				abort('0007','You do not have the expected access level');
		}
		
		C_dbIO::logged($this->dS_login,'(AI api)'); // sign any change in DB
...
class C_apicontext { // login with context loaded ( dS_account, dS_workcodes, dS_logins, dS_resources, etc...
	...
	public function __construct($xpected_alevel = false, $loadcontext = 1, $perfReport = false) {
		...
		if($web) { pad(); h2('Checking access rights'); }
	...
	public function timeStampToUTC($stamp, $utc = false) { // note that this function accepts time stamp format "YYYY-MM-DD weekday HH:MM" or UTC format 1293836400 
		...
		// so we are in ISO 8601 format "YYYY-MM-DD HH:MM" or we accept also this "YYYY-MM-DD weekday HH:MM" or "2011-01-01"
				$l = strlen($stamp);
		if($l==10) { $stamp .= ' 00:00'; $l = strlen($stamp); } // 'YYYY-MM-DD' that is a midnight stamp, we re-write it to 'YYYY-MM-DD 00:00:00' that is the start of a day (last previous day seconds was 23:59:59)

		$stamp = preg_replace('/[^0-9\-\s:]/', '', $stamp);
		$stamp = preg_replace('/\s{2,}/', ' ', $stamp); // replaces double spaces by single spaces (double spaces appear when you remove weekday from the "YYYY-MM-DD weekday HH:MM" version)
		$stamp = trim($stamp);
		
		$format = $this->isValidCueFormat($stamp);
		$value = $this->isValidCueValue($format); // returns an array like [2011-01-01 00:00, 1293836400]
		if($value[0]!=$stamp) warning($stamp.' has been reformatted to '.$value[0]);
		return $value[1];
	}
	
	public function isValidCueFormat($cue) { // validates the time stamp format 'YYYY-MM-DD HH:MM:SS'
		$l = strlen($cue);
		if($l==10) { $cue .= ' 00:00'; $l = strlen($cue); } // 'YYYY-MM-DD' that is a midnight stamp, we re-write it to 'YYYY-MM-DD 00:00:00' that is the start of a day (last previous day seconds was 23:59:59)
		if($l<>19&&$l<>18&&$l<>16&&$l<>15) abort('0010','time stamp wrong format (received:'.$cue.')');
		...
		if ($isdate = preg_match('/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1]) ([0-9]|0[0-9]|1[0-9]|2[0-3]):([0-9]|[0-5][0-9])$/', $cue)) 
			// this preg_match allows midnight timing like 2020-07-10 00:00, also "2024-12-31 09:00" or "2024-12-31 9:00"
			return $cue;
		else 
	...
	public function isValidCueValue($cue) { // uniform format as final output : "2024-12-31 09:00", even if the input format was "2024-12-31 Tuesday 9:00"
		$one = str_replace(' ','-',$cue);
		$two = str_replace(':','-',$one);
		$split = explode('-',$two);
