...
////////////////////////////////////////////////////////////////////////////////////////////////////////
//
//    M O B M I N D E R    Moby  A I   A P I    S P E C I F I C A T I O N S  
//
...
//
// http://localhost/app07/mobminder/ai/specs/api.php?lgn=B.Spoden.2024&pwd=bernards2024&kid=44367&web=1
// http://localhost/app07/mobminder/ai/specs/api.php?lgn=PASCAL99@mob!&pwd=2025vanhove&kid=44672&web=1
//
// monitor the corresponding account on the webapp :
//
// B.Spoden.2024 / bernards2024
// 
...
if(1) {
	...
	pad(); h2('Queries');
		...
		indent('0199: Query an account configuration from a key:',6);
			$l = $host.$uri_1.'/query/config.php'.'?'.$creds;
		...
		if(!$q->hits()) {
			...
			$fname3 = strtolower(substr($dS_visitor->firstname,0,5));
			...
			indent('Find a visitor by firstname match',9);
				$e = 'firstname='.$fname3;
			...
			if($rid) {
				...
				$dSresa = new C_dS_reservation($rid);
					$dSresa->cueIn-=3600;
					$dSresa->cueOut+=1800;
				$dSresa->magnify()->setstringtimeformat4AI($utc);
					$cin = 	$dSresa->cueIn;
					$cout = $dSresa->cueOut;
		...
		indent('0210: Track a visitor\'s history or planned appointments:',6);
			pad(0);
			indent('To return the full list of written C_dS_reservation instances, involving this visitor.',9);
				$e = 'id='.$dS_visitor->id.'&partof=0&deleted=0';
			$l = $host.$uri_1.'/query/visiapps.php'.'?'.$e.'&'.$creds;
			
			href($http.$l, 1, 9); pad(0);
			
			
			indent('To query the LATEST C_dS_reservation instance, that is when the visitor came last time.',9);
				$e = 'id='.$dS_visitor->id.'&partof=-1&deleted=0';
			$l = $host.$uri_1.'/query/visiapps.php'.'?'.$e.'&'.$creds;
			
			href($http.$l, 1, 9); pad(0);
			
				
			indent('To query only C_dS_reservation instances that are visible in the PAST, involving this visitor.',9);
				$e = 'id='.$dS_visitor->id.'&partof=1&deleted=0';
			$l = $host.$uri_1.'/query/visiapps.php'.'?'.$e.'&'.$creds;
			...
			indent('To query the NEXT TO START C_dS_reservation instance, that is when the visitor is planned to come next time.',9);
				$e = 'id='.$dS_visitor->id.'&partof=-2&deleted=0';
			$l = $host.$uri_1.'/query/visiapps.php'.'?'.$e.'&'.$creds;
			
			href($http.$l, 1, 9); pad(0);
			
				
			indent('To query only C_dS_reservation instances that are visible in the FUTURE.',9);
				$e = 'id='.$dS_visitor->id.'&partof=2&deleted=0';
			$l = $host.$uri_1.'/query/visiapps.php'.'?'.$e.'&'.$creds;
			
			href($http.$l, 1, 9); pad(2);
		...
		indent('0300: Read visitor appointments:',6);
			...
			href($http.$l, 1, 9); pad(1);
	
	
	
		indent('0400: Search for availabilities (sequential along timeline):',6);
			$wid = 'workcode='.$wkid.'&limit=1&from='.$d;
			$l = $host.$uri_1.'/query/availabilities.php'.'?'.$wid.'&'.$creds;
			
			href($http.$l, 1, 9); pad(0);
	
		indent('0401: Search for availabilities each on different calendar days:',6);
			$wid = 'workcode='.$wkid.'&limit=40&aggregate=1';
			$l = $host.$uri_1.'/query/availabilities.php'.'?'.$wid.'&'.$creds;
			
			href($http.$l, 1, 9); pad(0);
	
		indent('0402: Search for availabilities starting at least 6 hours from now:',6);
			$wid = 'workcode='.$wkid.'&limit=1&sameday=6';
			$l = $host.$uri_1.'/query/availabilities.php'.'?'.$wid.'&'.$creds;
		...
		pad(0);
		indent('0600: Geolocation',6);
				pad(0);
				
			indent('Read date, time and location of logged user',9);
				$l = $host.$uri_1.'/query/datetimelocation.php'.'?'.$creds;
				
				href($http.$l, 1, 9); pad(0);
				
			indent('Build up a date in the future or in the past',9);
				$incdays = '&incdays=1'; 
				$inchours = '&inchours=2'; 
				$incweeks = '&incweeks=3'; 
				$l = $host.$uri_1.'/query/datetimelocation.php'.'?'.$incdays.$inchours.$incweeks.'&'.$creds;
				
				href($http.$l, 1, 9); pad(0);
	
		pad(0);
		indent('0601: Read account planning',6);
				$rsc_ids = $context->resources->getIdsList('!');
				pad(0);
				
			indent('Read planning for all resources',9);
				$l = $host.$uri_1.'/query/planning.php'.'?'.$creds;
...
if(1) {	
	...
	pad(); h2('Posts');
		...
		pad();	
		h3('Post an SMS or email message');
			
		indent('1200: Post an sms message:',6);
			$lnamelike = $context->dS_login->lastname;
			$q = new Q('select id from visitors where lastname like "%'.$lnamelike.'%" and groupId = '.$aid.' limit 1;');
			if(!$q->hits()) {
				warning('you need to create a visitor on this account having your -same as login- name and a valid mobile number.');
			} else {
				...
				if($n) {	
							$d = new C_date(); $t = $d->getTimeString();
							$m = '&message='.urlencode('This is a test message sent by Moby at '.$t);
						$params = 'channel=1&recipient=+'.$n.$m.'&'.$creds;
					$l = $host.$uri_1.'/post/message.php'.'?'.$params;
				
					href($http.$l, 1, 6); pad(0);
				
				} else {
					warning('Still a valid mobile number is missing for '.$dS_visitor->getFullName().'.');
				}

			}
			
		indent('1200: Post an email:',6);
			$lnamelike = $context->dS_login->lastname;
			$q = new Q('select id from visitors where lastname like "%'.$lnamelike.'%" and groupId = '.$aid.' limit 1;');
			if(!$q->hits()) {
				warning('you need to create a visitor on this account having your -same as login- name and a valid mobile number.');
				...
				if($n) {	
							$d = new C_date(); $t = $d->getTimeString();
							$m = '&message='.urlencode('<span color="red">This is a test email sent by Moby at '.$t.'</span>');
							$t = '&title='.urlencode('Test email from api specifications.');
						$params = 'channel=0&recipient='.$eml.$t.$m.'&'.$creds;
					$l = $host.$uri_1.'/post/message.php'.'?'.$params;
				
					href($http.$l, 1, 6); pad(0);
				
				} else {
					warning('Still a valid emails address is missing for '.$dS_visitor->getFullName().'.');
				}
