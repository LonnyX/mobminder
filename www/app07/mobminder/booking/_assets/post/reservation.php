...
{
    //tmplog('computop_number='.$_POST['computop_number']);
    //tmplog('computop_securityCode='.$_POST['computop_securityCode']);
    //tmplog('computop_expiryDate='.$_POST['computop_expiryDate']);
    //tmplog('computop_brand='.$_POST['computop_brand']);
    //tmplog('computop_cardHolder='.$_POST['computop_cardHolder']);

    if(  (!isset($_POST['computop_number'])) 
        ||(!isset($_POST['computop_securityCode']))
        ||(!isset($_POST['computop_expiryDate']))
        ||(!isset($_POST['computop_brand']))
        ||(!isset($_POST['computop_cardHolder']))
        ||(!isset($_POST['computop_colordepth']))
        ||(!isset($_POST['computop_screenheight']))
        ||(!isset($_POST['computop_screenwidth']))
        ||(!isset($_POST['computop_timezoneoffset'])))

    {
        $msg = 'at least one computop parameter is missing';
        C_dS_exception::put('post/payment.php', 'computop error',$msg,$dS_account->id);
        die('PayconiqError!'.$msg); //PayconiqError sera traité comme une erreur de transaction => ok
    }
}

if($paymean!=paymean_onlinepayco && $paymean!=paymean_onlinecards && $paymean!=paymean_onlinebcontact) 
{
    session_write_close();
}
...
//webpage should always stand on same time zone as the account time zone 
...
if($visitors){
    ...
    // (*forceemail*) 
    // update attendee visitors email (if empty) with email from authentication form (identify.php) only in family mode
    if(isset($_SESSION['e-mail'])){
        $email = $_SESSION['e-mail']; 
        $mv = implode(',',$visitors);
        $q = new Q('update visitors set email = "'.$email.'" where groupId = '.$accountId.' and id in('.$mv.') and email = "" ;');
    }
...
$dS_reservation->billamount = 0;


//BSP 2023-12-28 : set color,status ans tags from account default values
...
$dS_reservation->cssColor = $dS_account->defaultCssAppColor;
$dS_reservation->cssPattern = $dS_account->defaultCssAppPattern;
$dS_reservation->cssTags = $dS_account->defaultCssAppTag;

if($workcodes) foreach($workcodes as $id) 
{
    $dS_workcode = new C_dS_workcode($id);
    if(!$dS_workcode->id) continue;

    $dS_reservation->billamount += $dS_workcode->price;

    //BSP 2023-12-28 : set color,status ans tags from workcode
    //'cssColor'	=> 'unique id of a C_dS_customCss of the color type (C_dS_customCss::cssType = 80, resaClass = 11)',
    //'cssPattern' 	=> 'unique id of a C_dS_customCss of the pattern type (C_dS_customCss::cssType = 81, resaClass = 11)',
    //'cssTags' 	=> 'unique ids of a C_dS_customCss of the tag type (C_dS_customCss::cssType = 82, resaClass = 11), separated by "!"',
    if(true)
    {
        if($dS_workcode->cssColor!=0) $dS_reservation->cssColor = $dS_workcode->cssColor;
        
        if($dS_workcode->cssPattern!=0) $dS_reservation->cssPattern = $dS_workcode->cssPattern;
        
        if($dS_workcode->tags)
        {
            if($dS_reservation->cssTags)
            {
                //merge old and new tags
                $oldtags = explode('!',$dS_reservation->cssTags);
                $workcodetags = explode('!',$dS_workcode->tags);
                $newtags = Array();
                foreach($oldtags as $tag) $newtags[] = $tag;
                foreach($workcodetags as $tag) 
                {
                    if(!in_array($tag,$newtags)) $newtags[] = $tag;
                }
                $dS_reservation->cssTags = implode('!',$newtags);
            }
            else
            {
                $dS_reservation->cssTags = $dS_workcode->tags;
            }
        }
    }
...
if($workcodes) foreach($workcodes as $id) {
    ...
    $q = new Q('select id from workcodes where id = '.$id.';'); 
    if(!$q->ids()) continue; // the workcode has been deleted from the account config by another user.
...
if($paymean==paymean_onlinepayco || $paymean==paymean_onlinecards || $paymean==paymean_onlinebcontact)
{
    ...
    $_SESSION['e-paymean'] = $paymean;
    session_write_close();
    
    //PAYCONIQ/////////////////////////////////////////////////////////////////////////
    if($paymean==paymean_onlinepayco)
    {
    ...
    else if($paymean==paymean_onlinecards || $paymean==paymean_onlinebcontact ) //Computop
    {
        $MerchantID = $dS_account->ePayComputopId;          //"done4you_test"; 
        $BlowfishPassword = $dS_account->ePayComputopFish;   //"6Mf_m[C72s=W*4Qe"; 
        $HmacPassword = $dS_account->ePayComputopHmac;       //"Yf4[*Fm7Ra6)p2_BTy9(?Me35!Nz=Aq8";
        ...
        //'TR' is added to transaction to avoid INVALID MAC when capture (error with simple transid like '10007072')
        //$transid = 'TR'.$dS_payment->id;
        ...
        //TODO TMP
        $capturemanual = tmpcomputopcapturemode; 

        
        if($paymean==paymean_onlinecards)
        {
            if($MerchantID=='done4you_test') //TESTING account
                $description = 'test:0000';
            else
                $description = $transnote;

            $number = @$_POST['computop_number'];
            $securityCode = @$_POST['computop_securityCode'];
            $expiryDate = @$_POST['computop_expiryDate'];
            $brand = @$_POST['computop_brand'];
            $cardholder = @$_POST['computop_cardHolder'];

            $colordepth = intval(@$_POST['computop_colordepth']);
            $screenheight = intval(@$_POST['computop_screenheight']);
            $screenwidth = intval(@$_POST['computop_screenwidth']);
            $timezoneoffset = strval(@$_POST['computop_timezoneoffset']);

            //load first visitor for VISA 3-D Secure transactions (mandatory fields)
            if(false) 
            {
                $request = $gw->createDirectPostRequest($transid,$amount,$description,$returnurl,$dS_account->id,$languageIso2,
                    $number,$securityCode,$expiryDate,$brand,$cardholder,
                    $colordepth,$screenheight,$screenwidth,$timezoneoffset); 
            }
            else //new call computop with billaccount fields (lastname,firstname,email)
            {
                $visitor = new C_dS_visitor($visitors[0]);

                $request = $gw->createDirectPostRequest($transid,$amount,$description,$returnurl,$dS_account->id,$languageIso2,
                    $number,$securityCode,$expiryDate,$brand,$cardholder,
                    $colordepth,$screenheight,$screenwidth,$timezoneoffset,$visitor); 
            }
        }
        else // paymean_onlinebcontact
        {
            if(true) //PPRO contract
            {
                if($MerchantID=='done4you_test') //TESTING account
                    //$description = 'test:0000';
                    ...
                    $description = $transnote;
                $request = $gw->createBancontactPPRORequest($transid,$amount,$description,$returnurl,$dS_account->id,$languageIso2); 
            }
            else //EMS contract
            {
                if($MerchantID=='done4you_test') //TESTING account
                    ...
                    $description = $transnote;
                $request = $gw->createBancontactEMSRequest($transid,$amount,$description,$returnurl,$timeoutsec,$dS_account->id,$languageIso2,$capturemanual); 
            }
        ...
        $dS_payment->transid = ''; //$request->GETUrl;
        //$dS_payment->transtatus= strtolower($result->status);
...
$rec_resas->addmeta_datetimeVerbose();
