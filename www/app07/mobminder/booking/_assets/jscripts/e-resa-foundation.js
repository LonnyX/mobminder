...
//
//DEV
e_MOB = function e_MOB(keyId, demo, language,returnVisitorId,returnresaid,computoptransid,familyIdentMode,bypasshomepage) {
	...
	const options = {
  		...
  		minute: '2-digit',
  		second: '2-digit',
  		timeZoneName: 'short' // Ajoute le fuseau horaire (ex: GMT+2)
	};

	const formattedDateTimeWithTimeZone = currentDateTime.toLocaleString('fr-FR', options);
	console.log("current browser datetime = "+formattedDateTimeWithTimeZone);

	if(vbs) vlog('e-resa.js','e_MOB','constructor','key:'+keyId); 
	if(vbs) vlog('e-resa.js','e_MOB','constructor','returnVisitorId:'+returnVisitorId); 
	if(vbs) vlog('e-resa.js','e_MOB','constructor','returnresaid:'+returnresaid); 
	if(vbs) vlog('e-resa.js','e_MOB','constructor','computoptransid:'+computoptransid); 
	//DEV
	this.state = { key:keyId, returnVisitorId:returnVisitorId, returnResaId:returnresaid,familyIdentMode:familyIdentMode,bypasshomepage:bypasshomepage };
	mobminder.app = this;
	mobminder.demo = demo;
	
	this.eids = { desk:'desk', remote:'eresa', buttons:'buttons',sinfosup:'s-info-sup',
	...
	if(computoptransid) 
	{
		this.calldeletecomputoppayment(computoptransid);
		computoptransid=0; //local var, not used anymore
	}
...
e_MOB.prototype = {
	...
	ajaxfeedback: function(callbacks, stream) { // called when the date is change from the TOP menu, applies jsdate on current planning screen
		...
		//DEV old version from eresa & eresa2
	...
	calldeletecomputoppayment: function(transid) {
		...
		let key = new C_iPASS(this.state.key);
		let paymean = new C_iPASS(C_dS_payment.type.onlinecards);
	...
	onconfigloaded: async function() {
		...
		if(this.state.bypasshomepage)
		{
			if(vbs) vlog('e-resa.js','e_MOB','onconfigloaded','calling start() because of bypasshomepage...');
			preventBrowserBackButton(window, document);
			this.start();
		}
		...
		/*DEV color depth issue: solved by provider
		//console.log("colorDepth="+screen.colorDepth);
		//console.log("ePayActive="+mobminder.account.ePayActive);
		if(screen.colorDepth==30 && mobminder.account.ePayActive)
		{
			let wcids = mobminder.context.surfer.eresaWorkcodes.split('!');
			for(let x in wcids) {
				let wkid = wcids[x];
				let workcode = C_dS_workcode.get(wkid);
				if(workcode.deposit)
				{
					//console.log("deposit="+workcode.deposit);

					$('#s-info-sup').html('<br><br><span id="info-payment-issue" >'+C_XL.w('Colordepth_problem_info').replace('{0}',mobminder.account.name)+'</span>');
					break;
				}
			}
		}*/
...
C_eRESA = function(resa, callbacks, preset) {
	...
	this.eids = { note:base+'note', notediv:base+'notediv', confirm:base+'confirm', paymsg:base+'paymsg',
		selectpaymsg:base+'selectpaymsg',
		payconiq:base+'payconiq', payconiqlogo:base+'payconiqlogo', payconiqblueprint:base+'payconiqblueprint',
		payvisa:base+'payvisa',payvisalogo:base+'payvisalogo',payvisablueprint:base+'payvisablueprint',
		paymc:base+'paymc',paymclogo:base+'paymclogo',paymcblueprint:base+'paymcblueprint',
		cancelcreditdebit:base+'cancelcreditdebit', confirmcreditdebit:base+'confirmcreditdebit',  
		paybancontact:base+'paybancontact',paybancontactlogo:base+'paybancontactlogo', paybancontactblueprint:base+'paybancontactblueprint',
		action:base+'action', creditdebit:base+'creditdebit',
		logovisa:base+'logovisa',logomc:base+'logomc', cardholder:base+'cardholder',
		number:base+'number',securitycode:base+'securitycode',expirydatemmyy:base+'expirydatemmyy',
		warn:base+'warn' }; //,brand:base+'brand'
	this.elements 	= new A_el();

	if(vbs) vlog('e-resa.js', 'C_eRESA', 'constructor', 'resaId:'+this.dataSet.id+', may delete:'+this.state.maydelete);
	

	//console.log("this.state.maydelete="+this.state.maydelete);
	let deleteallowed;
	if(this.state.maydelete)
	{
		let limit = new Date();

		//DEV now hours are used instead of day //limit.setDate(limit.getDate()+this.state.maydelete);
		limit.setHours(limit.getHours()+this.state.maydelete);

		//console.log("limit="+limit);
		//console.log("jsDateIn="+resa.jsDateIn);

		//too late to remove
		if(limit>resa.jsDateIn)
		{
			this.state.toolatetodelete=true;
			deleteallowed=false;
		}
		else //enough time to delete
		{
			this.state.toolatetodelete=false;
			deleteallowed=true;
		}
		//console.log("this.state.toolatetodelete="+this.state.toolatetodelete);
	}
	else
	{
		deleteallowed=false;
	}
	//console.log("deleteallowed="+deleteallowed);
	

	//let id			= new C_iPASS({id:this.dataSet.id, archived:0, peerId:0, bank:'visiapps' });
	//2025-08-14 bsp : preb value from 5 to 15 minutes because of timeout before computop payment validation by user! 
	// reason : payment is success but reservation has been deleted!
	//see (*pb01*)
	let id			= new C_iPASS({id:this.dataSet.id, archived:0, peerId:0, bank:'visiapps', amount:this.state.amount,transnote:this.state.transnote,preb:15});
	...
	let payconiq =  new C_iCLIK(this.eids.payconiq, { click:new A_cb(this, this.savepayconiq) }, { tag:'div', ui:'Payconiq', css:'e-button', 
		style:'display:flex; justify-content:center; align-items:center; margin:1em 0em;'}  );
	let payconiqimg = '<img src="../_assets/imgs/paylogo/Payconiq.jpg" style="max-height:40px;"></img>';
	let payconiqlogo =  new C_iCLIK(this.eids.payconiqlogo, { click:new A_cb(this, this.savepayconiq) }, { tag:'div', ui:payconiqimg, css:'e-button-logo', 
		style:'display:flex; justify-content:center; align-items:center; border:1px solid rgba(0,0,0, 0.2); border-radius:3px; '+(is.small?'margin:1em 1.5em 1em 0em':'margin:1em 1.5em')+'; padding:5px 5px; max-height:51px; width:80px;'}  );
		//same margin:1em (up and dow) as main button
	
	//DEV colordepth issue solved by provider
	//let depth = screen.colorDepth;
	//console.log("color depth="+depth);
	//let payvisaenabled = (depth!=30);
	
	let payvisa =  new C_iCLIK(this.eids.payvisa, { click:new A_cb(this, this.savevisa) }, { enabled:true, tag:'div', ui:'Visa', css:'e-button', 
		style:'display:flex; justify-content:center; align-items:center; margin:1em 0em;'}  );
	let payvisaimg = '<img src="../_assets/imgs/paylogo/Visa.jpg" style="max-height:40px;"></img>';
	let payvisalogo =  new C_iCLIK(this.eids.payvisalogo, { click:new A_cb(this, this.savevisa) }, { enabled:true, tag:'div', ui:payvisaimg, css:'e-button-logo', 
		style:'display:flex; justify-content:center; align-items:center; border:1px solid rgba(0,0,0, 0.2); border-radius:3px; '+(is.small?'margin:1em 1.5em 1em 0em':'margin:1em 1.5em')+'; padding:5px 5px; max-height:51px; width:80px;'}  );
		//same margin:1em (up and dow) as main button

	let paymc =  new C_iCLIK(this.eids.paymc, { click:new A_cb(this, this.savemc) }, { enabled:true, tag:'div', ui:'Mastercard', css:'e-button', 
		style:'display:flex; justify-content:center; align-items:center; margin:1em 0em;'}  );
	let paymcimg = '<img src="../_assets/imgs/paylogo/MC.jpg" style="max-height:40px;"></img>';
	let paymclogo =  new C_iCLIK(this.eids.paymclogo, { click:new A_cb(this, this.savemc) }, { enabled:true, tag:'div', ui:paymcimg, css:'e-button-logo', 
		style:'display:flex; justify-content:center; align-items:center; border:1px solid rgba(0,0,0, 0.2); border-radius:3px; '+(is.small?'margin:1em 1.5em 1em 0em':'margin:1em 1.5em')+'; padding:5px 5px; max-height:51px; width:80px;'}  );
		//same margin:1em (up and dow) as main button
	...
	let cancelcreditdebit =  new C_iCLIK(this.eids.cancelcreditdebit, { click:new A_cb(this, this.cancelpaycreditebit) }, { enabled:true, tag:'div', ui:C_XL.w('epay_cancel'), css:'e-button', style:'display:flex; justify-content:center; align-items:center;'}  );
	let confirmcreditdebit =  new C_iCLIK(this.eids.confirmcreditdebit, { click:new A_cb(this, this.savepaycreditdebit) }, { enabled:false, tag:'div', ui:C_XL.w('epay_pay')+' '+(this.state.amount/100).toFixed(2)+'€', css:'e-button', style:'display:flex; justify-content:center; align-items:center;'}  );

	let typing = new A_cb(this, this.typing, null, null, 1000); //1000
	let typingnumber = new A_cb(this, this.typingnumber, null, null, 800); //1000



	//let brand		= new C_iCRESTA(this.eids.brand, {}, { labels:{ 0:'VISA', 1:'MasterCard' } }, { skin:0, title:false, mode:-1, value:0, maxrows:1,locked:true }  );
	let cardholder = new C_iFIELD(this.eids.cardholder, { onfchange:typing }, { max:21, digits:this.state.cardholder, type:INPUT_CARDHOLDER, mandatory:true, focus:false, placeholder:C_XL.w('epay_cardholder'),dblclick:false });
	let number = new C_iFIELD(this.eids.number, { onfchange:typingnumber }, { max:16, digits:'', type:INPUT_CARDNUMBER, mandatory:true, focus:false, placeholder:C_XL.w('epay_cardnumber'),dblclick:false });
	let securitycode = new C_iFIELD(this.eids.securitycode, { onfchange:typing }, { max:3, digits:'', type:INPUT_CARDCVC, mandatory:true, focus:false, placeholder:C_XL.w('epay_securitycode'),dblclick:false });
	//let expirydatemm = new C_iFIELD(this.eids.expirydatemm, { onfchange:typing }, { max:2, digits:'', type:INPUT_CARDMONTH, mandatory:true, focus:false, placeholder:C_XL.w('epay_mm'),dblclick:false });
	//let expirydateyy = new C_iFIELD(this.eids.expirydateyy, { onfchange:typing }, { max:4, digits:'', type:INPUT_CARDYEAR, mandatory:true, focus:false, placeholder:C_XL.w('epay_yyyy'),dblclick:false });
	let expirydatemmyy = new C_iFIELD(this.eids.expirydatemmyy, { onfchange:typing }, { max:5, digits:'', type:INPUT_CARDDATE, mandatory:true, focus:false, placeholder:C_XL.w('epay_mmyy'),dblclick:false });
	
	//let expirydate 	= new C_iFIELD(this.eids.expirydate, { onfchange:new A_cb(this, this.changedate ), onenterkey:new A_cb(this, this.enterkey ) }, { digits:'dd', type:INPUT_BDATE, mandatory:true, placeholder:'Expiry date'});



	//C_XL.w('card_payment')
	//ui:'Payer avec Bancontact'
	//<span class="fa fa-arrow-alt-circle-up fa-1d5x" style="margin-right:5px;"></span>
	let paybancontact =  new C_iCLIK(this.eids.paybancontact, { click:new A_cb(this, this.savepaybancontact) }, { enabled:true, tag:'div', ui:'Bancontact', css:'e-button', 
		style:'display:flex; justify-content:center; align-items:center; margin:1em 0em;'}  );
	let paybancontact2img = '<img src="../_assets/imgs/paylogo/BC.jpg" style="max-height:40px;"></img>';
	let paybancontactlogo =  new C_iCLIK(this.eids.paybancontactlogo, { click:new A_cb(this, this.savepaybancontact) }, { enabled:true, tag:'div', ui:paybancontact2img, css:'e-button-logo', 
		style:'display:flex; justify-content:center; align-items:center; border:1px solid rgba(0,0,0, 0.2); border-radius:3px; '+(is.small?'margin:1em 1.5em 1em 0em':'margin:1em 1.5em')+'; padding:5px 5px; max-height:51px; width:80px;'}  );
		//same margin:1em (up and dow) as main button
	...
	this.captions = {
		summary: { caption:this.summary(), tip:'', csscolor:'', cssfa:'calendar-check', idle:!deleteallowed },
	...
	this.controls 	= new A_ct( { id:id, staff:staff, visitors:visitors, workcodes:workcodes, schedule:schedule, 
		note:note, confirm:confirm, action:action, payconiq:payconiq,payconiqlogo:payconiqlogo,payvisa:payvisa,payvisalogo:payvisalogo,paymc:paymc,paymclogo:paymclogo,cancelcreditdebit:cancelcreditdebit,
		confirmcreditdebit:confirmcreditdebit, paybancontact:paybancontact,paybancontactlogo:paybancontactlogo,cardholder:cardholder,number:number,securitycode:securitycode,expirydatemmyy:expirydatemmyy} ); //,brand:brand
...
C_eRESA.defauts = new A_df( { maydelete:false,toolatetodelete:false, confirm:true,payconiq:false,paycard:false,cardholder:'', paybancontact:false,amount:0,transnote:'',paymsg:false } );
C_eRESA.prototype = { 
	// public
	summary: function() { // returns time and date for this appointment		
		//DEV
		//let cin = this.dataSet.jsDateIn.HHmm();
		let cin = this.dataSet.cueIn_hhmm;
		//let cout = this.dataSet.jsDateOut.HHmm();
		let cout = this.dataSet.cueOut_hhmm();
		if(is.small)
		{
			//let date = C_XL.date(this.dataSet.jsDateIn, {abreviation:'abr', weekday:true, year:false });
			let date = C_XL.date(new DateAcc(this.dataSet), {abreviation:'abr', weekday:true, year:false });
			return date+' '+cin+'-'+cout;
		}
		else
		{
			//let date = C_XL.date(this.dataSet.jsDateIn, {abreviation:'full', weekday:true, year:true });
			let date = C_XL.date(new DateAcc(this.dataSet), {abreviation:'full', weekday:true, year:true });
	...
	display: function() { // appears where you can confirm your new appointment, just before it is saved
		...
		let n = this.controls.note ? this.controls.note.display('','erasa-confirm-note',this.state.confirm?'e- pls note&confirm':'note_payment') : '';
		let notefield = '<div id="'+this.eids.notediv+'">'+n+'</div>';
		
		let paymsg 	= '<div id="'+this.eids.paymsg+'" '+(this.state.paymsg?'':'style="display:none;"')+' class="e-msg bold">'+this.state.paymsg+'</div>';

		let selectpaymsg 	= '<div id="'+this.eids.selectpaymsg+'" '+(!this.state.confirm?'':'style="display:none;"')+' class="e-msg">'+C_XL.w('epay_selectmethod')+'</div>';

		//let bconfirm = this.controls.confirm.display({css:'e-button'});
		let bconfirm = this.controls.confirm.display();
		bconfirm = '<div '+(this.state.confirm?'':'style="display:none;"')+' class="e-msg text-center text-sm-left">'+bconfirm+'</div>';
	
		//let bpayconiq = this.controls.payconiq.display({css:'e-button'});
		let bpayconiq = (is.small?'':this.controls.payconiq.display());
		let bpayconiqlogo = this.controls.payconiqlogo.display();
		let bpayconiqblueprint = '<div id="'+this.eids.payconiqblueprint+'" class="blueprint flexinner" style="display:flex; justify-content:left; text-align: left !important; align-items:center; margin:auto 0; height:51px; flex-grow: 1; width:20px;">'+C_XL.w('epay_blueprintpayconiq')+'</div>';
		bpayconiq = '<div '+(this.state.payconiq?'style="align-items:center; flex-wrap:wrap;"':'style="display:none;"')+' class="e-msg text-center text-sm-left row">'+bpayconiq+bpayconiqlogo+bpayconiqblueprint+'</div>';

		
	
		//let bvisa = this.controls.payvisa.display({css:'e-button'});
		let bvisa = (is.small?'':this.controls.payvisa.display());
		let bvisalogo = this.controls.payvisalogo.display();
		let bvisablueprint = '<div id="'+this.eids.payvisablueprint+'" class="blueprint flexinner" style="display:flex; justify-content:left; text-align: left !important; align-items:center; margin:auto 0; height:51px; flex-grow: 1; width:20px;">'+C_XL.w('epay_blueprintmcvisa')+'</div>';
		bvisa = '<div '+(this.state.paycard?'style="align-items:center; flex-wrap:wrap;"':'style="display:none;"')+' class="e-msg text-center text-sm-left row">'+bvisa+bvisalogo+bvisablueprint+'</div>';


		let bmc = (is.small?'':this.controls.paymc.display());
		let bmclogo = this.controls.paymclogo.display();
		let bmcblueprint = '<div id="'+this.eids.paymcblueprint+'" class="blueprint flexinner" style="display:flex; justify-content:left; text-align: left !important; align-items:center; margin:auto 0; height:51px; flex-grow: 1; width:20px;">'+C_XL.w('epay_blueprintmcvisa')+'</div>';
		bmc = '<div '+(this.state.paycard?'style="align-items:center; flex-wrap:wrap;"':'style="display:none;"')+' class="e-msg text-center text-sm-left row">'+bmc+bmclogo+bmcblueprint+'</div>';

		let msgcreditdebit = '<div class="e-msg" style="display:inline-block">'+C_XL.w('epay_debitcreditcard')+'&nbsp;&nbsp;'+'</div>';
		let imgvisa = '<div id="'+this.eids.logovisa+'" style="display:inline-block;border:1px solid rgba(0,0,0, 0.2); border-radius:3px;padding:3px 3px; max-height:46px; width:57px; background-color:white;"><img src="../_assets/imgs/paylogo/Visa.jpg" style="max-height:30px;"></img></div>';
		let imgmc = '<div id="'+this.eids.logomc+'"     style="display:inline-block;border:1px solid rgba(0,0,0, 0.2); border-radius:3px;padding:3px 3px; max-height:46px; width:57px; background-color:white;"><img src="../_assets/imgs/paylogo/MC.jpg" style="max-height:30px;"></img></div>';
		let logocreditdebit = '<div style="margin-bottom:5px">'+msgcreditdebit+imgvisa+imgmc+'</div>';
		let cardholder 	= '<div>'+this.controls.cardholder.display()+'</div>';
		let number 	= '<div>'+this.controls.number.display()+'</div>';
		//let brand 	= '<div>'+this.controls.brand.display()+'</div>';
		
		let expirydatemmyyblueprint = '<div class="blueprint" style="display:inline-block">'+C_XL.w('epay_validitydate')+'</div>';
																						   
																						  
																	 
		// 
		let expirydatemmyy = '<div>'+this.controls.expirydatemmyy.display('alpha5')+'&nbsp;'+expirydatemmyyblueprint+'<div/>';

		let secuirytcodeblueprint = '<div class="blueprint" style="display:inline-block">'+C_XL.w('epay_blueprintcvc')+'</div>';
		let securitycode = '<div>'+this.controls.securitycode.display('alpha4')+'&nbsp;'+secuirytcodeblueprint+'</div>';
		
		let warn = '<div class="e-msg e-warner" id="'+this.eids.warn+'" style="display:none; text-align:left;">'+C_XL.w('warn red field')+'</div>';

		let bpaycardcancel = this.controls.cancelcreditdebit.display();
		bpaycardcancel = '<div class="e-msg text-center text-sm-left">'+bpaycardcancel+'</div>';
		let bpaycardconfirm = this.controls.confirmcreditdebit.display();
		bpaycardconfirm = '<div class="e-msg text-center text-sm-left">'+bpaycardconfirm+'</div>';
		
		let bpaycard = '<div id="'+this.eids.creditdebit+'" style="display:none;" >'+logocreditdebit+cardholder+number+expirydatemmyy+securitycode+warn+bpaycardconfirm+bpaycardcancel+'</div>'; //+brand+'<br/>'
		


		let bbancontact = (is.small?'':this.controls.paybancontact.display());
		let bbancontactlogo = this.controls.paybancontactlogo.display();
		let bbancontactblueprint = '<div id="'+this.eids.paybancontactblueprint+'" class="blueprint flexinner" style="display:flex; justify-content:left; text-align: left !important; align-items:center; margin:auto 0; height:51px; flex-grow: 1; width:20px;">'+C_XL.w('epay_blueprintbc')+'</div>';
		bbancontact = '<div '+(this.state.paybancontact?'style="align-items:center; flex-wrap:wrap;"':'style="display:none;"')+' class="e-msg text-center text-sm-left row">'+bbancontact+bbancontactlogo+bbancontactblueprint+'</div>';
	
		let paypadding = this.state.confirm?'auto':'0.5em';
		return notefield+paymsg+selectpaymsg+'<div style="padding-top:'+paypadding+';padding-bottom:'+paypadding+';">'+bconfirm+bbancontact+bpayconiq+bvisa+bmc+bpaycard+'</div>';
	},
	activate: function() {
	...
	/*setbuttonbusy(busy){
		...
		if(this.state.paycard) this.controls.payvisa.busy(busy);
    ...
    saved: function(dataSets, stream) {
		...
		//console.log('calling saved function');
		if(mobminder.context.surfer.id==18562) //Kinesens groupid=5094,kinesensfr loginid =18562,G-90E4LNV1T0
		{
			let event_category='mobminder_'+mobminder.context.surfer.eresaUrl;
			console.log('calling gtag with event_category='+event_category);
 			gtag('event', 'confirm_click', { 'event_category': event_category});
		}
		else if(mobminder.context.surfer.id==21798) //Cabinet de Kinésithérapie Hardy groupid=5386,kinehardy loginid =21798,G-3R6YFGBGDV
		{
			let event_category='mobminder_'+mobminder.context.surfer.eresaUrl;
			console.log('calling gtag with event_category='+event_category);
 			gtag('event', 'confirm_click', { 'event_category': event_category});
	...
	savepayconiq: function() {
		...
		this.setbuttonbusy(true);
		//this.controls.payconiq.enable(false);

		if(is.tactile) document.activeElement.blur(); // lets the soft keyboard disappear on touch devices
		
		let paymean = new C_iPASS(C_dS_payment.type.onlinepayco);
		this.controls['paymean']=paymean;
	...
	savevisa: function(){
		this.savepaycard(true);
	},
	savemc: function(){
		this.savepaycard(false);
	},
	savepaycard: function(isvisa) {
		if(vbs) vlog('e-resa.js', 'C_eRESA', 'savecard', '');
		this.state.isvisa = isvisa;
		//this.setbuttonbusy(true);
		if(is.tactile) document.activeElement.blur(); // lets the soft keyboard disappear on touch devices
		
		if(this.controls.note) $(this.elements.notediv).hide();

		if(!is.small) this.controls.payvisa.hide(true);
		this.controls.payvisalogo.hide(true);
		$(this.elements.payvisablueprint).hide();
		
		if(!is.small) this.controls.paymc.hide(true);
		this.controls.paymclogo.hide(true);
		$(this.elements.paymcblueprint).hide();
		
		$(this.elements.selectpaymsg).hide();

		this.controls.confirmcreditdebit.enable(false);
		$(this.elements.warn).show();

		
		if(!is.small && this.state.payconiq) this.controls.payconiq.hide(true);
		if(this.state.payconiq) this.controls.payconiqlogo.hide(true);
		if(this.state.payconiq) $(this.elements.payconiqblueprint).hide();

		if(!is.small && this.state.paybancontact) this.controls.paybancontact.hide(true);
		if(this.state.paybancontact) this.controls.paybancontactlogo.hide(true);
		if(this.state.paybancontact) $(this.elements.paybancontactblueprint).hide();
		
		$(this.elements.creditdebit).show();
		this.switchVisaMClogo(this.state.isvisa);
		$('html, body').animate({ scrollTop: $(this.elements.paymsg).offset().top }, 600);
	},
	switchVisaMClogo(isvisa)
	{
		if(isvisa)
		{
			//VISA
			$(this.elements.logovisa).show();
			$(this.elements.logomc).hide();
		}
		else
		{
			//Mastercard
			$(this.elements.logovisa).hide();
			$(this.elements.logomc).show();
		}
	},
	typing: function() {
		...
		this.controls.confirmcreditdebit.enable(newstate);
		if(newstate) $(this.elements.warn).hide();
		else $(this.elements.warn).show();
	},
	typingnumber: function(){
		...
		let number = this.controls.number.getpost();
		//console.log(number);
		if(number.startsWith('4'))
		{
			//VISA
			this.switchVisaMClogo(true);
		}
		else if(number.startsWith('5'))
		{
			//Mastercard
			this.switchVisaMClogo(false);
		}
		else
		{
			this.switchVisaMClogo(this.state.isvisa);
	...
	/*changedate: function(digits) 
	{ 
		//this.state.valid.bdate = this.controls.bdate.valid(); this.validident();  
	},*/
	/*enterkey: function(digits) 
	{ 
		//if(this.validident()) this.ident(); 
	},*/

	cancelpaycreditebit: function() {
		if(vbs) vlog('e-resa.js', 'C_eRESA', 'savepaycreditdebit', '');

		if(!is.small) this.controls.payvisa.show(true);
		this.controls.payvisalogo.show(true);
		$(this.elements.payvisablueprint).show();
		
		if(!is.small) this.controls.paymc.show(true);
		this.controls.paymclogo.show(true);
		$(this.elements.paymcblueprint).show();

		$(this.elements.selectpaymsg).show();

		if(this.controls.note) $(this.elements.notediv).show();

		
		if(!is.small && this.state.payconiq) this.controls.payconiq.show(true);
		if(this.state.payconiq) this.controls.payconiqlogo.show(true);
		if(this.state.payconiq) $(this.elements.payconiqblueprint).show();
		
		if(!is.small && this.state.paybancontact) this.controls.paybancontact.show(true);
		if(this.state.paybancontact) this.controls.paybancontactlogo.show(true);
		if(this.state.paybancontact) $(this.elements.paybancontactblueprint).show();
		
		$(this.elements.creditdebit).hide();

		this.controls.cardholder.clear();
		this.controls.number.clear();
		this.controls.expirydatemmyy.clear();
		this.controls.securitycode.clear(); //{propagate:false});

		$(this.elements.warn).hide();

		$('html, body').animate({ scrollTop: $(this.elements.paymsg).offset().top }, 600);
		
		//this.controls.brand.docheck(0);
		
	},
	savepaycreditdebit: function() {
		if(vbs) vlog('e-resa.js', 'C_eRESA', 'savepaycreditdebit', '');
		this.setbuttonbusy(true);
		if(is.tactile) document.activeElement.blur(); // lets the soft keyboard disappear on touch devices
		...
		//let brand = this.controls.brand.getpost()?'VISA':'MasterCard';
		let brand = (this.controls.number.getpost().startsWith('4')?'VISA':'MasterCard');
		let sbrand =  new C_iPASS(brand);
		this.controls['brand']=sbrand;

		//expirydate format must be YYMM (convert MM/YY => YYMM)
		let expirydate = this.controls.expirydatemmyy.getpost();
		expirydate = expirydate.split('/');
		expirydate = '20'+expirydate[1]+expirydate[0];
		let sexpirydate =  new C_iPASS(expirydate);
		this.controls['expirydate']=sexpirydate;
		...
		colordepth =  new C_iPASS(''+colordepth);
		this.controls['colordepth']=colordepth;

		let screenheight = screen.height;
		//console.log("screenheight="+screenheight);
		screenheight =  new C_iPASS(''+screenheight);
		this.controls['screenheight']=screenheight;

		let screenwidth = screen.width;
		//console.log("screenwidth="+screenwidth);
		screenwidth =  new C_iPASS(''+screenwidth);
		this.controls['screenwidth']=screenwidth;

		let timezoneoffset = new Date().getTimezoneOffset();
		//console.log("timezoneoffset="+timezoneoffset);
		timezoneoffset =  new C_iPASS(''+timezoneoffset);
		this.controls['timezoneoffset']=timezoneoffset;
		...
		let names = { schedule:{cin:'cueIn',out:'cueOut'}, id:{id:'id', archived:'archived', peerId:'peerId', bank:'bank', preb:'preb',paymean:'paymean',amount:'amount',transnote:'transnote' }
			...
			paymean:'paymean', number:'computop_number', securitycode:'computop_securityCode',expirydate:'computop_expiryDate',
			brand:'computop_brand', cardholder:'computop_cardHolder',
			colordepth:'computop_colordepth',screenheight:'computop_screenheight',screenwidth:'computop_screenwidth',timezoneoffset:'computop_timezoneoffset',
		 };
		mobminder.app.post(this.controls, names, '../_assets/post/reservation.php', new A_cb(this,this.saved), new A_cb(this,this.failed));
		if(this.dataSet.id>0) this.dataSet.unregister();
	},
	savepaybancontact: function() {
		if(vbs) vlog('e-resa.js', 'C_eRESA', 'savepaybancontact', '');
		this.setbuttonbusy(true);
		if(is.tactile) document.activeElement.blur(); // lets the soft keyboard disappear on touch devices
		
		let paymean = new C_iPASS(C_dS_payment.type.onlinebancontact);
		this.controls['paymean']=paymean;
...
C_ePERF = function(eid, callbacks, preset) {
	...
	this.optionspreset = [];
		...
		if(mobminder.context.surfer.eresaWorkcodes) { // this login uses only a subset of the defined e-workcodes
			...
			let options = C_dS_workcode.options({eWorkcodes:true, checked:'first', which:which, language:mobminder.context.surfer.language});
...
C_iSLOT = function(eid, dS_reservation, callbacks, preset) { // a single slot entry
	...
	let midnight = (this.resa.cueIn_yyyy*10000 + this.resa.cueIn_mm*100 + this.resa.cueIn_dd);
	...
	C_iSLOT.register.add(this, 'midnight', midnight, fCalsString, bCalId, uCalsString, this.resa.cueIn_hhmm);
	...
	//let time = new C_iCLIK(this.eids.date, { click:new A_cb(this, this.selected)} , { ui:this.resa.jsDateIn.HHmm(), tip:tip, tag:'span', style:'min-width:2em; float:left;' } );
	...
	let time = new C_iCLIK(this.eids.date, { click:new A_cb(this, this.selected)} , { ui:this.resa.cueIn_hhmm, tip:tip, tag:'span', style:'min-width:4.5em; text-align:center; float:left;' } );
...
C_eToken = function(eid, callbacks) {
	...
	let field 	= new C_iFIELD(this.eids.field, { onfchange:new A_cb(this, this.tokentype ), onenterkey:new A_cb(this, this.tokenenterkey ) }, { digits:'', type:INPUT_NUMER, mandatory:true, placeholder:C_XL.w('token') });
