...
C_eCheckIn = function(eid, callbacks, options) { 
	...
	this.eids = { 	own: { div:eid+'_div_all', prelist:eid+'_div_pre', postlist:eid+'_div_post', vlist:eid+'_div_vlist'
					...
					buttons: { ident:eid+'_ident', morenext:eid+'_morenext'}, //  more:eid+'_more', next:eid+'_next'},
	...
	//let more =  new C_iCLIK(this.eids.buttons.more, { click:new A_cb(this, this.more) }, { enabled:true, tag:'div', ui:C_XL.w('eresa register more'), css:'action', style:'display:flex; justify-content:center; align-items:center;'}  );
	//let next =  new C_iCLIK(this.eids.buttons.next, { click:new A_cb(this, this.next) }, { enabled:true, tag:'div', ui:C_XL.w('eresa ident continue one'), css:'action', style:'display:flex; justify-content:center; align-items:center;'}  );
	
	let more = {caption:C_XL.w('eresa register more'), tip:'', css:'action double', cssfa:'et fa-user-plus' };
	let next = {caption:C_XL.w('eresa ident continue one'), tip:'', css:'action double', cssfa:'fa-calendar' };
	let morenext = new e_DblButton(this.eids.buttons.morenext, {onleft:new A_cb(this, this.next), onright:new A_cb(this, this.more)}, {left:next, right:more });
	...
	this.buttons = new A_ct({ident:ident, morenext:morenext}); // more:more, next:next });
	this.appointments = new A_ct();
...
C_eCheckIn.prototype = {
	...
	display: function() {
			...
			welcome = '<div id="'+this.eids.wrappers.welcome+'" style="display:block;">'+welcome+email+mobile+warn+bident+'</div>';
				...
				let morenext = '<div class="e-btn">'+this.buttons.morenext.display()+'</div>';
			vlist = '<div id="'+this.eids.wrappers.vlist+'" style="display:none;">'+prelist+vlist+postlist+morenext+'</div>';
	...
	showcandidates: function() {
		...
		let vcount = 0; for(let vid in visitors) vcount++;
		...
		for(let vid in visitors) {
			...
			list += (vcount==1?'':listelemicon)+C_XL.gender(dS_v.gender, dS_v.language)+'&nbsp;'+dS_v.firstname+'&nbsp;'+dS_v.lastname+',<br/>';
			c++;
		}
	...
	candidates: function(inlineDataSets, stream) { // e-resa STEP 1: callback from initial email check
		...
		if(stream.startsWith("Blacklisted"))
		{
			//terminate processing
			if(this.callbacks.blacklisted) this.callbacks.blacklisted.cb();
		}
		else if(stream.slice(0,5)==='error') 
		{
			//terminate processing
			if(this.callbacks.blacklisted) this.callbacks.blacklisted.cb();
		}
		else
		{
			...
			let vcount = 0; for(let vid in this.state.visitors) vcount++;
			
			//console.log("vcount"+vcount);
			//console.log("eresaSignin"+mobminder.context.surfer.eresaSignin);

			if(vcount==0 && !mobminder.context.surfer.eresaSignin)
			{
				this.callbacks.blacklisted.cb();
			}
			else
			{
...
C_eRegister = function(eid, callbacks, preset) {
	...
	this.eids = { 	id:b+'id', gender:b+'gender', fname:b+'fname', lname:b+'lname', mobile:b+'mobile'
					...
					, email:b+'email', bdate:b+'bdate', save:b+'regsave', warn:eid+'_div_warn' ,optin:b+'optin'
				};
	this.elements = new A_el();
	this.state = C_eRegister.defaults.align(preset);
		...
		let typingfn = new A_cb(this, this.typingfn, null, null, 1500); /* special cast for firstname whom we will try to guess the gender from */
	...
	let lname 		= new C_iFIELD(this.eids.lname, { onfchange:typing, onenterkey:onenter }, { digits:this.dataSet.lastname, type:INPUT_NAMES, mandatory:true, focus:true, placeholder:po_ln });
	let fname 		= new C_iFIELD(this.eids.fname, { onfchange:typingfn, onenterkey:onenter }, { digits:this.dataSet.firstname, type:INPUT_NAMES, mandatory:true, placeholder:po_fn });
	...
	let addr 		= new C_iFIELD(this.eids.addr, { onfchange:typing, onenterkey:onenter }, { digits:this.dataSet.address, type:INPUT_NAMES, mandatory:!shortform, placeholder:C_XL.w('address') });
	let zip 		= new C_iFIELD(this.eids.zip, { onfchange:typing, onenterkey:onenter }, { digits:this.dataSet.zipCode, type:INPUT_NAMES, mandatory:!shortform, placeholder:C_XL.w('zipCode') });
	let city 		= new C_iFIELD(this.eids.city, { onfchange:typing, onenterkey:onenter }, { digits:this.dataSet.city, type:INPUT_NAMES, mandatory:!shortform, placeholder:C_XL.w('city') });
	let country		= new C_iFIELD(this.eids.country, { onfchange:typing, onenterkey:onenter }, { digits:this.dataSet.country, type:INPUT_NAMES, mandatory:!shortform, placeholder:C_XL.w('country') });
...
C_eRegister.prototype = { 
	...
	// event handling
	typing: function(d) {
		let optin = !!this.controls.optin.value(); 
...
C_eVappointments.prototype = {
	...
	vdistinctcount: function() {
		let c = [];
		let vs = this.state.dSvisitors;
		for(let vid in vs) 
		{ // total count for all visitors
			let dS_v = vs[vid];
			for(let rid in dS_v.apps) 
				if(dS_v.apps[rid].deletorId==0) 
					if(!c.includes(rid)) c.push(rid);
		}
		return c.length;
	},
	...
	appsdisplay: function() {
		...
		if(count) {
			...
			for (let i=0;i<2;i++)
			{
				...
				if(i==0)
				{
					if(!options.maydelete) 
					{
						let m = C_XL.w('e- deletion unavailable'); 
						list +='<div style="margin: 1em 0; text-align:right;">'+m+'</div>';
					}
					else
					{
						let m = is.tactile? C_XL.w('e- touch to delete') : C_XL.w('e- click to delete');
						list +='<div style="margin: 1em 0 0.5em 0; text-align:right;">'+m+'</div>';
						let m2;
						
						if(options.maydelete==1) 		m2 = C_XL.w('Cancellation_before_1h');
						else if(options.maydelete==12) m2 = C_XL.w('Cancellation_before_12h');
						else if(options.maydelete==24) m2 = C_XL.w('Cancellation_before_24h');
						else if(options.maydelete==48) m2 = C_XL.w('Cancellation_before_48h');
						else if(options.maydelete==72) m2 = C_XL.w('Cancellation_before_72h');
						else m2 = C_XL.w('Cancellation_before_7days');

						list += '<div style="margin: 0.5em 0 1em 0; text-align:right;"><b>'+m2+'</b></div>';
					}
			...
			$(this.elements.wrp).html(msgabove+list+confirmmsg);
	...
	//DEV : OLD VERSION OVERRIDEN BY PASCAL
	// ajax callbacks
...
C_eVselect.prototype = {
	...
	{
		this.elements.vselect.innerHTML = '';
	},*/
	activate: function() {	
		this.elements.collect(this.eids.own);
		this.controls.activate('C_eCheckIn');
...
C_eProcess = function(eid, callbacks) { // FIVE STEPS RESERVATION PROCESS IS OVERALL MANAGED HERE
	...
	let checkin = new C_eCheckIn(this.eids.controls.chkin, { identified:new A_cb(this, this.identified), blacklisted: new A_cb(this, this.blacklisted)} ); // includes an instance of C_eRegister
	...
	let staff = new C_iSTAFF(this.eids.controls.staff, 'staffing', new A_cb(this,this.staff), {}, { show:staffshow, maxrows:(is.small?false:false), maxcols:1 });
		...
		let more = {caption:C_XL.w('e- more app'), tip:'', css:'action double', cssfa:'fa-plus' };
		let done = {caption:C_XL.w('e- done ok'), tip:'', css:'action double', cssfa:'fa-sign-out' };
	...
	this.controls = new A_ct({progress:progress, capps:capps, vselect:vselect, visitor:new C_iPASS('-'), auth:auth, checkin:checkin, eperf:eperf, duration:duration, before:before, ampm:ampm, staff:staff, tboxing:tboxing, options:options, snexton:snexton, limit:limit, limitdate:limitdate});
...
C_eProcess.prototype = {
	...
	display: function(containerEid) {
		...
		// pane 2 - search options
			...
			let perfarea = eperf+perftip;
						...
						let displaybefore = this.controls.before.controls.radio.options.count>1;
						let before = displaybefore? '<div class="">'+this.controls.before.display()+'</div>':'';
				...
				let soptablebefore = displaybefore?'<div style="display:block; flex-grow:1;">'+before+'</div>':'';
				let soptableampm = withampm?'<div style="display:block; flex-grow:1;">'+ampm+'</div>':'';
		...
		// pane 3 - choosing out of availabilities
			...
			let s3 	= '<div class="" style="">'+this.controls.progress.display(3)+'</div>';		
				//BSP 2024-05-14 add display=none at slothere sentence (ambigous message because of from date not understood)
				let slotshere 	= '<div class="e-msg"  id="'+this.eids.msgs.slotshere+'" style="display:none;margin-bottom:0.5em;">'+C_XL.w('slots here')+'</div>';
			...
			let availipane = '<div id="'+this.eids.panes.p3+'" class="e-msg" class="step-pane">'+change1+slotshere+selectmsg+backslots+slotspane+moreslots+'</div>';
	...
	before: function(date,option) { 
		...
		if(this.state.initializing) 
			return; // this function is called during this.controls.activate process, so do not jump to step 2 now
		else 
			this.change();
	...
	workcodeSelect: function(workcodeIdsArray) { // a workcode has been checked or unchecked
		...
		let hasmoney = mobminder.account.ePayActive && ( dS_wk.price||dS_wk.deposit );

		// show the notice that is linked with the selected workcode
		let name = dS_wk.name; // name is used outside the if {} 
		let l = mobminder.context.surfer.language; // language specified for the web page

		if(l == dS_wk.altLanguage1) { if(dS_wk.altName1) name = dS_wk.altName1; }
		if(l == dS_wk.altLanguage2) { if(dS_wk.altName2) name = dS_wk.altName2; }

		let note=false;
		if(dS_wk.webpagenote) 
		{
			note = dS_wk.webpagenote;
			// reset the name and note when alternative languages are specified in the workcode
			if(l == dS_wk.altLanguage1) { if(dS_wk.altwebpagenote1) note = dS_wk.altwebpagenote1; }
			if(l == dS_wk.altLanguage2) { if(dS_wk.altwebpagenote2) note = dS_wk.altwebpagenote2; }
		}
		let hasnote = !!note;
		...
		if(hasnote||hasmoney) {
			...
			$(this.elements.msgs.wkname).html(name.htmlize()).show();
			if(hasnote) $(this.elements.msgs.wknotice).html(note.htmlize()).show();
		} else {
	...
	cappsloaded: function(visitors) {
		...
		let eresaMax = mobminder.context.surfer.eresaMax;
		let ac = this.controls.capps.vdistinctcount(); //take distinct app! no duplicate if same app for several visitors //this.controls.visi.appscount();
		//console.log("eresaMax"+eresaMax);
		//console.log("ac="+ac);
	...
	search: function() { // the 'search' button was hit
	...
	ispayconiqenabled()
	{
		//ePayWebActive => 1:'VISA/MasterCard', 2:'Online Payconiq', 4:'Bancontact', 8:'Carte Bleue' } };
		let ePayWebActive = parseInt(mobminder.account.ePayWebActive);
		return (mobminder.account.ePayActive==1 
			&& mobminder.account.ePayconiqKey 
			&& ((ePayWebActive & 2) === 2));
	},
	ispaycardenabled()
	{
		...
		return (mobminder.account.ePayActive==1 
			&& ((ePayWebActive & 1) === 1)
			&& mobminder.account.ePayComputopId 
			&& mobminder.account.ePayComputopFish 
			&& mobminder.account.ePayComputopHmac);

	},
	ispaybancontactenabled()
	{
		//ePayWebActive => 1:'VISA/MasterCard', 2:'Online Payconiq', 4:'Bancontact', 8:'Carte Bleue' } };
		let ePayWebActive = parseInt(mobminder.account.ePayWebActive);
		return (mobminder.account.ePayActive==1 
	...
	{
		//1 account name
		let res = mobminder.account.name;

		//date = 24 septembre 2022 à 15h30
		res += ' - ' + C_XL.date(new DateAcc(resa), { abreviation:'full', weekday:true, time:true });
		...
		let bcalsname = resa.bCal.name;
		res += ' - '+ bcalsname;
		//console.log("bcalsname="+bcalsname);

	

		//prestation-------------------------------------
		if(resa.performances)
		{
		...
		//console.log("getpaydescription="+res);
	...
	slotclick: function(resa) {
		...
		let paycard = this.ispaycardenabled()&&price;
		let paybancontact = this.ispaybancontactenabled()&&price;

		// -----------------------------------------------------------------------------
		// disable all pay means if payconiq server is down ----------------------------
		// -----------------------------------------------------------------------------
		if(mobminder.context.provider_threshold!=0) //if check counter condition is enabled
		{
			if(payconiq) //if payconiq payment is activated
			{
				//if payconiq server is down
				if(mobminder.context.is_Payconiq_alive>mobminder.context.provider_threshold)
				{
					//then payconiq AND paycard are disabled!
					payconiq=false;
					if(paycard) paycard=false;
					if(paybancontact) paybancontact=false;
				}
			}
		}
		// -----------------------------------------------------------------------------
		...
		{
			paymsg=false;
			this.elements.msgs.tipconfirm.innerHTML = mobminder.context.surfer.eresaAllowNote ? '' : C_XL.w('e- pls confirm');
		}
		else
		{
			if(perfprice==amount)
				paymsg=C_XL.w('epay_payamount').replace('{0}',(amount/100).toFixed(2));
			else
				paymsg=C_XL.w('epay_paydeposit').replace('{0}',(amount/100).toFixed(2));

			this.elements.msgs.tipconfirm.innerHTML = mobminder.context.surfer.eresaAllowNote ? '' : C_XL.w('confirm_payment');
		}
		...
		let eresa = new C_eRESA(resa, { saved:new A_cb(this,(confirm?this.newresasaved:this.newresasavedandpay)), failed:new A_cb(this,this.overbooking) },{ confirm:confirm,payconiq:payconiq,paycard:paycard,cardholder:'',paybancontact:paybancontact,amount:amount,transnote:transnote,paymsg:paymsg } );
		this.controls.progress.caption(3, eresa.summary()).slideto(2);
	...
	newresasavedandpay: function(dataSets) { // SCREEN STEP 5
		...
		if(payment.paymean==C_dS_payment.type.onlinepayco)
		{
			//console.log("1:payment.qrcodestring="+payment.qrcodestring);
			//console.log("2:qrcodeshort="+qrcodeshort);

			let qrcodeshort = payment.qrcodestring;
			if(!qrcodeshort.includes('?')) qrcodeshort = qrcodeshort.substr(0, qrcodeshort.indexOf('?')); 
			...
			if(!is.small) //TODO switch mobile not
			{
				...
				this.elements.msgs.payinfo.innerHTML = C_XL.w('scan_pay')+' ('+(payment.amount/100).toFixed(2)+' €)'; 
			}
			else //smartphone
			{
				...
				isios =  !!(navigator.userAgent.match(/iPhone/) || navigator.userAgent.match(/iPad/));
				if(isios)
					window.location.replace(payment.qrcodestring);
				else
					this.ouvrirDeepLink(payment.qrcodestring);
			}
			
			this.buttons.paycancel.show(true);
		}
		else// if(payment.paymean==C_dS_payment.type.onlinecards || payment.paymean==C_dS_payment.type.onlinebancontact ) //= cards OR bancontact
		{
			this.elements.msgs.payinfo.innerHTML = C_XL.w('mobile_payment_processing'); //clean qrcode before next booking
			this.elements.qrcode.innerHTML = '';
			this.buttons.paycancel.hide(true);
	...
	ouvrirDeepLink: function(urlDeepLink) {
		var anchor = document.createElement('a');
		anchor.href = urlDeepLink;
	...
	backhere:function(dataSets, stream) { // periodic payconiqdog callback
		...
		if(this.watchdogcounter>=204) //20250815 bsp correction : 17min * 12 (60s/5s) = 204 (17 min = 15 min for prebooking timeout + 2 (margin)) //see (*pb01*)
		{
		...
		if(payment.transtatus==C_dS_payment.status.code.pending||payment.transtatus==C_dS_payment.status.code.identified)
		{
		...
		else if(payment.transtatus==C_dS_payment.status.code.success||payment.transtatus==C_dS_payment.status.code.reserved)
		{
			//this.watchdog.suspend(true);
	...
	appsummary: function() { // heads back to step 1 after a new appointment was taken
		...
		//DEV reset firstsignin after first rdv creation, otherwise appsummary goes directly to booking process
		if(this.controls.checkin.state.firstsignin==true) this.controls.checkin.state.firstsignin=false;

		this.controls.capps.appload(vids);
		
		//hide vselect
		$(this.elements.vslct).hide();
	...
	slotsfresh: function(inlineDataSets) { // this is where we display this stuff on the slots pad
		...
		if(this.days) { 
			/*	//let c = this.controls.contin...value(); 
			...
			s = new Date(s*1000); s.toMidnight();
				// console.log('setting conti.. to :'+s.getUnixTime());
			this.controls.contin..d.set(s.stamp()); */
	...
	activateforreturnurl: function() {
		//console.log("call activateforreturnurl");
	...
	visidataforreturnurl: function(inlineDataSets, stream) { // e-resa STEP 1: callback from initial email check
		...
		//DEV 23-03-08 because error if modify options and click on search
		this.controls.capps.appload(vids)
		...
		if(payment.paymean==C_dS_payment.type.onlinepayco)
		{
			...
			else
			{
				...
				this.elements.msgs.payinfo.innerHTML = C_XL.w('mobile_payment_processing')+' ('+(payment.amount/100).toFixed(2)+' €)'; 
			}
			this.buttons.paycancel.show(true);
		}
		else //cards
