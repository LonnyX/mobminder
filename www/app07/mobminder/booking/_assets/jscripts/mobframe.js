...
var measurePlitem = true; // measure the display time of entering plitems data
...
var monitorFramework = {  0:'C_iMENU', 1:'A_ct', 3:'C_KEY'}; // 0:'A_ct', 1:'A_cb', 2:'C_iMODAL',, 4:'C_iCLIK', 5:'C_iWIN', 6:'C_iBUTTON'
...
var monitorModalResa 		= { 0:'M_RESA', 1:'C_iINOUT', 2:'C_iCUE', 3:'C_iSPAN', 4:'C_iDPpop', 5:'C_iDP', 6:'C_iOFFD', 7:'C_iCSS', 8:'C_iMENU', 9:'C_iDDSTACK', 10:'C_iCLIK', 10:'C_iEDIT' /*, 10:'C_dS_reservation', 11:'C_dS_attendee', 12:'C_dS_att_visitor', 13:'C_dS_performance'*/};
var monitorModalResaSerie 	= { 0:'M_RESA', 2:'C_iDPpop', 3:'C_iDP', 4:'C_iRECUR', 5:'C_iWDAYs' };
var monitorModalResaPayments= { 0:'M_RESA', 1:'M_PAYMENT', 2:'C_dS_payment', 3:'C_iBILL', 4:'C_iPAY' };
var monitorModalRscr 		= { 0:'M_RESC', 1:'C_iPLUS', 3:'M_Huser', 4:'M_Hourly', 5:'C_iDP', 6:'C_iOFFD', 7:'C_iCSS', 9:'C_iYEAR' };
...
var monitorFinder = { 0:'C_backFIND', 1:'C_iARRAY', 2:'C_iSTAFF', 3:'C_iACPICK', 4:'C_iDP', 5:'M_duplacc' };


	//// application
var monitorHorizontal = { 0:'C_iMOB', 1:'C_iDNAV', 2:'P_AgendaHVL', 3:'P_horiz', 4:'P_htype', 5:'P_hrow', 6:'C_iPLAN', 9:'P_RULER', 10:'P_Vertical'};
var monitorVertical = { 0:'C_iMOB', 2:'P_AgendaHVL', 3:'P_Vertical', 3:'P_column', 5:'C_iPLAN', 6:'P_RULER', 7:'P_GRID' }; //, 4:'P_column',, 1:'R_search'
var monitorListView = { 0:'C_iMOB', 1:'R_search', 2:'C_iPLAN', 3:'P_AgendaHVL', 4:'P_List'};
var monitorStickers = { 0:'P_Sticker', 1:'P_GRID', 2:'P_column', 3:'P_horiz', 4:'P_htype', 5:'P_hrow'};
...
var monitorSearch 	= { 0:'R_search', 1:'C_iACPICK', 2:'C_iAC', 3:'C_iPLAN', 4:'C_iAMPM', 5:'C_iBEFORE', 6:'C_iSLOT', 8:'C_iTABLE' };
var monitorTopMenu 	= { 0:'C_iMOB', 1:'T_bar', 2:'T_login', 3:'T_notes', 4:'T_tasks', 5:'C_iCAL', 6:'T_backo', 7:'T_dp' };
var monitorEresa 	= { 0:'e_MOB', 1:'C_eSearch', 2:'C_eRESA', 3:'C_eREG', 4:'C_eVisitor', 5:'C_eCheckIn', 6:'C_iSTAFF', 7:'C_eProcess', 8:'C_iSLIDE', 9:'C_iEDIT', 10:'C_iFIELD' };
...
//var pay_provider_alive_limit = 0; // if paymean alive counter is greater than limit, then paymean is disabled
								   // 0 => disable the check of alive counter exceeded before enabling/disabling payment


////// function mode setting
//
//	
	//DEV true for debugging
	//var verboseOn = true; //DEV : true/false to log traces
	var verboseOn = false; //DEV : true/false to log traces

	//DEV false or defined array
	//var classOnlyVerbose = monitorEresa;
	var classOnlyVerbose = false; // false; //monitorEresa; // (defines which filter to apply) default: false (verbose for all classes)
...
A_ct.prototype = {
	...
	validation: function(sub) {
		...
		if(!sub) // (*act01*) only the highest level of nesting reports the problem (so to report it only once).
			if(!isvalid) {  
				new C_iMSG(C_XL.w('invalid form'));
			}
		return isvalid;
	},
...
var is = { 
  small:
		...
		!!(/webos|iphone|ipod|blackberry|nokia|htc|opera mini|mobile safari|windows phone|iemobile|android|ipad|huawei/i).test(navigator.userAgent.toLowerCase())
, tactile:
		!!(   /webos|iphone|ipod|blackberry|nokia|htc|opera mini|mobile safari|windows phone|iemobile|android|ipad|huawei/i.test(navigator.userAgent.toLowerCase())
		   || (navigator.maxTouchPoints && navigator.maxTouchPoints > 1 && /macintosh/i.test(navigator.userAgent)))

		// ||('ontouchstart' in window)||(navigator.msMaxTouchPoints) PROBLEM WITH HYBRID DEVICES
...
if(verboseOn) {
	...
	console.log('is - tactile:'+is.tactile+', small screen:'+is.small+', navigator.maxTouchPoints:'+navigator.maxTouchPoints);
...
function symbol(which,css,style) {
	...
	switch(which) {
		...
		case 'plus task': return '<div style="display:inline-block;" class="fa fa-13x fa-thumbtack fa-rotate-45 fa-gray"></div>'+'<div style="display:inline-block;" class="fa fa-09x fa-plus fa-gray"></div>';
...
//	date & time are formatted by backend using account location time zone and sent to js frontend dataset in new reservation properties (cueIn_yyyy, etc)
//	DateAcc emulates a Date structure by containing separate fields like yyyy,mm,dd,wd,hhmm and provide all properties and functions used by C_XL.date function
// 	see php/addmeta_datetimeverbose function, see (*dtacc01*)
//
class DateAcc
{
    constructor (resa) {
        this.yyyy = resa.cueIn_yyyy;
		this.mm = resa.cueIn_mm;
		this.dd = resa.cueIn_dd;
		this.wd = resa.cueIn_wkday;
		this.hhmm = resa.cueIn_hhmm;
		//console.log("new DateAcc="+this.yyyy+"-"+this.mm+"-"+this.dd+"-"+this.wd+"-"+this.hhmm);
    }
	getDate() //day in month
	{
		return this.dd;
	}
	getMonth() //month, january=0
	{
		//this.mm : january=1
		return this.mm-1;
	}
	/*getWeekNumber() //number of the week
	{
		return 0;
	}*/
	getPHPday() //day of the week
	{
		//this.wd : value 0 is for Sunday. value 6 is for Saturday
		if(this.wd==0) return 7; // PHP standard has monday = 1 and sunday =7
		return this.wd;
	}
	getFullYear()
	{
...
C_XL.date = function(jsdateORstamp, preset, language) {
	...
	if(jsdateORstamp) {
		...
		else if(jsdateORstamp instanceof DateAcc) 
		{
			//console.log("DateAcc");
			jsdate = jsdateORstamp; // you passed a Date object
		}
		//else if(typeof (jsdateORstamp|0) == 'number') //issue : object|0 is always typeof number!
		else if(typeof jsdateORstamp == 'number') //solved
		{
			//console.log("number");
	...
	switch(language) { 
		...
		case mobminder.language.codes.english: // reversed English sequence
			post = 'th'; if(date=='1') post='st'; if(date=='2') post='nd';if(date=='3') post='rd';
			s = [month, date+post]; 
		...
		case mobminder.language.codes.german:
			///////////////////////////////////////////////////////////////////////////////////////
			//2022-08-19 DEV correction "Donnerstag 21 Juli 2022" => "Donnerstag, 21. Juli 2022"
			post = '.';
			s = [date+post,month]; 
...
C_XL.gender.codes = new Array(); // set by setContextLanguage() in defined in language.js
...
C_iCLIK.prototype = {
	...
	display: function(css, rowspan, tag) {
		...
		//////////////////////////////////////////////////////////////////////////////////////
		//DEV : force 'cursor:default' if idle == true
		//let style  = ' style="cursor:pointer; '+this.state.style+indent+'"'; // not used for tag 'input'
		let style  = ' style="'+(this.state.idle?'cursor:default;':'cursor:pointer; ')+this.state.style+indent+'"'; // not used for tag 'input'
		//////////////////////////////////////////////////////////////////////////////////////
	...
	inlabel: function(regexp, options) { // highlights in label the digits given through regexp, option: elsehide
		...
		let where = this.state.ui.search(regexp); // returns start position
		let found = (where >= 0);
...
const INPUT_CARDHOLDER 	= 260
const INPUT_CARDMONTH 	= 261
const INPUT_CARDNUMBER 	= 262
const INPUT_CARDYEAR	= 263
const INPUT_CARDCVC		= 264
const INPUT_CARDDATE 	= 265
...
, 260:'INPUT_CARDHOLDER'
, 261:'INPUT_CARDMONTH'
, 262:'INPUT_CARDNUMBER'
, 263:'INPUT_CARDYEAR'
, 264:'INPUT_CARDCVC'
, 265:'INPUT_CARDDATE'
...
C_iEDIT.defauts = new A_df( { 
	/* user presets */ label:'', placeholder:'', digits:'-', type:INPUT_TEXT, enabled:true, hidden:false, focus:false
						, tabindex:false, max:false, mandatory:false, loginId:0, dblclick:true, units:''
...
C_iEDIT.prototype = {
	...
	display: function(css, label, labelcss, options) { // return top bottom like: <div>a label</div><input>input field</input>
		...
		let textinput, type='text', pattern='', name='', autocomplete='autocomplete="off"';
		...
		switch(this.state.type) {
			...
			case INPUT_NUMER: 
				//type='number'; 
				if(is.tactile) pattern = 'inputmode="numeric" pattern="[0-9]*"'; // for soft keyboards switching, this misses the + option
				break; 
			case INPUT_PHONE: 
			case INPUT_MOBILE: 
				type='tel'; 
			...
			case INPUT_CARDHOLDER: 
				//name='name="cardholder"';
				//this.eids.ui='cardholder'; 
				autocomplete='autocomplete="cc-name"';
				break;
			case INPUT_CARDNUMBER: 
				if(is.tactile) pattern = 'inputmode="numeric" pattern="[0-9]*"'; 
				autocomplete='autocomplete="cc-number"'; //this.eids.ui='cardNumber'; name='name="cardnumber"';  // for soft keyboards switching, this misses the + option
				break; 
			case INPUT_CARDMONTH: 
				//if(is.tactile) 
				pattern = 'pattern="[0-9]*"'; //this.eids.ui='cardExpirationMonth'; name='name="cardExpirationMonth"'; // for soft keyboards switching, this misses the + option
				break; 
			case INPUT_CARDYEAR: 
				//if(is.tactile) 
				pattern = 'pattern="[0-9]*"'; //this.eids.ui='cardExpirationYear'; name='name="cardExpirationYear"'; // for soft keyboards switching, this misses the + option
				break; 
			case INPUT_CARDCVC: 
				if(is.tactile) pattern = 'inputmode="numeric" pattern="[0-9]*"'; 
				autocomplete='autocomplete="cc-csc"'; //this.eids.ui='cvc2'; name='name="cvc2"'; // for soft keyboards switching, this misses the + option
				break; 
			case INPUT_CARDDATE: 
				if(is.tactile) pattern = 'inputmode="numeric" pattern="(0[1-9]|1[0-2])\/\d{2}"'; 
				autocomplete='autocomplete="cc-exp"'; //this.eids.ui='cardExpiration'; name='name="cardExpiration"'; // for soft keyboards switching, this misses the + option
				break; 
		...
		if(this.state.rows) 
			textinput = '<textarea'+cssclass+' rows="'+this.state.rows+'" id="'+this.eids.ui+'" '+tabindex+placeholder+'>'+display+'</textarea>';
		else 
			textinput = '<input type="'+type+'" '+name+' '+pattern+' '+autocomplete+' '+cssclass+' id="'+this.eids.ui+'" value="'+display+'"'+tabindex+placeholder+' '+autocap+'/>';
		
		let dlabel = ''; if(label) {
			this.state.label = options.xl?C_XL.w(label):label;
			//dlabel = '<div class="select-header textcolor-light '+(labelcss||'')+'">'+this.state.label+'</div>';
			dlabel = '<div class="'+(labelcss||'')+'">'+this.state.label+'</div>';
		}
		let units = this.state.units || ''; if(units) units = ' '+units;

		return dlabel+textinput+units;
	},
	...
	activate: function() { // must be called after display
		...
		if(is.tactile) 
		{
			if(is.android) 
			{
				$(this.elements.ui).bind('input', (e) => { that.androidkeys(e); } );
			}
			else if(is.appleios)
			{
				$(this.elements.ui).bind('input', (e) => { that.appleioskeys(e); } );
				//console.log('add input event==>appleioskeys');
			}
		}
		return this;
	},
	...
	androidkeys: function(e) { // only for Android smartphone keyboards, replaces the keypress function. 
		...
		if(this.state.type==INPUT_CARDDATE) { 
	...
	appleioskeys: function(e) { // only for Android smartphone keyboards, replaces the keypress function. 
		
		
		let el = this.elements.ui;
		let newvalue = el.value; // value as is
		//console.log('==>appleioskeys newvalue=,',newvalue);
		//console.log('==>appleioskeys oldvalue=,',this.state.digits);
		if(vbs) vlog('mobframe.js','C_iEDIT','appleioskeys','eid:'+this.eids.ui+', old value = '+this.state.digits+',new value ='+newvalue);


		if(this.state.digits!=newvalue)
		{
			//console.log('==>appleioskeys calling changed');
			this.changed(true);
		}
		else
		{
			//console.log('==>appleioskeys NO calling changed');
		}
	...
	validate: function(newdigits) { // sets this.state.valid according to expected format type and mandatory attributes
		...
		switch(this.state.type) {
			...
			case INPUT_CARDMONTH: this.state.valid = this.isCreditMonth(newdigits); break;
			case INPUT_CARDYEAR:  this.state.valid = this.isCreditYear(newdigits); break;
			case INPUT_CARDNUMBER: this.state.valid = this.isCreditNumber(newdigits); break;
			case INPUT_CARDCVC: this.state.valid = this.isCVC(newdigits); break;
			case INPUT_CARDDATE: this.state.valid = this.isCreditDate(newdigits); break;
		}
	...
	filterfor: function(inputtype) { // provides a filter for allowed chars depending on field type.
		...
		switch(inputtype) { // keep this inline with (*re01*), but keep in mind that they might differ
			...
			case INPUT_EMAIL: 	f = '._a-zA-Z0-9-@'; break;
			
			case INPUT_PRICE: 	f = '.0-9-'; 		break;
			...
			case INPUT_URL: 	f = '.a-zA-Z0-9-\:\/\?\&='; break;
			...
			case INPUT_TEXT: // single line text, with extended points set, parentheses
					f = language+extpoints; break;
			case INPUT_ML_TEXT: // multi line notes 
					f = language+extpoints+cr; break;
			...
			case INPUT_ML_HTML:  // allows emoji's
			case INPUT_ML_MAIL: 
			case INPUT_ML_SMS: 
			
			case INPUT_TEXT_WE: // basically you accept anything BUT the forbidden chars ( see this.disallowed() )
			default:
				f = false; break;
		}	
		return { 'filter':f };
	...
	//////////////////////////////////////////////////////////////////////////////////////
	//DEV : replace Pascal function by DEV function (to confirm with Pascal)
	//////////////////////////////////////////////////////////////////////////////////////
	...
	setvalidtip: function() {
		...
		var msg = false; 
		//////////////////////////////////////////////////////////////////////////////////////
		//DEV display error tooltip only if field is filled!
		//////////////////////////////////////////////////////////////////////////////////////
		if(this.state.filled) 
			if(!this.state.valid)
					switch(this.state.type) {
						...
						case INPUT_BDATE: 	msg = C_XL.w('not bdate'); break;
						case INPUT_PRICE: 	msg = C_XL.w('not price'); break;
						case INPUT_IBAN: 	msg = C_XL.w('bad format'); break; //DEV : new case
						case INPUT_BIC: 	msg = C_XL.w('bad format'); break; //DEV : new case
						case INPUT_ALPHA: 	msg = C_XL.w('not alpha'); break;
						case INPUT_TOKEN: 	msg = C_XL.w('bad e-url'); break;
						case INPUT_URL: 	msg = C_XL.w('not url'); break;
						case INPUT_LOGIN: 	msg = C_XL.w('bad login'); break;
						case INPUT_PASSWD: 	msg = C_XL.w('not pass'); break;
						case INPUT_CARDMONTH: 	msg = C_XL.w('bad format'); break;
						case INPUT_CARDNUMBER: 	msg = C_XL.w('bad format'); break;
						case INPUT_CARDYEAR: 	msg = C_XL.w('bad format'); break;
						case INPUT_CARDCVC: 	msg = C_XL.w('bad format'); break;
						case INPUT_CARDDATE: 	msg = C_XL.w('bad format'); break;
					}	
		//////////////////////////////////////////////////////////////////////////////////////
		//DEV disable tooltip if mandatory and empty because a local message is displayed in case of empty fields
		//if(this.state.mandatory)
		//	if(!this.state.filled) { msg = C_XL.w('not filled in'); }
		//////////////////////////////////////////////////////////////////////////////////////
		
		//////////////////////////////////////////////////////////////////////////////////////
		//DEV display red border if(tooltip msg is defined) OR (field not filled AND mandatory)
		//////////////////////////////////////////////////////////////////////////////////////
	...
	keyup: function(e) { // this event is triggered after the character appeared in the input field
		let key = e.which; // key code - Netscape/Firefox/Opera
		if(vbs) vlog('mobframe.js','C_iEDIT','keyup','eid:'+this.eids.ui+', key:'+key);
	...
	isPrice: function(sText) {
		if(sText == '0') return true; // entering no price at all is also ok (up to mandatory feature to further decide)
	...
	isMobile: function(sText) {
		...
		let NSNdigits = function(digits) { // National Significant Numbers, check also (*nsn*)
			...
			if(prefixis('+30',digits)) return 10; // Greece, mobiles like +30 6xx 1234567890
	...
	isCVC: function(sText){
		//console.log("isCreditMonth="+sText);
		//if(sText=='') return false;
		//var intVal = parseInt(sText);
		//if(intVal<1) return false;
		//if(intVal>12) return false;

		//if(sText=='0') return false;
		//if(sText=='00') return false;
	...
	isCreditYear: function(sText){
		...
		{
			if(sText.startsWith('20')) return true;
			else return false;
		}
		else return false;
	...
	isCreditDate: function(sText){
		//console.log("isCreditDate="+sText);
		if(sText.length!=5) return false;

		var regex = new RegExp("^(0[1-9]|1[0-2])\/?([0-9]{2})$");
		if(!regex.test(sText)) return false;

		return true;
	},
	isCreditNumber: function(sText){
		if(sText == '') return true;
		var regex = new RegExp("^[0-9]{16}$");
		if(!regex.test(sText)) return false;
		
		if(!sText.startsWith('4') && !sText.startsWith('5')) return false;
		
		return this.luhnCheck(sText);
	},
	...
	isURL: function(sText) {
		...
		if(sText.search(/^((https?):\/\/)?(www\.)?[a-z0-9\-\.]{3,}\.[a-z]{2,3}(\/[a-zA-Z0-9\-\.]{1,}){0,5}?$/) == -1) 
...
C_iMODAL = function(html, callbacks, preset) { 
	...
	if(this.status.position.drop||this.status.position.side) { // the modal is positionned relatively to another element on page (case of e.g. a drop down list)
		...
		if(this.status.position.offset.y == 0) {
				const yscroll = bodyscroll(); // put modal on visible body even when body is scrolled
...
C_iMODAL.prototype = {
	...
	busy: function(onoff) {
		if(onoff && !this.status.busy) {
			...
			const busy = '<div class="modal-busy" id="'+this.eids.busy+'" style="position:absolute; z-index:'+zindex+'; top:0; left:0; width:100%; height:100%; overflow:hidden;"></div>';
			$(this.elements.inset).append(busy);
			C_iWIN.cursor('wait');
		} else {
	...
	display: function(html) {
		...
		if(this.status.fullscreen) // then modals are full screen
			...
			outset = '<div class="'+c.outset+' '+c.shape+'" id="'+this.eids.outset+'" style="position:absolute; z-index:'+zindex+'; overflow:hidden;'+bottom0px+'">'+inset+'</div>';
		}
	...
	show: function() {
		...
		if(this.status.fullscreen) { 
			...
			else if(this.status.size.maxy) { $(this.elements.body).css('max-height', maxy ); }
...
//////////////////////////////////////////////////////////////////	
//DEV : disable anonymous function
//////////////////////////////////////////////////////////////////
/*(function (w,d) {
...
//////////////////////////////////////////////////////////////////
preventBrowserBackButton = function (w,d) {
