...
C_dS_context.defaults = new A_df({groupId:0, loginId:0, keyId:0, pivotStamp:0, gmtshift:0, is_Payconiq_alive:0, is_SoftPay_alive:0, is_HardPay_alive:0, provider_threshold:0});
...
C_dS_workcode.options = function(preset) { // for C_iCRESTA, see (*aco01*)
	...
	if('eWorkcodes' in preset) { // preset.eWorkcodes undefined => return all workcodes
		...
		for(let id in register) {
			...
			if(preset.tip) {
				let tip = '<strong>'+name+'</strong>'; if(note) tip+= ':<br/>'+note;
				presets[id].tip = tip;
			}
		}
		order.sort(function(a,b){return (register[a].name>register[b].name)?1:-1; });
		...
		// show all kind of workcodes, sort them into 2 sections  PREPARED BUT NOT IN USE, we would like to display e-reservables in a second section and non eresa in the top section
...
C_dS_payment.type = { notset:-1, cash:0, mobqrcode:1, payconiq:2, cards:4, softpos:11, hardpos:21, onlinepayco:33, onlinecards:44, onlinebancontact:55 };
C_dS_payment.status = { // see (*ep10*)
	code:{ isnew:999, unauthorized:0, identified:17, pending:10, success:20, expired:30, failed:40, cancelled:50 },
	translate:{ 999:'ep-st-isnew', 0:'ep-st-unauthorized', 10:'ep-st-pending', 17:'ep-st-identified', 20:'ep-st-success', 30:'ep-st-expired', 40:'ep-st-failed', 50:'ep-st-cancelled' }
};
...
C_dS_resource.names = {  // watch out ! mobile app has copied this list, if you add another alias here, you need to add it also in the smartphone baseline
	...
	resources:0, workrooms:10, workplaces:20, classrooms:25, carerooms:30, 
...
C_dS_reservation.defaults = new A_df( { cueIn:0, cueOut:86400	// !! watch this when you add new members to this object (*r00*)
	...
	, v:0, archived:0
	, cueIn_yyyy:0,cueIn_mm:0,cueIn_dd:0,cueIn_wkday:0,cueIn_hhmm:'00:00' } ); //date & time are formatted by backend using account location time zone and sent to js frontend dataset in new reservation properties
...
C_dS_reservation.prototype = { 
	...
	cueOut_hhmm: function() {
		let durationmin = (this.cueOut-this.cueIn)/60; // is a number minutes

		let [inhour, inmin] = this.cueIn_hhmm.split(':').map(Number);
    
		let outtotalmin = inhour * 60 + inmin + durationmin;
		
		let outhour = Math.floor(outtotalmin / 60) % 24; // Modulo 24 for reest to 0 after 23h
		let outmin = outtotalmin % 60;
		
		// Format
		let southour = String(outhour).padStart(2, '0');
		let soutmin = String(outmin).padStart(2, '0');
		
		return `${southour}:${soutmin}`;
