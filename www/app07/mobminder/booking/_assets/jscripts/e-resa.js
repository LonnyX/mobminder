...
C_eOptin.prototype = { 
	...
	value: function(){
		return !!this.controls.optin.value();  // which is [0,1] turned into [false,true]
	},
...
C_eREG = function(eid, callbacks, preset) {
	...
	let lname 		= new C_iFIELD(this.eids.lname, { onfchange:typing }, { digits:this.dataSet.lastname, type:INPUT_NAMES, mandatory:true, focus:true, placeholder:C_XL.w('lastname') });
	let fname 		= new C_iFIELD(this.eids.fname, { onfchange:typingfn }, { digits:this.dataSet.firstname, type:INPUT_NAMES, mandatory:true, placeholder:C_XL.w('firstname') });
	...
	let addr 		= new C_iFIELD(this.eids.addr, { onfchange:typing }, { digits:this.dataSet.address, type:INPUT_NAMES, mandatory:!shortform, placeholder:C_XL.w('address') });
	let zip 		= new C_iFIELD(this.eids.zip, { onfchange:typing }, { digits:this.dataSet.zipCode, type:INPUT_NAMES, mandatory:!shortform, placeholder:C_XL.w('zipCode') });
	let city 		= new C_iFIELD(this.eids.city, { onfchange:typing }, { digits:this.dataSet.city, type:INPUT_NAMES, mandatory:!shortform, placeholder:C_XL.w('city') });
	let country		= new C_iFIELD(this.eids.country, { onfchange:typing }, { digits:this.dataSet.country, type:INPUT_NAMES, mandatory:!shortform, placeholder:C_XL.w('country') });
	...
	//(see *optin*)
	let optin		= new C_eOptin(this.eids.optin, { onswitch:typing});
...
C_eREG.prototype = { 
	...
	display: function(css) {
		...
		//let opswitch = this.controls.optin.display();
		//let optin = '<div class="" style="margin:2em 0 0 0;">'+opswitch+optinlbl+'</div>';
		let optin = '<div>'+this.controls.optin.display()+'</div>';
...
C_eCheckIn.prototype = {
	...
	checkin: function(inlineDataSets, stream) { // e-resa STEP 1: callback from initial email check
		...
		//2023-03-16 security check
		if(stream.slice(0,5)==='error') 
			if(this.callbacks.identified)
				this.callbacks.identified.cb(false,true /*as if you were blacklisted */);
...
C_eVisitor.prototype = {
	...
	filterdapps: function(){
		...
		{	
			...
			{
				//DEV exclude prebooking rdv from displayed list/////////////////////////////////
	...
	appsdisplay: function(options) {
		options = options || {  maydelete:mobminder.context.surfer.eresaCancel };
		let list = '';
		let apps = this.state.dSvisitor.apps;
		this.appointments = new A_ct();
...
C_eProcess.prototype = {
	...
	currentApps: function(options) { // displays the list of appointments
		...
		if(!options.maydelete) 
		{
			...
			msgunder = '<div style="margin: 1em 0; text-align:right;">'+m+'</div>';
		}
		else
		{
			let m = is.tactile? C_XL.w('e- touch to delete') : C_XL.w('e- click to delete');
			msgunder = '<div style="margin: 1em 0 0.5em 0; text-align:right;">'+m+'</div>';
			...
			msgunder+= '<div style="margin: 0.5em 0 1em 0; text-align:right;"><b>'+m2+'</b></div>';
		}
	...
	ispayconiqenabled()
	{
		//console.log("mobminder.account.ePayWebActive="+ePayWebActive);
		//console.log("mobminder.account.visa="+(ePayWebActive&1));
		//console.log("mobminder.account.payco="+(ePayWebActive&2));
		//console.log("mobminder.account.bancontact="+(ePayWebActive&4));
	...
	ispaycardenabled() //computop credit cards
	{
		//TODO
		//mobminder.account.paycardkey = '1234567890';
		//mobminder.account.paycardkey = false;
	...
	slotclick: function(resa) {
		...
		let paymsg=false;
		let transnote='';
		if(confirm) //simple resa without payment
		{
			transnote='';
			paymsg=false;
		...
		{
			transnote = this.getpaydescription(resa);
			if(perfprice==amount)
		...
		//let cardholder = this.controls.visi.state.dSvisitor.firstname+' '+this.controls.visi.state.dSvisitor.lastname;
		//cardholder = cardholder.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toUpperCase().replace(' a-zA-Z0-9','');
		cardholder='';
	...
	newresasavedandpay: function(dataSets) { // SCREEN STEP 5
		...
		if(payment.paymean==C_dS_payment.type.onlinepayco)
		{
			...
			if(!is.small) //TODO switch mobile not
			{
				...
				//console.log("smartphone payconiq url = "+payment.qrcodestring);
			...
			else //smartphone
			{
				...
				//console.log("smartphone payconiq url = "+url);
				...
				//+(isios?"is.ios = true":"is.ios = false")+"---"+navigator.userAgent; 
				
				//some iphone do not like <a> click for launching payconiq deelink
				if(isios)
				{
	...
	openWindowWithPost: function(fullurl) 
	{
		/*if(true)
		{
			//console.log("before color depth="+depth);
			...
			const handler = {
				get(target, prop) {
				  ...
				  const normal = [48, 32, 16, 15, 8, 4, 2, 1];
				  return normal.find((depth) => depth <= target[prop]) || 16;
				}
			  };
			
			if("Proxy" in window) {
				//console.log("windows is proxy");
				window.screen = new Proxy(window.screen, handler);
				let depth = screen.colorDepth;
				//console.log("after color depth="+depth);
			}

			let url = "http://192.168.0.37/testing/test/colordepth.php";
	...
	backhere:function(dataSets, stream) { // periodic payconiqdog callback
		...
		if(this.watchdogcounter>=204) //20250815 bsp correction : 17min * 12 (60s/5s) = 204 (17 min = 5 min for prebooking timeout + 2 (margin)) //see (*pb01*)
		{
	...
	appdeleted: function() {
		...
		this.checklimit({afterdelete:true, maydelete:mobminder.context.surfer.eresaCancel});
	},
	...
	visidataforreturnurl: function(id,inlineDataSets, stream) { // e-resa STEP 1: callback from initial email check
		...
		if(vbs) vlog('e-resa.js','e_MOB','visidataforreturnurl','');
		...
		else
		{
			...
			if(!resa)
			{
				if(vbs) vlog('e-resa.js','e_MOB','visidataforreturnurl','rdv not found (id='+mobminder.app.state.returnResaId+')');
				mobminder.app.state.returnVisitorId=0;
				mobminder.app.state.returnResaId=0;
				this.reload();
