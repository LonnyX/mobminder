...
new Q('update '.$sats.' set fetchlastmonth=fetchthismonth, fetchthismonth=0, today404=0;');
$q = new Q('select sum(fetchlastmonth) as c from '.$sats.';');	
$totalo = $q->c();
...
new Q('update '.$qtable.' set pushlastmonth=pushthismonth, pushthismonth=0;');
$q = new Q('select sum(pushlastmonth) as c from '.$qtable.';');	
$totalq = $q->c();
...
$end_time = time();
$done = 'O '.Date('Y-m-d H:i:s',$end_time).' - last month total outbound: '.$totalo.' - last month total queued: '.$totalq;
