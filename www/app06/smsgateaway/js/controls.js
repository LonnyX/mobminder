...
function C_iMENU(eid, callbacks, options, preset) { // options is { labels:{}, order:{}, presets:{}, count: } 
	...
	//
		if(!options.order) { options.order = []; var i; for(i in options.labels) options.order.push(i); } // then make the order from the labels list (depending on the browser, the creation sequence in labels is OR IS NOT respected)
		if(!options.count) options.count = options.order.length;
		if(!options.presets) options.presets = {};
	for(var x in options.order) { 
			var v = options.order[x];
			var l = options.labels[v]; // note that the same value can come many times in the menu, using different labels
			var p = options.presets[v] || {};
			var morep = { value:v, ui:l, tag:'td', css:'menu-label'};
		var label = new C_iCLIK(eid, { click: new A_cb(this,this.onlabel) }, omerge(p,morep) );
		this.labels.add1(label, x);
		if(label.state.selected) this.state.selected = x;
	}
	this.options = options; 
	
		var title = this.state.title?new C_iCLIK(this.eids.title,{click:new A_cb(this, this.ontitle)},{ui:this.state.title, tag:'div'}):false;
		var table = new C_iTABLE(this.eids.eid, this.labels, {maxrows:preset.maxrows, maxcols:preset.maxcols});
...
C_iMENU.prototype = {
	display: function(css) {
		...
		if(!this.options.count) return ''; // removes also the title when no items to display
				css = css||'';
			var table = this.controls.table.display(this.state.cssclass+' '+css);
			var title = this.controls.title? this.controls.title.display('select-header textcolor-light '+(css||'')) : '';
		
		return title+table;
	},
	activate: function() { 
		this.elements.collect(this.eids);
		var isdisplayed = this.controls.table.isdisplayed(); 
	...
	getelement: function(value) { // used for scrolling to a given element, see (*e01*)
		for(var x in this.options.order) if(this.options.order[x]==value) return this.labels[x].element();
		return false;
	},
	select: function(value, onoff, options) { // options like { reset:true }
		if(options) if(options.reset) {
			for(var x in this.options.order) this.labels[x].select(false);
			this.state.selected = false;
		}
		for(var x in this.options.order) if(this.options.order[x]==value) {
	...
	highlight: function(value, onoff, options) {
		if(options) if(options.reset) for(var x in this.options.order) this.labels[x].highlight(false);
		for(var x in this.options.order) if(this.options.order[x]==value) this.labels[x].highlight(onoff);
	},
	lock: function(values, onoff, options) {
		if(options) if(options.reset) for(var x in this.options.order) this.labels[x].lock(false);
		if(!(values instanceof Array)) values = [values];
		for(var i in values)
			for(var x in this.options.order) 
	...
	show: function(which, onoff) { // which is one value or an array of value like [ v1, v2, v3 ]
		if(!which) for(var x in this.options.order) this.labels[x].hide(!onoff);
			else {
				if(typeof which !== 'object') which = [which];
				for(var y in which) { 
					var v = which[y]; // value to show/hide
					for(var x in this.options.order) 
	...
	subset: function(letters) { // highlights letters if found in labels, and hides all other labels
		...
		var spaced = letters.join('[ \-]*'); // covers the case of pre-fixed names like 'Da Costa' or 'de-Roy'
			spaced = spaced.replace(/\//g,'\\/'); // back slashes for slashes e.g. '/95', '/' must be slashed for the regexp to work (registration field allows this character)
		var regexp = '('+spaced+')(?![^<>]*<\/|[^><]*>)'; // (?![^<>]*<\/|[^><]*>) does not search inside html tags (used e.g. for bullets...)
			regexp = new RegExp(regexp, 'gi'); // g = global search, i = case insensitive
		
		var matchcount = 0;
		for(var x in this.labels.get) 
	...
	skip: function(labelindex) { // when arrow keys are used, this function defines which item are skipped when rotating with the arrows
		var label = this.labels.get[labelindex];
		var skip = label.state.hidden || label.state.section || label.state.selected || (!label.state.enabled);
		return skip;
	},
	next: function() { // highlights the next not hidden item in the list of options
			var current = this.state.highlighted; 
			...
			var s = current+1; s%=len; // keep it in range 
			var round = 0; 
	...
	previous: function() { // highlights the previous not hidden item in the list of options
			...
			var s = current-1; s+=len; s%=len; // keep it in range 
			var round = 0; 
	...
	arrows: function(keycode) {
		switch(keycode) {
			...
			case C_KEY.code.s.tab: 
				...
				var high = this.state.highlighted;
...
function C_iCOMMENT(eid, callbacks, preset) {
	...
	var comment = new C_iFIELD(this.eids.comment, { onfchange:new A_cb(this, this.fchange), onfclear:false}, { digits:this.state.comment, type:this.state.inputype, placeholder:this.state.placeholder });
	this.controls = new A_ct({comment:comment});

}
C_iCOMMENT.defauts = new A_df({ value:0, label:'', comment:'', placeholder:'', locked:false, inputype:INPUT_ALPHANUMSTRICT });
C_iCOMMENT.prototype = {

	// PUBLIC: initialize
	display: function() {
		var label = '<td id="'+this.eids.label+'" class="label" style="white-space:nowrap;">'+this.state.label+'</td>';
		var input = '<td>'+this.controls.comment.display()+'</td>';
		return label+input;
	},
	activate: function() { 
...
function C_iTXTA(eid, callbacks, options, preset) { // options is { labels:{}, order:{}, presets:{}, count: } 
	...
	for(var x in options.order) { 
		var v = options.order[x];
		var l = options.labels[v];
		var c = options.presets[v]||'';
		var item = new C_iCOMMENT(eid, this.callbacks, { value:v, label:l, comment:c});
		this.items.add1(item, x);
	}
	this.options = options; 
	
		var title = this.state.title?new C_iCLIK(this.eids.title,{click:new A_cb(this, this.ontitle)},{ui:this.state.title, tag:'div'}):false;
		var table = new C_iTABLE(this.eids.eid, this.items, {maxrows:preset.maxrows, maxcols:preset.maxcols});
	this.controls = new A_ct({title:title, table:table});	
}
...
C_iTXTA.prototype = {
	...
	lock: function(values, onoff, options) {
		if(options) if(options.reset) for(var x in this.options.order) this.items[x].lock(false);
		if(!(values instanceof Array)) values = [values];
		for(var i in values)
			for(var x in this.options.order) 
...
function C_iTEM(eid, callbacks, c /*correlator*/, label, preset) { 
	...
	this.tip = false; // C_iTIP
			...
			var ruby = '&nbsp;<ruby style="position:absolute; top:20%; left:0.6em;">'+'</ruby>'; // the checker container
		var check = new C_iCLIK(this.eids.check, { down:new A_cb(this, this.cdown), click:new A_cb(this, this.cclick)} , { tag:'td', ui:ruby , css:'cresta-check', style:'position:relative; padding-left:1.6em;' } );
		var label = new C_iCLIK(this.eids.label, { down:new A_cb(this, this.ldown), click:new A_cb(this, this.lclick)} , { tag:'td', ui:label, css:'cresta-label', style:'width:99%;' } );
...
C_iTEM.preset = function(values) { // values like { checked:false, highlight:false, locked:false, tip:false, hidden:false }
	for(var v in values) this[v] = values[v];
...
C_iTEM.prototype = {
	...
	display: function() {
		...
		var label = this.controls.label.caption()?this.controls.label.display():'';
		var check = this.controls.check.display();
			
			var inset = '<table style="width:100%">'+'<tr>'+(this.state.flip?label+check:check+label)+'</tr>'+'</table>';
		return '<div id="'+this.eids.own.wrappy+'" style="white-space:nowrap;">'+inset+'</div>'; // classes added to this are highlight, disabled
	},
	activate: function() { 
	...
	newtip: function(tip) { this.settip(tip); if(!this.elements.wrappy) return this; this.tip.activate(); return this; },
...
C_iCRESTA.prototype = {
	...
	labelled: function(english, css) { // an alternative way to display by returning <td>title</td><td>control</td>
		var control = '<td>'+this.table(css)+'</td>';
		var label = '<td class="label textcolor-light" style="white-space:nowrap; text-align:right; vertical-align:top;">'+C_XL.w(english)+'</td>';
	...
	lock: function(v, onoff) {
		if(!v) 
			for(var x in this.options.order) this.items[x].lock(onoff); // lock every options
		else 
			for(var x in this.options.order) if(this.options.order[x]==v) this.items[x].lock(onoff); // loop is necessary as multiple labels may have the same value
		return this;
	},
	...
	docheck: function(v, onoff, options) { // value is a scalar value OR an array of values like [32, 45, 64]
		...
		if(this.state.mode==-1) { // only one value, set this value
			...
			for(var i in v) {
				verbose.push(v[i]);
				for(var x in this.options.order) if(this.options.order[x]==v[i]) 
					switch(onoff) {
	...
	reset: function(newvalues, options) {
		...
		var vnv = false; // valid new values (check that at least one 'to check' value is part of the options)
		for(var i in newvalues) {
			...
			var verbose = []; for(var i in newvalues) verbose.push(newvalues[i]);
	...
	prevent: function() { // only checked options remain enabled, others are disabled
		...
		for(var x in this.options.order) this.items[x].lock(!this.isckecked(x));
	},
	...
	getpost: function(names) {
		...
		switch(this.state.postmode) {
			case 'bitmap': var bm=0; for(var x in checked) bm|=checked[x]; return bm;
	...
	getvalue: function(v) {
		if(this.state.mode==-1) // only one value is selected, return this value
			for(var x in this.items.get) if(this.isckecked(x)) return this.options.order[x];
			
		if(v!==undefined) { // v is the only value of concern for the caller
			for(var x in this.options.order) if(this.options.order[x]==v) return this.isckecked(x);
			return undefined;
		}
		
		// for all other cases, return an array with checked values
		var checked = new Array(); 
		for(var x in this.items.get) if(this.items[x].state.checked) checked.push(this.options.order[x]);
	...
	setting: function(options) { // returns all setting, like { valueA:1, valueB:0, valueC:1 } or (if options.byorder == true) like { 0:1, 1:0, 2:1 }
		options = options||{};
		var setting = [];
		if(options.byorder) for(var x in this.options.order) setting[x] = this.isckecked(x)?1:0;
		else for(var x in this.options.order) setting[this.options.order[x]] = this.isckecked(x)?1:0;
		return setting;
	},
	hasvalue: function(v) {
	...
	add: function(value, label, optionpreset) { // add one more label
		...
		this.options.count = this.options.order.length;
			var position = this.options.count-1; // adds this new option in last position
	...
	table: function(css) { // displays the checker control part (not the title)
		var skin = this.state.skin ? 'checks' : 'radios';
		var flipped = this.state.flip ? ' flipped' : '';
			css = css || '';
		var table = this.controls.table.display(css+' '+'cresta-table '+skin+flipped);
		return table;
	},
	
	count1: function() { var c = 0; for(var i in this.items.get) if(this.items[i].state.checked) c++; return c; },
	count0: function() { var c = 0; for(var i in this.items.get) if(!this.items[i].state.checked) c++; return c; },
	count2: function() { var c = 0; for(var i in this.items.get) if(this.items[i].state.highlight) c++; return c; },
	countX: function() { var c = 0; for(var i in this.items.get) c++; return c; }, // why not this.options.order.length ?
	
	// private
	countactives: function() { var c = 0; for(var i in this.items.get) if(this.items[i].state.checked && !this.items[i].state.locked) c++; return c; },
	checkall: function(onoff) {	for(var i in this.items.get) this.check(i, onoff); return this; },
	...
	highall: function(onoff) { for(var i in this.items.get) this.items[i].highlight(onoff); return this; },
	...
	additem: function(v, l, x, optionpreset) {
		optionpreset = optionpreset || {}; // like { tip:C_iTIP, onlabel:A_cb, sortrule:function(a,b) { return (labels[a]>labels[b])?1:-1; }; }
			...
			var onlabel = optionpreset.onlabel || new A_cb(this, this.onlabel); // specifying an own event on label overrules the standard behaviour
		if(this.state.flip) optionpreset.flip = true;
		var label = new C_iTEM(this.eids.eid, { onlabel:onlabel, oncheck:new A_cb(this, this.oncheck), onlocked:optionpreset.onlocked }, x, l, optionpreset);
...
function C_iMSG(message, callbacks, preset) { // callbacks like { onChoice } 
		...
		var base = 'msg_layer'+C_iMODAL.layer+'_';
	this.eids = { buttons:'modal_msg_layer_'+C_iMODAL.layer, b0:base+'b0', b1:base+'b1' };
	this.callbacks = callbacks || {};
	this.state = C_iMSG.defauts.align(preset);

	var msgs = {};
	switch(this.state.interactivity) {
		case 'confirm': msgs.b0 = 'cancel'; msgs.b1 = 'confirm'; break;
		case 'yesno': msgs.b0 = 'no'; msgs.b1 = 'yes'; break;
		case 'ok': msgs.b0 = 'ok'; msgs.b1 = 'ok'; break;
	} 
		msgs = C_XL.w(msgs);
	
		var cssclass = 'button button-msg strong'; 
		var bpreset = { tag:'div', css:cssclass, style:'min-width:6em; width:8em; height:3em; line-height:3em; display:inline-block;' }
	var b0 = new C_iCLIK(this.eids.b0, { click:new A_cb(this, this.click, 0) }, omerge(bpreset, { ui:msgs.b0 })  );
	var b1 = new C_iCLIK(this.eids.b1, { click:new A_cb(this, this.click, 1) }, omerge(bpreset, { ui:msgs.b1 })  );
	
	this.controls = new A_ct( {b0:b0, b1:b1} );
	
	// display
	var left = '<td><div class="" style="min-width:80px; min-height:84px;"></div></td>';
	var message = '<div class="modal-msg '+(this.state.css.body||'')+'" style="text-align:center; margin:1em 0;">'+message+'</div>';
	var buttons = '';
	switch(this.state.interactivity) {
		case 'ok': buttons = b1.display(); 
			break;
		case 'confirm':
		case 'yesno': 
			var paddy = '<span style="width:2em;">&nbsp;</span>';
			buttons = b1.display()+paddy+b0.display(); break;
	}
	
	var buttons = '<div style="text-align:center">'+buttons+'</div>';
	var right = '<td>'+message+buttons+'</td>';
	var layout = '<table style="width:100%;" class="'+(this.state.css.image||'')+'"><tr>'+left+right+'</tr></table>';
	
	// activate
	this.modal = new C_iMODAL({body:layout}, { escape:new A_cb(this, this.escape) }, { size:this.state.size, fullscreen:is.newtouch } );
	this.controls.activate('C_iMSG');
}
...
function C_iPOP(eid, callbacks, preset) { // pops a C_modal from a clickable item
	...
	var face = new C_iCLIK(this.eids.ui, { click:new A_cb(this, this.drop), dblclick:this.callbacks.dblclick }, preset  );
...
C_iPOP.prototype = {
	...
	labelled: function(english, css) {
		...
		var range = '<td id="'+this.eids.tds.label+'">'+this.controls.face.display(css)+'</td>';
		var label = '<td id="'+this.eids.tds.face+'" class="label textcolor-light" style="white-space:nowrap; text-align:right;">'+C_XL.w(english)+'</td>';
		return label+range;
	},
	...
	//events
	drop: function() {
		var html = this.callbacks.enter.cb(); // html = { header:'html', body:'html'} fetch this part from instanciator object
		
		if(is.newtouch) { // touch device. We display options on a the full screen
			
			var cartouche = new C_iDQS(this.eids.cartouche, { onquit:new A_cb(this, this.close), onconfirm:new A_cb(this, this.confirmed) }, { remove:false, save:false } );
			
							var c = '<td id="'+this.eids.report+'">'+cartouche.display()+'</td>';
				...
				var row = '<tr>'+c+title+'</tr>';
			header = '<table summary="details" style="width:100%; padding:0 0 1em 0;">'+row+'</table>';
			
					var ovstyle = 'z-index:20; position:absolute; top:2.5em; bottom:0; width:100%; overflow:hidden;';
				var overlay = '<div class="touchdev-overlay" id="'+this.eids.overlay+'" style="'+ovstyle+'">'+header+'</div>';
			$('#body').append(overlay);
			
			this.elements.collect(this.eids);
			
			// display in a droppy that hangs at the input field corner
				var inner = '&nbsp;'; if(html.body) inner = html.body; // when inner is empty, the body div does not repond to the height:100% css rule
...
C_iBUTTON.prototype = {
	display: function(preset) { // preset like {caption:'caption', width:4, height:4, css:'acss', tabindex:2, tip:'tiptext' }
		...
		var css = 'button '+this.state.css;
		var width = this.state.width ? ' width:'+this.state.width+'em;' : '';
		var height = this.state.height ? ' height:'+this.state.height+'em;' : '';
		var hidden = this.state.hidden ? ' display:none;' : '';
		var button = '<input '+tabindex+' id="'+this.eids.button+'" type="button" value="'+this.state.caption+'" style="'+width+height+hidden+'" class="'+css+'"/>';
		return button;
	},
	activate: function() {
		this.elements.collect(this.eids);
...
function C_iONOFF(eid, callbacks, preset) {
	...
	// var cursor = '<div class="cursor fa fa-13x fa-circle"></div>';
		var cursor = '<div class="cursor"></div>';
	var onoff 	= new C_iCLIK(this.eids.onoff, { click:new A_cb(this, this.onswitch) }
		, { enabled:preset.enabled, tip:preset.tip||false, css:'switch', ui:cursor, tag:'div' });

	this.controls = new A_ct({onoff:onoff});
}
C_iONOFF.defaults =  new A_df( { value:1, state:true } );
C_iONOFF.prototype = {
	
	// public
	display: function(css) {
		var onoff = this.controls.onoff.display(css||'');
		return onoff;
	},
	labelled: function(english, css, options) { // css applies to label
			css = css || '';
			options = options || {};
		var range = '<td>'+this.controls.onoff.display()+'</td>';
		var label = '<td class="label textcolor-light '+css+'" style="white-space:nowrap; text-align:right;">'+C_XL.w(english)+'</td>';
		
		if('table' in options) return '<table style="display:inline-block;"><tr>'+label+range+'</tr></table>';
		return label+range;
	},
	activate: function() {
...
function C_iDQS(eid, callbacks, preset) {
	...
	this.state = C_iDQS.defaults.align(preset);
		...
		var removekbshorcut = this.state.removekbshorcut ? [C_KEY.code.s.ctrl+C_KEY.code.s.backspace /*MAC standard*/] : false; 
	
	var remove 	= new C_iCLIK(this.eids.remv, { click:new A_cb(this, this.remove) }
	...
	var quit 	= new C_iCLIK(this.eids.quit, { click:new A_cb(this, this.quit) }
	...
	var save 	= new C_iCLIK(this.eids.save, { click:new A_cb(this, this.save) }
	...
	var confrm 	= new C_iCLIK(this.eids.confrm, { click:new A_cb(this, this.confirmed) }
...
C_iDQS.prototype = {
	...
	display: function() {
		this.controls.reset();
			...
			var remove = this.controls.remove.display();
			var quit = this.controls.quit.display();
			var save = this.controls.save.display();
			var confrm = this.controls.confrm.display();
	...
	enable: function(onOff, which) { // global disabling for all buttons, enabling restores the setting in this.status
		if(which)
			...
			var buttons = this.controls.get;
			if(onOff===undefined) for(var c in buttons) buttons[c].enable(this.state[c]); // enables according to initial state
			else for(var c in buttons) buttons[c].enable(onOff&&this.state[c]); // enables all according to provided onOff
		}
	},
	failuremsg: function() {
	...
	removeChoice: function(v) { // confirmation response
		if(v) if(this.callbacks.ondelete) this.callbacks.ondelete.cb();
		return false;
	},
...
function C_iDDWN(eid, callbacks, options, preset) {
	...
	this.options = options;
		...
		var face = new C_iCLIK(this.eids.face, { down:new A_cb(this, this.facedown), click:new A_cb(this, this.drop), dblclick:this.callbacks.dblclick }, { tip:preset.tip, enabled:preset.enabled, tag:(preset.tag||'input'), css:preset.css, style:preset.style, ui:ui }  );
		var menu = new C_iMENU(this.eids.menu, { onlabel:new A_cb(this, this.onlabel), onarrow:new A_cb(this, this.onarrow) }, options, { value:preset.value, maxcols:preset.maxcols, maxrows:preset.maxrows, title:false, tabselect:true } );
...
C_iDDWN.prototype = {
	...
	labelled: function(english, css) {
		var range = '<td id="'+this.eids.tdlabel+'">'+this.controls.face.display(css||'alpha12')+'</td>';
		var label = '<td id="'+this.eids.tdddwn+'" class="label textcolor-light" style="white-space:nowrap; text-align:right;">'+C_XL.w(english, {cap:true})+'</td>';
		return label+range;
	},
	activate: function() {
		this.controls.activate();
	...
	drop:function() { // why is this not included in the C_iMENU  ?? 
		...
		if(is.newtouch) { // touch device. We display options on a the full screen
			...
			var cartouche = new C_iDQS(this.eids.cartouche, { onquit:new A_cb(this, this.quit)}, { remove:false, save:false } );
	...
	scrollto: function(value) {
		var el = this.controls.menu.getelement(value); // (*e01*)
	...
	onlabel: function(value) {
		this.controls.menu.select(this.state.value=value, true, {reset:true});
			var ui = this.options.labels[this.state.value] || '?';
...
function C_iSPEAK(eid, callbacks, preset) {
	...
	this.state = C_iSPEAK.defaults.align(preset);
		...
		var options = C_iSPEAK.options(preset);
	var ddwn = new C_iDDWN(this.eids.ddwn, callbacks, options, preset );
...
C_iSPEAK.options = function(preset) { // miscellaneous like { exclude:screen to be excluded from list, include:[misc options] }
	...
	var labels = mobminder.language.names; // (*ml*)
	var presets = [];
...
function C_iDDSTACK(eid, leadclass, callbacks, preset) {
	...
	var ddwn = new C_iDDWN(this.eids.ddwn, { onselect:new A_cb(this, this.onddwn), dblclick:new A_cb(this, this.ddwncleared) }, leadclass.options(preset), { value:0, tag:'div', invite:preset.invite, maxrows:0 } );

		var optpreset = {};
		
		// presetting the picker preset options (items of type leadclass)
		var labels = [], order=[], presets = [];
		var onlabel = this.state.onlabelclick ? new A_cb(this, this.onlabel) : false;
		for(var id in this.state.ids) {
			if(this.leadclass.get(id)) {
				labels[id] = this.leadclass.ACoptions.label(id);
				order.push(id);
					var tip = this.leadclass.ACoptions.tip ? this.leadclass.ACoptions.tip(id) : false; // checks first if the function is defined in ACoptions
	...
	var stack = new C_iCRESTA(this.eids.stack, { onchange:new A_cb(this, this.onchecks) }, optpreset, { emptypost:preset.emptypost, skin:1, mode:0, title:false } );	
...
C_iDDSTACK.prototype = {
	...
	display: function(css) {
			css = css || { ddwn:'', stack:'' };
		var ddwn 	= '<tr><td>'+this.controls.ddwn.display('alpha20 textcolor-light like-input '+(css.ddwn||''))+'</td></tr>';
		var stack 	= '<tr><td>'+this.controls.stack.display(css.stack||'')+'</td></tr>';
		var table 	= '<table id="'+this.eids.wrp+'" class="">'+ddwn+stack+'</table>';
		return table;
	},
	labelled: function(english, css) {
		return '';
	...
	more: function(optionsIdArray, unset) {
		for(var x in optionsIdArray) {
			var id = optionsIdArray[x];
	...
	value: function() { 
		var checked = this.controls.stack.getvalue();
		var items = [];
	...
	refresh: function(id) { // label refresh, if this id belongs to the displayed list
		...
		var options = this.leadclass.options();
			var tip = options.presets[id] ? (options.presets[id].tip||false) : false; // checks first if the function is defined in ACoptions
			var label = options.labels[id];
	...
	onlabel: function(x) {
		var id = this.controls.stack.options.order[x];
	...
	onddwn: function(id, options) {
		...
		options = options || {};
		var doptions = this.controls.ddwn.options; // drop options
		if(!(id in doptions.labels)) return; // this id is not present locally in dbAccess
		var tip = doptions.presets[id] ? (doptions.presets[id].tip||false) : false; // checks first if the function is defined in ACoptions
			var check = options.unset?(!options.unset):true;
...
C_iTIP.prototype = {
	...
	tipSwap: function() { // called while you move the mouse pointer
		var x = C_iTIP.status.mouse.x; var y = C_iTIP.status.mouse.y;
...
$(document).ready(function() {
	if(!C_iTIP.elements.outset) {
		var inset = '<div id="'+C_iTIP.eids.inset+'" style=""></div>';
		var outset = '<div id="'+C_iTIP.eids.outset+'" style="pointer-events:none; position:absolute; display:none; z-index:200; max-width:50%;">'+inset+'</div>'; // (*i01*) max-width relates to screen width
		$('#body').append(outset);
...
C_iTIP.handlers = {
	...
	delayed:function(eid) {
		...
		var tip = C_iTIP.register[C_iTIP.status.eids.current];
...
function C_iTAB(eid, callbacks, preset) {
	...
	this.eids = { f:eid+'_f', b:eid+'_b', c:eid+'_c', busy:eid+'_busy' };
		var b = new C_iCLIK(this.eids.b, { click:callbacks.ontab }, { value:preset.value, tag:'div', ui:preset.label, css:'inner' });
...
C_iTAB.prototype = {
	...
	busy:function(onoff) {
		var el = document.getElementById(this.eids.busy);
		if(onoff && !el) {
			var style = 'z-index:20; position:absolute; top:0; left:0; width:100%; height:100%; overflow:hidden;';
			var overlay = '<div class="tab-busy" id="'+this.eids.busy+'" style="'+style+'"></div>';
...
function C_iTABS(eid, options, callbacks, preset) { 	// options like { 0:'tab1', 1:'tab2', 2:'tab3' }
	...
	this.controls = new A_ct();
	this.callbacks = callbacks || {};
	var cb = new A_cb(this, this.ontab);
	for(var v in this.options) {
				var label = this.options[v];
				var tid = eid+'_'+v;
			var b = new C_iTAB(tid, { ontab:cb }, { value:v, label:label });
...
C_iTABS.prototype = {
	...
	display: function(css, options) { // displays the upper control part, which are the tabs themselves
		...
		var ftds = []; 
		for(var v in this.controls.get)
			ftds.push(this.controls.get[v].td_front());
			
			var side = '<td width=30% class="tab-side">&nbsp;</td>'; 
		ftds.unshift(side); ftds.push(side);

		var ftable = '<table width=100% class="tabs '+css+'" style=""><tr>'+ftds.join('')+'</tr></table>';
		return ftable;
	},
	...
	ontab: function(o) { 
		var which = o.value(); // o is the C_iCLIK instance nested in the C_iTAB
		this.swap(which);
...
C_iDRAG.handlers = {
	down: function(e) {
		...
		$(document).mousemove(C_iDRAG.handlers.move).mouseup(C_iDRAG.handlers.anywhere); // this covers any case of dropping above a non droppable area on the screen
			...
			var draggy = C_iDRAG.register[this.id];
		...
		var position = $(this).offset(); // catch the clicked object  position
		...
		var clicked = { width:$(this).width(), height:$(this).height(), inner:this.innerHTML, css:$(this).attr('class') }
	...
	move: function(e) {
		...
		// see if we drifted far enough to call this a drag and drop
			var deltax = C_iDRAG.state.click.mouse.x - e.pageX; 
			var deltay = C_iDRAG.state.click.mouse.y - e.pageY; 
			var draggy = C_iDRAG.register[C_iDRAG.state.click.eid];
			
			var far = deltax*deltax+deltay*deltay; // we do not sqrt() this figure because we compare to 10^2
			var farenough = draggy.state.fartrigger*draggy.state.fartrigger;
		...
		var x = draggy.state.movable.x ? e.pageX-C_iDRAG.state.click.offset.x : C_iDRAG.state.click.target.x;
		var y = draggy.state.movable.y ? e.pageY-C_iDRAG.state.click.offset.y : C_iDRAG.state.click.target.y;
		var offx = draggy.state.image.offx||0; var offy = draggy.state.image.offy||0;
...
C_iDROP.handlers = {
	up: function(e) {
		...
		// catch the context
		var draggy = C_iDRAG.state.dragging;
		var drops = C_iDROP.register[this.id]; 
		var droppy = drops[draggy.state.family];
	...
	enter: function() {
		if(!C_iDRAG.state.dragging) return true; 
		var draggy = C_iDRAG.state.dragging;
		var droppy = C_iDROP.register[this.id][draggy.state.family];
...
//
var ccsstype = { color:80, pattern:81, tag:82 }; // DB values defined in datasets.js
var skin = { color:ccsstype.color, pattern:ccsstype.pattern, tag:ccsstype.tag, acctag:83, rsctag:84, tbxtag:85, wkctag:86, logtag:87, eresa:89 }; // defined in datasets.js
...
function C_iSKIN(eid, callbacks, preset) { // TBD =>  Use C_iCLIK here, No reaction on iPad !!
	...
	this.state = C_iSKIN.defaults.align(preset);
		var css = preset.css||'skin-face alpha12 like-input';
		var poppreset = { tag:'div', ui:'', css:css, } 
	var pop = new C_iPOP(this.eids.ui, { escape:new A_cb(this, this.escape), enter:new A_cb(this, this.enter), popped:new A_cb(this, this.popped) }, poppreset );
	this.controls = new A_ct( {pop:pop} );
	this.handlers = new A_hn();
}
C_iSKIN.defaults = new A_df( { type:skin.color, value:90, allownone:false } );
C_iSKIN.prototype = {
	// public
	...
	activate: function() {
		...
		var css = '', text = '', value = this.state.value, preset = this.state.value;
	...
	enter: function() {  // event handler, ui hit, lets the pad appears
		var options = C_iSKIN.options.display(this.state.type, this.eids.options, this.state.allownone);
		var title = '';
		switch(this.state.type) {
		...
		var header = '<h1 style="text-align:left; padding:.5em;">'+C_XL.w(title)+'</h1>';
	...
	popped: function(modal) {
		this.elements.collect(this.eids);
		var select = this.handlers.add('select', new A_cb(this, this.select));
		var selectedcss;
		switch(this.state.type) {
	...
	select: function(value) { // callback from element click
		...
		switch(this.state.type) {
			...
			case skin.tag: 		this.state.value = value; 
			case skin.acctag: 	this.state.value = value; 
			case skin.rsctag: 	this.state.value = value; 
			case skin.tbxtag: 	this.state.value = value; 
			case skin.logtag: 	this.state.value = value; 
...
[{c:'fa fa-user',v:'1005'}			,{c:'fa fa-link',v:'1015'}			,{c:'fa fa-grimace',v:'1125'}		,{c:'fa fa-tag',v:'1035'}			,{c:'fa fa-credit-card',v:'1045'}		,{c:'fa fa-university',v:'1055'}	,{c:'fa fa-mars-double',v:'1065'}	,{c:'fa fa-fish',v:'1075'}		],
...
[{c:'fa fa-user-tag',v:'4006'}	,{c:'fa fa-brain',v:'4016'}		,{c:'fa fa-chair-office',v:'4026'}		,{c:'fa fa-comments-alt',v:'4036'}		,{c:'fa fa-recycle',v:'4046'}		],
...
logins:[
	...
	//
	//          WHERE						CONDITION							 TOOLS									TIME								ACTIVITY								SYMBOL
[{c:'fa fa-car-side',v:'7000'}		,{c:'fa fa-child',v:'7010'}			,{c:'fa fa-person-booth',v:'7020'}		,{c:'fa fa-hourglass-half',v:'7030'},{c:'fa fa-highlighter',v:'7040'}		,{c:'fa fa-recycle',v:'7050'}				],
[{c:'fa fa-home',v:'7001'}			,{c:'fa fa-baby',v:'7011'}			,{c:'fa fa-hammer',v:'7021'}			,{c:'fa fa-play',v:'7031'}			,{c:'fa fa-coffee',v:'7041'}			,{c:'fa fa-shapes',v:'7051'}				],
[{c:'fa fa-hospital-alt',v:'7002'}	,{c:'fa fa-users',v:'7012'} 		,{c:'fa fa-car-mechanic',v:'7022'}		,{c:'fa fa-stop',v:'7032'}			,{c:'fa fa-chalkboard-teacher',v:'7042'},{c:'fa fa-info',v:'7052'}					],
[{c:'fa fa-hospital',v:'7003'}		,{c:'fa fa-user-tie',v:'7013'}		,{c:'fa fa-medkit',v:'7023'}			,{c:'fa fa-infinity',v:'7033'}		,{c:'fa fa-glass-cheers',v:'7043'}		,{c:'fa fa-exclamation-triangle',v:'7053'}	],
[{c:'fa fa-chair',v:'7004'}			,{c:'fa fa-user-friends',v:'7014'}	,{c:'fa fa-stethoscope ',v:'7024'}		,{c:'fa fa-pause',v:'7034'}			,{c:'fa fa-phone-volume',v:'7044'}		,{c:'fa fa-lightbulb-exclamation',v:'7054'}	],
[{c:'fa fa-person-carry',v:'7005'}	,{c:'fa fa-user-md',v:'7015'}		,{c:'fa fa-balance-scale',v:'7025'}		,{c:'fa fa-stopwatch',v:'7035'}		,{c:'fa fa-lock-open-alt',v:'7045'}		,{c:'fa fa-ban',v:'7055'}					],
[{c:'fa fa-procedures',v:'7006'}	,{c:'fa fa-users-cog',v:'7016'}		,{c:'fa fa-spa',v:'7026'}				,{c:'fa fa-watch ',v:'7036'}		,{c:'fa fa-lock-alt',v:'7046'}			,{c:'fa fa-arrow-square-up',v:'7056'}		],
[{c:'fa fa-warehouse-alt',v:'7007'}	,{c:'fa fa-users-class',v:'7017'}	,{c:'fa fa-video',v:'7027'}				,{c:'fa fa-sync',v:'7037'}			,{c:'fa fa-handshake',v:'7047'}			,{c:'fa fa-at',v:'7057'}					],
[{c:'fa fa-school ',v:'7008'}		,{c:'fa fa-user-clock',v:'7018'}	,{c:'fa fa-laptop',v:'7028'}			,{c:'fa fa-clock',v:'7038'}			,{c:'fa fa-hands',v:'7048'}				,{c:'fa fa-tasks',v:'7058'}					]
]
};
(C_iSKIN.tagsreset = function() { // sets up C_iSKIN.bank from C_iSKIN.csstags
	C_iSKIN.bank[0] = 'tag-none';
	for(var category in C_iSKIN.csstags)
		for(var x in C_iSKIN.csstags[category]) {
			var line = C_iSKIN.csstags[category][x];
			for(var item in line) { var pair = line[item]; C_iSKIN.bank[pair.v] = pair.c; }
		}
})();

C_iSKIN.options = {
	colors: function(eid, allownone) {
		var tds = new Array, table;
		var colspan = 9;
		if(allownone) {
	...
	eskins: function(eid, allownone) {
		var trs = new Array, table;
		trs.push('<th colspan=2 class="header">&nbsp;</th>');
		if(allownone) {	trs.push('<tr><td colspan=2>'+C_XL.w('none')+'<css>none</css><val></val></td></tr>'); }
		var td = function(v) { return '<td>'+C_XL.w(v)	+'<css>'+v+'</css>	<val>'+v+'</val></td>'; } 
		var tr = function(l,r) { return '<tr>'+td(l)+td(r)+'</tr>'; }
		var left = [ 'mobminder', 'business', 'doctors', 'mecas', 	'edoc', 	'doctortie', 'handpeople', 	'stoneleaves', 	'flowers' ];
		var right = [ 'medminder', 'clinics', 'lawyers', 'wellness','emedical', 'stetosblue', 'caduceus', 	'bambstones', 	'flowerpetals' ];
		for(var x in left) trs.push(tr(left[x], right[x])); // like '<td>'+C_XL.w('business')	+'<css>business</css>	<val>business</val></td>'
		table = '<table id="'+eid+'" class="skin-picker e-skins">'+trs.join('')+'</table>';
		return table;
	},
	tags: function(eid, bank, allownone) {
		var trs = new Array;
			var dress = function(cvarray) {
				var l = []; for(var x in cvarray) {
					var css = cvarray[x].c, value = cvarray[x].v;
		...
		for(var x in bank) { var line = bank[x]; trs.push(dress(line));	}
		
		var table = '<table id="'+eid+'" class="skin-picker tags">'+'<tr style="white-space:nowrap;">'+trs.join('</tr><tr style="white-space:nowrap;">')+'</tr>'+'</table>';
		return table;
	},
	display: function(skinmode, eid, allownone) {
		var that = C_iSKIN.options;
		switch(skinmode) {
	...
	activate: function(table, selected /*must be the css value*/, onclick) {
		...
		$(table).find('td').each(
			function(){
				...
				var i = this.innerHTML; // which is the content of the cell as built so far
				if(i.search('<css>')==-1) { // treating here tds having no <css></css><val></val> structure
					...
					$(this).click(function() { return onclick(this.innerHTML) } );
						...
						var css = i.between('<css>','</css>'); $(this).addClass(css);
						var val = i.between('<val>','</val>'); $(this).click(function() { return onclick(val) } );
...
C_iIMG.prototype = {
	...
	setsize: function(size) {
		...
		if(!img) return;
		var s = { w:img.naturalWidth, h:img.naturalHeight }; // original image size in pixels
		var spc = this.state.size.slice(0, -1)|0; // turns 69% into 69
		var ns = { w:s.w*spc/100|0, h:s.h*spc/100|0 } // new image size in pixels
...
function C_iCSS(eid, callbacks, preset) { // preset like { enabled:true, hidden:false, cssclass:class_chat, csstype:color, value:0, span:false }
		preset = preset || {};
	var csstype = preset.csstype, cssclass = preset.cssclass;
	this.eids = { ui:eid+'_ui', checks:eid+'_cks' };
	this.elements = new A_el();
	this.callbacks = callbacks; // like { select:o_callback, escaped:o_callback } 
	var popui = preset.blind ? eid : this.eids.ui; // when used on C_iDAY, pops from a day square like [16]
		var css = 'skin-face alpha12 like-input';
		var poppreset = { tag:'div', ui:'', css:css, blind:preset.blind } 
	var pop; 
	switch(csstype) {
		case ccsstype.color: 
		case ccsstype.pattern: 
			...
			break;
		case ccsstype.tag:
					var precheck = preset.value?preset.value.split('!'):false;
...
C_iCSS.prototype = {
	...
	setCcssClass: function(ccssClass, value) { // changes the set of options according to ccssClass
			var tagspreset=0; if(this.controls.pop instanceof C_iCRESTA) tagspreset = value.split('!');
	...
	setvalue: function(value) {
		...
		if(this.controls.pop instanceof C_iPOP)
			if(value in this.options.labels) {
				this.state.value = value;
				var dS = C_dS_customCss.getByType(this.state.cssclass, this.state.csstype, value); 
...
function C_iGENDER(eid, callbacks, preset) {
	...
	var gender = new C_iDDWN(eid, { onselect:new A_cb(this, this.select) }, { labels:C_XL.w(genders.english) }, { value:this.state.gender, maxcols:1, maxrows:1, mandatory:new A_cb(this, this.valid) } );
...
C_iGENDER.prototype = {
	...
	digits: function(digits) {
		...
		if(!digits) return;
		var post = new C_iPASS({digits:digits});
...
function C_iNOTE(eid, textpreset, preset) {
	...
	this.field = new C_iFIELD(eid, false, { digits:textpreset, type:INPUT_TEXTAREA, placeholder:preset.placeholder, rows:preset.rows, focus:preset.focus, dblclick:false }); 
}
...
var msgmedium = { email:139, sms:200 };
...
function C_iTAGS(eid, callbacks, preset) {
	...
	var captions = C_XL.w(mobtags.fusion); //  mobtags.fusion (*T01*) in language.js, see also C_iTAGS
	var clicks = []; 
	for(var m in captions) {
		var caption = captions[m];
...
C_iTAGS.prototype = {
	display: function(cssTitle, css) {
		...
		var isemail = (this.state.medium==msgmedium.email)
		
		var lis = { templ:[], resa:[], visi:[], att:[] };
		...
		lis.resa.push(this.controls.bookingcode.display());
			var haswk = mobminder.account.has.workcodes||mobminder.account.has.eworkcodes;
		...
		var rowsAll = '<tr>'+rows.join('</tr><tr>')+'</tr>';
...
function C_iAC(eid, leadClass, callbacks, preset) { // in here you select ONLY ONE OPTION by typing letters in an alphanum field
	...
	var digits = this.state.value ? this.ACoptions.label(this.state.value) : '';
		if(preset.digits) digits = preset.digits;
		
		var typing = new A_cb(this, this.typing);
	var ui = new C_iFIELD(this.eids.ui, 
	...
	// trigger: It sets how many digits must be typed before options are displayed
	...
	if(this.state.trigger==0) { // automatic adaptative trigger (default when not ruled by genesis class)
		var c = this.ACoptions.count();
		if(c>=30) this.state.trigger = 1;
		if(c>=90) this.state.trigger = 2;
		if(c>=200) this.state.trigger = 3;
	}
	
	if(preset.buttons) 
		for(var bname in preset.buttons)
			this.buttons.add1(preset.buttons[bname], bname);
}
C_iAC.defauts = new A_df( { 
/* mode */ 	  value:'', button:false, exclude:false, autoclose:false, postlabel:false, css:false, trigger:3, placeholder:''
/* state */ , queried:''
} );
C_iAC.prototype = {
	...
	display: function(css) {	
		this.state.css = css||'';
		var face = this.controls.ui.display(this.state.css);
		var buttons = ''; for(var bname in this.buttons.get) buttons += this.buttons[bname].display();
			
		// var button = this.controls.button ? this.controls.button.display() : '';
		return face+buttons;
	},
	labelled: function(english, css) {
	...
	load: function(digits, options, presets) {  // lets the pad appear
		...
		this.menu = new C_iMENU(this.eids.ops, { onarrow:new A_cb(this, this.onarrow), onlabel:new A_cb(this, this.select) }, this.options, { maxcols:1, tabselect:true } );
	...
	drop: function() { // when used in drop mode (not appropriate for small touch devices)
		...
		var body = this.menu.display();
		// display in a droppy that hangs at the input field corner
			var css = 'pad'; if(is.newtouch) css = 'touchdev';
		this.droppy = new C_iMODAL(
			{ body:body, header:'-' }, // header is intentionnally not empty, that forces the control to display it
			{ escape:new A_cb(this, this.escape) }, 
			{ css:css, position:{ drop:{ui:this.ui.elements.ui}, offset:{x:0,y:5} }, size:{maxy:'18em'} } 
		);
		this.menu.activate(); 
	...
	subset: function() {
		...
		var digits = this.ui.state.digits; // we take the actual ui digits because some users continue on typing to select a firstname further in the list
			digits = digits.replace(/\./gi,'');
		var remains = this.menu.subset(digits); // displays in the list of options only the subset that contains the given digits
		...
		var msg = C_XL.w('no reference for')+' <b class="match">"'+digits+'"</b>'+'&nbsp;[X]';
		var noref = new C_iCLIK(this.eids.noref, { click:new A_cb(this, this.close)}, { tag:'a', ui:msg } );
	...
	changed: function(digits) {  // event handler, any change typed in to the input ui field
		...
		var query = digits.replace(/[ ,\-]/gi,''); 
			...
			// 
		var length = query.length; // takes into account only the number of valid letters, special chars are ignored
		var trigger = this.state.trigger;
		var figspattern = new RegExp(/[0-9]/); var figures = figspattern.test(query); if(figures) trigger+=2;
		if(length<trigger) {
			this.ui.removeClass('loading');
		...
		if(length>=trigger && !this.options) { // if no pad is displayed yet
			...
			if(vbs) vlog('controls.js','C_iAC', 'changed', 'digits:'+digits+', query:'+query+', first fetch');
				var x = this.callbacks.acxtrapass ? this.callbacks.acxtrapass.cb() : undefined;
	...
	escape: function() { // the modal has been closed by the esc or layer click action
		...
		var layerclick = !this.controls.ui.isfocused(); // the modal was escaped by a layer click (alternative is using the Esc keyboard key)
...
function C_iACPICK(eid, leadclass, callbacks, preset) {
	...
	var ac = new C_iAC(this.eids.ac, this.leadclass, { acselect:new A_cb(this, this.acSelect), acclear:new A_cb(this, this.ACcleared), typing:this.callbacks.typing, acxtrapass:this.callbacks.acxtrapass }
		, preset /*{ buttons:preset.buttons, placeholder:preset.placeholder, exclude:preset.exclude, focus:preset.focus }*/ );
	...
	for(var id in this.state.ids) {
		if(this.leadclass.get(id)) {
			...
			order.push(id);
				var tip = ac.ACoptions.tip ? ac.ACoptions.tip(id) : false; // checks first if the function is defined in ACoptions
			presets[id] = { checked:true, tip:tip, onlabel:onlabel }
		}
	}
	this.sortrule = function(a,b) { return (labels[a]>labels[b])?1:-1; }; order.sort(this.sortrule);
	...
	var checks = new C_iCRESTA(this.eids.checks, { onchange:new A_cb(this, this.onchecks) }, optpreset, { emptypost:preset.emptypost, skin:1, mode:0, title:false } );
...
C_iACPICK.prototype = {
	...
	display: function() {
		...
		var ac = this.controls.ac.display(this.controls.ac.ACoptions.css); 
		var checks = this.controls.checks.display();
	...
	value: function() { // read currently checked options
		var checked = this.controls.checks.getvalue();
		var items = [];
		for(var x in checked) {
			var id = checked[x];
			items[id] = this.leadclass.get(id);
	...
	refresh: function(id) { // label refresh, if this id belongs to the displayed list
		...
		if(!this.controls.checks.hasvalue(id)) return; // the given id is not displayed
			var tip = this.controls.ac.ACoptions.tip ? this.controls.ac.ACoptions.tip(id) : false; // checks first if the function is defined in ACoptions
			var label = this.controls.ac.ACoptions.label(id);
	...
	onlabel: function(x) {
		var id = this.controls.checks.options.order[x];
	...
	acSelect: function(id, options) {
		...
		if(!this.controls.checks.hasvalue(id)) { // prevent adding twice the same id in the list
			...
			this.controls.checks.docheck(id, check);
			if(this.callbacks.changed) this.callbacks.changed.cb(this.value()); // passes array like [id1=>leadclassobject, id2=>leadclassobject]
		}
	},
	ACcleared: function() {
...
function C_iAKEYS(eid, callbacks, preset) { 
	...
	this.state = C_iAKEYS.defaults.align(preset);
		...
		var options = C_dS_accesskey.accounts(this.state.loginId);
	var akeys = new C_iMENU(this.eids.akeys, {  }, options, { maxcols:this.state.maxcols, maxrows:this.state.maxrows, title:C_XL.w('accounts') } );
...
C_iAKEYS.prototype = {
	// public
	display: function(css) { 
		var akeys = this.controls.akeys.display();
		return akeys;
	},
	activate: function() {
		this.elements.collect(this.eids);
	...
	audit: function() {
		var d = this.forloggedaccount;
		var count = '<tr><td class="label textcolor-light" style="white-space:nowrap; text-align:right;">'+C_XL.w('akeys count')+'</td><td>'+d.count+'</td></tr>';
		var related = '<tr><td class="label textcolor-light" style="white-space:nowrap; text-align:right;">'+C_XL.w('akey related')+'</td><td>'+d.akey.id+'</td></tr>';
		var account = '<tr><td class="label textcolor-light" style="white-space:nowrap; text-align:right;">'+C_XL.w('account id')+'</td><td>'+mobminder.account.id+'</td></tr>';
		var audit = '<table summary="keys" class="" style="margin-top:1em;">'+count+related+account+'</table>';
		return audit;
	}, 
...
C_iALLOW.prototype = {
	// public
	display: function(css) { 
		var granting = this.controls.granting.display();
		var wrappy = '<div id="'+this.eids.wrappy+'">'+granting+'</div>';
		return wrappy;
	},
	activate: function() {
		this.elements.collect(this.eids);
	...
	getpost: function() {
		...
		// disallowed (they loose their access to this account)
			var disallowed = '-';
			var loggable = C_dS_loggable.get(); // array like loggable[lid] = o_dS_login;
			var currents = []; for(var lid in loggable) if(loggable[lid].accessLevel>=aLevel.operator) currents.push(lid);
			var selected = this.controls.granting.value([aLevel.operator,aLevel.supervisor,aLevel.manager,aLevel.seller,aLevel.admin]);
			for(var x in selected) { var lid = selected[x]; for(var y in currents) if(currents[y]==lid) { currents.splice(y,1); break; }; };
			var remains = currents.length; if(remains) disallowed = currents.join('!');
		
		// granted: they gain an access to this account
			var granted = '-';
			var candidates = this.controls.granting.value([0/*guests*/]);
			var elligibles = []; for(var x in candidates) { 
				var lid = candidates[x]; 
				var akeys = C_dS_accesskey.toaccount(lid, mobminder.account.id).count;
	...
	setupcontrols: function() {
			var granted = {}; // granted[mobminder.context.surfer.id]= {checked:true, locked:true };
			var loggables = C_dS_accesskey.get('config');
		for(var lid in loggables) {
				var keyscount = 0; for(var kid in loggables[lid]) keyscount++;
		...
		var granting = new C_iUSERS(this.eids.granting, { changed:new A_cb(this, this.onchange) }, { title:C_XL.w('access allowance'), orientation:'vertical', guests:true }, granted );
	...
	onchange: function() {
		...
		var admins = this.controls.granting.value([aLevel.seller,aLevel.admin]); // restricts to the concerned access levels
		var guestslogged = this.controls.granting.value([0/*guests*/]); // restricts to the concerned access levels
		for(var x in guestslogged) {
			var lid = guestslogged[x];
...
function C_iUSERS(eid, callbacks, preset, optionspreset) {
	...
	this.state = C_iUSERS.defaults.align(preset);
		...
		var bylevel = new Array();
	for(var l in aLevel.names) { // one list by access level
		if(this.state.hide) { var hidden = arrayINVERT(this.state.hide); if(l in hidden) continue; }
			var options = C_dS_loggable.cresta(l, optionspreset); if(options.count==0) continue;
			var eid = this.eids.picker+'_'+l;
			var title = C_XL.w(aLevel.name(l));
	...
	if(this.state.guests) { // this options lists also logins that are connected on this session, and having no access yet to the logged account (Access tab of account preferences)
			var eid = this.eids.picker+'_g';
			var options = C_dS_loggedIn.cresta(optionspreset, {excludeloggables:true, excludelevels:[aLevel.synchro,aLevel.eresa]} ); 
		if(options.count)
...
C_iUSERS.prototype = {
	...
	display: function(options) { // options  like { titlescss, tablecss, }
		...
		var rows = {rXin:'',rXout:'',r1in:'<tr>',r1out:'</tr>'}; if(this.state.orientation == 'vertical') rows = {rXin:'<tr style="border-top:1em solid transparent;">',rXout:'</tr>',r1in:'',r1out:''};
			for(var l in this.controls.get) 
		...
		var title = ''; if(this.state.title) title = '<h2 style="margin:0 0 1em 0">'+this.state.title+'</h2>';
		return title+logins;
	},
	activate: function() {
		this.elements.collect(this.eids);
	...
	lock: function(v, onoff, alevels) {
		if(alevels) {
			for(var x in alevels) { var al = alevels[x]; if(al in this.controls.get) this.controls.get[al].lock(v, onoff); }
			return;
		}
		for(var l in this.controls.get) this.controls.get[l].lock(v, onoff);
	},
	getpost: function(alevels) {
		var users = '-', collect = this.value(alevels);
	...
	value: function(alevels) { // alevels is an optional array like [aLevel.seller,aLevel.admin]
		...
		if(alevels) {
			var regs = [];
			for(var x in alevels) { var al = alevels[x]; if(al in this.controls.get) regs = regs.concat(this.controls[al].getvalue()); }
			return regs;
		}
		for(var l in this.controls.get) collect = collect.concat(this.controls[l].getvalue());
		return collect;
	},
	check: function(v, onoff, alevels) { // (*c03*)
		...
		for(var l in this.controls.get) this.controls.get[l].docheck(v, onoff);	
	},
	// private
...
function C_iSTAFF(eid, mode, onSelect, optcheck, preset) { // mode can be 'staffing', 'empty', or 'multi' 
	...
	optcheck = optcheck || {}; if(optcheck===true) { optcheck = agClassMatrix(true,true,false); }
		...
		var bcount = C_dS_resource.count(class_bCal);
		var ucount = C_dS_resource.count(class_uCal);
		var fcount = C_dS_resource.count(class_fCal);
		
			var buftotal = bcount+ucount+fcount; 
	...
	var selmode = {
		...
		,'multi':	{ bCal:1, uCal:1, fCal:0 } // used by M_LOGIN for view definition: no staffing number display
	}
	...
	if(this.has.uCals && mode=='staffing') { // then we need a staff size selector
		var o = {labels:[]}, t = C_dS_resource.count(class_uCal); for(var c = 1; c<=t; c++) o.labels[c] = c;
		var s = new C_iCRESTA(this.eids.staffsize, {}, o, { title:C_XL.w('staffing')+'&nbsp;', skin:0, mode:-1, maxrows:12, value:preset.size||1 });
...
C_iSTAFF.prototype = {
	...
	display: function(css) {
		css = css || '';
		var bCal = ''; var uCal = ''; var fCal = '';  var s = ''; 
	...
	reset: function(valuesArray) {
				var verbose = []; for(var i in valuesArray) verbose.push(valuesArray[i]);
	...
	prevent: function() { // items that are not currently selected become disabled (used for e-performance)
		var rtypes = [class_bCal, class_uCal, class_fCal];
		for(var x in rtypes) {
			var c = rtypes[x];
	...
	getpost: function() { 
		...
		var post = { bCals:'', uCals:'' , fCals:'', size:1 };
		var checked = { b: 0, u: 0, f: 0 };
		var count = { b: 0, u: 0, f: 0 };
	...
	valid: function() {
		...
		var count = 0;
		if(this.controls[class_bCal]) { var b = this.controls[class_bCal].getvalue(); count += b.length; }
		if(this.controls[class_uCal]) { var u = this.controls[class_uCal].getvalue(); count += u.length; }
		if(this.controls[class_fCal]) { var f = this.controls[class_fCal].getvalue(); count += f.length; }
		return !!count;
	},
	
	// private
	type: function(id) { return C_dS_resource.get(id).resourceType; },
	more: function(ids) { 
		// e.g when visitor's preferences are loaded, staff preferences are higlighted through this function
		for(var x in ids) {
			var type = this.type(ids[x]);
	...
	remove: function(ids) {
		for(var x in ids) this.controls[this.type(ids[x])].docheck(ids[x], false);
	},
	gaugesize: function() { // reduce the staffing size to the staffing selection
		if(!this.has.size) return this;
		var countSelected = this.controls[class_uCal].getvalue().length;
		var countOptions = this.controls.staffsize.count();
		var sizeSeleted = this.controls.staffsize.getvalue();
		...
		for(var i=(countSelected+1); i<=countOptions; i++) 
...
//
function C_iTRIGGER(eid, callbacks, dS, preset) { // pick out from a list of future dates
	...
	this.eids = { autosend:base+'auto'
				...
				, wrappers: { delivt:base+'_wrp_dt', delay:base+'_wrp_dly', ahead:base+'_wrp_ahd'
							...
							, targetnote:base+'_wrp_trgnote',  triggernote:base+'_wrp_trinote', disablednote:base+'_wrp_disnote'
							} 
				};
	this.dS = dS;
	this.callbacks = callbacks || {};
	...
	var autosend	= new C_iDDWN(this.eids.autosend, { onselect:new A_cb(this, this.onautosend) }, { labels:C_XL.w({ 0:'default disabled', 1:'default enabled' })}, { value:this.dS.sendComms } );			

		var sps = mobminder.account.secondsPerSlice;
	var deliv 	= new C_iCUE(this.eids.deliv, false, { seconds:this.dS.deliveryTime, rangein:(6*3600/sps), rangeout:(21*3600/sps), increment:'hour' } );
		
		var delays = { 0:C_XL.w('no delay'), 5:'5 mn', 10:'10 mn', 15:'15 mn', 20:'20 mn', 30:'30 mn', 45:'45 mn', 60:'60 mn', 90:'90 mn', 120:'120 mn'}
	var delay	= new C_iDDWN(this.eids.delay, { onselect:new A_cb(this, this.ondelay) }, { labels:delays }, { value:this.dS.deliveryDelay, maxcols:1 } );
	
		var aheads = { 0:C_XL.w('on time'), 5:'5 mn', 10:'10 mn', 15:'15 mn', 20:'20 mn', 30:'30 mn', 45:'45 mn', 60:'60 mn', 90:'90 mn', 120:'2 h', 180:'3 h', 240:'4 h', 480:'8 h'}
	var ahead	= new C_iDDWN(this.eids.ahead, { onselect:new A_cb(this, this.onahead) }, { labels:aheads }, { value:this.dS.advance, maxcols:1 } );
	
			var watchoverlabel = function(rsctype) { return C_XL.w('logins watching over')+' '+C_XL.w(rsctype); }
		var targets = []; targets[class_visitor]=C_XL.w('visitors'); targets[class_bCal] = watchoverlabel(class_bCal);
	...
	var target	= new C_iDDWN(this.eids.target, { onselect:new A_cb(this, this.ontarget) }, { labels:targets }, { value:this.dS.target } );
	
	var trigger	= new C_iDDWN(this.eids.trigger, { onselect:new A_cb(this, this.ontrigger) }, mobminder.communication.triggers(), { value:this.dS.triggerId, maxcols:1 } );
	
	var prelist = new C_iONOFF(this.eids.prelist, { onswitch:new A_cb(this, this.onprelist) }, { state:this.dS.presenceList } );

	var factions = new C_iONOFF(this.eids.factions, { onswitch:new A_cb(this, this.onfactions) }, { state:this.dS.filterOnActions } );
	var actions = new C_iACTIONS(this.eids.actions, {}, { actions:this.dS.actions } );
	
	var flogins = new C_iONOFF(this.eids.flogins, { onswitch:new A_cb(this, this.onflogins) }, { state:!!this.dS.filterOnLogins } );
	var logins 	= new C_iUSERS(this.eids.logins, {}, {validation:'none', hide:[aLevel.synchro]}, arrayINVERT(this.dS.logins.split('!')) ); // limit this message to reservations taken by given logins
	
	var fresources = new C_iONOFF(this.eids.fresc, { onswitch:new A_cb(this, this.onfresources) }, { state:!!this.dS.filterOnResources } );
		var precheck = [], ids = arrayINVERT(this.dS.resources.split('!'));
			precheck[class_bCal] = precheck[class_uCal] = precheck[class_fCal] = ids;
	var resources = new C_iSTAFF(this.eids.resources, 'empty', false, precheck, {postmode:'merged', validation:'none' });
...
C_iTRIGGER.prototype = {
	// public
	display: function() {
		var target = '<tr>'+this.controls.target.labelled('delivery target', 'alpha24')+'</tr>';
		var targetnote = '<tr id="'+this.eids.wrappers.targetnote+'"><td></td><td><div class="alpha24 mobtext right pad">'+C_XL.w('logins target note')+'</div></td></tr>';
		var disablednote = '<tr id="'+this.eids.wrappers.disablednote+'"><td></td><td><div class="alpha24 mobtext right pad">'+C_XL.w('com default disabled note')+'</div></td></tr>';
		
		var trigger = '<tr>'+this.controls.trigger.labelled('delivery trigger', 'alpha20')+'</tr>';
		var triggernote = '<tr id="'+this.eids.wrappers.triggernote+'"><td></td><td><div class="alpha24 mobtext right pad">'+C_XL.w('action default note')+'</div></td></tr>';
		
		var autosend = '<tr id="'+this.eids.wrappers.autosend+'">'+this.controls.autosend.labelled('comm autotrigger', 'alpha14')+'</tr>';	
		var deliv = '<tr id="'+this.eids.wrappers.delivt+'">'+this.controls.deliv.labelled('delivery time', 'alphaCUE')+'</tr>';
		var delay = '<tr id="'+this.eids.wrappers.delay+'">'+this.controls.delay.labelled('delivery delay', 'alpha14')+'</tr>';
		var ahead = '<tr id="'+this.eids.wrappers.ahead+'">'+this.controls.ahead.labelled('delivery ahead', 'alpha14')+'</tr>';

		var paddy = '<tr><td colspan=2>&nbsp;</td></tr>';	
		
		var prelist = '<tr id="'+this.eids.wrappers.prelist+'">'+this.controls.prelist.labelled('presence list')+'</tr>';
		var prelistnote = '<tr id="'+this.eids.wrappers.prelistnote+'"><td></td><td><div class="alpha24 mobtext right pad">'+C_XL.w('presence list note')+'</div></td></tr>';	
		
		var factions = '<tr>'+this.controls.factions.labelled('filter on actions')+'</tr>';	
		var actions = '<tr id="'+this.eids.wrappers.actions+'"><td></td><td style="padding-bottom:1em;">'+this.controls.actions.display()+'</td></tr>';
		
		var flogins = '<tr>'+this.controls.flogins.labelled('filter on logins')+'</tr>';	
		var logins = '<tr id="'+this.eids.wrappers.logins+'"><td width=50%></td><td style="padding-bottom:1em;">'+this.controls.logins.display()+'</td></tr>'; 
			
		var fresources = '<tr>'+this.controls.fresources.labelled('filter on resources')+'</tr>';	
		var resources = '<tr id="'+this.eids.wrappers.resources+'"><td width=50%></td><td style="padding-bottom:1em;">'+this.controls.resources.display()+'</td></tr>';
			
		var triggers = '<table summary="triggers" style="">'+target+targetnote+trigger+triggernote+deliv+delay+ahead+autosend+disablednote+paddy+prelist+prelistnote+factions+actions+flogins+logins+fresources+resources+'</table>';
		return triggers;
	},
	activate: function() {
	...
	// private
	dshow: function(e, d) { // delayed element show
		if(!d) d = 600;
	...
	onprelist: function(v) {
		var note = this.elements.prelistnote;
	...
	ontrigger: function(v) {
		...
		var time = this.elements.delivt; 
		var delay = this.elements.delay;
		var ahead = this.elements.ahead;
		var trinote = this.elements.triggernote;
		var prelist = this.elements.prelist;
		 
		if(v==99) { // manual notification
		...
		else if(v==81) { // visitor's birthday
			this.controls.flogins.set(false).enable(false); 
			this.controls.fresources.set(false).enable(false); 
			this.controls.factions.set(false).enable(false); 
		}
		else if(v==80) { // visitor's file record
			this.controls.fresources.set(false).enable(false); 
		}
		else { // any other 
		...
		if(v==0 || v==99 || v==80 || v==90 || v==81) { // any not scheduled mode
		...
		switch(v) {
			case 0: // communication after action
				$(ahead).hide(); this.dshow(delay); 
				var a = this.controls.factions.value(); a = a|0;
				if(a == 0) this.dshow(trinote);
				break;
				
			case 99: // manual communication
				$(trinote).hide(); $(ahead).hide(); $(delay).hide(); break; 
				
			case 80: // on visitor's file record
				$(trinote).hide(); $(ahead).hide(); this.dshow(delay); break; 
				
			case 90: // hour - X reminder
				$(trinote).hide(); $(delay).hide(); this.dshow(ahead); break; 
				
			case 81: // on visitor's birthday
...
C_iTRIGGER.deliveryStatus = function(dS_template, dS_resa, dS_target, enabled) { // applies only for SMS
	...
	// note that in mobminder.sms, targetId is read as resourceId (that should be fixed)
		...
		var time = dS_template.deliveryTime; // is a number of seconds counted from midnight
		var advance = dS_template.advance; // is a number of minutes counted from the reservation cueIn
		var delay = dS_template.deliveryDelay;
		var actions = dS_template.filterOnActions; // { 1:'creation', 2:'modification', 4:'deletion' }
		var triggerId = dS_template.triggerId;
		var visicreation = jsDateFromUnixTime(dS_target.created);
		var visichange = jsDateFromUnixTime(dS_target.changed);
		var resacreation = jsDateFromUnixTime(dS_resa.created);
		var resachange = jsDateFromUnixTime(dS_resa.changed);
		var resadeleted = jsDateFromUnixTime(dS_resa.deleted);
		var resaInDate = dS_resa.jsDateIn.clone({midnight:1});
		var leadclass = (dS_template instanceof C_dS_emailTemplate) ? C_dS_emailTemplate : C_dS_smsTemplate;
		var recipient = leadclass.recepient(dS_target); // depending on the leadclass, is a mobile number or an email address
		var now = new Date(); var yesterday = now.clone().increment({d:-1, midnight:1});
	
			if(enabled===undefined) enabled = dS_template.isEnabled(dS_resa.id, dS_target.id);
	...
	var status = C_dS_sms.status.sms_nosms; 
	...
	var planned = false; // now we define a planned date for the message
	switch(triggerId) {
		case 0: // communication after action  ( creation, change or delete )
		...
		case 99: // manual communication ( action triggered )
			 planned = C_XL.w('on activation');
			break; 
			
		case 80: // on visitor's file record ( action triggered )
			break; 
			
		case 81: // on visitor's birthday (deliveryTime rules the delivery time on that visitor's birthday)
			planned = false; // TBD
			break;
			
		case 90: // hour - X reminder ( advance rules the number of minutes ahead of cueIn )
	...
	if(dS_target) { // then the target is known, we need to check if the MSG was actually sent already
		...
		if(msg) { // the message was sent
			...
			// we need to forecast the message, because it was not really compiled yet
		...
		if(sent) 
			...
			if(status == C_dS_sms.status.sms_nomobile) { 
				...
				case 0: // immediate sending at reservation creation time
					sent = C_XL.w('not yet sent');
					break;
				case 81: // on visitor's birthday
					sent = C_XL.w('should be actioned'); // TO BE DONE
					break;
				case 99: // notification_manual (*)
				case 80: // on visitor's file record
	...
	return { title:title, message:message, planned:planned, sent:sent, status:status, statustring:statustring, statustime:statustime };
 }
...
function C_iCOMM(eidPost, dS_reservation, leadClass, onMsg) { // one instance of this ctrl by msg_medium
	...
	this.leadClass = leadClass; // leadClass is C_dS_smsTemplate or C_dS_emailTemplate
	this.msgMedium = leadClass.msgMedium;
	this.msgClass = leadClass.childs; // will be C_dS_sms or C_dS_email
...
C_iCOMM.prototype = {
	...
	any: function() { // tells if any communicatin of the given leadClass exists
		var c=0; for(var tc in this.templates) for(var templId in this.templates[tc]) c++;
		return c; 
	},
	display: function(css, attendees) {
		...
		var b = this.picker(class_bCal, css);
		var u = this.picker(class_uCal, css);
		var f = this.picker(class_fCal, css);
		var v = this.picker(class_visitor, css);
		return v+b+u+f;
	},
	activate: function() {
		this.elements.collect(this.eids);
	...
	more: function(ids, action) {
		for(var x in ids) this.controls[this.resctype(ids[x])].set(ids[x], action || true);
	},
	...
	getpost: function() {
		...
		for(var templId in this.controls.get) { // for each template that was displayed
				var setting = this.controls.get[templId].getvalue(); // checked resource ids like [1201180, 1201190]
				var o_dS_template = this.usedTemplates[templId];
				var tc = o_dS_template.target; 
				var autosend = o_dS_template.sendComms; // is 1 if the template sends comm by default, 0 if disabled by default
			var toggled = autosend ? arrayREMOVE(this.addresseesIds[tc],setting) : setting; // we store only toggled communications
			for(var x in toggled) {
				var rscId = toggled[x];
				var bundle = templId+'-'+rscId; // this bundle is like "2502-1201180" is unleashed here (*bu1*), 
				bundles.push(bundle); 
			}
		}
	...
	displayMSG: function(templId, addresseeId) { // triggered when you click a message status
			...
			var dS_template = this.usedTemplates[templId];
			var dS_target = this.addressees[dS_template.target][addresseeId];
			var enabled = this.controls[templId].setting()[addresseeId];

		var d = C_iTRIGGER.deliveryStatus(dS_template, this.resa, dS_target, enabled); // delivery is like { title: , message: , planned: , sent: , status: , statustime:  }
		...
		var title = '', route = '';
		var labeltd = '<td class="label textcolor-light">';
		var tr = '<tr style="vertical-align:top;">';
		if(this.msgMedium == msgmedium.email) 
		...
		paddy 		= tr+labeltd+'&nbsp;</td>'+'<td style="text-align:left;">&nbsp;</td></tr>';
			...
			var table = '<table>'+title+message+paddy+planned+senton+route+status+'</table>';
			
		new C_iMSG(table, false, { css:{image:this.msgClass.getImgCss() }, size:{x:500,y:''} } );
	},
	
	// private
	setaddressees: function(attendees) { // set up the list of adressees based on list of attendees = array[class][x] = id
		...
		for(var tc in attendees) { // for each different class of adressee
			this.addressees[tc] = [];
			this.addresseesIds[tc] = [];
			switch(tc|0) {
				case class_visitor: 
					var visitors = attendees[tc];
				...
				default: // that is bCal, uCal and fCal: in those cases, addressees are the 'watching over logins'
					var resources = attendees[tc];
					for(var x in resources) {
						var rid = resources[x]; // is the appointment attendeee, an account resource
						var watchovers = C_dS_loggable.getWatchovers(rid); // are the logins defined to get notifications for this resource
						for(var lid in watchovers) {
							var dS_login = watchovers[lid];
		...
		for(var tc in this.addresseesIds) { // for each different class of adressee (tc can be class_visitor, class_bCal, class_uCal, class_fCal)
			
			this.commPreset[tc] = [];
			for(var templId in this.templates[tc]) { // for each template
				this.commPreset[tc][templId] = [];
				var dS_template = this.templates[tc][templId];
				var autosend = dS_template.sendComms;
				
				for(var x in this.addresseesIds[tc]) { // for each attendee of the concerned target class (target class is defined by the template)
					
					var id = this.addresseesIds[tc][x];
					var onitem = new A_cb(this, this.onslabel, {templId:templId, addrId:id});
					var preset = { checked:autosend, locked:false, onlocked:onitem, onlabel:onitem };
					
					if(this.resa.id >0) { // existing resa, been saved with possibly some disabled communication
						preset.checked = dS_template.isEnabled(this.resa.id, id); // was disabled and saved in the past
					} else 
						{ // new resa, keep default comm sending in consideration
						var p = true;
						for(var x in attendees[class_bCal]) { // only bCals have the setting that disables the comm by default, keep in mind that only one bCal can be invited on an appointment
							var rid = attendees[class_bCal][x];
							var dS_rsc = C_dS_resource.get(rid);
						...
						if(autosend==0) preset.checked = false; // if the template has disabled comm by default, the preset goes to "not set"
					}
					if(this.msgClass.get(this.resa.id, id, templId)) { preset.locked = true; }; // was sent, not possible to choose on scheduling anymore
					
					this.commPreset[tc][templId][id] = preset;
				}
			}
		}
		...
		for(var tc in this.addresseesIds) { // for each different class of adressee
			this.commStatus[tc] = [];
			
			for(var templId in this.templates[tc]) { // many templates may define the same given target class
				this.commStatus[tc][templId] = [];
				var dS_template = this.templates[tc][templId]; 
				switch(tc|0) {
					case class_visitor:
						for(var x in this.addresseesIds[tc]) {	// many visitors might be addressees	
							var vid = this.addresseesIds[tc][x];
							var dS_target = C_dS_visitor.get(vid); // that is a dS_visitor
							var preset = this.commPreset[tc][templId][vid];
							this.commStatus[tc][templId][vid] = C_iTRIGGER.deliveryStatus(dS_template, this.resa, dS_target, preset.checked);
						} break;
					default: // that is bCal, uCal and fCal: in those cases, addressees are the 'watching over logins'
						for(var x in this.addresseesIds[tc]) {	// many logins might be addressees
							var lid = this.addresseesIds[tc][x];
							var dS_target = C_dS_loggable.get(lid); // if dS_target is undefined, you might have read an accesskey while the dS_login is not available in the config query (this has been fixed in the query/config by not sending accesskeys of higher accessLevel than the logged in level)
							var preset = this.commPreset[tc][templId][lid];
	...
	picker: function(tc, css) {
		...
		// display names of attendees in the first left column
			var labels = [], addressees = this.addressees[tc];
			switch(tc|0) {
				case class_visitor: for(var id in addressees) labels[id] = addressees[id].vname(); break;
				default: for(var id in addressees) labels[id] = addressees[id].lname();
			}
		var o_control = new C_iMENU(this.eids.target[tc], {}, { labels:labels }, { title:C_XL.w(tc) } );
		tds.push(o_control.display()); // this control is left not activated with intention
		
		for(var templId in this.templates[tc]) { 
			var dS_template = this.templates[tc][templId]; 
			...
			if(dS_template.resources) { // we do this only if the template defines restrictions on certain resources
					var ronly = dS_template.resources.split('!'); for(var x in ronly) ronly[x] = ronly[x]|0;
					var byclass = this.attendees; var byid = new Array(); 
				for(var ac in byclass) if(ac == class_visitor) continue; else for(var x in byclass[ac]) byid.push(byclass[ac][x]); // all attendees in a flat array
				var applies = false; for(var k in ronly) for(var x in byid) if(byid[x]==ronly[k]) { applies = true; break; } // find a match of resa attendees in temaplate restricted resources
				...
				var onpicker = new A_cb(this, this.onpicker, templId);
			this.usedTemplates[dS_template.id] = dS_template; // keep this for getpost function operation
				var title = dS_template.tname();
				var eid = this.eids.target[tc]+'_'+dS_template.id;
				var labels = []; for(var id in this.commStatus[tc][templId]) labels[id] = this.commStatus[tc][templId][id].statustring;
				var options = { labels:labels, presets:this.commPreset[tc][templId] };
			var o_control = new C_iCRESTA(eid, { onchange:onpicker }, options, { title:title, mode:0 });
			this.controls.add1(o_control, dS_template.id);
			tds.push(o_control.display());
		}
		var td = '<td style="padding-right:2em;">';
	...
	onpicker: function(templId, selections, spot, hitvalue) { // the user wants to de-activate or activate a message
		...
		var ctrl = this.controls[templId];
		var addrId = hitvalue; // addressee id, is undefined if you clicked the control title
		var dS_template = this.usedTemplates[templId];
		
		if(!addrId) { // control header was clicked, we need to execute this function for each possible addressee
			
			var targets = this.addressees[dS_template.target];
			for(var tid in targets) this.onpicker(templId, selections, spot, tid); 
			return;
		}
		
		var enabled = ctrl.setting()[hitvalue];		
		if(vbs) vlog('controls.js', 'C_iCOMM', 'onpicker', 'templId:'+templId+', addrId:'+addrId+', enabled:'+enabled);
				
				var dS_target = this.addressees[dS_template.target][addrId];
			var delivery = C_iTRIGGER.deliveryStatus(dS_template, this.resa, dS_target, enabled);
			var label = delivery.statustring; // display the real status
...
C_iCOMM.smsSema = function(css) { // displays SMS status legend
	...
	switch(css) {
		...
		var sema = '<div style="line-height:1.2em; '+c+'" class="sms-sema '+css+' fa '+s+'"></div>';
	// var div = '<div style="line-height:1.2em; position:relative;" class="sms-status">'+sema+'</div>';
	return sema;
}
C_iCOMM.legend = function() { // displays SMS status legend
	var trs = new Array();
	
	var table = '<table>'+trs.join('')+'</table>';
	return table;
}
...
function C_iDUR(eid, callbacks, preset) { // preset = { duration:slices }
	...
	var options = { radio:{ labels:{}, presets:{} }, drop:{} };
	
	for(var s=mobminder.account.durationShortest; s<=mobminder.account.durationLongest; s+=mobminder.account.durationSteps) options.radio.labels[s] = this.slice(s);
	for(var t=s; s<=(mobminder.account.durationLongest*3); s+=mobminder.account.durationSteps) options.drop[s] = this.slice(s);
	
	var presets = { radio:new Array() };
	options.radio.presets[this.state.slice] = { checked:true }; // specifies which radio to set at init time
	
	
	var drop = new C_iDDWN(this.eids.drop, { onselect:new A_cb(this, this.drop) }, {labels:options.drop}, {tag:'span', value:t, maxrows:0, nested:true } );
	options.radio.labels['drop'] = drop.display('dur-more')+' +'; // add the drop ui as last option of the radio picker
	
	var radio = new C_iCRESTA(this.eids.radio, { onchange:new A_cb(this, this.radio) }, options.radio, { title:C_XL.w('duration'), skin:0, mode:-1 } );
...
C_iDUR.prototype = {
	...
	display: function(css) {
		var radio = this.controls.radio.display(); 
	...
	// private
	slice: function(s) {
		var seconds = (mobminder.account.secondsPerSlice*s)|0;
...
function C_iDETAILS(eid, callbacks, preset) { // see (*dt*)
	...
	var is = { v:this.state.mode==planning.vertical, h:this.state.mode==planning.horizontal, t:this.state.mode==planning.text }
	var onchange = new A_cb(this, this.changed);
	var order, labels, presets, options;
	var sortrule = function(a,b) { return (labels[a]>labels[b])?1:-1; }; // alphabetical order
	var preset = C_dS_details.get(this.state.mode, this.state.rsctype).details; 
	...
	// options for resa attributes
		...
		for(var x in labels) order.push(x);
		for(var bit in labels) if(preset&bit) presets[bit] = { checked:true };
			options = { order:order.sort(sortrule), labels:labels, presets:presets, count:order.length };
		var mresa = new C_iCRESTA(this.eids.mresa, {onchange:onchange}, options, { mode:0, title:C_XL.w('reservation'), maxcols:3, maxrows:10 });
	...
	// options for visitor attributes
		...
		for(var bit in labels) if(preset&bit) presets[bit] = { checked:true };
			options = { order:order.sort(sortrule), labels:labels, presets:presets, count:order.length };
		var mvisi = new C_iCRESTA(this.eids.mvisi, {onchange:onchange}, options, { mode:0, title:C_XL.w('visitor'), maxcols:3, maxrows:10 });
	...
	// options for the display mode
		...
		for(var bit in labels) if(preset&bit) presets[bit] = { checked:true };
			options = { order:order.sort(sortrule), labels:labels, presets:presets, count:order.length };
		var mmisc = new C_iCRESTA(this.eids.mmisc, {onchange:onchange}, options, { mode:0, title:C_XL.w('miscellaneous'), maxcols:3, maxrows:10 });
...
C_iDETAILS.prototype = {
	// public
	display: function(css) { 
		var mresa = '<td>'+this.controls.mresa.display()+'</td>';
		var mvisi = '<td>'+this.controls.mvisi.display()+'</td>';
		var mmisc = '<td>'+this.controls.mmisc.display()+'</td>';
		
		var t = '<table><tr style="vertical-align:top;">'+mresa+mvisi+mmisc+'</tr></table>';
		return t;
	},
	activate: function() {
		this.elements.collect(this.eids);
	...
	getpost: function() {
		var checked, encoded = 0;
			checked = this.controls.mresa.getvalue(); for(var x in checked) encoded += checked[x]|0;
			checked = this.controls.mvisi.getvalue(); for(var x in checked) encoded += checked[x]|0;
			checked = this.controls.mmisc.getvalue(); for(var x in checked) encoded += checked[x]|0;
		return encoded;
	},	
	
	// feedbacks
	changed: function() {
		var encoded = this.getpost();
...
function C_iTDONE(eid, callbacks, preset) {
	...
	var progr = new C_iCRESTA(this.eids.progr, { onchange:new A_cb(this, this.progr) }, { order:[0,1], labels:C_XL.w({ 0:'in progress', 1:'archived on' }), count:2 }, { skin:0, mode:-1, value:this.state.status } );
	var dpOut = new C_iDP(this.eids.dpOut, {dayclick:new A_cb(this, this.dpOut)}, { stmp:this.state.midnOut, display:{abreviation:'full', weekday:true} } );
	var f4all = new C_iCRESTA(this.eids.f4all, { onchange:new A_cb(this, this.f4all) }, { order:[1], labels:{1:C_XL.w('4all assignees')}, presets:{}, count:1 }, { mode:0 } );
...
C_iTDONE.prototype = {
	...
	display: function(css) {
		var progr = '<td style="width:10em; padding:0 1em;">'+this.controls.progr.display()+'</td>';
		var dpOut = '<td style="width:26em; text-align:left;">'+this.controls.dpOut.display('bold alpha20')+'</td>';
		var f4all = '<td style="width:15em; text-align:left;">'+this.controls.f4all.display()+'</td>';
		var table = '<table style=""><tr style="vertical-align:bottom">'+progr+dpOut+f4all+'</tr></table>';
		return table;
	},
	activate: function() {
		this.elements.collect(this.eids);
	...
	getpost: function() { 
		...
		var f4all = this.controls.f4all.getvalue(1); f4all = f4all ? 1 : 0;
...
function C_iPLUS(eid, callbacks, preset) {
	...
	var plus = new C_iBUTTON.standard(this.eids.plus, new A_cb(this, this.plus), 'plus' );
...
C_iPLUS.prototype = {
	// public
	display: function(css) {
		var table = '<table class="plus '+(css||'')+'" id="'+this.eids.table+'">'+'</table>';
		return table;
	},
	labelled: function(english, css) {
	...
	prefixed: function(prefix, css) {
		return '<td class="label textcolor-light" style="white-space:nowrap; text-align:right;">'+prefix+'</td><td>'+this.display(css)+'</td>';
	},
	activate: function() {
		this.elements.collect(this.eids);
		this.refresh();
	...
	refresh: function() {
		var trs = new Array();
		var pluslist = this.state.plusclass.list();
		this.order = pluslist.order;
		for(var x in this.order) {
			var id = this.order[x], number = (x|0)+1;
			var label = pluslist.labels[id];
			var eid = this.eids.click+this.state.plusclass.eid+'_'+x;
			var click = new C_iCLIK(eid, { down:new A_cb(this, this.down), click:new A_cb(this, this.item, id) } , { tag:'td', ui:label } );
			this.controls.add1(click, x);
			trs.push('<tr><td style="text-align:right;">'+number+'.</td>'+click.display()+'</tr>'); // starts by listing all options, each on ONE row
		}
	...
	item: function(id) {
		...
		var item = this.state.plusclass.get(id);
	...
	plus: function() {
		...
		var newitem = this.state.plusclass.plus();
	...
	enter: function(draggy, droppy) {
		...
		if(!draggy.state.hand) { // first droppy we are dragging over
			...
			var t = draggy.elements.draggy.innerHTML;
			draggy.elements.draggy.innerHTML = droppy.elements.droppy.innerHTML;
			droppy.elements.droppy.innerHTML = t;
			
			var a = draggy.state.value, b = droppy.state.value, k = this.order[a]; this.order[a] = this.order[b]; this.order[b] = k;
			...
			var t = draggy.state.hand.elements.droppy.innerHTML;
			draggy.state.hand.elements.droppy.innerHTML = droppy.elements.droppy.innerHTML;
			droppy.elements.droppy.innerHTML = t;
			
			var a = draggy.state.hand.state.value, b = droppy.state.value, k = this.order[a]; this.order[a] = this.order[b]; this.order[b] = k;
		}
	...
	drop: function(draggy, droppy) {
		...
		// var neworder = this.state.plusclass.neworder(this.order);
...
function C_iPERM(eid, callbacks, preset) {
	...
	var onchange = new A_cb(this, this.changed);
		...
		var usage = parseInt('00000FFFF', 16);
		var setup = parseInt('FFFF00000', 16);
		
			var owned = mobminder.context.surfer.permissions; // a given surfer may not grant persmissions he does not own
			var preset = this.state.permissions;
				if(mobminder.context.surfer.accessLevel==aLevel.admin) owned = usage|setup; // admins do see all permissions
				if(this.state.forced) preset = usage|setup; // admins are forced to keep all permissions
		usage = new C_iCRESTA(this.eids.usage, {onchange:onchange}, permissions.options(owned&usage, preset), { mode:0, title:C_XL.w('reg usage'), locked:this.state.forced });
		setup = new C_iCRESTA(this.eids.setup, {onchange:onchange}, permissions.options(owned&setup, preset), { mode:0, title:C_XL.w('setup'), locked:this.state.forced });
...
C_iPERM.prototype = {
	// public
	display: function(css) { 
		var usage = '<td>'+this.controls.usage.display()+'</td>';
		var setup = '<td>'+this.controls.setup.display()+'</td>';
		
		var t = '<table><tr style="vertical-align:top;">'+usage+setup+'</tr></table>';
		return t;
	},
	activate: function() {
		this.elements.collect(this.eids);
	...
	getpost: function() {
		var checked, encoded = 0;
			checked = this.controls.usage.getvalue(); for(var x in checked) encoded += checked[x]|0;
			checked = this.controls.setup.getvalue(); for(var x in checked) encoded += checked[x]|0;
		return encoded;
	},	
...
C_iARROW.prototype = {
	// public
	display: function(values, types, rowcss, order) { // types[member] = "A" (for text alignment) or 0 (for figure alignment), order[index] = member, values[member] = the value to display
		var tds = new Array();
		for(var m in values) {
			var csstype = types[m]?'text':'figure';
			tds.push('<td class="'+csstype+' '+m+'">'+values[m]+'</td>'); 
		}
	...
	enable: function(onOff) { // can be called before or after activation
		...
		if(is.tactile) { 
			...
			if(this.state.enabled) {
				var handlers = new A_hn({tstart:new A_cb(this, this.tstart), tend:new A_cb(this, this.tend)});
			...
			if(this.state.enabled) {
				var handlers = new A_hn({down:new A_cb(this, this.down), up:new A_cb(this, this.up), enter:new A_cb(this, this.enter), leave:new A_cb(this, this.leave)});
...
function C_iARRAY(eid, catalyst, callbacks, preset) { 
	...
	this.eids = { wrapper:eid+'_wrp', headers:eid+'_hd', rows:eid+'_row', plus:eid+'_plus', xport:eid+'_xport', busy:eid+'_busy' };
	
	this.callbacks = callbacks || {}; // like { onrow: }
	this.elements = new A_el();
	this.semicols = {}; // for eids of semicolumns
	this.catalyst = catalyst;
	this.headers = this.catalyst.headers();
	
	var xport = new C_iCLIK(this.eids.xport, { click:new A_cb(this, this.xport) }, { ui:C_XL.w('export iArray') } );	
	this.controls = new A_ct({xport:xport});
	this.heads = new A_ct({});
...
C_iARRAY.prototype = {
	...
	display: function(c) {
			var xport = '', endoflist = '';
			if(this.state.xport) xport = '<div style="margin:1em 0; text-align:center;">'+this.controls.xport.display()+'</div>';
	...
	busy:function(onoff) {
		...
		if(onoff && !el) {
			var style = 'z-index:20; position:absolute; top:0; left:0; width:100%; height:100%; overflow:hidden;';
			var overlay = '<div class="array-busy" id="'+this.eids.busy+'" style="'+style+'"></div>';
			$(this.elements.wrapper).append(overlay);
		} else 
			if(onoff==false && el) $(el).remove();
		return this;
	},
	header: function(onidx) { var optidx = this.catalyst.on[onidx];	return this.headers[optidx];	},
	headline: function(options) { // options like {file:true}
		...
		if(options.file) {
			for(var x in this.catalyst.on) ths.push(''+this.header(x)+';');
			trs.push(''+ths.join('')+newline);
			return trs;
		}
		
		this.heads = new A_ct({});
			var cols = new Array(), eids = new Array(), dds = new Array();
			
			var drop = function(eid, right) {
				var floaty = '<div id="'+eid+'" style="width:2em; height:100%; float:right;"></div>';
				var absolute = '<div style="position:absolute; bottom:0px; height:100%; width:1em; '+(right||'')+' border:none;">'+floaty+'</div>';
				return absolute;
			}
			var last = this.catalyst.on.length-1;
			this.round^=1;
		for(var x in this.catalyst.on) {
			var csssorting = '', sortorder = '', member = this.catalyst.on[x]; 
			...
			var eid = this.eids.headers+'_'+x+'_'+this.round;
			var callbacks = { click:new A_cb(this, this.onheader, x), hold:new A_cb(this, this.viewset, x), down:new A_cb(this, this.downheader) };
			var iclik = new C_iCLIK(eid, callbacks, { ui:(this.header(x)+sortorder), tag:'th', css:csssorting } );
			
					var eidL = eid+'_L', eidR = eid+'_R';
				// ths.push('<th id="'+eid+'" class="'+csssorting+'">'+iclik.display()+'</th>');
				ths.push(iclik.display());
					var drops = drop(eidR); if(x==last) drops+=drop(eidL,'right:-1em;');
	...
	refresh: function() {
		...
		// headers
		var trs = this.headline();
var perf = false; if(perf) perf = new microperf('iArray');
		// values display
		var controws = new A_ct({});
		var ids = this.catalyst.sort(this.state.ids); // sorts according to catalyst options, this is no sort on id
		var types = this.catalyst.types();
		var order = this.catalyst.on;
if(perf) perf.cue('done with sorting');

		for(var x in ids) {
				var values = this.catalyst.dSui(ids[x]); // is an array like [ 0:val1, 1:val2, 2:val3, 3:val4, ...]
				var row = new C_iARROW(this.eids.rows+'_'+ids[x], { click:new A_cb(this, this.onrow) }, { selected:false, correlator:ids[x] } );
				var rowcss = this.catalyst.dScss?this.catalyst.dScss(ids[x]):undefined;
			trs.push(row.display(values, types, rowcss, order));
...
if(perf) perf.cue('done with creating rows');
		...
		if(this.state.sum||this.state.max) 
			if(this.catalyst.sum) {
				var sumandmax = this.catalyst.sum(ids);
				if(this.state.max) {
					var tds = [], values = sumandmax.max;
					for(var m in values) tds.push('<td class="max">'+values[m]+'</td>'); 
					trs.unshift('<tr>'+tds.join('')+'</tr>');
				}
				if(this.state.sum) {
					var tds = [], values = sumandmax.sum;
					for(var m in values) tds.push('<td class="sum">'+values[m]+'</td>'); 
					trs.unshift('<tr>'+tds.join('')+'</tr>');
				}
			}
...
if(perf) perf.cue('done with summing');
		...
		if(this.state.count) 
			if(this.catalyst.count) {
				var count = this.catalyst.count(ids);
				var tds = [], values = count.counters;
				for(var m in values) tds.push('<td class="count">'+values[m]+'</td>');
				trs.unshift('<tr class="counters">'+tds.join('')+'</tr>');
			}
if(perf) perf.cue('done with counting');
		
		var table = '<table style="position:relative; width:'+this.state.width+';">'+trs.join('')+'</table>';
...
if(perf) perf.cue('done with displaying');
		...
		var family = this.eids.headers;
		var elems = new A_el(this.semicols);
		for(var x in elems.get) { x = x|0;
			var CLRels = elems[x];
...
if(perf) perf.report('C_iARRAY::refresh()');
	...
	file: function() {
		...
		// headers
		var trs = this.headline({file:true});
		
		// values display
		var ids = this.catalyst.sort(this.state.ids), types = this.catalyst.types();
		for(var x in ids) {
				var values = this.catalyst.dSui(ids[x], {file:true});
				var tds = new Array();
			for(var m in values) {
				var quotes = types[m]?'"':'';
				tds.push(quotes+values[m]+quotes+';'); // string types are surrounded by double quotes for export in .csv files
			}	
			trs.push(''+tds.join('')+crlf);
		}
		...
		if(this.state.sum||this.state.max) 
			if(this.catalyst.sum) {
				...
				if(this.state.max) {
					var tds = [], values = sumandmax.max;
					for(var m in values) tds.push(''+values[m]+';'); 
	...
	onheader: function(idx) { // set the order to happen on this header, or change the order direction
		...
		if(this.state.ids.length > 999) { this.busy(true); var that = this; setTimeout(function() { return that.refresh() },100); }
			else this.refresh();
		return true;
	},
	downheader: function() {
	...
	viewset: function(idx) { // opens a modal for fields selection
	...
	xport: function() {
		...
		var uri = 'data:text/xlsx;charset=utf-8,'+encodeURIComponent(this.file());
		var el = document.createElement('a'); el.href = uri; el.download = C_XL.w(this.catalyst.title)+'.csv';
	...
	drop: function(draggy, droppy) { // change columns display order
		this.leave(draggy, droppy);
			var dragIdx = draggy.state.value|0, dropIdx = droppy.state.value|0;
...
C_iARRAY.fields = function(catalyst, callbacks, preset) { // Modal display: Select fields to be displayed in C_iARRAY
	...
	var base = 'flds'+'_';
	this.eids = { buttons:base+'butns', tabs:base+'tabs', fields:base+'flds' };
	this.elements = new A_el();
	
	var cartouche	= new C_iDQS(this.eids.buttons, {onsave:new A_cb(this, this.save), onquit:new A_cb(this, this.quit)}, { remove:false } );
	
	var pass	= new C_iPASS( {} );
		var options = this.catalyst.cresta();
	var fields = new C_iCRESTA(this.eids.fields, {}, options, { maxcols:3, maxrows:10 } );
	var tabs	= new C_iTABS(this.eids.tabs, C_XL.w({0:'display in table'}), false, {current:preset.tab||0} );

	this.controls 	= new A_ct( { pass:pass, cartouche:cartouche, tabs:tabs, fields:fields } );
...
C_iARRAY.fields.prototype = {
	// private
	header: function() {
		var buttons = '<td>'+this.controls.cartouche.display()+'</td>';
		var title = '<td style="align:right;"><h1>'+this.title()+'</h1></td>';
		var header = '<tr>'+buttons+title+'</tr>';
		var divHeader = '<div style="padding:0 0 1em 0;"><table summary="header layout">'+header+'</table></div>';
		var divTabs = '<div style="text-align:center;">'+this.controls.tabs.display()+'</div>';
		return divHeader+divTabs;
	},
	body: function(css) {
	
		// tab 0 schedule & hourly
		var fields = this.controls.fields.display();
		var tab0 = this.controls.tabs.container(0, fields);
		
		return tab0;
	},
	activate: function() {
		this.controls.activate('C_iARRAY.fields');
	},
	...
	// event handling
	save: function() { 
		var checked = this.controls.fields.getvalue() ;
		// feature: we want to keep the display order that is written in this.catalyst.on but absent in checked
		
		for(var x in checked) { // add new fields at the end
			var f = checked[x];
			if(arrayHAS(this.catalyst.on,f)) continue; 
			this.catalyst.on.push(f);
		}
		var clear = true; while(clear) {
			clear = false;
			for(var x in this.catalyst.on) { // remove unwished fields
				var f = this.catalyst.on[x];
				if(arrayHAS(checked,f)) continue; 
...
function C_iCUE(eid, onSelect, preset) { // displays all resources
	...
	// options
	var labels = { };
		var increment = (preset.increment=='hour')?(mobminder.account.timeSlice):1;
		var padding = (preset.increment=='hour')?0:1;
		
		var start = this.state.rangein-padding; /*-increment;*/
		var stop = this.state.rangeout+padding; /*+increment;*/ // <= bugs when C_iSPAN cues push the limit outside the visible range, to be solved!
	for(var s=start; s<=stop; s+=increment) // rangein and rangeout are in slices
		labels[s] = C_iCUE.options[s];
	
	
	// drop preset
	var slice = preset.slice || this.state.rangein; // select first option by default
	...
	if(vbs) vlog('controls.js','C_iCUE','constructor','rangein:'+this.state.rangein+', rangeout:'+this.state.rangeout+', preset:'+slice+', iscueout:'+this.state.iscueout); 
		var order = []; for(var x in labels) order.push(x);
		cueoptions = { order:order, labels:labels, presets:{}, count:order.length };
		
	var drop = new C_iDDWN(eid, { onselect:new A_cb(this, this.select) }, cueoptions, { maxcols:1, value:slice /*preset*/ }  );
...
C_iCUE.prototype = {
	...
	setSeconds: function(sec) { var slice = (sec/mobminder.account.secondsPerSlice)|0; this.setSlice(slice); },
	...
	sliceslock: function(options, locked) {
		...
		if(options && typeof(options)=='object') {
			var order = this.controls.drop.options.order;
			var tobelocked = [];
	...
	getpost: function(options) { 
		...
		var slices = this.controls.drop.getpost();
		if(options.seconds)	{ 
			// if(this.state.iscueout) if(slices==this.state.iscueout) slices = 0;
			return slices*mobminder.account.secondsPerSlice; 
		}
		else return slices;
	},
...
C_iCUE.init = function() { // called at account config setting - dependance with account time slicing
	...
	var sliceSec = mobminder.account.secondsPerSlice;
	for(var seconds=0, slice=0; seconds<=86400; seconds+=sliceSec, slice++) 
...
function C_iRANGE(eid, callbacks, preset) {
	...
	preset.setin = preset.setin||0;
		...
		var sps = mobminder.account.secondsPerSlice;
		var accountIn = (mobminder.account.rangeIn/sps);
		var accountOut = (mobminder.account.rangeOut/sps);
	
	if(this.state.units=='seconds') {
		var min = (preset.min!==undefined)?(preset.min/sps):accountIn; // min & max are in slices
		var max = preset.max?(preset.max/sps):accountOut;
	} else { // units=='slices'
		var min = preset.min||accountIn; // min & max are in slices
		var max = preset.max||accountOut;
...
if(vbs) vlog('controls.js','C_iRANGE','construction','units:'+this.state.units+', cueIn:'+time(preset.setin)+', cueOut:'+time(preset.setout)+', min:'+time(min*sps)+', max:'+time(max*sps));
	...
	var rangein = new C_iCUE(this.eids.rangein, new A_cb(this, this.rangein), { seconds:preset.setin, rangein:min, rangeout:max, increment:this.state.increment } );
	var rangeout = new C_iCUE(this.eids.rangeout, new A_cb(this, this.rangeout), { seconds:preset.setout, rangein:min, rangeout:max, increment:this.state.increment } );
...
C_iRANGE.prototype = {
	display: function(css) {
		css = css||'alphaCUE';
		var rangein = this.controls.rangein.display(css||'');
		var rangeout = this.controls.rangeout.display(css||'');
		var range = C_XL.w('fromtime',{cap:0})+' '+rangein+' '+C_XL.w('to',{cap:0})+' '+rangeout;
		return range;
	},
	labelled: function(english, css) {
		var range = '<td>'+this.display(css||'alphaCUE')+'</td>';
		var label = '<td class="label textcolor-light" style="white-space:nowrap; text-align:right;">'+C_XL.w(english)+'</td>';
	...
	getpost: function() {
		var seconds = this.state.units=='seconds';
		var cueIn = this.controls.rangein.getpost({seconds:seconds});
		var cueOut = this.controls.rangeout.getpost({seconds:seconds});
	...
	rangein: function(s) { // changing rangein => adjust rangeout
		...
		var lockbefore = (s|0)+this.increment; 
	...
	rangeout: function(s) { // changing rangeout
		...
		var lockafter = (s|0)-this.increment;
...
function C_iOFFD(eid, callbacks, preset) {
	...
	var dpIn  = new C_iDP(this.eids.dpIn, {dayclick:new A_cb(this, this.dpIn)}, { jsdate:this.state.cin, display:{abreviation:'abr', weekday:true} } );
	var dpOut = new C_iDP(this.eids.dpOut, {dayclick:new A_cb(this, this.dpOut)}, { jsdate:this.state.out, display:{abreviation:'abr', weekday:true} } );
...
C_iOFFD.prototype = {
	...
	display: function(css) {
		var dateIn = this.controls.dpIn.display('bold');
		var dateOut = this.controls.dpOut.display('bold');
	...
	getpost: function() { 
		var midnOut = this.state.out.clone().addDay(1).stamp();
		var midnIn = this.state.cin.stamp();
...
function C_iSPAN(eid, callbacks, preset) {
	...
	var	cueIn = new C_iCUE(this.eids.cueIn, new A_cb(this, this.cueIn), { jsdate:this.state.cin } );
	var	cueOut = new C_iCUE(this.eids.cueOut, new A_cb(this, this.cueOut), { jsdate:this.state.out } );		
...
C_iSPAN.prototype = {
	...
	display: function(css) {
		var cueIn = this.controls.cueIn ? this.controls.cueIn.display('bold alphaCUE') : '';
		var cueOut = this.controls.cueOut ? this.controls.cueOut.display('bold alphaCUE') : '';
		var dateIn = this.controls.dpIn.display('bold');
		var dateOut = this.controls.dpOut.display('bold');
	...
	getpost: function() { 
		var midnOut = this.controls.dpOut.getpost();
		var midnIn = this.controls.dpIn.getpost();
		var cueOut = this.controls.dpOut.jsdate().timeshift()+this.controls.cueOut.getpost({seconds:true});
		var cueIn = this.controls.dpIn.jsdate().timeshift()+this.controls.cueIn.getpost({seconds:true});
	...
	dpIn: function(jsdate, stmp) { 		// date cueIn has changed
		...
		var sameday = this.state.cin.sameday(this.state.out);
		if(sameday)
	...
	cueOut: function(slice, seconds) { 	// time cueOut has changed
		...
		var sameday = this.state.out.sameday(this.state.cin);
		if(sameday)
...
function C_iINOUT(eid, callbacks, preset) { // preset like { cin:jsDateIn, out:jsDateOut }
	...
	// meta data
	var secondsIn = preset.cin.seconds();
	var secondsOut = preset.out.seconds();
	var fulldays = (!secondsIn && !secondsOut);
	this.is = { sameday:this.state.cin.sameday(this.state.out), fulldays:fulldays }
	
	// time display controls
	var cueIn = false, cueOut = false;
	if(this.is.fulldays) { // when cueIn and cueOut are midnight ( =0 ) there is no time display
		this.state.out.addDay(-1);
	} else { 
	
		var s = this.state.out.getSlice(); if(s==0) this.state.out.addDay(-1);
	...
	var dateIn = new C_iDPpop(this.eids.dateIn, this.state.cin, this.callbacks, { abreviation:'full', weekday:true } );
	var dateOut = new C_iDPpop(this.eids.dateOut, this.state.out, this.callbacks, { abreviation:'full', weekday:true } );
...
C_iINOUT.prototype = {
	...
	display: function(prefix) {
			var timeIn = this.controls.cueIn ? this.controls.cueIn.display('bold alphaCUE') : '';
			var timeOut = this.controls.cueOut ? this.controls.cueOut.display('bold alphaCUE') : '';
			var dateIn = this.controls.dateIn.display('bold');
			var dateOut = this.is.sameday ? '' : this.controls.dateOut.display('bold');
		prefix = prefix || '';
		
			var splitline = is.newtouch ? '<br/>' : ' ';
	...
	getpost: function() { 
		var cueIn = 0;
		var cueOut = 0;
		var dateOut = this.controls.dateOut.getpost().clone().toMidnight();
		var dateIn = this.controls.dateIn.getpost().clone().toMidnight();
		if(this.is.fulldays) {
			dateOut.addDay(1);
		} else {
			var cueIn = dateIn.timeshift()+this.controls.cueIn.getpost({seconds:true});
			var cueOut = dateOut.timeshift()+this.controls.cueOut.getpost({seconds:true});
		}
		return { cin:(dateIn.stamp()+cueIn), out:(dateOut.stamp()+cueOut) }; 
	},
	getcues: function() {
		var cueIn = this.controls.cueIn.getpost({seconds:true});
		var cueOut = this.controls.cueOut.getpost({seconds:true});
		return { cin:cueIn, out:cueOut }; 
	},
	setduration: function(s) { // impacts only cueOut, s is a number of slices
			var neverlater = mobminder.account.sliceOut;
			var out = this.controls.cueIn.slice()+s;
	...
	getduration: function() { // returns the duration is number of slices
			var cueIn = this.controls.cueIn.getpost({seconds:false});
			var cueOut = this.controls.cueOut.getpost({seconds:false});
		return cueOut-cueIn;
	},

	// private
	after: function() {
		var neverlater = mobminder.account.sliceOut-this.state.span;
		var maxspan = this.state.cues.max ? this.state.cues.max-this.state.span : mobminder.account.rangeOut;
		var after = neverlater < maxspan ? neverlater : maxspan;
		return after;
	},
	before: function() {
...
function C_iMONTH(eidNest, scope, callbacks, preset) {
	...
	var y = this.scope.y; var m = this.scope.m+1;
...
C_iMONTH.prototype = {
	display: function(today) { // returns a <table>, today must be a PHP stamp
		var tds = new Array(new Array, new Array, new Array, new Array, new Array, new Array); // months are displayed on 5 rows most often, but sometime 6 are needed, very exceptionally 4 are sufficient (february 2015)

		var tableCss = 'dpicker-month';
		var enabled, istoday, iscurrentweek = 0;
			
		var css = function(daycode, istoday) {  
			var css = 'daycode'+daycode%7; 
			if(istoday) css += ' today'; 
			return css; 
		}
		var dayclick = new A_cb(this,this.dayclick);
		
		var row, col, cell; // row[0-5], col[0-6], cell[0-41]  <= those counters run accross the heading, filling and trailing loops
		var wn = this.scope.range.wn1; // weeknumber of first row
		var dwn = this.state.weeknumb; // display week number
		...
		//		
		var date = new Date(this.scope.range.date1);
		var stop = this.scope.range.day31;
		for(var d = 0;  d<stop  ; d++, col++, cell++, col%=7) {
			
				var o_DAY = new C_iDAY(this.eids.nest, date, 0, { click:dayclick }, {enabled:this.allow(date,col)});
		...
		//
		for(var r in tds) tds[r] = tds[r].join(''); // for each row, join tds into an html list of tds
		
		// join rows
		//
		var rows = '<tr>'+tds.join('</tr><tr>')+'</tr>';
		
		// month name and weekdays names header
		//
		var wnpadding = dwn ? '<th></th>' : ''; // there is one additionnal column on screen when weeknumbers are displayed
		var monthname = '<tr><th colspan=7 class="dpicker-monthname">'+this.name()+' '+this.scope.y+'</th>'+wnpadding+'</tr>';
		var daynames = new Array(); // build day names column headers
		for(var daycode=6;daycode<13;daycode++) daynames.push(C_XL.weekday(1+daycode%7,'min'));
...
C_iMONTH.scope = function(PHPstmp) { // a sub object that contains only the time related variables of a given month
	...
	var date = new Date(PHPstmp*1000); 
...
C_iMONTH.scope.prototype = {
	...
	setRange:function() {
			var date1 = new Date(this.y, this.m, 1);
			var date31= new Date(this.y, this.m+1, 1); date31.setDate(date31.getDate()-1);
			var wnumb = date1.getWeekNumber().wn; // week number according to ISO-8601
			var daycode1 = date1.getDay(); // day of the week for the first date
...
C_iCAL.defauts = new A_df( { jsdate:false, stmp:0, hidden:false, mpr:2, rows:4, pastrows:1
	...
	, now:{ midnight:(function(){ var d=new Date(); return d.toMidnight().stamp(); })() }	} );
C_iCAL.prototype = {
	// public
	display: function() {
		var rows2display = this.state.rows, mpr = this.state.mpr; // months per row
		var monthsDisplayed = rows2display * mpr; // that is a multiple of an entire row
		...
		var rows = this.rows(rows2display, this.frame.first); // e.g 6 rows makes 12 months display
	...
	getcurrentcoord: function() { // used for scrolling a div to the current element of the calendar, using pages (snap feature of iScroll)
		var td = $(this.frame.selected.elements.table).parent('td');
		var row = $(td).parent('tr');
		var rowIndex = row[0].sectionRowIndex; // [0] to access the DOM element (because we work here with jquery objects)
	...
	increment: function(i, options) { // options like {feedback:false}
		...
		var y = this.state.jsdate.getFullYear();
		var m = this.state.jsdate.getMonth();
	...
	monthDisplay: function(scope) { // offset is a number of month, defaults to 0
			var presets = {mindate:this.state.mindate, maxdate:this.state.maxdate, weekdays:this.state.weekdays, weeknumb:this.state.weeknumb};
		var o_MONTH = new C_iMONTH(this.eids.table, scope, {dayclick:new A_cb(this,this.dayclick)}, presets);
	...
	highlight: function() {
		...
		var year = this.state.jsdate.getFullYear();
		var month = this.state.jsdate.getMonth()+1;
		var stamp = this.state.jsdate.stamp();
	...
	busy: function(where) {
		var div = '<div class="busy" style="min-height:5em;">&nbsp;</div>';
		var tr = '<tr id="'+this.eids.busytr+'">'+'<td colspan='+this.state.mpr+' style="">'+div+'</td>'+'</tr>';
		switch(where) {
	...
	feedback: function(is) { // inc: the change comes from programmatic incremmentation
		if(is) if(is.inc) 
			if(this.callbacks.incremented) {
				this.callbacks.incremented.dcb(600 /*delay in ms*/, this.state.jsdate, this.state.stmp);
				return;
			}
		if(this.callbacks.dayclick) this.callbacks.dayclick.dcb(400 /*delay in ms*/, this.state.jsdate, this.state.stmp);
	},
	...
	moretop: function(rows) { 
		var rows2display = (rows||2);
		var keepscrollon = this.getfirstelement(); 
		var monthsDisplayed = rows2display * this.state.mpr; // that is a multiple of an entire row
		$(this.elements.table).prepend(this.rows(rows2display, this.frame.first.decMonth(monthsDisplayed) )); // display
		for(var o_DAY; o_DAY = this.notActivated.shift(); o_DAY.activate()); // activate
...
function C_iDP(eid, callbacks, preset) { // preset like { display:{abreviation:'abr', weekday:true}, mindate:jsdate, maxdate:jsdate, enabled:true, stmp:astamp }
	...
	var ui = new C_iCLIK(this.eids.ui, { down:new A_cb(this, this.facedown), click:new A_cb(this, this.pop) } , { tag:this.state.display.tag, ui:'X', enabled:preset.enabled } );	
	this.controls = new A_ct({ui:ui});
}
...
C_iDP.prototype = {
	...
	displayDate: function() { // on the ui
		var abreviation = this.state.display.abreviation;
		var weekday = this.state.display.weekday;
		var html = C_XL.date(this.state.jsdate, {abreviation:abreviation, weekday:weekday});
	...
	pop: function() { 
		...
		var cal = new C_iCAL(this.eids.cal, {dayclick:new A_cb(this, this.dayclick)}, { jsdate:this.state.jsdate, mpr:2, rows:6, pastrows:1, mindate:this.state.mindate, maxdate:this.state.maxdate, weekdays:this.state.weekdays });
		this.controls.add1(cal, 'cal');
		var position = { drop:{ui:this.controls.ui.elements.ui}, offset:{x:0,y:5} };
		this.modal = new C_iMODAL(
	...
	top: function() { 
		...
		var keepscrollon = this.controls.cal.moretop();
...
function C_iDPpop(eid, jsdate, callbacks, preset) { // preset like {abreviation:'abr', weekday:true, year:false }
	...
	this.state = C_iDPpop.defauts.align(preset);
		var date = C_XL.date(jsdate, preset||{abreviation:'abr', weekday:true, year:false } );
	var ui = new C_iCLIK(this.eids.date, { click:new A_cb(this, this.click)} , { ui:date, tag:preset.tag, tip:preset.tip } );
	this.controls = new A_ct({ui:ui});
}
...
C_iDPpop.prototype = { 
	// public
	display: function(css) { 
		var date = this.controls.ui.display(css);
		return date; 
	},
...
function C_iYEAR(eid, callbacks, preset) { // element id of the container element in which we build this control
	...
	this.now.midnight = new Date(this.now.y, this.now.m, this.now.d); this.now.midnight = this.now.midnight.stamp(); 
		...
		var labels = []; for(var y=this.now.y-2; y<(this.now.y+5); y++) labels[y] = y;
		var order = []; for(var x in labels) order.push(x);
		var yoptions =  { order:order, labels:labels, presets:{}, count:order.length };
	var year = new C_iDDWN(this.eids.year, { onselect:new A_cb(this, this.year) }, yoptions, { maxcols:1, maxrows:1, value:this.now.y } );
...
C_iYEAR.prototype = {
	...
	// public
	display: function() {
		var left = '<td id="'+this.eids.left+'" style="width:1%">'+this.months(this.eids.months)+'</td>';
		var right = '<td id="'+this.eids.right+'"></td>';
		var grid = '<div style="position:relative;" id="'+this.eids.grid+'"><table class="year-"><tr>'+left+right+'</tr></table></div>';
		var stats = '<div id="'+this.eids.stats+'">stats:</div>';
		return grid+stats;
	}, 
	activate: function() {
		this.elements.collect(this.eids);
	...
	query: function() {
		...
		this.busy(true);
		var date = new Date(this.state.year, 0, 1);
		var post = new C_iPASS({key:mobminder.context.keyId,stamp:date.stamp(), days:365, rescId:this.state.rescid, fulldays:1, sms:0, peers:0 });
		var names = {post:{key:'k',stamp:'stamp',days:'days',rescId:'rescId',fulldays:'fulldays',sms:'sms', peers:'peers'}};
	...
	set: function(year) {
		...
		this.query();
		if(year) {
			var table = this.planning(this.eids.planning);
	...
	planning: function(eid) {
		var trs = new Array;
		var tds;
		var stmp, i, j,date31, day31, dayCode1, tri;
		var date = new Date(this.state.year, 0, 1); // initialized to january 1st
		var index = 0; // days count ranges [0 - 364]
		...
		for(var m=0;m<12;m++) {
			...
			for(i=0; i<day31; i++, j++, date.addDay(1)) { // note that j was not re-initialized in this second loop
				var stmp = date.stamp();
				var o_calday = new C_iDAY(this.eid, date, index++, this.events );
	...
	months: function(eid) { // months names left heading grid rows
		var trs = new Array('<tr><th>'+this.controls.year.display('alpha04 boldy')+'</th></tr>');
		var css;
		for(var m=0;m<12;m++) {
	...
	crossCss: function(td) { // returns references to 2 DOM objects
		var row = $(td).parent("tr");
		var rowIndex = row[0].sectionRowIndex-1;
	...
	fill: function(o_easyDAYin, o_easyDAYout, css, options) {
		...
		var ordered = this.order(o_easyDAYin, o_easyDAYout);
		for(var x = ordered.cin.index; x<= ordered.out.index; x++ ) // no dragging allowed over stickers
			if(this.state.days[x].state.sticker) { this.state.dragging.busy = false; return; };
		for(var x = ordered.cin.index; x<= ordered.out.index; x++ )
			$(this.state.days[x].elements.td).addClass(css);
	},
	cleanup: function(css) {
		if(css)
			return $(this.elements.planning).find('td.'+css).removeClass(css);
		for(var x in this.state.days) this.state.days[x].resetcss();
	},
	order: function(o_easyDAYin, o_easyDAYout) {
		var xin = o_easyDAYin;
		var xout = o_easyDAYout;
		if(xout.index<xin.index) { var x = xout; xout = xin, xin = x; }
		return { cin:xin, out:xout }
	},
	busy: function(onoff) {
		if(onoff && !this.state.busy) {
			var overlay = '<div class="modal-busy" id="'+this.eids.busy+'" style="z-index:20; position:absolute; top:0; left:0; width:100%; height:100%; overflow:hidden;"></div>';
	...
	stats: function(colors) {
		var trs = new Array();
		for(var id in colors.labels) {
			var name = colors.labels[id];
			var counter = colors.counters[id];
		...
		var table = '<table class="year-stats">'+trs.join('')+'</table>';
	...
	dayup: function(o_easyDAY) {
		...
		if(this.state.dragging.busy) {
			var ordered = this.order(this.state.dragging.cin, this.state.dragging.out);
	...
	plitems: function(dataSets) {
		this.busy(false);
		var attendees = dataSets['C_dS_attendee'];
		var reservations = dataSets['C_dS_reservation'];
		var newyearsday = new Date(this.state.year, 0, 1); var nyStamp = newyearsday.stamp();
		var newyearseve = new Date(this.state.year+1, 0, 1); var eveStamp = newyearseve.stamp();
		var colors = C_dS_customCss.options(resaclass.event, ccsstype.color);
	if(vbs) vlog('controls.js','C_iYEAR','plitems','newyearsday:'+newyearsday.sortable()+'newyearseve:'+newyearseve.sortable()); 
		...
		for(id in attendees) {
			...
			var resa = reservations[attendee.resaId];
			if(resa.deletorId) continue; // we do not show deleted stuff
			var parts = resa.getparts();
			for(var x in parts) {
				var part = parts[x];
				var midnight = part.midnight; 
...
function C_iRECUR(eid, callbacks, preset) {
	...
	this.state = C_iRECUR.defaults.align(preset);
		var din  = new Date(this.state.midnIn*1000 ); this.state.midnIn  = din.toMidnight().getPHPstamp();
	
		// recurrence mode
			var rmodopt = C_XL.w({ daily:'daily', weekly:'weekly', monthly:'monthly', yearly:'yearly' });
	var rmode = new C_iDDWN(this.eids.rmode, { onselect:new A_cb(this, this.onrmode) }, { labels:rmodopt }, { value:'weekly' } );
	
		// daily repetition (step)
			var drepopt = C_XL.w({ 1:'every day' });
			for(var x = 2; x<15; x++) drepopt[x] = C_XL.w('every')+' '+x+' '+C_XL.w('days');
	var drepete = new C_iDDWN(this.eids.drepete, { onselect:new A_cb(this, this.ondrepete) }, { labels:drepopt }, { value:1 } );
	
		// weekly pattern
			var wrepopt = C_XL.w({ 1:'every week' });
			for(var x = 2; x<27; x++) wrepopt[x] = C_XL.w('every -f')+' '+x+' '+C_XL.w('weeks');
	var wrepete = new C_iDDWN(this.eids.wrepete, { onselect:new A_cb(this, this.onwrepete) }, { labels:wrepopt }, { value:1 } );
	
			var jsdate = new Date(this.state.cue*1000); var encoday = 1<<(jsdate.getPHPday()-1);
	var wdays = new C_iWDAYs(this.eids.wdays, { onwdays:new A_cb(this, this.onwdays) }, { encoded:encoday, disabled:encoday } );
	
		// monthly pattern
			var mrepopt = C_XL.w({ 1:'every month' });
			for(var x = 2; x<12; x++) mrepopt[x] = C_XL.w('every')+' '+x+' '+C_XL.w('months');
	var mrepete = new C_iDDWN(this.eids.mrepete, { onselect:new A_cb(this, this.onmrepete) }, { labels:mrepopt }, { value:1 } );
	var mwdays = new C_iWDAYs(this.eids.mwdays, { onwdays:new A_cb(this, this.onmwdays) }, { encoded:encoday, disabled:encoday } );
	
		var mwpopts = C_XL.w({ 1:'on first', 2:'on second', 3:'on third', 4:'on fourth', 9:'on the last' });
			var jsdpos = jsdate.daypos();
			var jsdposlast = jsdate.dayposlast();
			var jsddatelast = jsdate.datelast();
	
		var mpatopts = { 0:C_XL.w('on a date')+':&nbsp;'+C_XL.w('ondate')+'&nbsp;'+jsdate.getDate() };
		if(jsdpos<5) mpatopts[jsdpos] = mwpopts[jsdpos];
		if(jsdposlast==-1) mpatopts[9] = mwpopts[9];
		
				var mlastopt = C_XL.w({ 1:'on last day', 2:'penulti day' });
			for(var x = 3; x<=10; x++) mlastopt[x] = x+' '+C_XL.w('days before eom');
		if(jsddatelast>=-10) mpatopts[jsddatelast] = mlastopt[-jsddatelast];
	
	var mpattern = new C_iCRESTA(this.eids.mpattern, { onchange:new A_cb(this,this.onmpattern)}, { labels:mpatopts }, { skin:0, title:false, mode:-1, value:0 });

		// yearly
			var yrepopt = C_XL.w({ 1:'every year' });
			for(var x = 2; x<6; x++) yrepopt[x] = C_XL.w('every')+' '+x+' '+C_XL.w('years');
	var yrepete = new C_iDDWN(this.eids.yrepete, { onselect:new A_cb(this, this.onyrepete) }, { labels:yrepopt }, { value:1 } );
	var ydate = new C_iDPpop(this.eids.ydate, jsdate, { setdate:false }, { abreviation:'full', weekday:false, year:false } );
	
		// iterations
			var iteropt = C_XL.w({ 1:'one time' });
			for(var x = 2; x<53; x++) iteropt[x] = x+' '+C_XL.w('x times');
	var iterate = new C_iDDWN(this.eids.iterate, { onselect:new A_cb(this, this.oniterate) }, { labels:iteropt }, { value:1 } );
	
	var dpOut = new C_iDP(this.eids.dpOut, {dayclick:new A_cb(this, this.dpOut)}, { stmp:this.state.cue, display:{abreviation:'full', weekday:true} } );
...
C_iRECUR.prototype = {
	...
	display: function(css) {
			var rmode = '<tr>'+this.controls.rmode.labelled('recurrence')+'<td></td></tr>';
			var drepete = '<tr id='+this.eids.wrappers.rmode.daily+'>'+this.controls.drepete.labelled('repeats every')+'<td></td></tr>';
			var wrepete = '<tr id='+this.eids.wrappers.rmode.weekly+'>'+this.controls.wrepete.labelled('repeats every')+'<td>'+this.controls.wdays.display()+'</td></tr>';
			var mrepete = '<tr id='+this.eids.wrappers.rmode.monthly+'>'+this.controls.mrepete.labelled('repeats every')+'<td></td></tr>';
				var mwdays 	= '<td id='+this.eids.wrappers.mpattern.mwdays+'>'+this.controls.mwdays.display()+'</td>';
				var mdate 	= '<td id='+this.eids.wrappers.mpattern.mdate+' class="vtop">'+'</td>'; // returns a double <td> inside a table
				var mlast 	= '<td id='+this.eids.wrappers.mpattern.mlast+' class="vbottom">'+'</td>'; // there is no further choice here
			var mpattern 	= '<tr id='+this.eids.wrappers.rmode.monthframe+'><td></td><td>'+this.controls.mpattern.display()+'</td>'+mwdays+mdate+mlast+'</tr>';
			var yrepete = '<tr id='+this.eids.wrappers.rmode.yearly+'>'+this.controls.yrepete.labelled('repeats every')+'<td>'+'&nbsp;'+C_XL.w('ondate')+this.controls.ydate.display()+'</td></tr>';
			
			var iterate = '<tr>'+this.controls.iterate.labelled('happens xt')+'<td></td></tr>';
			var dpOut = '<td></td>'+this.controls.dpOut.labelled('up to date','bold alpha18');
			
		var recur = '<table style="margin:1em 0;">'+rmode+drepete+wrepete+mrepete+mpattern+yrepete+iterate+dpOut+'</table>';	
		
			var preview = '<div class="textcolor-light">'+C_XL.w('preview')+'</div>';
			var inner = '<div id="'+this.eids.own.prvws+'" class="" style="border:1px solid rgba(0,0,0, 0.1); border-radius:3px; padding:1em; width:44em; min-height:10em; margin-bottom:1em;">'+'</div>';
		var outer = '<div class="wide100" style="min-height:10em; height:10em; overflow-y;visible;">'+preview+inner+'</div>';
	...
	getpost: function() { 
		var post = new Array();
		var dates = this.controls.previews.getpost();
		for(var x in dates) post.push(dates[x].stamp());
		return post.join('!'); 
	},
	isopen: function() {
		var midnIn = this.controls.dpIn.isopen();
		var midnOut = this.controls.dpOut.isopen();
	...
	resume: function(cue) { // displays a preview of all generated dates according to user settings
		...
		var d = this.state.drive;	// what let the serie stop [ iterate, or date (dpout) ]
		var r = this.state.recurrence;	// the mode of reccurence [ daily, weekly, monthly, yearly ]
			var i = this.controls.iterate.value()|0; // number of times the appointment should happen (relevant if drive is iterate)
			var t = this.controls.dpOut.jsdate(); // jsdate term date to stop the serie (relevant if drive is dpout)
		var s = 1; 	// the stepping (every 2 weeks eg.)
		var w = [];	// the concerned weekdays
		var m = this.controls.mpattern.getvalue()|0; // m is negative to reach last days of the months, is 0 for a specific date, is positive for the x'th week of the month and is 9 for the last week of the month
		...
		var jsd = new Date(this.state.cue*1000); // initial date
		...
		var first = new C_iDPpop(this.eids.own.prvws+'_0', jsd, { setdate:new A_cb(this, this.onpreview, 1) }, this.dpoptions );
		var previews = new A_ct({}); 
			previews.add1(first,jsd.stamp()); // the date selected on the planning is the first an mandatory step.

		// produce iterations
			var o = 1; // occurance
				var goon; switch(d) { // defines if more iterations must be found, depends on the drive
					case 'iterate': goon = function(o,j) { return o<i;  }; break;
					case 'dpout': goon = function(o,j) { return j.isbefore(t); }; break;
				}
				var increment; switch(r) { // defines how the next iteration is computed, depends on the reccurence mode
			...
			var jsdi = jsd.clone(), lastmidnite = first.midnight, iterpop;
	...
	prevdisplay: function(warnings) {
		...
		warnings = warnings||[];
		var el = this.elements.own.prvws; el.innerHTML = '';
		var warnicon = '<div style="font-size:1.2em; display:inline-block;" class="orange fa fa-gray fa-warning"></div>';
		...
		// excluded dates rows
		var trsX = {}; // Xcluded
		for(var c in this.xcluded) {
				var jsd = new Date(c*1000);
			var dppop = new C_iDPpop(this.eids.xcluded+'_'+c, jsd, { setdate:new A_cb(this, this.onexcluded) }, this.dpoptions );
			var date = dppop.display('textcolor-light');
				var cue = jsd.stamp();
			this.controls.xcluded.add1(dppop,cue);
			var index = '<td>'+'</td>'; // no index for excluded items
					var bin = '<div style="font-size:1.4em; display:inline-block;" class="fa fa-gray fa-recycle"></div>';
				var incl = new C_iCLIK(this.eids.icludes+c+'_clk', { click:new A_cb(this, this.reinclude, cue) }, { ui:bin+' '+C_XL.w('include date'), visible:false, enabled:true, tip:C_XL.w('tip include this date'), css:'', tag:'td' });
				this.controls.icludes.add1(incl,cue);
			var warning = '<td class="textcolor-light" style="text-align:right;">('+C_XL.w('excluded')+')</td>';
			var include = incl.display();
		...
		var trsP = {}; // Preview
		var o = 1; 
		for(var c in this.controls.previews.get) { 
			var dppop = this.controls.previews.get[c];
			var date = dppop.display();
			var index = '<td style="text-align:right; padding-left:1em;">'+o+++'. </td>';
				var cue = dppop.jsdate.stamp();
			var warning = (cue in warnings)?'<td>('+C_XL.w('overbooking')+') '+warnicon+'</td>':'<td></td>';
					var bin = '<div style="font-size:1.4em; display:inline-block;" class="fa fa-gray fa-trash"></div>';
				var xcl = new C_iCLIK(this.eids.xcludes+cue+'_clk', { click:new A_cb(this, this.xclude, cue) }, { ui:bin+' '+C_XL.w('xclude date'), visible:false, enabled:true, tip:C_XL.w('tip xclude this date'), css:'', tag:'td' });
				this.controls.xcludes.add1(xcl,cue);
			var exclude = xcl.display();
			trsP[cue] = '<tr>'+warning+index+date+exclude+'</tr>';
		}
		
		// merged rows 
		var trs = new Array(); for(var c in trsX) trs.push(c); for(var c in trsP) trs.push(c); trs.sort(); for(var x in trs) trs[x] = trsP[trs[x]]||trsX[trs[x]];
	...
	onrmode: function(which) {
		if(vbs) vlog('controls.js','C_iRECUR','onrmode','which:'+which);
			var els = this.elements.wrappers.rmode.get;
		for(var x in els) $(els[x]).hide(); $(els[which]).show();
	...
	onmpattern: function(which) {
				which = which|0;
			var els = this.elements.wrappers.mpattern.get; 
		var wrapper = 'mwdays'; 
			if(which==0) wrapper = 'mdate';
			if(which<0) wrapper = 'mlast';
		for(var x in els) $(els[x]).hide(); $(els[wrapper]).show();
...
function C_iPERIOD(eid, callbacks, preset) {
	...
	this.state.mode = this.mode(); // decodes the DB setting
		// var din  = new Date(this.state.midnIn*1000 ); this.state.midnIn  = din.toMidnight().getPHPstamp();
		// var dout = new Date(this.state.midnOut*1000); this.state.midnOut = dout.toMidnight().getPHPstamp();
	...
	var picker = new C_iCRESTA(this.eids.picker, { onchange:new A_cb(this,this.onpicker)}, { labels:C_XL.w(options) }, { skin:0, title:false, mode:-1, value:this.state.mode });
	
	var dpIn  = new C_iDP(this.eids.dpIn, {dayclick:new A_cb(this, this.dpIn)}, { stmp:this.state.midnIn, display:{abreviation:'full', weekday:true} } );
	var dpOut = new C_iDP(this.eids.dpOut, {dayclick:new A_cb(this, this.dpOut)}, { stmp:this.state.midnOut, display:{abreviation:'full', weekday:true} } );
...
C_iPERIOD.prototype = {
	...
	display: function(css, style) {
			css = css||''; style = style||'';
		var dpIn = '<td style="width:20em; text-align:right;">'+this.controls.dpIn.display('bold')+'</td>';
		var picker = '<td style="width:10em; padding:0 1em;">'+this.controls.picker.display()+'</td>';
		var dpOut = '<td style="width:20em; text-align:left;">'+this.controls.dpOut.display('bold')+'</td>';
			if(css) css=' class="'+css+'"'; if(style) style='style="'+style+'"';
		var table = '<table '+style+css+'><tr>'+dpIn+picker+dpOut+'</tr></table>';
		return table;
	},
	activate: function() {
		this.elements.collect(this.eids);
	...
	getpost: function() { 
		var midnIn = 0, midnOut = 0; // takes care of the BD protocol
	...
	mode: function() { // bit map: 00b = always, 01b = up to, 10b = from, 11b = period, 110b = 1day
		var upto = this.state.midnOut ? 1 : 0;
		var from = this.state.midnIn  ? 1 : 0;
		var none = !(upto+from);
		if(none) return 0;
		var aday = this.state.midnOut==this.state.midnIn;
...
function C_iBEFORE(eid, callbacks, preset) { // pick out from a list of future dates
	...
	this.state = C_iBEFORE.defauts.align(preset); this.state.jsDate = new Date();
		var withdp = (this.state.mode=='radio')&&(this.state.eresa==false);
	...
	this.elements = new A_el();
	var presets = new Array(); // specifies which radio to set at init time
	var yesterday = new Date(); yesterday.increment( {d:-1} );
	
		var precheck = false, which = preset.selection||C_iBEFORE.encode([10, 11, 12, 13, 20, 21, 23, 26]); // default setting for run mode
	...
	var options = C_iBEFORE.options(which, precheck, { eresa:this.state.eresa }, this.enlabels); // translates the block of options in one shot
	if(this.is.withdp) {
		var dp = new C_iDP(this.eids.dp, {dayclick:new A_cb(this, this.dpSelect)}, { display:{abreviation:'abr', weekday:true, tag:'span'}, mindate:yesterday, nested:true } );
	...
	this.controls = new A_ct(this.is.withdp?{dp:dp}:{});
		var mode = (this.state.mode=='radio')?-1:1, skin = (this.state.mode=='radio')?0:1;
	var radio = new C_iCRESTA(this.eids.radio, { onchange:new A_cb(this, this.onselect) }, options, { title:C_XL.w(this.state.title), skin:skin, mode:mode, maxrows:(preset.maxrows||12) } );
...
C_iBEFORE.encode = function(values) { var bits = 0; for(var x in values) bits|=C_iBEFORE.bits[values[x]]; return bits; }
...
C_iBEFORE.prototype = {
	...
	getpost: function() { 
		if(this.is.presetmode) { // checker mode for recording setup preferences
			var bits = C_iBEFORE.encode(this.controls.radio.getvalue());
...
function C_iWDAYs(eid, callbacks, preset) { // AM PM preferences
	...
	this.state = C_iWDAYs.defaults.align(preset); // preset like { encodedAMPM: }
			...
			var a = this.state.abreviation;
		var owd = { labels:{ 1:C_XL.weekday(1,a), 2:C_XL.weekday(2,a), 4:C_XL.weekday(3,a), 8:C_XL.weekday(4,a), 16:C_XL.weekday(5,a), 32:C_XL.weekday(6,a), 64:C_XL.weekday(7,a) } };
	var wd = new C_iCRESTA(this.eids.days, { onchange:new A_cb(this, this.onselect) }, owd, { mode:1, skin:1, title:false /*C_XL.w('weekdays')*/, cssclass:'' } );
...
C_iWDAYs.prototype = {
	...
	value: function() { 
		var vs = this.controls.wd.getvalue();
		var ds = []; for(var x in vs) {
			var v = vs[x];
			for(var s = parseInt('10000000', 2), p=7; s>>=1; p--) if(v&s) ds.push(p%7); // jsDate 0 is sunday, 1 is monday to 6 saturday
		}
	...
	set: function(setting, options) {
		...
		this.state.encoded = setting;
			var nv = []; 
		for(var s = parseInt('10000000', 2); s>>=1; ) if(setting&s) nv.push(s); // jsDate 0 is sunday, 1 is monday to 6 saturday
	...
	lock: function(encoded, onoff) {	
		for(var s = parseInt('10000000', 2); s>>=1; ) // jsDate 0 is sunday, 1 is monday to 6 saturday
			if(encoded&s) this.controls.wd.lock(s, onoff);
	},
	bits: function(value) { // display in bit map format
		// return '';
		var bits = value&parseInt('7F',16);
		for(var bytie = '', scan=parseInt('80',16); scan; scan>>=1) bytie += (scan&bits ? '1' : '0');
		return '|'+bytie+'|';
	},
	...
	onselect: function(selections) { 
		...
		for(var x in selections) this.state.encoded += (selections[x]|0); // set corresponding bits
...
function C_iAMPM(eid, callbacks, preset) { // AM PM preferences
	...
	var options = { 
		...
		var am = false,wd = false,pm = false;
		var m = this.state.mode;
		var pm = this.state.showpm;
...
C_iAMPM.prototype = {
	...
	and: function(encoded) {
		...
		var newsetting = encoded&this.state.encoded;
	...
	set: function(setting) {
		...
		if(this.state.showpm) {
			this.state.encoded = setting;
			var AMmask = parseInt('00010000',16); // 16 is the hex base
			var PMmask = parseInt('00000100',16);
			this.controls.am.reset();
			this.controls.pm.reset();
			for (var v=1, encoded = setting|0; v<=64; v<<=1, encoded>>= 1) {
				if(AMmask&encoded) this.controls.am.docheck(v);
				if(PMmask&encoded) this.controls.pm.docheck(v);
			}
		} else {
			var WKmask = parseInt('00000001',16);
			for (var v=1, encoded = setting|0; v<=64; v<<=1, encoded>>= 1) {
	...
	bits: function(value) { // display in bit map format
		// return '';
		var bits = { pm:(value&parseInt('0000FF00',16))>>8, am:(value&parseInt('00FF0000',16))>>16 };
		for(var byteAM = '', scan=parseInt('80',16); scan; scan>>=1) byteAM += (scan&bits.am ? '1' : '0');
		for(var bytePM = '', scan=parseInt('80',16); scan; scan>>=1) bytePM += (scan&bits.pm ? '1' : '0');
		return byteAM+'|'+bytePM;
	},
	
	WDchanged: function(selections) {
		this.state.encoded = 0;
		for(var x in selections) this.state.encoded += (selections[x]|0); // set bits in WD byte
	...
	selectAM: function(selections) { 
		...
		for(var x in selections) this.state.encoded += (selections[x]|0)<<16; // set bits in AM byte
		if(this.callbacks.onampm) this.callbacks.onampm.cb();
	...
	selectPM: function(selections, target, value) { 
		...
		for(var x in selections) this.state.encoded += (selections[x]|0)<<8; // set bits in PM byte
		if(this.callbacks.onampm) this.callbacks.onampm.cb();
...
C_iDATEPRESET = function(eid, callbacks, preset) { // pick out from a list of future dates
	...
	this.elements = new A_el();
	var preset = new Array(); preset[0] = { set:true }; // specifies which radio to set at init time
	
		var yesterday = new Date(); yesterday.increment( {d:-1} );
	var dp = new C_iDP(this.eids.dp, {dayclick:new A_cb(this, this.dpSelect)}, { display:{abreviation:'abr', weekday:true, tag:'span'}, mindate:yesterday, jsdate:this.state.jsdate } );
	
		var options = { labels:{ 0:C_XL.w('current date'), 99:dp.display('-') }, order:[0, 99], count:2 }; // translates the block of options in one shot
	var radio = new C_iCRESTA(this.eids.radio, { onchange:new A_cb(this, this.radioSelect) }, options, { title:C_XL.w('upper left date'), skin:0, mode:-1, value:(this.state.jsdate?99:0) } );
...
C_iDATEPRESET.prototype = {
	// public
	display: function(css) {
		var radio = this.controls.radio.display('select-header '+(css || '')); 
		return '<table style="text-align:left; layout:fixed"><tr><td>'+radio+'</td></tr></table>';
	},
	activate: function() {
	...
	getpost: function() { 
		...
		var utstmp = this.state.jsdate.getUnixTime();
		return utstmp;
	},
...
function C_iACTIONS(eid, callbacks, preset) { // pick out from a list of future dates
	...
	this.state = C_iACTIONS.defauts.align(preset);
		...
		var filters = C_XL.w({ 1:'creation', 2:'modification', 4:'deletion' });
		var forder = [1,2,4];
		var presets = { 1:{checked:this.state.actions&1}, 2:{checked:this.state.actions&2}, 4:{checked:this.state.actions&4} };
		var options = { order:forder, labels:filters, presets:presets, count:forder.length };
	var filter = new C_iCRESTA(this.eids.filter, { onchange:new A_cb(this, this.filter) }, options, { skin:1, mode:1, title:this.state.title, postmode:'bitmap' } );
...
C_iBAR = function(eid, points, scale, options) { // points like { 0:{measure:13, css:'css1'}, 1:{measure:5, css:'css2'} }
	...
	for(var x in points) { this.segments.push(new C_iBAR.segment(points[x].measure, (points[x].label||''), points[x].css, this.scale, this.options)); }
...
C_iBAR.prototype = {
	display : function(css) {
		var tdLegend = '', tdLegendPad = '';
		var tdTotal = '', tdTotalPad = '';
		if(this.options.legend) { 
			switch(this.options.legendDisplay) {
				...
				case 'fixfront': 
					var inner = '<div style="position:absolute; right:0; bottom:0; text-align:right; white-space:nowrap;">'+this.options.legend+'&nbsp;</div>';
					var outer = '<div style="width:100%; position:relative">&nbsp;'+inner+'</div>'; // &nbsp; forces div to have an height, to witch the absolute positionning refers
		...
		var tdVals = '', tdBars = '', tdLabels = '';
		var tableLabels = '', tableValues = '', tableBars = '';
		...
		if(this.options.labelDisplay!='inbar') {
			var graph = '<table class="iBAR-labels '+(css||'')+'" '+C_iBAR.tblstyle+'><tr>'+tdLabels+'<td style="width:auto;"></td></tr></table>';
			tableLabels = '<tr>'+tdLegendPad+'<td>'+graph+'</td>'+tdTotalPad+'</tr>';
		}
			
		if(this.options.valueDisplay!='inbar') {
			var graph = '<table class="iBAR-values '+(css||'')+'" '+C_iBAR.tblstyle+'><tr>'+tdVals+'<td style="width:auto;"></td></tr></table>';
			tableValues = '<tr>'+tdLegendPad+'<td>'+graph+'</td>'+tdTotalPad+'</tr>';
		}
			var graph = '<table class="iBAR-bars '+(css||'')+'"   '+C_iBAR.tblstyle+'><tr>'+tdBars+'<td style="width:auto;"></td></tr></table>';
		var tableBars 	= '<tr>'+tdLegend+'<td>'+graph+'</td>'+tdTotal+'</tr>';
...
C_iBAR.segment.prototype = {
	tdBar : function() { 
		var style = 'style="'+C_iBAR.segment.tdstyle+' vertical-align:'+this.options.barTextVertical+'; width:'+this.width+'; height:'+this.options.barHeight+';"';
		var cclass = 'class="'+this.options.barSkinCss+' '+this.css+'"';
		var value = '', label = '';
...
C_iURL = function(eid, callbacks, preset) {
	...
	var lbl 	= new C_iFIELD(this.eids.lbl, { onfchange:new A_cb(this, this.linklabel, null, null, 500) }, { digits:preset.label||'', type:INPUT_TEXT, mandatory:this.state.mandatory, placeholder:C_XL.w(this.state.placeholder) });
	var url 	= new C_iFIELD(this.eids.url, false, { digits:preset.url||'', type:INPUT_URL, mandatory:false, placeholder:C_XL.w('link url') });
...
C_iURL.prototype = {
	display: function(css) {
		var tdtd = '<td>'+this.controls.lbl.display('alpha14')+'</td><td>'+this.controls.url.display('alpha28')+'</td>';
		return tdtd; 
	},
	labelled: function(english) {
			var linktr = '<tr>'+this.display()+'</tr>';
			var label = '<tr><td colspan=2 class="textcolor-light">'+C_XL.w(english)+'</td></tr>';
		var html = '<table summary="directions link">'+label+linktr+'</table>';
		return html; 
	},
	activate:function() { 
		this.controls.activate();
	...
	getpost: function() { 
		var lbl = this.controls.lbl.getpost();
		var url = this.controls.url.getpost();
		var post = { url:url, label:lbl };		
		return post;
	},	
	// private
...
C_iFONT = function(eid, callbacks, preset) {
	...
	var options = C_iFONT.options(preset);
		var value = preset.value==''?0: preset.value; // js objects do not accept negative values as member name
	var ddwn = new C_iDDWN(this.eids.ddwn, { onselect:new A_cb(this, this.selected) }, options, { maxcols:1, maxrows:0, value:value } );
	this.controls = new A_ct({ddwn:ddwn});
}
...
C_iFONT.prototype = {
	...
	activate:function() { this.controls.activate();	},
	getpost: function() { 
		var value = this.controls.ddwn.getpost();
...
C_iFONT.options = function(options) {
	...
	var labels = lists[options.fonttype];
	var order = new Array(); for(var x in labels) order.push(x);
	var sortrule = function(a,b) { return (labels[a]>labels[b])?1:-1; }; order.sort(sortrule);
...
C_iGMT = function(eid, callbacks, preset) {
	...
	var options = C_iGMT.options();
		var value = (preset.value|0)<=0 ? 'n'+(-(preset.value|0)) : preset.value; // js objects do not accept negative values as member name
	var ddwn = new C_iDDWN(this.eids.ddwn, { onselect:false }, options, { maxcols:1, maxrows:0, value:value } );
	this.controls = new A_ct({ddwn:ddwn});
}
...
C_iPRO = function(eid, callbacks, preset) {
	...
	var options = C_iPRO.options();
		var mandatory = preset.mandatory ? new A_cb(this, this.isvalid) : false;
	var ddwn = new C_iDDWN(this.eids.ddwn, { onselect:false }, options, { maxcols:1, maxrows:0, value:preset.value, mandatory:mandatory } );
	this.controls = new A_ct({ddwn:ddwn});
}
...
C_iPRO.prototype = {
	...
	valid: function() {
		...
		var valid = this.controls.ddwn.value()!==0;
		return valid;
	},
...
C_iPRO.options = function() {
	var sections = C_XL.w({ 100:'dentistry', 200:'doctors', 300:'alt medicines', 400:'psychology', 500:'wellness and aesthetic', 800:'agences & services' });
	var labels = { 
		100: C_XL.w({ 
				101:'dental surgeon',
				102:'dentist',
	...
	var order = { 100:new Array(),200:new Array(),300:new Array(),400:new Array(),500:new Array(),800:new Array() }; 			
	for(var s in labels) { // s is the section id
		
		for(var x in labels[s]) order[s].push(x);
			var sortrule = function(a,b) { return (labels[s][a]>labels[s][b])?1:-1; }; 
	...
	for(var s in sections) {
		...
		for(var i in labels[s]) labelsmerge[i] = labels[s][i];
	}
	
	var ordermerge = new Array();
	for(var s in order) {
		ordermerge.push(s);
		for(var i in order[s]) ordermerge.push(order[s][i]);
	}
	
	var count = ordermerge.length;
		var bullet = C_XL.w('b2');
	var lockies = { 100:{section:bullet}, 200:{section:bullet}, 300:{section:bullet}, 400:{section:bullet}, 500:{section:bullet}, 800:{section:bullet} };
...
//
C_iSEC = function(eid, callbacks, preset) {
		preset  = preset || {};
	this.eids = { ddwn:eid+'_ddwn' };	
	this.elements = new A_el();
	this.callbacks = callbacks || {}; // like {  }
...
C_iSEC.options = function() {
	...
	var sections = C_XL.w({ 10:'medical sector', 20:'wellness and aesthetic', 30:'agences & services', 40:'financial', 50:'building' });
	...
	var order = { 10:new Array(),20:new Array(),30:new Array(),40:new Array(),50:new Array() }; 			
	for(var s in labels) { // s is the section id
	...
	for(var s in order) {
		...
		var bullet = C_XL.w('a2');
	var lockies = { 10:{section:bullet}, 20:{section:bullet}, 30:{section:bullet}, 40:{section:bullet}, 50:{section:bullet} };
...
//////////////////////////////////////////////////////////////////////////////////////////////
//
//   C H A T    D I S P L A Y    (is used by )
//
C_iCHAT = function(eid, callbacks, preset) {
		preset  = preset || {};
	var b = eid+'_'; this.eids = { conv:b+'conv', interv:b+'interv', bsend:b+'bsnd', bcpy:b+'bcpy' };
	...
	this.state = C_iCHAT.defaults.align(preset);
		...
		var bpreset = { tag:'div', css:'button steel', style:'' };
		var ui = '<div style="width:5em; height:4em; display:table-cell; vertical-align:middle; text-align:center;">'+symbol('send msg')+'</div>';
	var bsend 	= new C_iCLIK(this.eids.bsend, { click:new A_cb(this, this.onsend) }, omerge(bpreset, { ui:ui, tip:C_XL.w('tip send phylac'), keys:[C_KEY.code.s.shift+C_KEY.code.s.enter] }));
	
		var ui = '<div style="width:3em; height:4em; display:table-cell; vertical-align:middle; text-align:center;">'+symbol('to clipboard')+'</div>';
	var bclip 	= new C_iCLIK(this.eids.bcpy, { click:new A_cb(this, this.oncopy) }, omerge(bpreset, { ui:ui, tip:C_XL.w('tip to clipboard'), keys:[C_KEY.code.s.ctrl+C_KEY.code.alpha.c] }));
	
	var interv 	= new C_iFIELD(this.eids.interv, {}, { digits:'', type:INPUT_TEXTAREA, rows:3, placeholder:'', dblclick:false } );
	
	this.controls = new A_ct({ bsend:bsend, interv:interv, bclip:bclip });
	
		var dS_chat_participant = this.state.chat.participants.bylogin.get(mobminder.context.loginId);
...
C_iCHAT.defaults = new A_df({chat:false}); // chat is a C_dS_chat_thread
C_iCHAT.prototype = {
	display: function(css) { 
		
		var convdiv = '<tr style="height:30em; vertical-align:bottom;"><td>'+this.phylacteries()+'</td></tr>';
			var interv = C_XL.w('chat never been');
		...
		interv = '<tr><td style="text-align:center;" class="">'+interv+'</td></tr>';
	...
	focus: function() {
		var el = this.controls.interv.elements.ui;
	...
	prefresh: function() { // phylacteries refresh
		...
		if(!this.state.chat.id) return; // new chat
			var phylas = this.state.chat.phylacteries.bycue.get();
			var trs = new Array();
		for(var cue in phylas) trs.push(this.phylisplay(phylas[cue]));
		this.elements.conv.innerHTML = trs.join('');
			
			var physseen = 0;
			if(this.surferparticipates) {
				var dS_chat_participant = this.state.chat.participants.bylogin.get(mobminder.context.surfer.id);
		...
		var ph = C_XL.w('start chat'); if(cue) ph = C_XL.w('answer here'); this.controls.interv.placeholder(ph); 
		return true;
	},
	
	// private: 
	fetchphylas: function() {
			var chatid = this.state.chat.id;
		if(chatid<=0) return;
		var p = new C_iPASS({chatid:chatid, partid:this.partid, bank:this.state.chat.bank }); // bank: helps bringing back phylacteries in the same mem bank as where the C_dS_chat_thread is stored
	...
	intervention: function() { // a string of controls that allow intervention in a chat
		var interv = '<td style="width:36em;">'+this.controls.interv.display()+'</td>';
		var bipost = '<td style="padding-left:1em;">'+this.controls.bsend.display();+'</td>';
		var bclip = navigator.clipboard?'<td style="padding-left:1em;">'+this.controls.bclip.display()+'</td>':'';
		return '<table style="margin:0 auto;" class=""><tr>'+interv+bipost+bclip+'</tr></table>';
	},
	phylacteries: function() {
		var t = '<table id="'+this.eids.conv+'" style="width:100%;" class="chat-phyls">'+'</table>';
		return t;
	},
	phylisplay: function(dS) { // returns a <tr>, dS is an o_dS_chat_phylactery
	
			var	who = C_dS_loggable.getname(dS.who, {phylactag:true});
				who = '<div class="'+who.css+'">'+who.nickname+'</div>';
			var css = dS.who==mobminder.context.surfer.id?'right':'left'; // surfer speaks right, other participants speak left
		var wholeft = '<th style="width:1%;" class="who-left">'+(css=='left'?who:'')+'</th>'; 
		var whoright = '<th style="width:1%;" class="who-right">'+(css=='right'?who:'')+'</th>';
		
			var d = new Date(dS.cue*1000);
			var when = '<span class="when">&nbsp;'+d.datetimestr()+'&nbsp;</span>';
			var phyla = '<span class="phy">'+dS.bla+'</span>';
			var says = '<td class="who-says"><div class="'+css+'">'+phyla+when+'</div></td>';
		var tr = '<tr>'+wholeft+says+whoright+'</tr>';
		return tr;
	},
	
	// callback
	onsend: function() {
			var bla = this.controls.interv.getpost();
			var chatid = this.state.chat.id;
			var cue = new Date();
			var who = mobminder.context.surfer.id;
			if(!bla.length) return;
		this.controls.bsend.busy(true);
		var p = new C_iPASS({chatid:chatid, partid:this.partid, cue:cue.stamp(), who:who, bla:bla });
		...
		this.controls.interv.set('');
	},
	oncopy: function() {
		
		var phyltext = function(dS) { // returns a raw text version of the philactery, dS is an o_dS_chat_phylactery
		
			var nl = String.fromCharCode(10)+String.fromCharCode(13);
			
				var dS_login = C_dS_loggable.get(dS.who);
				var	who = dS_login.firstname+' '+dS_login.lastname;
			
					var d = new Date(dS.cue*1000);
				var when = d.datetimestr();
			var tr = '('+when+') '+who+':'+nl+dS.bla+nl+nl;
			return tr;
		}
		
			var phylas = this.state.chat.phylacteries.bycue.get();
			var lines = new Array();
		for(var cue in phylas) lines.push(phyltext(phylas[cue]));
...
C_iPLAY = function(presets, altformats) { // provide alternative formats for browsers portability
	...
	for (var i=1; i<arguments.length; i++){
		var filename = this.state.path+arguments[i];
		var sourceel = document.createElement('source'); sourceel.setAttribute('src', filename)
...
C_iFILE = function(eid, dS, callbacks, presets) { 
	...
	this.callbacks = callbacks || {}; // like { preupload, uploading, uploaded }
		...
		var msg = this.is.newfile ? this.finvite():this.filenexticon();
		var dcss = this.is.newfile ? 'ifile ifile-new':'ifile'; 
		
	var droppy = new C_iCLIK(this.eids.droppy, { click:new A_cb(this, this.ondroppy)}, { tag:'div', ui:msg, css:dcss, style:'display:inline-block; text-align:center;', tip:this.state.tip }  );
...
C_iFILE.defaults = new A_df({ enabled:true, key:0, baseline:0, codename:'no code', busy:false, fname:false, uploaded:false, postpath:'./post/file.php', querypath:'./queries/file.php', fileclass:'C_dS_file', tip:false });
C_iFILE.state = { down:false };
C_iFILE.prototype = {
	
	// public
	display: function(css) {
		if(vbs) vlog('controls.js','C_iFILE','display','eid:'+this.eids.drop);
		var droppy = this.controls.droppy.display();
		var input = '<input style="display:none" type="file" id='+this.eids.finput+'>'; // display:none; (used for the file selection window open interactivity)
		return droppy+input;
	},
	labelled: function(english, css) {
	...
	// private 
	upload: function(file) { // (replaced by mupload for multi-part content sending)
		...
		this.busy(true);
		var fuploaded = new A_cb(this, this.fuploaded);
	...
	mupload: function(file) { // multipart upload. Before posting, this function grabs from its parent some post parameter through a call to this.callbacks.preupload()
		...
		var pre = {}; // specifies form data that you want to upload along with the file (e.g. login parameters)
	...
	download: function() {
		...
		var key=''; if(this.state.key) key = '&k='+this.state.key+'&b='+this.state.baseline;
		var uri = this.state.querypath+'?id='+this.dS.id+'&c='+this.state.fileclass+key;
		var el = document.createElement('a'); el.href = uri; el.download = this.state.fname;
		document.body.appendChild(el); el.click(); document.body.removeChild(el);
	},
	busy: function(onoff) {
		onoff = !!onoff;
		var changed = this.state.busy != onoff;
		this.state.busy = onoff;
		if(changed)
	...
	innerbox: function(msg, fextension) { // check here how to center a piece of text inside a div (using table)
		fextension = fextension||'file'; msg = msg||C_XL.w('drop file here');
			var s = '<td>'+symbol(fextension,'fa-2x','padding-right:0.3em;')+'</td>';
			var n = '<td style="line-height:1.1em; font-size:85%; white-space:normal; max-width:12em; text-align:left; vertical-align:middle;">'+msg+'</td>';
	...
	fselect: function() {
		...
		var files = this.elements.finput.files; // when using input of type = file
		var file = files[0]; 
		this.mupload(file);
	},
...
C_iCHECKIN.prototype = {
	...
	checkin: function(login, pass) {
		var data = new C_iPASS({l:login, p:pass, x:this.state.xpectedalevel});
	...
	lpvalidation: function(stream) { // ajax feedback
		...
		var key = split.shift()|0;
		var login = split.shift()|0;
		var account = split.shift()|0;
		var language = split.shift()|0;
...
C_iWatchdog.prototype = {
	...
	again: function() {
		this.timer = false;
			var docall = true; if(this.callbacks.predog) docall = this.callbacks.predog.cb();
		if(docall!==false) { 
			if(typeof docall === 'object') { // like { values:{date:o.date,ics:o.ics,tcs:o.tcs}, names:{date:'date',ics:'ics',tcs:'tcs'} }
				if(docall.values&&docall.names) { 
					var p = new C_iPASS(docall.values); // values and names are defined by docall return values
