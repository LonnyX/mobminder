...
//////////////////////////////////////////////////////////////////////////////////////////////
//
//   E X T E N D I N G     A r r a y     a n d     D a t e    o b j e c t s 
//
...
Date.prototype.datetimestr = function(options) { // options like { y:true } <= displays year
	...
	var dateandtime = this.sortable(options)+' '+this.HHmm(); 
	return dateandtime;
}
...
now = function() { var d=new Date(); return d; };
...
datetime = function(unixTime, options) {
	var jsdate = new Date(unixTime*1000); 
...
C_iMODAL.prototype = {
	...
	display: function(html) {
		...
		var p = this.status.fullscreen?'fs-':'zi-'; // fs for full screen, zi for z-indexed
		...
		var header = html.header==undefined ? '' : '<div class="'+c.header+'" id="'+this.eids.header+'">'+html.header+'</div>';
			var inner = '<div></div>'; if(html.body) inner = html.body; // when inner is empty, the body div does not repond to the height:100% css rule
			var maxheightpad = is.newtouch?' height:100%; max-height:100%;':'';
		var body = '<div class="'+c.body+'" id="'+this.eids.body+'" style="position:relative;'+maxheightpad+'">'+inner+'</div>'; // relative necessary for iscroll and for process (*sc03*).
		var inset = '<div class="'+c.inset+' '+c.shape+'" id="'+this.eids.inset+'" style="position:relative;'+maxheightpad+'">'+header+body+'</div>';
		...
		if(this.status.fullscreen) // then modals are full screen
			outset = '<div class="'+c.outset+'" id="'+this.eids.outset+'" style="min-width:100%; min-height:'+C_iWIN.size.h+'px; position:absolute; z-index:'+zindex+'">'+inset+'</div>';
				...
				var bottom0px = is.newtouch?'bottom:0px;':'';
	...
	show: function() {
		...
		if(this.status.fullscreen) { 
			...
			$(this.elements.body).width(this.status.size.x).height(this.status.size.y);
				var maxy = this.status.size.maxy; // is a string like '18em', cannot be devided
			if(is.newtouch) {
				maxy=false; 
...
C_tMODAL.prototype = {
	...
	// public
	display: function() {
		var header = '<div id="'+this.eids.header+'">'+this.html.header+'</div>';
			
			var padder = '<div style="height:10em; border-bottom:5px solid silver;">&nbsp;</div>'
		var slide = '<div id="'+this.eids.slide+'" class="iSlide">'+this.html.body+padder+'</div>';
		var wrapper = '<div id="'+this.eids.wrapper+'" style="position:relative; height:100%; max-height:100%; width:100%;">'+slide+'</div>';
		var vwport = '<div id="'+this.eids.vwport+'" style="position:absolute; top:'+this.state.position.top+'; bottom:0; min-width:100%; max-width:100%; overflow:hidden;">'+wrapper+'</div>';

		var btop = '<div id="'+this.eids.bouncytop+'" class="minder-background" style="z-index:10; position:absolute; top:'+this.state.position.top+'; min-width:100%; max-width:100%; overflow:hidden; display:none;">'+'TOP'+'</div>';
		var bbot = '<div id="'+this.eids.bouncybottom+'" class="minder-background" style="z-index:10; position:absolute; bottom:0; min-width:100%; max-width:100%; overflow:hidden; display:none;">'+'BOTTOM'+'</div>';
		
		return header+vwport+btop+bbot;
	},	
	activate: function() {
		this.elements.collect(this.eids);
		var handlers = new A_hn({ scrolstart:new A_cb(this, this.scrolstart), scrolldone:new A_cb(this, this.scrolldone), bounce:new A_cb(this, this.bounce) });
		var options = { 
	...
	bounce: function(to, on) {
		...
		var cancel = false; if(nocancel===false) cancel = true; // callback should be defined and should return false to cancel menu pop up
	...
	menu: function(which, onoff, options) {
		...
		// organize fade away of menu
			var h = new A_hn({ over:new A_cb(this, this.faderdelayover, {el:el, state:state})});
	...
	faderdelayover: function(which) {
		...
		var h = new A_hn({ fading:new A_cb(this, this.fading, which.state),faded:new A_cb(this, this.faded, which.state)});
...
C_iTABLE.prototype = {
	...
	rows: function() { // display with top/bottom mode: <div>title</div><table>control</table>
		...
		for(td = 0, x = 0, l = r; td<c; td++, l = (td<f)?r:r-1 ) // we keep empty cells in the bottom row
			for(tr = 0; tr<l; tr++, x++)
				if(x<i) tds.grid.add(tr, td, '<td class="">'+this.items[x].display()+'</td>');
...
(C_iWIN.setbodyclass = function(cursor) {
		...
		var bodysize = is.small ? 'small-device' : 'large-device';
	// $('body').addClass(bodysize+bodymode).noContext();
		var oriclass = is.landscape?'landscape':'portrait';
